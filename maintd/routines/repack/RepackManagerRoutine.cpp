...
#include "RepackManagerRoutine.hpp"
#include "scheduler/Scheduler.hpp"
#include "common/semconv/Attributes.hpp"

namespace cta::maintd {

RepackManagerRoutine::RepackManagerRoutine(cta::log::LogContext& lc, cta::Scheduler& scheduler, int rmrtte, int timeout)
    : m_lc(lc),
  ...
  m_lc.log(cta::log::INFO, "Created RepackManagerRoutine");
}

void RepackManagerRoutine::execute() {
  ...
  m_lc.log(log::DEBUG, "In RepackManagerRoutine::execute(): before promoteRepackRequestsToToExpand()");
  ...
  m_lc.log(log::DEBUG, "In RepackManagerRoutine::execute(): after promoteRepackRequestsToToExpand()");
  ...
  if (const auto repackRequest = m_scheduler.getNextRepackRequestToExpand(); repackRequest != nullptr) {
    m_lc.log(log::DEBUG, "In RepackManagerRoutine::execute(): after getNextRepackRequestToExpand()");
    ...
    try {
      try {
        ...
        m_lc.log(log::DEBUG, "In RepackManagerRoutine::execute(): after expandRepackRequest()");
      ...
      m_lc.log(log::WARNING,
               "In RepackManagerRoutine::execute(), RepackRequest object does not exist in the objectstore");
    }
  }
...
void RepackManagerRoutine::reportBatch(std::string_view reportingType, GetBatchFunc getBatchFunc) const {
  ...
  while (totalTime.secs() < m_softTimeout && moreBatch) {
    ...
    if (!reportBatch.empty()) {
      ...
      m_lc.log(log::INFO, "In RepackManagerRoutine::reportBatch(), reported a batch of reports.");
    } else {
      moreBatch = false;
    }
  }
