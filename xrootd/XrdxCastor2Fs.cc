...
//
   ...
   if (rcode == SFS_REDIRECT) {
     ...
     // we add the user defined stager selection also to the redirection in case the client comes back from a failure
     if ((val = Open_Env.Get("svcClass"))) {
       redirectionhost+="svcClass="; redirectionhost+= val; redirectionhost+="&";
     }

     if ((val = Open_Env.Get("stagerHost"))) {
       redirectionhost+="stagerHost="; redirectionhost+= val; redirectionhost += "&";
     }
...
/******************************************************************************/
int XrdxCastor2Fs::stageprepare( const char* path, XrdOucErrInfo &error, const XrdSecEntity *client, const char* ininfo) {
			   ...
			   const XrdSecEntity *client)
{
  ...
  if (oenv.Get("firstbytewritten")) {
    ...
    if (!XrdxCastor2Stager::FirstWrite(error, path,oenv.Get("reqid"), fileid, nds, oenv.Get("stagehost"), oenv.Get("serviceclass"))) {
      TIMING(xCastor2FsTrace,"END",&preparetiming);  
      preparetiming.Print(xCastor2FsTrace);
      return SFS_ERROR;
    }
...
{  
  ...
  if ((cmd == SFS_FSCTL_LOCATE) || ( cmd == 16777217 ) ) {
    ...
    XrdSfsFileExistence file_exists;
    if ((_exists(path.c_str(),file_exists,error,client,0)) || (file_exists==XrdSfsFileExistNo)) {
      return SFS_ERROR;
    }
