...
XrdxCastor2Stager::Prepare2Get(XrdOucErrInfo &error, uid_t uid, gid_t gid, const char* path, const char* stagehost, const char* serviceclass, XrdOucString &redirectionhost, XrdOucString &redirectionpfn, XrdOucString &status) 
{
  ...
  XrdOucString ErrPrefix = "uid=";ErrPrefix+=(int)uid;ErrPrefix+=" gid=";ErrPrefix+=(int)gid;ErrPrefix+=" path=";ErrPrefix+=path;ErrPrefix+=" stagehost=";ErrPrefix+=stagehost;ErrPrefix+=" serviceclass=";ErrPrefix+=serviceclass;
  ...
  castor::client::VectorResponseHandler rh(&respvec); int mask = umask(0);
  umask(mask);
  ...
  if (fr->errorCode()) {
    TRACES("received error errc=" << fr->errorCode() << " errmsg=\"" << fr->errorMessage().c_str() << "\" " << ErrPrefix << " subreqid=" << fr->subreqId().c_str() << " reqid="<<fr->reqAssociated().c_str());
