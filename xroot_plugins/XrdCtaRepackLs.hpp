...
public:
  RepackLsStream(cta::Scheduler& scheduler,
  ...
  m_vid(vid) {
    ...
    if (!vid) {
      ...
      m_repackList.sort(
        [](cta::common::dataStructures::RepackInfo repackInfo1, cta::common::dataStructures::RepackInfo repackInfo2) {
  ...
  /*!
    ...
    */
  virtual bool isDone() const { return m_repackList.empty(); }
  ...
  virtual int fillBuffer(XrdSsiPb::OStreamBuffer<Data>* streambuf) override {
    ...
    std::transform(m_repackList.begin(), m_repackList.end(), std::inserter(tapeVids, tapeVids.begin()),
                   [](cta::common::dataStructures::RepackInfo& ri) { return ri.vid; });

    cta::common::dataStructures::VidToTapeMap tapeVidMap =
      m_catalogue.Tape()->getTapesByVid(tapeVids);  // throws an exception if a vid does not exist
    ...
    for (bool is_buffer_full = false; !m_repackList.empty() && !is_buffer_full; m_repackList.pop_front()) {
      ...
      repackRequestItem->set_total_failed_files(repackRequest.failedFilesToRetrieve +
                                                repackRequest.failedFilesToArchive);
      ...
      repackRequestItem->set_last_expanded_fseq(
        repackRequest.lastExpandedFseq != 0 ? repackRequest.lastExpandedFseq - 1 : 0);
