...
std::string DiskSystemList::getDSName(const std::string& fileURL) const {
  ...
  {
    auto pri = std::find_if(m_pointersAndRegexes.begin(),
                            m_pointersAndRegexes.end(),
...
//------------------------------------------------------------------------------
void DiskSystemFreeSpaceList::updateFreeSpaceEntry(const std::string& diskSystemName,
                                                   uint64_t freeSpace,
  ...
  if (updateCatalogue) {
    catalogue.DiskInstanceSpace()->modifyDiskInstanceSpaceFreeSpace(diskInstanceSpace.name,
                                                                    diskInstanceSpace.diskInstance,
                                                                    freeSpace);
  }
}
...
uint64_t DiskSystemFreeSpaceList::fetchFreeDiskSpaceWithScript(const std::string& scriptPath,
  ...
  try {
    sp = std::make_unique<cta::threading::SubProcess>(scriptPath,
                                                      std::list {scriptPath, diskInstanceName, spaceName},
                                                      jsonInput);
  }
