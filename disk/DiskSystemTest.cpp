...
public:
  ...
  public:
    const char* what() const throw() { return "Failed to get catalogue"; }
  };
  ...
  virtual void TearDown() { m_catalogue.reset(); }
...
private:
  void insertDiskSystemSpinner() {
    ...
    catalogue.DiskInstanceSpace()->createDiskInstanceSpace(
      m_cliId, diskInstanceSpaceName, diskInstanceName, freeSpaceQueryURL, refrestInterval, diskInstanceSpaceComment);

    m_diskSystemSpinner.name = "DiskSystemNameSpinner";
    m_diskSystemSpinner.fileRegexp =
      "root://ctaeos.archiveretrieve-1215709git0e38ccd0-xi98.svc.cluster.local//eos/ctaeos/cta(.*)eos.space=spinners";
    m_diskSystemSpinner.targetedFreeSpace = 1;
    m_diskSystemSpinner.sleepTime = 1;
    m_diskSystemSpinner.comment = "Comment";

    catalogue.DiskSystem()->createDiskSystem(m_cliId, m_diskSystemSpinner.name, diskInstanceName, diskInstanceSpaceName,
                                             m_diskSystemSpinner.fileRegexp, m_diskSystemSpinner.targetedFreeSpace,
  ...
  void insertDiskSystemDefault() {
    ...
    m_diskSystemDefault.fileRegexp =
      "root://ctaeos.archiveretrieve-1215709git0e38ccd0-xi98.svc.cluster.local//eos/ctaeos/cta(.*)eos.space=default";
    m_diskSystemDefault.targetedFreeSpace = 1;
    m_diskSystemDefault.sleepTime = 1;
    m_diskSystemDefault.comment = "Comment";

    catalogue.DiskSystem()->createDiskSystem(m_cliId, m_diskSystemDefault.name, diskInstanceName, diskInstanceSpaceName,
                                             m_diskSystemDefault.fileRegexp, m_diskSystemDefault.targetedFreeSpace,
...
TEST_F(DiskSystemTest, getDSNameWithFileDstURLFromEOS) {
  ...
  std::string dstURL = "root://ctaeos.archiveretrieve-1215709git0e38ccd0-xi98.svc.cluster.local//eos/ctaeos/cta/"
                       "54065a67-a3ea-4a44-b213-6f6a6f4e2cf4?eos.lfn=fxid:7&eos.ruid=0&eos.rgid=0&eos.injection=1&eos."
                       "workflow=retrieve_written&eos.space=spinners&toto=5";

  ASSERT_EQ(m_diskSystemSpinner.name, allDiskSystem.getDSName(dstURL));

  dstURL = "root://ctaeos.archiveretrieve-1215709git0e38ccd0-xi98.svc.cluster.local//eos/ctaeos/cta/"
           "54065a67-a3ea-4a44-b213-6f6a6f4e2cf4?eos.lfn=fxid:7&eos.ruid=0&eos.rgid=0&eos.injection=1&eos.workflow="
           "retrieve_written&eos.space=default";
...
TEST_F(DiskSystemTest, fetchDiskSystemFreeSpace) {
  ...
  uint64_t refrestInterval = 1;
  std::string diskInstanceSpaceComment = "Comment";
  catalogue.DiskInstanceSpace()->createDiskInstanceSpace(m_cliId, diskInstanceSpaceName, diskInstanceName,
                                                         freeSpaceQueryURL, refrestInterval, diskInstanceSpaceComment);
  // create disk system
  ...
  catalogue.DiskSystem()->createDiskSystem(m_cliId, constantFreeSpaceDiskSystem.name, diskInstanceName,
                                           diskInstanceSpaceName, constantFreeSpaceDiskSystem.fileRegexp,
                                           constantFreeSpaceDiskSystem.targetedFreeSpace,
                                           constantFreeSpaceDiskSystem.sleepTime, constantFreeSpaceDiskSystem.comment);
