...
def main():
    ...
    if env_var_defined("CUSTOM_XROOTD_VERSION", ci_input_vars):
        ...
        if xrootd_version != project_xrootd_version:
            sys.exit(f"ERROR: CUSTOM_XROOTD_VERSION must be equal to value in project.json ({xrootd_version} != {project_xrootd_version}). Please verify the logic in the modify-project-json job.")
    ...
    if env_var_defined("CUSTOM_EOS_VERSION", ci_input_vars):
        ...
        if eos_version != project_eos_version:
            sys.exit(f"ERROR: CUSTOM_EOS_VERSION must be equal to value in project.json ({eos_version} != {project_eos_version}). Please verify the logic in the modify-project-json job.")

    if env_var_defined("CUSTOM_SYSTEM_TEST_IMAGE_TAG", ci_input_vars):
        check_image_tag_available(ci_input_vars['CUSTOM_SYSTEM_TEST_IMAGE_TAG'], project_json["dev"]["ctaImageRepository"])
    ...
    if env_var_defined("CUSTOM_EOS_IMAGE_TAG", ci_input_vars):
        ...
        if eos_image_tag != project_eos_image_tag:
            sys.exit(f"ERROR: CUSTOM_EOS_IMAGE_TAG must be equal to value in project.json ({eos_image_tag} != {project_eos_image_tag}). Please verify the logic in the modify-project-json job.")
        ...
        # Check that at this point the project.json contains the same version
        if eos_image_tag != project_eos_image_tag:
            sys.exit(f"ERROR: EOS image tag deduced from CUSTOM_EOS_VERSION must be equal to value in project.json ({eos_image_tag} != {project_eos_image_tag}). Please verify the logic in the modify-project-json job.")
