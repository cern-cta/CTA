...
- `cta-pg.Dockerfile` dockerfile for creating an image per service based on locally-built RPMs (compiled with the Postgres scheduler). The images that can be built from this Dockerfile are immutable and are suitable for production.
- `cta.Dockerfile` dockerfile for creating an image per service based on locally-built RPMs (compiled with the Objectstore scheduler). The images that can be built from this Dockerfile are immutable and are suitable for production.
