...
# TODO: This is not working correctly on CI but it works for buildtree. Not needed at the moment but when grpc frontend is ready we will need to restart restart tpsrv-1 pod.
# (
# TIMEOUT=360
# SECS=0
# while [ -z "${MESSAGE}" ]; do
...
# done
#
# kubectl -n ${NAMESPACE} delete pod tpsrv-1
#
# SECS=0
# while test 1 == "$(kubectl -n ${NAMESPACE} get pods | grep tpsrv-1 | wc -l)"; do
#   if test ${SECS} -eq ${TIMEOUT}; then
#     echo "HOST: ERROR. Timed out while waiting for tpsrv-1 to be deleted."
#   fi
#   echo "HOST: Waiting for tpsrv-1 to be deleted..."
#   sleep 1
#   SECS=$(( ${SECS} + 1 ))
# done
#
# # Find the yaml file used to generate the pod.
# tpsrv-1_yaml=$(find /tmp | grep pod-tpsrv-1.yaml)
