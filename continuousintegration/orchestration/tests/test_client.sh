...
kubectl -n ${NAMESPACE} exec client -- bash -c "${TEST_PRERUN} && /root/client_timestamp.sh ${TEST_POSTRUN}" || exit 1
...
# We don't care about the tapesrv logs so we don't need the TEST_[PRERUN|POSTRUN].
