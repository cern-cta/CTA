...
################################################################################
# Define list of test files and the corresponding SURLs.
################################################################################
...
################################################################################
# Trigger gfal2 retrieve workflow
################################################################################

# NB_RETRIEVE times for each file

echo
echo "Trigering gfal2 retrieve workflow as poweruser1:powerusers..."
echo "${NB_RETRIEVES} retrieve requests per file"

# We need the -s as we are staging the files from tape (see xrootd prepare definition)
for ((retrieve_req=0; retrieve_req < ${NB_RETRIEVES}; retrieve_req++)); do
  KRB5CCNAME=/tmp/${EOSPOWER_USER}/krb5cc_0 XrdSecPROTOCOL=krb5 gfal-bringonline --from-file ${TEST_FILES_SURL}
done
...
STARTING_COUNTER_VAL=$((${NB_RETRIEVES}))
...
for ((expected_counter_val=${STARTING_COUNTER_VAL}; expected_counter_val > 0; expected_counter_val--)); do
  ...
  # At the momento we are only doing 1 retrieve.
  #rm -f ${FAILED_LIST}
  #touch ${FAILED_LIST}
