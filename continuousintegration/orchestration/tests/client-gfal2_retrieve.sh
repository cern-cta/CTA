...
for ((subdir=0; subdir < ${NB_DIRS}; subdir++)); do
  ...
  cat ${SURLs} | grep /${subdir}/ | xargs --max-procs=${NB_PROCS} -iTEST_FILE_SURL bash -c "XRD_LOGLEVEL=Dump KRB5CCNAME=/tmp/${EOSPOWER_USER}/krb5cc_0 XrdSecPROTOCOL=krb5 gfal-bringonline TEST_FILE_SURL 2>${ERROR_DIR}/RETRIEVE_${subdir}_\$(echo TEST_FILE_SURL | awk -F/ '{ print \$10 }') && rm ${ERROR_DIR}/RETRIEVE_${subdir}_\$(echo TEST_FILE_SURL | awk -F/ '{ print \$10 }')  || echo Error with gfal-bringonline prepare stage for file TEST_FILE_NAME, full logs in ${ERROR_DIR}/RETRIEVE_${subdir}_\$(echo TEST_FILE_SURL | awk -F/ '{ print \$9 }') " | tee ${LOGDIR}/prepare_${subdir}.log | grep ^ERROR
  echo Done.
