...
test_and_check_cmd "Adding dis 'ctaeos_dis'" "${command}" "add" "-n ctaeos_dis --di ${EOS_INSTANCE} -i 10 -u eosSpace:default -m 'Add dis ctaeos_dis'"\
...
test_and_check_cmd "Changing dis 'ctaeos_dis'" "${command}" "ch" "-n ctaeos_dis --di ${EOS_INSTANCE} -i 100 -m 'Change dis ctaeos_dis'"\
...
test_command "Deleting dis 'ctaeos_dis'" "${command}" "rm" "-n ctaeos_dis --di ${EOS_INSTANCE}" || exit 1
test_assert || exit 1
...
admin_cta dis add  -n ctaeos_dis --di ${EOS_INSTANCE} -i 10 -u eosSpace:default -m 'Add dis ctaeos_dis' || { log_message "Error creating disk instance space for disk system test."; exit 1; }

test_and_check_cmd "Adding ds 'ctaeos_ds'" "${command}" "add" "-n ctaeos_ds --di ${EOS_INSTANCE} --dis ctaeos_dis -r root://${EOS_INSTANCE}//eos/ctaeos/cta/ -f 10000 -s 10 -m 'Adding ds ctaeos_ds'"\
...
admin_cta dis rm -n ctaeos_dis --di ${EOS_INSTANCE}
...
xrdcp /root/testfile root://${EOS_INSTANCE}//eos/ctaeos/cta/testfile
sleep 15
...
eos root://${EOS_INSTANCE} rm /eos/ctaeos/cta/testfile
sleep 1
test_assert || exit 1
...
#KRB5CCNAME=/tmp/${EOSPOWER_USER}/krb5cc_0 XrdSecPROTOCOL=krb5 xrdfs ${EOS_INSTANCE} prepare -s /eos/ctaeos/cta/testfile
...
test_command "Adding repack request" "${command}" "add" "-v ${vids[0]} -m -u repack_ctasystest -b root://${EOS_INSTANCE}//eos/ctaeos/cta"\
