...
if [[ $DONOTARCHIVE == 1 ]]; then
    ...
    eos root://${EOS_INSTANCE} ls -d ${EOS_BASEDIR}/${TARGETDIR} || die "target directory does not exist and there is no archive phase to create it."
fi

if [[ $TAPEAWAREGC == 1 ]]; then
    echo "Enabling tape aware garbage collector"
    ssh ${SSH_OPTIONS} -l root ${EOS_INSTANCE} eos space config default space.filearchivedgc=off || die "Could not disable filearchivedgc"
fi
...
TOKEN=$(eos "root://${EOS_INSTANCE}" token --tree --path '/eos/ctaeos' --expires "${LATER}" --owner user1 --group eosusers --permission rwxd)

TOKEN_EOSPOWER=$(eospower_eos root://"${EOS_INSTANCE}" token --tree --path '/eos/ctaeos' --expires "${LATER}")
