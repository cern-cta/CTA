...
(cd eos; git fetch; git checkout ${EOS_COMMIT})

# Install debuginfo for other dependencies
dnf -y debuginfo-install eos-xrootd-5.8.4
...
# Look into 'build_el9' CI stage for the eos RPM name
gdb_eos_config=/gdb_eos_config
cat > $gdb_eos_config <<EOF
set debuginfod enabled off
set substitute-path /root/rpmbuild/BUILD/5.3.23-20251007020343git0f4721f14 /eos
directory /eos
EOF
