...
def pytest_collection_modifyitems(config, items):
    ...
    if not config.getoption("--no-setup"):
        # Note that the order of these is important -> first to run should be prepended last
        add_test_into_existing_collection("system_tests/test_init_tape_infra.py", items, prepend=True)
        add_test_into_existing_collection("system_tests/test_init_catalogue.py", items, prepend=True)
        add_test_into_existing_collection("system_tests/test_init_kerberos.py", items, prepend=True)
