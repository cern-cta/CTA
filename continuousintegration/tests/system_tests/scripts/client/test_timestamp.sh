...
fileInfoAfterArchive=$(eos root://${EOS_MGM_HOST} fileinfo ${TEST_DIR}/${TEST_FILE_NAME})
...
fileInfoAfterRetrieve=$(eos root://${EOS_MGM_HOST} fileinfo ${TEST_DIR}/${TEST_FILE_NAME})
...
KRB5CCNAME=/tmp/${EOSPOWER_USER}/krb5cc_0 XrdSecPROTOCOL=krb5 xrdfs ${EOS_MGM_HOST} prepare -e ${TEST_DIR}/${TEST_FILE_NAME}
wait_for_evict ${EOS_MGM_HOST} "${TEST_DIR}/${TEST_FILE_NAME}"

fileInfoAfterEvict=$(eos root://${EOS_MGM_HOST} fileinfo ${TEST_DIR}/${TEST_FILE_NAME})
