...
def test_idempotent_prepare(env):
    ...
    no_prepare_dir=f"{env.eos_cta_test_dir}/no_prepare"
...
# TODO: these greps are extremely error-prone. They should include the filename, but that's for later
...
def test_retrieve_queue_cleanup(env):
    ...
    for i in range(num_copies):
        ...
        env.eosmgm[0].exec(f"eos attr set sys.archive.storage_class=ctaStorageClass_{i + 1}_copy {multi_copy_dir}")
