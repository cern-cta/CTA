...
void DriveLsResponseStream::init(const admin::AdminCmd& admincmd) {
    ...
    for (auto dr_it = m_tapeDrives.begin(); dr_it != m_tapeDrives.end();) {
        ...
        if (shouldKeep && !m_listAllDrives) {
            ...
            auto config_it = std::find_if(driveConfigs.begin(),
                        driveConfigs.end(),
            ...
            if (config_it == driveConfigs.end()) {
                m_lc.log(cta::log::ERR, "DriveLsStream::fillBuffer could not find SchedulerBackendName configuration for drive " + dr_it->driveName);
            }
