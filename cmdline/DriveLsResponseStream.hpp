...
cta::xrd::Data DriveLsResponseStream::next() {
    ...
    // Skip if not listing all drives and scheduler backend doesn't match
    if (!m_listAllDrives && m_schedulerBackendName.value_or("") != driveSchedulerBackendName) {
        return next(); // Recursively get the next valid drive
    }
