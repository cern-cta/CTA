...
void TextFormatter::flush() {
  ...
  for (auto& l : m_outputBuffer) {
    if (l.size() != m_colSize.size()) {
      throw std::runtime_error("TextFormatter::flush(): incorrect number of columns");
    }
    for (size_t c = 0; c < l.size(); ++c) {
      if (m_colSize.at(c) < l.at(c).size()) {
        m_colSize[c] = l.at(c).size();
      }
    }
  }
  ...
  for (auto& l : m_outputBuffer) {
    ...
    for (size_t c = 0; c < l.size(); ++c) {
      ...
      if (is_header && c == l.size() - 1 && (l.at(c) == "comment" || l.at(c) == "path" || l.at(c) == "reason")) {
...
void TextFormatter::printAdminLsHeader() {
  push_back("HEADER");
  push_back("user", "c.user", "c.host", "c.time", "m.user", "m.host", "m.time", "comment");
}
...
void TextFormatter::printArchiveRouteLsHeader() {
  push_back("HEADER");
  push_back("storage class", "copy number", "tapepool", "c.user", "c.host", "c.time", "m.user", "m.host", "m.time",
            "comment");
}

void TextFormatter::print(const ArchiveRouteLsItem& arls_item) {
  push_back(arls_item.storage_class(), arls_item.copy_number(), arls_item.tapepool(),
...
void TextFormatter::printFailedRequestLsHeader() {
  push_back("HEADER");
  push_back("object id", "request type", "copy no", "tapepool/vid", "requester", "group", "path");
}
...
void TextFormatter::printFailedRequestLsSummaryHeader() {
  push_back("HEADER");
  push_back("request type", "total files", "total size");
}

void TextFormatter::print(const FailedRequestLsSummary& frls_summary) {
  std::string request_type = frls_summary.request_type() == RequestType::ARCHIVE_REQUEST  ? "archive" :
                             frls_summary.request_type() == RequestType::RETRIEVE_REQUEST ? "retrieve" :
                                                                                            "total";

  push_back(request_type, frls_summary.total_files(), dataSizeToStr(frls_summary.total_size()));
}

void TextFormatter::printGroupMountRuleLsHeader() {
  push_back("HEADER");
  push_back("instance", "group", "policy", "c.user", "c.host", "c.time", "m.user", "m.host", "m.time", "comment");
}
...
void TextFormatter::printListPendingArchivesSummaryHeader() {
  push_back("HEADER");
  push_back("tapepool", "total files", "total size");
}

void TextFormatter::print(const ListPendingArchivesSummary& lpa_summary) {
  push_back(lpa_summary.tapepool(), lpa_summary.total_files(), dataSizeToStr(lpa_summary.total_size()));
}

void TextFormatter::printListPendingRetrievesHeader() {
  push_back("HEADER");
  push_back("vid", "archive id", "copy no", "fseq", "block id", "size", "user", "group", "path");
}
...
void TextFormatter::printListPendingRetrievesSummaryHeader() {
  push_back("HEADER");
  push_back("vid", "total files", "total size");
}

void TextFormatter::print(const ListPendingRetrievesSummary& lpr_summary) {
  push_back(lpr_summary.vid(), lpr_summary.total_files(), dataSizeToStr(lpr_summary.total_size()));
}

void TextFormatter::printLogicalLibraryLsHeader() {
  push_back("HEADER");
  push_back("library", "disabled", "reason", "c.user", "c.host", "c.time", "m.user", "m.host", "m.time", "comment");
}

void TextFormatter::print(const LogicalLibraryLsItem& llls_item) {
  push_back(llls_item.name(), llls_item.is_disabled(), llls_item.disabled_reason(), llls_item.creation_log().username(),
            llls_item.creation_log().host(), timeToStr(llls_item.creation_log().time()),
            llls_item.last_modification_log().username(), llls_item.last_modification_log().host(),
            timeToStr(llls_item.last_modification_log().time()), llls_item.comment());
}
...
void TextFormatter::printRepackLsHeader() {
  ...
  push_back("c.time", "repackTime", "c.user", "vid", "tapepool", "providedFiles", "totalFiles", "totalBytes",
            "filesToRetrieve", "filesToArchive", "failed", "status");
}
...
void TextFormatter::printRequesterMountRuleLsHeader() {
  push_back("HEADER");
  push_back("instance", "username", "policy", "c.user", "c.host", "c.time", "m.user", "m.host", "m.time", "comment");
}
...
void TextFormatter::printTapeLsHeader() {
  push_back("HEADER");
  push_back("vid", "media type", "vendor", "library", "tapepool", "vo", "encryption key name", "capacity", "occupancy",
            "last fseq", "full", "from castor", "state", "state reason", "label drive", "label time", "last w drive",
            "last w time", "w mounts", "last r drive", "last r time", "r mounts", "c.user", "c.host", "c.time",
            "m.user", "m.host", "m.time", "comment");
}
...
void TextFormatter::print(const TapeLsItem& tals_item) {
  ...
  push_back(tals_item.vid(), tals_item.media_type(), tals_item.vendor(), tals_item.logical_library(),
            tals_item.tapepool(), tals_item.vo(), tals_item.encryption_key_name(), dataSizeToStr(tals_item.capacity()),
            dataSizeToStr(tals_item.occupancy()), tals_item.last_fseq(), tals_item.full(), tals_item.from_castor(),
            tals_item.state(), state_reason, tals_item.has_label_log() ? tals_item.label_log().drive() : "",
...
void TextFormatter::print(const TapeFileLsItem& tfls_item) {
  ...
  push_back(tfls_item.af().archive_id(), tfls_item.tf().copy_nb(), tfls_item.tf().vid(), tfls_item.tf().f_seq(),
            tfls_item.tf().block_id(), tfls_item.df().disk_instance(), fxid.str(), dataSizeToStr(tfls_item.af().size()),
            checksumType, checksumValue, tfls_item.af().storage_class(), tfls_item.df().owner_id().uid(),
            tfls_item.df().owner_id().gid(), timeToStr(tfls_item.af().creation_time()));
}
...
void TextFormatter::print(const TapePoolLsItem& tpls_item) {
  uint64_t avail =
    tpls_item.capacity_bytes() > tpls_item.data_bytes() ? tpls_item.capacity_bytes() - tpls_item.data_bytes() : 0;

  double use_percent =
    tpls_item.capacity_bytes() > 0 ?
      (static_cast<double>(tpls_item.data_bytes()) / static_cast<double>(tpls_item.capacity_bytes())) * 100.0 :
      0.0;
  ...
  push_back(tpls_item.name(), tpls_item.vo(), tpls_item.num_tapes(), tpls_item.num_partial_tapes(),
            ...
            tpls_item.encrypt(), tpls_item.supply(), tpls_item.created().username(), tpls_item.created().host(),
...
void TextFormatter::printDiskInstanceLsHeader() {
  push_back("HEADER");
  push_back("name", "c.user", "c.host", "c.time", "m.user", "m.host", "m.time", "comment");
}
...
void TextFormatter::printVirtualOrganizationLsHeader() {
  push_back("HEADER");
  push_back("name", "read max drives", "write max drives", "max file size", "disk instance", "c.user", "c.host",
...
void TextFormatter::print(const VirtualOrganizationLsItem& vols_item) {
  push_back(vols_item.name(), vols_item.read_max_drives(), vols_item.write_max_drives(),
            dataSizeToStr(vols_item.max_file_size()), vols_item.diskinstance(), vols_item.creation_log().username(),
            vols_item.creation_log().host(), timeToStr(vols_item.creation_log().time()),
            vols_item.last_modification_log().username(), vols_item.last_modification_log().host(),
            timeToStr(vols_item.last_modification_log().time()), vols_item.comment());
}
...
void TextFormatter::print(const RecycleTapeFileLsItem& rtfls_item) {
  ...
  push_back(rtfls_item.archive_file_id(), rtfls_item.copy_nb(), rtfls_item.vid(), rtfls_item.fseq(),
            rtfls_item.block_id(), rtfls_item.disk_instance(), fxid.str(), dataSizeToStr(rtfls_item.size_in_bytes()),
            checksumType, checksumValue, rtfls_item.storage_class(), rtfls_item.disk_file_uid(),
            rtfls_item.disk_file_gid(), timeToStr(rtfls_item.recycle_log_time()), rtfls_item.disk_file_path(),
