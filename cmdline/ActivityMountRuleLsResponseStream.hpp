...
void ActivityMountRuleLsResponseStream::init(const admin::AdminCmd& admincmd) {
    // ActivityMountRuleLs command doesn't take any parameters, just get all activity mount rules
    auto activityMountRules = m_catalogue.RequesterActivityMountRule()->getRequesterActivityMountRules();
    
    // Convert list to our internal list
    m_activityMountRules = std::move(activityMountRules);
}
