...
//------------------------------------------------------------------------------
void EosNamespaceInjection::updateDiskFileIdAndDiskInstanceInCatalogue(const std::string& archiveId,
...
void EosNamespaceInjection::compareJsonAndCtaMetaData(const MetaDataObject& metaDataJson,
  ...
  compareValues(metaDataJson.size,
                metaDataCatalogue.size,
                "Error: File size does not match when comparing the provided json file and the catalogue.");
  compareValues(metaDataJson.checksumType,
                metaDataCatalogue.checksumType,
                "Error: Checksum type does not match when comparing the provided json file and the catalogue.");
  compareValues(metaDataJson.checksumValue,
                metaDataCatalogue.checksumValue,
...
EosNamespaceInjection::getArchiveFileIdAndChecksumFromEOS(const std::string& diskInstance,
...
bool EosNamespaceInjection::checkEosCtaConsistency(const uint64_t& archiveId,
  ...
  if (eosArchiveFileId == archiveId && eosChecksum == ctaChecksum
      && g_metaDataObjectCatalogue.diskFileId == newDiskFileId) {
