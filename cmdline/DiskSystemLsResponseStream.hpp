...
void DiskSystemLsResponseStream::init(const admin::AdminCmd& admincmd) {
    // DiskSystemLs command doesn't take any parameters, just get all disk systems
    auto diskSystemList = m_catalogue.DiskSystem()->getAllDiskSystems();
    
    // Convert DiskSystemList to our internal list
    for (const auto& diskSystem : diskSystemList) {
        m_diskSystems.emplace_back(diskSystem);
    }
}
