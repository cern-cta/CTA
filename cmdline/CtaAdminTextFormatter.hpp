...
public:
   ...
   */
  explicit TextFormatter(unsigned int bufLines = 1000) : m_bufLines(bufLines) { m_outputBuffer.reserve(bufLines); }
...
private:
  ...
  bool m_lastColumnFlushLeft = false;                    //!< Flag indicating if last collumn should be aligned left
