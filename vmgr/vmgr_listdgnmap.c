...
vmgr_listdgnmap(int flags, vmgr_list *listp)
{
	int bol = 0;
	int c;
	char func[16];
	gid_t gid;
	int listentsz = sizeof(struct vmgr_tape_dgnmap);
	...
	if (listp->nbentries == 0	/* no data in the cache */
		...
		if (flags == VMGR_LIST_END) {
			...
			marshall_LONG (sbp, VMGR_LISTDGNMAP);
		}
		...
		/* unmarshall reply into vmgr_tape_dgnmap structures */

		listp->nbentries = nbentries;
		lp = (struct vmgr_tape_dgnmap *) listp->buf;
		while (nbentries--) {
	...
	lp = ((struct vmgr_tape_dgnmap *) listp->buf) + listp->index;
	listp->index++;
