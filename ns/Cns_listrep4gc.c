...
#ifndef lint
static char sccsid[] = "@(#)$RCSfile: Cns_listrep4gc.c,v $ $Revision: 1.2 $ $Date: 2006/01/26 15:36:18 $ CERN IT-GD/CT Jean-Philippe Baud";
#endif /* not lint */
...
Cns_listrep4gc(const char *poolname, int flags, Cns_list *listp)
{
	int bol = 0;
	int c;
	char func[15];
	...
	if (strlen (poolname) > CA_MAXPOOLNAMELEN) {
		serrno = EINVAL;
		return (NULL);
	}
	...
	if (listp->len == 0	/* no data in the cache */
		...
		if (flags == CNS_LIST_END) {
			...
			marshall_LONG (sbp, CNS_LISTREP4GC);
		}
