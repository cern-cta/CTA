                            ...
                            struct Cns_srv_request_info *reqinfo)
{
  ...
  sprintf (reqinfo->logbuf, "LastModTime=%lld NewVid=\"%s\" OldVid=\"%s\"",
           (long long int)last_mod_time, newvid, oldvid);
  ...
  for (i = 0; i < nbseg; i++){
    nslogit("MSG=\"New segment information\" REQID=%s NSHOSTNAME=%s "
            ...
            new_smd_entry[i].side, new_smd_entry[i].fseq,
            new_smd_entry[i].blockid[0], new_smd_entry[i].blockid[1],
                      ...
                      struct Cns_srv_request_info *reqinfo)
{
  ...
  /* Construct log message */
  sprintf (reqinfo->logbuf,
           "Guid=\"%s\" FileSize=%llu ChecksumType=\"%s\" ChecksumValue=\"%s\" "
           "NewModTime=%lld LastModTime=%lld",
           guid, filesize, csumtype, csumvalue, (long long int)new_mod_time,
           (long long int)last_mod_time);
                        ...
                        struct Cns_srv_request_info *reqinfo)
{
  ...
  /* Construct log message */
  get_cwd_path (thip, cwd, cwdpath);
  sprintf (reqinfo->logbuf, "Cwd=\"%s\" Path=\"%s\" LastModTime=%lld",
           cwdpath, path, (long long int)last_mod_time);
  ...
  /* Check that the file in nameserver is not newer than what is given This can
   * happen in case of multiple stagers concurrently modifying a file. In such
   * a case, raise the appropriate error and ignore the request.
   */
  if ((filentry.mtime > last_mod_time) && (last_mod_time > 0))
    RETURN (ENSFILECHG);
  ...
  for (i = 0; i < nbseg; i++) {
    ...
    if ((magic >= CNS_MAGIC4) && (nbseg == 1)) {
      ...
      /* Checking for a checksum in the file metadata table if we have only one
       * segment for a file. We have different names for checksum type in
       * Cns_seg_metadata table and Cns_file_metadata. adler32==AD
       */
    ...
    if (Cns_insert_smd_entry (&thip->dbfd, &smd_entry)) {
      if (serrno != EEXIST ||
          Cns_get_smd_by_fullid (&thip->dbfd,
                                 ...
                                 smd_entry.fsec, &old_smd_entry, 1,
                                 &rec_addrs) ||
  ...
  /* Delete old segments if they were too numerous */
  fsec = nbseg + 1;
