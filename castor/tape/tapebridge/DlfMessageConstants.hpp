...
TAPEBRIDGE_RECEIVED_NOMOREFILES=106, /* "Received NoMoreFiles from client" */
TAPEBRIDGE_RECEIVED_ACK_OF_NOTIFICATION=107, /* "Received acknowledge of notification from client" */
...
TAPEBRIDGE_DROPPING_RTCP_FINISHED_DUE_TO_SHUTTING_DOWN=149, /* "Dropping RTCP_FINISHED message from rtcpd daemon because the rtcpd session is being shutdown" */
TAPEBRIDGE_DROPPING_TAPEBRIDGE_FLUSHEDTOTAPE_DUE_TO_SHUTTING_DOWN=150, /* "Dropping TAPEBRIDGE_FLUSHEDTOTAPE message from rtcpd daemon because the rtcpd session is being shutdown" */
TAPEBRIDGE_FIRST_SESSION_ERROR=151, /* "The first session-error has occurred" */
TAPEBRIDGE_NO_MOUNT_DUE_TO_NO_TAPE=152, /* "Mount not started because the client did not give the tape to be mounted" */
TAPEBRIDGE_EMPTY_MIGRATION_MOUNT=153, /* "Empty migration mount" */
TAPEBRIDGE_EMPTY_RECALL_MOUNT=154 /* "Empty recall mount" */
}; // enum TapeBridgeDlfMessages
} // namespace tapebridge
} // namespace tape
} // namespace castor
