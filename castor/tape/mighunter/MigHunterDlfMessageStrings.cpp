...
{NOT_ENOUGH_DATA_TO_CREATE_STREAMS, "not enough data to create streams"},
{NO_DRIVES, "no drive assigned to this service class"},
{MIGRATION_POLICY_INPUT, "input to call the migration policy"},
...
{GRACEFUL_SHUTDOWN_DUE_TO_ERROR, "Graceful shutdown due to error"},
{APPLY_STREAM_POLICY_TO_SVCCLASS_EXCEPT, "Exception raised when applying stream-policy to service-class"},
{GOT_TAPE_POOLS_FOR_STREAM_POLICY, "Got the tape-pools for the stream-policy"},
{FAILED_TO_FIND_TAPE_POOL_FOR_STREAM, "Failed to find tape-pool for stream"},
{FAILED_TO_MOVE_CREATED_STREAMS_TO_STOPPED, "Failed to move created streams with tape-copies to stopped"},
{FOUND_CREATED_STREAMS_WITH_TAPECOPIES, "Found newly-created streams with tape-copies attached to them"},
{-1, ""}};
