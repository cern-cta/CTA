/******************************************************************************
 *                      NameServerSynchronizationThread.cpp
 *
 ...
 *
 * @(#)$RCSfile: NameServerSynchronizationThread.cpp,v $ $Revision: 1.1 $ $Release$ $Date: 2008/02/29 11:06:12 $ $Author: mmartins $
 *
...
// Include files
#include "castor/gc/NameServerSynchronizationThread.hpp"
...
// Definitions
#define DEFAULT_NS_GCINTERVAL 300
#define DEFAULT_NS_SYNCINTERVAL 10
#define DEFAULT_NS_CHUNKSIZE 1000


//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
castor::gc::NameServerSynchronizationThread::NameServerSynchronizationThread() {
  
  syncIntervalConf = "NS_SYNCINTERVAL";
  syncInterval = DEFAULT_NS_GCSYNCINTERVAL;
  chunkSizeConf = "NS_CHUNKSIZE";
  chunkSize = DEFAULT_NS_CHUNKSIZE;
...
// Run
//-----------------------------------------------------------------------------
void castor::gc::NameServerSynchronizationThread::run(void *param) {  

  // "Starting Garbage Collector Daemon" message
  castor::dlf::Param param[] =
    { castor::dlf::Param("GC SynchronizationThread:","Name Server")};
  castor::dlf::dlf_writep(nullCuuid, DLF_LVL_SYSTEM, 18, 1, param);
  
  // Get the synchronization interval and chunk size
  unsigned int syncInterval = DEFAULT_NS_SYNCINTERVAL;
  unsigned int chunkSize = DEFAULT_NS_CHUNKSIZE;
  readConfigFile(true);
	  ...
	  if (stat(filepath.c_str(), &filebuf) < 0) {
	    ...
	    continue;  /* not a file */
		  ...
		  castor::dlf::Param("NameServer", fid.first)};
              castor::dlf::dlf_writep(nullCuuid, DLF_LVL_DEBUG, 31, 4, params);
...
castor::gc::NameServerSynchronizationThread::fileIdFromFileName
...
// synchronizeFiles
//-----------------------------------------------------------------------------
void castor::gc::NameServerSynchronizationThread::synchronizeFiles
(std::string nameServer,
 ...
 const std::map<u_signed64, std::string> &paths) throw() {
  ...
  if (0 == cfids) {
    ...
    castor::dlf::dlf_writep(nullCuuid, DLF_LVL_SYSTEM, 29, 3, params);
  }  
  ...
  castor::dlf::dlf_writep(nullCuuid, DLF_LVL_SYSTEM, 33, 3, params);
  ...
  for (std::vector<u_signed64>::const_iterator it = orphans.begin();
	...
	{ castor::dlf::Param("GC SynchronizationThread:","Name Server"),
	  castor::dlf::Param("FileName", paths.find(*it)->second),
	  castor::dlf::Param("Error", strerror(errno))};
      castor::dlf::dlf_writep(nullCuuid, DLF_LVL_SYSTEM, 28, 3, params, &fid);
    }
  }
}
