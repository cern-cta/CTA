        ...
        if jobtype == 'd2dsource':
            ...
            for machine in self._jobsLocations[jobid]:
                if machine != diskserver:
                    if verbose: print 'Calling d2dSourceReady on ' + machine + ' for jobid ' + jobid
            ...
            if jobid not in self._jobsLocations:
                ...
                if verbose: print "Race condition detected : this job has already started : " + jobid
                self._lock.release()
                raise ValueError
            # if a destination job wants to start, check whether the source is ready
            if jobtype == 'd2ddest' and not self[diskserver][jobid][3]:
                if verbose: print "Source is not ready yet for jobid " + jobid + ' and diskserver ' + diskserver
                self._lock.release()
            ...
            while True:
                ...
                if srId.getvalue() != None:
                    ...
                    if int(reqType.getvalue() == 133): # OBJ_StageDiskCopyReplicaRequest
                        ...
                        print jobid + '(d2d source) : ', schedSourceCandidates[0]
                        ...
                        print jobid + '(d2d destinations) : ', schedDestCandidates
...
class SchedulerService(rpyc.Service):
    ...
    def exposed_bhosts(self, svcClass=None):
        ...
        if verbose: print 'bhosts called for svcClass', svcClass
    ...
    def exposed_badmin(self, svcClass=None):
        ...
        if verbose: print 'badmin called for svcClass', svcClass
    ...
    def exposed_bjobs(self, svcClass=None):
        '''bqueues lists the number of running and pending jobs per queue'''
        if verbose: print 'bjobs called for svcClass', svcClass
    ...
    def exposed_jobStarting(self, diskserver, jobid, jobtype):
        ...
        if verbose: print 'jobStarting called for job ' + jobid + ' (' + jobtype + ') on ' + diskserver
    ...
    def exposed_getQueuingJobs(self, diskserver):
        ...
        if verbose: print 'getQueuingJobs called for ' + diskserver
    ...
    for svcClass in dslist:
        for ds in dslist[svcClass]:
            try:
                ...
                print 'No queue could be retrieved from ' + ds + ' (' + str(e) + ')'
                pass
