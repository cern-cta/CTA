...
//------------------------------------------------------------------------------
static castor::CnvFactory<castor::io::StreamDumpParametersCnv>
  s_factoryStreamDumpParametersCnv;
...
void castor::io::StreamDumpParametersCnv::createRep(castor::IAddress* address,
  ...
  ad->stream() << obj->errAction();
  ad->stream() << obj->startFile();
  ad->stream() << obj->maxFile();
  ad->stream() << obj->fromBlock();
  ad->stream() << obj->toBlock();
  ad->stream() << obj->id();
}
...
castor::IObject* castor::io::StreamDumpParametersCnv::createObj(castor::IAddress* address)
  ...
  int errAction;
  ad->stream() >> errAction;
  object->setErrAction(errAction);
  int startFile;
  ad->stream() >> startFile;
  object->setStartFile(startFile);
  int maxFile;
  ad->stream() >> maxFile;
  object->setMaxFile(maxFile);
  int fromBlock;
  ad->stream() >> fromBlock;
  object->setFromBlock(fromBlock);
  int toBlock;
  ad->stream() >> toBlock;
  object->setToBlock(toBlock);
  u_signed64 id;
  ad->stream() >> id;
  object->setId(id);
  return object;
