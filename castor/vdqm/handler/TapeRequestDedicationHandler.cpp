...
void castor::vdqm::handler::TapeRequestDedicationHandler::run() {
  ...
  while ( !_stopLoop ) {
    ...
    try {
       ...
       */
      ptr_IVdqmService->OLDmatchTape2TapeDrive(&freeTapeDrive, &waitingTapeRequest);
...
void castor::vdqm::handler::TapeRequestDedicationHandler::handleDedication(
   ...
   */
  castor::vdqm::DatabaseHelper::updateRepresentation(freeTapeDrive, nullCuuid);
  castor::vdqm::DatabaseHelper::updateRepresentation(waitingTapeRequest,
    nullCuuid);  
...
void *castor::vdqm::handler::TapeRequestDedicationHandler::dedicationRequest(
  ...
  if (0 != freeTapeDrive) {
    ...
    try {
      ...
      if ( !rtcpConnection.OLDsendJobToRTCPD(freeTapeDrive) ) {
