...
const std::string castor::db::cnv::DbGCFileCnv::s_bulkInsertStatementString =
"INSERT /* bulk */ INTO GCFile (diskCopyId, id, request) VALUES (:1,ids_seq.nextval,:2) RETURNING id INTO :3";
...
const std::string castor::db::cnv::DbGCFileCnv::s_bulkSelectStatementString =
"DECLARE \
   TYPE RecordType IS RECORD (diskCopyId INTEGER, id INTEGER, request INTEGER); \
   ...
   BEGIN \
     ...
     OPEN objs FOR SELECT diskCopyId, id, request \
                     FROM GCFile t, bulkSelectHelper h \
                    WHERE t.id = h.objId; \
...
const std::string castor::db::cnv::DbGCFileCnv::s_storeTypeBulkStatementString =
...
// ObjType
//------------------------------------------------------------------------------
unsigned int castor::db::cnv::DbGCFileCnv::ObjType() {
...
// objType
//------------------------------------------------------------------------------
unsigned int castor::db::cnv::DbGCFileCnv::objType() const {
  return ObjType();
}
...
// fillRep
//------------------------------------------------------------------------------
void castor::db::cnv::DbGCFileCnv::fillRep(castor::IAddress*,
...
// fillObj
//------------------------------------------------------------------------------
void castor::db::cnv::DbGCFileCnv::fillObj(castor::IAddress*,
                                           castor::IObject* object,
                                           unsigned int type,
                                           bool endTransaction)
  throw (castor::exception::Exception) {
...
// createRep
//------------------------------------------------------------------------------
void castor::db::cnv::DbGCFileCnv::createRep(castor::IAddress*,
                                             castor::IObject* object,
                                             bool endTransaction,
                                             unsigned int type)
  throw (castor::exception::Exception) {
...
// bulkCreateRep
//------------------------------------------------------------------------------
void castor::db::cnv::DbGCFileCnv::bulkCreateRep(castor::IAddress*,
  ...
  if (0 == nb) return;
  // Casts all objects
  std::vector<castor::stager::GCFile*> objs;
  for (int i = 0; i < nb; i++) {
    objs.push_back(dynamic_cast<castor::stager::GCFile*>(objects[i]));
  }
  ...
  try {
    ...
    m_bulkInsertStatement->setDataBuffer
      (1, diskCopyIdBuffer, castor::db::DBTYPE_UINT64, sizeof(diskCopyIdBuffer[0]), diskCopyIdBufLens);
    ...
    for (int i = 0; i < nb; i++) {
      requestBuffer[i] = (type == OBJ_GCFileList && objs[i]->request() != 0) ? objs[i]->request()->id() : 0;
