...
const std::string castor::db::cnv::DbStageRequestQueryRequestCnv::s_bulkSelectStatementString =
"DECLARE \
   TYPE CurType IS REF CURSOR RETURN StageRequestQueryRequest%ROWTYPE; \
   ...
   BEGIN \
     ...
     OPEN objs FOR SELECT flags, userName, euid, egid, mask, pid, machine, svcClassName, userTag, reqId, creationTime, lastModificationTime, id, svcClass, client \
                     FROM StageRequestQueryRequest t, bulkSelectHelper h \
                    WHERE t.id = h.objId; \
...
// bulkCreateRep
//------------------------------------------------------------------------------
void castor::db::cnv::DbStageRequestQueryRequestCnv::bulkCreateRep(castor::IAddress* address,
  ...
  if (0 == nb) return;
  // Casts all objects
  std::vector<castor::stager::StageRequestQueryRequest*> objs;
  for (int i = 0; i < nb; i++) {
    objs.push_back(dynamic_cast<castor::stager::StageRequestQueryRequest*>(objects[i]));
  }
  try {
    // Check whether the statements are ok
    if (0 == m_insertStatement) {
...
castor::db::cnv::DbStageRequestQueryRequestCnv::bulkCreateObj(castor::IAddress* address)
  throw (castor::exception::Exception) {
  // Prepare result
