...
const std::string castor::db::cnv::DbQueryParameterCnv::s_bulkSelectStatementString =
"DECLARE \
   TYPE RecordType IS RECORD (value VARCHAR2(2048), id INTEGER, query INTEGER, queryType INTEGER); \
   ...
   BEGIN \
     ...
     OPEN objs FOR SELECT value, id, query, queryType \
                     FROM QueryParameter t, bulkSelectHelper h \
                    WHERE t.id = h.objId; \
...
void castor::db::cnv::DbQueryParameterCnv::bulkCreateRep(castor::IAddress*,
  ...
  if (0 == nb) return;
  // Casts all objects
  std::vector<castor::stager::QueryParameter*> objs;
  for (int i = 0; i < nb; i++) {
  ...
  try {
    ...
    for (int i = 0; i < nb; i++) {
      if (objs[i]->value().length()+1 > valueMaxLen)
