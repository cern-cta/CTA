...
/*
 * Csec_api_common.c - Common function in the Csecurity API 
 */
...
int Csec_setup_trace() {
  ...
  if (envar != NULL) {
    ...
    if (envar != NULL) {
      strncpy(thip->trace_file, envar, CA_MAXNAMELEN-1);
    }
  }
...
int Csec_trace(char *func, char *msg, ...) {
  ...
  if (thip->trace_file[0] != '\0') {
    int fd;
    fd = open(thip->trace_file, O_CREAT|O_WRONLY|O_APPEND);
    if (fd <0) return -1;
...
/**
 * Sends a gss_buffer_t over a socket
 */
     ...
     int token_type;
{
  ...
  Csec_trace(func, "Sending packet Magic: %xd Type: %d, Len: %d\n",
	     magic, type, datalen);

  p = headbuf;
  marshall_LONG(p, htonl(magic));
  marshall_LONG(p, htonl(type));
  marshall_LONG(p, htonl(datalen));
  headlen =  p - headbuf;
  ...
  if (ret < 0) {
    ...
    Csec_errmsg(func, "Error sending token length");
    return -1;
  } else if (ret != headlen) {
    serrno = ESEC_SYSTEM;
  ...
  if (ret < 0) {
    /* We keep the serrno from netwrite */
    Csec_errmsg(func, "Error sending token data");
    return -1;
  } else if (ret != tok->length) {
    serrno = ESEC_SYSTEM;
    Csec_errmsg(func, "Bad token data length");
    return -1;
  }
     ...
     int *rtype;
{
  ...
  if (magic != CSEC_TOKEN_MAGIC_1) {
    ...
    Csec_errmsg(func, "Received magic: %d expecting %d\n",
  ...
  if (data_already_read>0) {
    memcpy(prefetched_data, tok->value, data_already_read);
  }
