...
[[ $(echo ${WELL_KNOWN} | jq -r .sitename | wc -l) -eq 1 ]] && echo "Site name:  $(echo ${WELL_KNOWN} | jq -r .sitename)"  || { echo "ERROR: Sitename not in the response from .well-known" >&2; exit 1; }
echo "Description: $(echo ${WELL_KNOWN} | jq -r .description)"
[[ $(echo ${WELL_KNOWN} | jq -r .endpoints[0].uri | wc -l) -eq 1 ]] && echo "URI:       $(echo ${WELL_KNOWN} | jq -r .endpoints[0].uri)" || { echo "ERROR: URI not found in the response from .well-known." >&2; exit 1; }
[[ $(echo ${WELL_KNOWN} | jq -r .endpoints[0].version | wc -l) -eq 1 ]] && echo "Version:   $(echo ${WELL_KNOWN} | jq -r .endpoints[0].version)" || { echo "ERROR: Metadata not found in the response from .well-known" >&2; exit 1;}
echo "Metadata: $(echo ${WELL_KNOWN} | jq -r .endpoints[0].metadata)"

# Archive the file.

eos root://"${EOS_MGM_HOST}" rm $EOS_BASEDIR/test_http-rest-api 2>/dev/null # Delete the file if present so that we can run the test multiple times
INIT_COUNT=$(eos root://"${EOS_MGM_HOST}" ls -y $EOS_BASEDIR/ | grep test_http-rest-api | wc -l)
[[ "${INIT_COUNT}" -eq 0 ]] || { echo "Test file test_http-rest-api already in EOS before archiving."; exit 1; }
tmp_file=$(mktemp)
echo "Dummy" > "${tmp_file}"

curl -L --insecure -H "Accept: application/json" -H "Authorization: Bearer ${TOKEN_EOSUSER1}" "https://${EOS_MGM_HOST}:8443$EOS_BASEDIR/test_http-rest-api" --upload-file "${tmp_file}"
...
while [[ "${FINAL_COUNT}" -eq 0 ]]; do
  ...
  FINAL_COUNT=$(eos root://"${EOS_MGM_HOST}" ls -y $EOS_BASEDIR | grep "test_http-rest-api" | grep 'd0::t1' | wc -l)
...
eos root://"${EOS_MGM_HOST}" ls -y $EOS_BASEDIR

# Archive Info Request information about the progression of writing files
# to tape.
echo "$(date +%s): Showing archiveinfo..."
curl ${CURL_OPTS} -L --capath /etc/grid-security/certificates -H "Accept: application/json" -H "Authorization: Bearer ${TOKEN_EOSPOWER1}" ${HTTPS_URI}/archiveinfo/ -d "{\"paths\":[\"$EOS_BASEDIR/test_http-rest-api\"]}" | jq .

# Stage
# Request that tape-stored files are made available with disk latency.

REQ_ID=$(curl ${CURL_OPTS} -L --capath /etc/grid-security/certificates -H "Accept: application/json" -H "Authorization: Bearer ${TOKEN_EOSPOWER1}" ${HTTPS_URI}/stage/ -d "{\"files\":[{\"path\":\"$EOS_BASEDIR/test_http-rest-api\"}]}" | jq -r .requestId)
...
while [[ "${FINAL_COUNT}" -eq 0 ]]; do
  ...
  FINAL_COUNT=$(eos root://"${EOS_MGM_HOST}" ls -y $EOS_BASEDIR | grep 'test_http-rest-api' | grep 'd1::t1' | wc -l)
...
curl ${CURL_OPTS} -L --capath /etc/grid-security/certificates -H "Accept: application/json" -H "Authorization: Bearer ${TOKEN_EOSPOWER1}" "${HTTPS_URI}/release/${REQ_ID}" -d "{\"paths\":[\"$EOS_BASEDIR/test_http-rest-api\"]}"

EVICT_COUNT=$(eos root://"${EOS_MGM_HOST}" ls -y $EOS_BASEDIR | grep 'test_http-rest-api' | grep 'd0::t1' | wc -l)
...
OLDIFS=$IFS
IFS='::'
tmp=$(eos root://"${EOS_MGM_HOST}" ls -y $EOS_BASEDIR | grep test_http-rest-api | awk '{ print $1 }')
split1=( $tmp )
EVICT_INIT=${split1[0]}
...
curl ${CURL_OPTS} -L --capath /etc/grid-security/certificates -H "Accept: application/json" -H "Authorization: Bearer ${TOKEN_EOSPOWER1}" "${HTTPS_URI}/stage/${REQ_ID}/cancel/" -d "{\"paths\":[\"$EOS_BASEDIR/test_http-rest-api\"]}"
...
eos root://"${EOS_MGM_HOST}" rm $EOS_BASEDIR/test_http-rest-api
IFS=$OLDIFS
