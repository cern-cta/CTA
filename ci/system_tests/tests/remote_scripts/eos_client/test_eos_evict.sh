...
if [[ 0 == "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE}  | jq -r '.locations[] | select(.schedgroup | startswith("default")) | .schedgroup' | wc -l)" ]]; then
...
if [[ 0 == "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE} | jq -r '.locations[] | select(.schedgroup | startswith("tape")) | .schedgroup' | wc -l)" ]]; then
...
while [[ 0 == "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE}  | jq -r '.locations[] | select(.schedgroup | startswith("default")) | .schedgroup' | wc -l)" ]]; do
...
for ((retrieve_req=${NB_RETRIEVES}; retrieve_req > 0; retrieve_req--)); do
  ...
  if [[ ${retrieve_req} != "$(eospower_eos --json root://${EOS_MGM_HOST} attr get sys.retrieve.evict_counter ${TEMP_FILE} | jq -r '.attr.get[].sys.retrieve.evict_counter')" ]]; then
...
if [[ 4 != "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE} | jq -r '.locations[] | .schedgroup' | wc -l)" ]]; then
...
if [[ 3 != "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE} | jq -r '.locations[] | .schedgroup' | wc -l)" ]]; then
  error "The number of replicas is not the expected one"
else
  echo "Disk replica count is as expected"
fi
...
if [[ 2 != "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE} | jq -r '.locations[] | .schedgroup' | wc -l)" ]]; then
  error "The number of replicas is not the expected one"
else
  echo "Disk replica count is as expected"
fi
...
if [[ 1 != "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE} | jq -r '.locations[] | .schedgroup' | wc -l)" ]]; then
  error "The number of replicas is not the expected one"
else
  echo "Disk replica count is as expected"
fi
if [[ 1 != "$(eospower_eos --json root://${EOS_MGM_HOST} info ${TEMP_FILE} | jq -r '.locations[] | select(.schedgroup | startswith("tape")) | .schedgroup' | wc -l)" ]]; then
