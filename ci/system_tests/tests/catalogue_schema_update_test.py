...
def test_init_catalogue_updater(
    ...
    top_path = Path(env.exec_local("git rev-parse --show-toplevel").stdout)
    ...
    if catalogue_schema_update_params.schema_checkout_ref:
        ...
        catalogue_schema_path = (
            Path(env.exec_local("git rev-parse --show-toplevel").stout) / "catalogue" / "cta-catalogue-schema"
        ).resolve()
        catalogue_schema_ref = env.exec_local(f"git -C {catalogue_schema_path} rev-parse --short HEAD").stdout
...
def test_liquibase_update(cta_frontend, catalogue_updater, catalogue_to_version):
    result = catalogue_updater.exec("/launch_liquibase.sh update", capture_output=True, throw_on_failure=False)
    assert result.success, "Liquibase update failed\n" f"stdout:\n{result.stdout}\n" f"stderr:\n{result.stderr}"
