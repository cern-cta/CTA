...
class VersionTest(unittest.TestCase):
    ...
    def test_parses_canonical_versions(self) -> None:
        version = CTAVersion.parse("v6.10.11.0-1.rc1")
        assert version.core == (6, 10, 11, 0, 1)
    ...
    def test_constructs_supported_tag_components(self) -> None:
        release_version = CTAVersion.parse("v6.10.11.0-1", require_base=True)
        assert release_version.with_components(variant=BuildVariant.PGSCHED).text == "v6.10.11.0-1.pgsched"
        assert (
            release_version.with_components(release_candidate=2, variant=BuildVariant.PGALL).text
            == "v6.10.11.0-1.rc2.pgall"
        )
    ...
    def test_rejects_unsupported_or_manual_suffixes(self) -> None:
        for value in (
            ...
            "v6.10.11.0-1.pg1",
            "v6.10.11.0-1.rc0",
            "v6.10.11.0-1.pgsched.rc1",
        ):
            with self.subTest(value=value), pytest.raises(VersionError):
                CTAVersion.parse(value)

    def test_requires_unsuffixed_command_input(self) -> None:
        with pytest.raises(VersionError, match=r"release tag v6\.10\.11\.0-1 --suffix pgsched"):
            CTAVersion.parse("v6.10.11.0-1.pgsched", require_base=True)

    def test_manually_appended_release_candidate_suggests_option(self) -> None:
        with pytest.raises(VersionError, match=r"release tag v6\.10\.11\.0-1 --release-candidate"):
            CTAVersion.parse("v6.10.11.0-1.rc2", require_base=True)
    ...
    def test_rc_selection_handles_complete_and_partial_families(self) -> None:
        ...
        assert select_release_candidate(release_version, ["v6.10.11.0-1.rc1"]) == 2
        assert (
            select_release_candidate(
                release_version,
                ["v6.10.11.0-1.rc2", "v6.10.11.0-1.rc2.pgsched"],
            )
            == 3
        )
    ...
    def test_explicit_variant_starts_a_new_rc_family(self) -> None:
        release_version = CTAVersion.parse("v6.10.11.0-1", require_base=True)
        assert select_release_candidate(release_version, ["v6.10.11.0-1.rc3"]) == 4
        assert (
            select_release_candidate(
                release_version,
                ["v6.10.11.0-1.rc3.pgsched"],
            )
            == 4
        )
    ...
    def test_rc_family_always_allocates_after_existing_base(self) -> None:
        ...
        assert (
            select_release_candidate(
                release_version,
                ["v6.10.11.0-1.rc3", "v6.10.11.0-1.rc3.pgsched"],
            )
            == 4
        )

    def test_previous_release_uses_numeric_order(self) -> None:
        tags = ["not-a-release", "v6.9.99.0-1", "v6.10.2.0-1", "v6.10.11.0-1.rc1"]
        result = previous_release(CTAVersion.parse("v6.10.12.0-1"), tags)
        assert result.text == "v6.10.11.0-1.rc1"
