...
class ReleaseContextTest(unittest.TestCase):
    ...
    def test_closed_changelog_merge_requests_are_ignored(self) -> None:
        source_branch = "v6.12.0.0-1-main-changelog-update"
        self.api.get_all.return_value = [
            {"state": "closed", "target_branch": "main", "source_branch": source_branch},
        ]
        assert self.context.find_changelog_merge_request("v6.12.0.0-1", "main") is None

    def test_target_branch_is_part_of_changelog_branch(self) -> None:
        assert self.context.config.changelog_branch("v6.12.0.0-1", "release/5.12") == (
            "v6.12.0.0-1-release-5.12-changelog-update"
        )
    ...
    def test_active_source_branch_collision_is_rejected(self) -> None:
        self.api.get_all.return_value = [
            {
                "state": "opened",
                "title": "[Misc] Update changelog for release 6.12.0.0-1",
                "target_branch": "other",
                "source_branch": "v6.12.0.0-1-main-changelog-update",
            }
        ]
        with pytest.raises(ReleaseWorkflowError, match="targeting another branch"):
            self.context.find_changelog_merge_request("v6.12.0.0-1", "main")
    ...
    def test_tag_pipeline_is_filtered_to_tag(self) -> None:
        ...
        self.context.find_pipeline("abc", "v6.12.0.0-1")
        self.api.get_all.assert_called_once_with("pipelines", {"sha": "abc", "ref": "v6.12.0.0-1"})
