...
test_and_check_cmd "Adding tape 'ULT108 '" "${command}" "add" "-v ULT108  --mt LTO8 --ve vendor -l ${lls[1]} -t ctasystest -f true --purchaseorder order1"\
  "select(.vid==\"ULT108 \" and .mediaType==\"LTO8\" and .logicalLibrary==\"${lls[1]}\" and .physicalLibrary==\"phys1\" and .full==true and .purchaseOrder==\"order1\") | .vid"\
  "1" "adding tape 'ULT108 '" || exit 1
test_and_check_cmd "Reclaiming tape 'ULT108 '" "${command}" "reclaim" "-v ULT108 "\
  "select(.vid==\"ULT108 \" and .mediaType==\"LTO8\" and .logicalLibrary==\"${lls[1]}\" and .full==false) | .vid"\
  "1" "reclaiming tape 'ULT108 '" || exit 1
test_and_check_cmd "Changing tape ULT108  state to REPACKING" "${command}" "ch" "-v ULT108  -s 'REPACKING' -r 'Test admin-cta ta ch'"\
  "select(.vid==\"ULT108 \" and .mediaType==\"LTO8\" and .logicalLibrary==\"${lls[1]}\" and .state==\"REPACKING\") | .vid"\
  "1" "changing tape ULT108  state" || exit 1
test_and_check_cmd "Changing tape ULT108  order to order2" "${command}" "ch" "-v ULT108  --purchaseorder 'order2' -r 'Test admin-cta ta ch'"\
  "select(.vid==\"ULT108 \" and .purchaseOrder==\"order2\") | .vid"\
  "1" "changing tape ULT108  order" || exit 1
test_command "Removing tape ULT108 " "${command}" "rm" "-v ULT108 " || exit 1
