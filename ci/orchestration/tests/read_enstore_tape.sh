...
# Ensure required tools are available inside the rmcd pod.
...
write_with_retry "Writing VOL1 image" dd if="${vol1}" of=/enstore-tape.img bs=80
write_with_retry "Writing VOL1 to tape" dd if=/enstore-tape.img of="${device}" bs=80
mt -f ${device} status

list_payload_contents "${payload}"
write_with_retry "Writing payload to tape" dd if="${payload}" of="${device}" bs=1048576
