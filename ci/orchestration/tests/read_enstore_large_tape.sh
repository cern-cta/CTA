...
write_with_retry "Writing VOL1 image" dd if="${vol1}" of=/enstorelarge-tape.img bs=80
write_with_retry "Writing VOL1 to tape" dd if=/enstorelarge-tape.img of="${device}" bs=80
mt -f ${device} status
write_with_retry "Writing header to tape" dd if="${header}" of="${device}" bs=262144
mt -f ${device} status
write_with_retry "Writing payload to tape" dd if="${payload}" of="${device}" bs=262144
mt -f ${device} status
write_with_retry "Writing trailer to tape" dd if="${trailer}" of="${device}" bs=262144
