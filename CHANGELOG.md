...
-  #943 - Resolve "Handling of 0-length files on CTA dev side"

-  #968 - Add support to build_deploy.sh to handle --upgrade and --skip-image-reload simultaneously

-  #950 - Introduce build_deploy.sh --upgrade and refactor of underlying workflow in build_deploy


### Bug Fixes

-  #311 - Resolve "Remove wrong 'job owned after destruction' INFO message after request is requeued"

-  #957 - Resolve "Clear outdated log parameters when starting a new tape session"

-  #987 - Fix incorrect localhost for rmcd

-  #957 - Resolve "Clear outdated log parameters when starting a new tape session"

-  #983 - Fix bugs in build_deploy.sh script for postgres scheduler workflows and cached cmake variables

-  #978 - Resolve "Add log param `mountAttempted` to DataTransferSession"

-  #776 - Fix gfal2 coredumping on eviction step using http due to `double free or corruption`

-  #969 - Fix select() always timing out in rmc_serv

-  #974 - Fix incorrect scheduler type in build scripts and fix bug with incorrect postgres connection URLs

-  #976 - Add install of 'which' to ensure gdb-add-index functions correctly on latest alma9 release

-  #914 - Resolve "Log when a file cannot be deleted from the Catalogue"

-  #971 - Ensure git fetch has proper depth for submodules

-  #970 - Deleted duplicated shift from build_deploy.sh

-  #965 - Fix pipeline triggering on no changes when branch created through web UI

-  #966 - Fix liquibase-test failing due to CTA not supporting previous catalogue version

-  #952 - Fix docs path in GitLab issue template
...
-  #994 - Fix initialization of hp in rmc to ensure SonarCloud quality gate passes

-  #956 - Resolve "Add missing documentation on `archiveroutetype`"

-  #982 - Fix some low-hanging fruit SonarCloud issues

-  #984 - Add CI test to ensure creation timestamps and modification timestamps are the...

-  #954 - Further clean-up and performance improvements to the Helm setup

-  #973 - List clang-format issues in job output and remove tag fetching

-  #945 - Adding template for CTA log parameter issues

-  #972 - Make the cmake build type explicit in the CI

-  #729 - Resolve "Remove external dependecy on cpp-check CI test"

-  #964 - Improve CI pipeline performance by relying on shallow clones

-  #961 - Replace usage of strlen in CmdLineTool.cpp

-  #963 - Replace leftover yum-plugin-versionlock with python3-dnf-plugin-versionlock

-  #948 - Update delete_instance.sh so that it does not rely on a central log mount

-  #932 - Move latest ReleaseNotes.md entries to CHANGELOG.md and remove ReleaseNotes.md

-  #937 - Increase default resource limits on Helm setup


### feature/bug/maintenance

-  <id> - Resolve "Add support for separate repack scheduler DB to `cta-admin dr ls` output"

-  <id> - Resolve "Handle new protobuf fields in all workflow events"

-  <id> - clang format changes for MR 719 (cta-admin drive ls)

-  <id> - Resolve "Adjusting clang format configuration to prevent grouping all...

-  <id> - Fix failing valgrind tests due to EAGAIN error
