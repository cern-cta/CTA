...
void RepackRequestManager::runOnePass(log::LogContext& lc, const size_t repackMaxRequestsToExpand) {
  ...
  if (const auto repackRequest = m_scheduler.getNextRepackRequestToExpand(); repackRequest != nullptr) {
    ...
    try {
      ...
      lc.log(log::WARNING,
             "In RepackRequestManager::runOnePass(), RepackRequest object does not exist in the objectstore");
    }
  }
