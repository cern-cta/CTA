...
struct SchedulerDatabaseTestParam {
  ...
  SchedulerDatabaseTestParam(cta::SchedulerDatabaseFactory& dbFactory) : dbFactory(dbFactory) {}
};  // struct SchedulerDatabaseTestParam
 ...
 */
class SchedulerDatabaseTest : public ::testing::TestWithParam<SchedulerDatabaseTestParam> {
public:
...
const cta::common::dataStructures::SecurityIdentity
  SchedulerDatabaseTest::s_adminOnAdminHost(SchedulerDatabaseTest::s_admin, SchedulerDatabaseTest::s_adminHost);
const cta::common::dataStructures::SecurityIdentity
  SchedulerDatabaseTest::s_adminOnUserHost(SchedulerDatabaseTest::s_admin, SchedulerDatabaseTest::s_userHost);

const cta::common::dataStructures::SecurityIdentity
  SchedulerDatabaseTest::s_userOnAdminHost(SchedulerDatabaseTest::s_user, SchedulerDatabaseTest::s_adminHost);
const cta::common::dataStructures::SecurityIdentity
...
TEST_P(SchedulerDatabaseTest, createManyArchiveJobs) {
  ...
  do {
    ...
    tfw.vid = "vid";
    auto am =
      mountInfo->createArchiveMount(common::dataStructures::MountType::ArchiveForUser, tfw, "drive", "library", "host");
    bool done = false;
    size_t count = 0;
    while (!done) {
  ...
  tfw.tapePool = "tapePool";
  tfw.vid = "vid";
  auto am = moutInfo->createArchiveMount(tfw, "drive", "library", "host", "vo", "mediaType", "vendor", 123456789,
...
TEST_P(SchedulerDatabaseTest, putExistingQueueToSleep) {
  ...
  cta::common::dataStructures::DiskInstanceSpace diskInstanceSpace {"dis-A",
                                                                    "di-A",
                                                                    "constantFreeSpace:999999999999",
                                                                    60,
                                                                    60,
                                                                    0,
                                                                    "No Comment",
  ...
  cta::disk::DiskSystem diskSystem {"ds-A",
                                    diskInstanceSpace,
                                    "$root://a.disk.system/",
                                    10UL * 1000 * 1000 * 1000,
                                    15 * 60,
...
TEST_P(SchedulerDatabaseTest, popRetrieveRequestsWithDisksytem) {
  ...
  catalogue.DiskInstanceSpace()->createDiskInstanceSpace(common::dataStructures::SecurityIdentity(), "dis-A", "di",
                                                         "constantFreeSpace:999999999999", 60, "No comment");
  catalogue.DiskSystem()->createDiskSystem(common::dataStructures::SecurityIdentity(), "ds-A", "di", "dis-A",
                                           "$root://a.disk.system/", 10UL * 1000 * 1000 * 1000, 15 * 60, "No comment");

  catalogue.DiskInstanceSpace()->createDiskInstanceSpace(common::dataStructures::SecurityIdentity(), "dis-B", "di",
                                                         "constantFreeSpace:999999999999", 60, "No comment");
  catalogue.DiskSystem()->createDiskSystem(common::dataStructures::SecurityIdentity(), "ds-B", "di", "dis-B",
                                           "$root://b.disk.system/", 10UL * 1000 * 1000 * 1000, 15 * 60, "No comment");
  ...
  std::list<std::function<void()>> lambdas;
  for (size_t id = 0; id < filesToDo; id++) {
    lambdas.emplace_back([id, &db, &lc, diskSystemList]() {
      cta::common::dataStructures::RetrieveRequest rr;
      cta::log::LogContext locallc = lc;
      cta::common::dataStructures::RetrieveFileQueueCriteria rfqc;
      rfqc.mountPolicy.name = "mountPolicy";
...
TEST_P(SchedulerDatabaseTest, popRetrieveRequestsWithBackpressure) {
  ...
  catalogue.DiskInstance()->createDiskInstance(common::dataStructures::SecurityIdentity(), "di", "No comment");
  catalogue.DiskInstanceSpace()->createDiskInstanceSpace(common::dataStructures::SecurityIdentity(), "dis-A", "di",
                                                         "constantFreeSpace:6000", 60, "No comment");
  catalogue.DiskSystem()->createDiskSystem(common::dataStructures::SecurityIdentity(), "ds-A", "di", "dis-A",
                                           "$root://a.disk.system/", 0UL, 15 * 60, "No comment");
...
TEST_P(SchedulerDatabaseTest, popRetrieveRequestsWithDiskSystemNotFetcheable) {
  ...
  catalogue.DiskInstance()->createDiskInstance(common::dataStructures::SecurityIdentity(), "di", "No comment");
  catalogue.DiskInstanceSpace()->createDiskInstanceSpace(common::dataStructures::SecurityIdentity(), "dis-error", "di",
                                                         "constantFreeSpace-6000", 60, "No comment");

  catalogue.DiskSystem()->createDiskSystem(common::dataStructures::SecurityIdentity(), "ds-Error", "di", "dis-error",
                                           "$root://error.disk.system/", 0UL, 15 * 60, "No comment");
...
INSTANTIATE_TEST_CASE_P(MockSchedulerDatabaseTest,
...
INSTANTIATE_TEST_CASE_P(PostgresSchedDBSchedulerDatabaseTest,
                        SchedulerDatabaseTest,
...
INSTANTIATE_TEST_CASE_P(OStoreSchedulerDatabaseTestVFS,
                        SchedulerDatabaseTest,
...
INSTANTIATE_TEST_CASE_P(OStoreSchedulerDatabaseTestRados,
                        SchedulerDatabaseTest,
