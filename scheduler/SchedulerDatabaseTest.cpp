...
TEST_P(SchedulerDatabaseTest, createManyArchiveJobs) {
  ...
  tfw.tapePool = "tapePool";
  tfw.vid = "vid";
  auto am = moutInfo->createArchiveMount(tfw,
                                         "drive",
                                         "library",
                                         "host",
                                         "vo",
...
TEST_P(SchedulerDatabaseTest, putExistingQueueToSleep) {
  ...
  std::function<void()> lambda = [&db, &lc, diskSystemList]() {
    ...
    rfqc.mountPolicy.lastModificationLog = {"u",
                                            "h",
                                            std::chrono::system_clock::to_time_t(std::chrono::system_clock::now())};
...
TEST_P(SchedulerDatabaseTest, popRetrieveRequestsWithDisksytem) {
  ...
  catalogue.DiskInstanceSpace()->createDiskInstanceSpace(common::dataStructures::SecurityIdentity(),
                                                         "dis-A",
                                                         "di",
                                                         "constantFreeSpace:999999999999",
                                                         60,
                                                         "No comment");
  catalogue.DiskSystem()->createDiskSystem(common::dataStructures::SecurityIdentity(),
                                           "ds-A",
                                           "di",
                                           "dis-A",
                                           "$root://a.disk.system/",
