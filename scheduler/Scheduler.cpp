...
// deleteArchive
//------------------------------------------------------------------------------
void Scheduler::deleteArchive(const std::string& instanceName,
...
// cancelRepack
//------------------------------------------------------------------------------
void Scheduler::cancelRepack(const common::dataStructures::SecurityIdentity& cliIdentity,
...
// expandRepackRequest
//------------------------------------------------------------------------------
void Scheduler::expandRepackRequest(std::unique_ptr<RepackRequest>& repackRequest,
...
// reportDriveConfig
//------------------------------------------------------------------------------
void Scheduler::reportDriveConfig(const cta::tape::daemon::TpconfigLine& tpConfigLine,
  ...
  auto schedulerDbTime = t.secs();
  log::ScopedParamContainer spc(lc);
  spc.add("drive", tpConfigLine.unitName).add("schedulerDbTime", schedulerDbTime);
  ...
  for (auto& em : mountInfo->existingOrNextMounts) {
    ...
    if (em.driveName != driveName) {
      ...
      existingMountsBasicTypeSummaryPerVo[VirtualOrganizationMountPair(
                                            tapepoolVoMap.at(em.tapePool).name,
                                            common::dataStructures::getMountBasicType(em.type))]
        .totalMounts++;
      if (em.activity) {
...
bool Scheduler::getNextMountDryRun(const std::string& logicalLibraryName,
  ...
  if (logicalLibrary) {
    if (logicalLibrary.value().isDisabled) {
      log::ScopedParamContainer params(lc);
      params.add("logicalLibrary", logicalLibraryName).add("catalogueTime", getLogicalLibrariesTime);
