...
std::string Scheduler::queueArchiveWithGivenId(const uint64_t archiveFileId,
  ...
  const common::dataStructures::ArchiveFileQueueCriteriaAndFileId catalogueInfo(
    archiveFileId, queueCriteria.copyToPoolMap, queueCriteria.mountPolicy);
...
void Scheduler::expandRepackRequest(const std::unique_ptr<RepackRequest>& repackRequest,
  ...
  if (repackInfo.noRecall) {
    archiveFilesFromCatalogue.remove_if([&repackInfo,
                                         &filesInDirectory](const common::dataStructures::ArchiveFile& archiveFile) {
      //We remove all the elements that are not in the repack buffer so that we don't recall them
      return std::find_if(filesInDirectory.begin(), filesInDirectory.end(),
                          [&archiveFile, &repackInfo](const std::string& fseq) {
                            ...
                            return std::find_if(archiveFile.tapeFiles.begin(), archiveFile.tapeFiles.end(),
  ...
  while (!archiveFilesFromCatalogue.empty()) {
    ...
    auto storageClassOfArchiveFile = std::find_if(
      storageClasses.begin(), storageClasses.end(),
  ...
  try {
    ...
    nbRetrieveSubrequestsQueued = repackRequest->m_dbReq->addSubrequestsAndUpdateStats(
...
common::dataStructures::DesiredDriveState Scheduler::getDesiredDriveState(const std::string& driveName,
  ...
  for (const auto& driveState : driveStates) {
    lc.log(log::DEBUG, "In Scheduler::getDesiredDriveState(): checking driveName: " + driveName +
...
void Scheduler::sortAndGetTapesForMountInfo(
  ...
  for (const auto& pm : mountInfo->potentialMounts) {
    if (pm.tapePool.empty()) {
      // log an error that tapepool in PotentialMounts contained an empty string
      lc.log(log::ERR, "In Scheduler::sortAndGetTapesForMountInfo(): for Potential Mounts, "
      ...
      mountInfo->potentialMounts.erase(
        std::remove_if(mountInfo->potentialMounts.begin(), mountInfo->potentialMounts.end(),
                       [](const SchedulerDatabase::PotentialMount& mount) { return mount.tapePool.empty(); }),
  ...
  for (const auto& em : mountInfo->existingOrNextMounts) {
    if (em.tapePool.empty()) {
      // log an error that tapepool in ExistingOrNextMounts contained an empty string
      lc.log(log::ERR, "In Scheduler::sortAndGetTapesForMountInfo(): for Existing or Next Mounts, "
                       "tapePool is an empty string.");
      // Remove elements where tapepool is empty to don't have any problem later
      mountInfo->existingOrNextMounts.erase(
        std::remove_if(mountInfo->existingOrNextMounts.begin(), mountInfo->existingOrNextMounts.end(),
                       [](const SchedulerDatabase::ExistingMount& mount) { return mount.tapePool.empty(); }),
  ...
  for (auto& em : mountInfo->existingOrNextMounts) {
    ...
    if (em.driveName != driveName) {
      ...
      if (isRepackingMount && !defaultRepackVo.has_value()) {
        ...
        lc.log(log::ERR, "In Scheduler::sortAndGetTapesForMountInfo(): existingOrNextMounts found a repack mount while "
                         "there is no default repack VO defined.");
        continue;
      }
      ...
      existingMountsBasicTypeSummaryPerVo[VirtualOrganizationMountPair(
                                            voName, common::dataStructures::getMountBasicType(em.type))]
        .totalMounts++;
      if (em.activity) {
  ...
  for (auto m = mountInfo->potentialMounts.begin(); m != mountInfo->potentialMounts.end(); ++m) {
    ...
    if (isRepackingMount) {
      if (defaultRepackVo.has_value()) {
        ...
        lc.log(log::ERR, "In Scheduler::sortAndGetTapesForMountInfo(): potential repack mount not considered due to "
                         "lack of default repack VO");
  ...
  if (std::count_if(mountInfo->potentialMounts.cbegin(), mountInfo->potentialMounts.cend(),
...
std::optional<common::dataStructures::LogicalLibrary> Scheduler::getLogicalLibrary(const std::string& libraryName,
  ...
  auto logicalLibraryItor = std::find_if(
    logicalLibraries.begin(), logicalLibraries.end(),
...
std::optional<common::dataStructures::PhysicalLibrary> Scheduler::getPhysicalLibrary(const std::string& libraryName,
  ...
  auto physicalLibraryItor = std::find_if(
    physicalLibraries.begin(), physicalLibraries.end(),
...
void Scheduler::checkNeededEnvironmentVariables() {
  ...
  if (!environmentVariablesNotSet.empty()) {
    ...
    listVariablesNotSet += "]";
    std::string errMsg =
      "In Scheduler::checkNeededEnvironmentVariables(), the following environment variables: " + listVariablesNotSet +
      " are not set.";
...
bool Scheduler::getNextMountDryRun(const std::string& logicalLibraryName,
  ...
  sortAndGetTapesForMountInfo(mountInfo, logicalLibraryName, driveName, timer,
                              existingMountsDistinctTypeSummaryPerTapepool, existingMountBasicTypeSummaryPerVo,
                              tapesInUse, tapeList, getTapeInfoTime, candidateSortingTime, getTapeForWriteTime, lc);
  ...
  switch (new_state) {
    ...
    case Tape::BROKEN:
      try {
        ...
        throw catalogue::UserSpecifiedAnEmptyStringReasonWhenTapeStateNotActive(
          std::regex_replace(ex.getMessageValue(), std::regex(Tape::stateToString(Tape::BROKEN_PENDING)),
    ...
    case Tape::REPACKING:
      if (prev_state == Tape::REPACKING_DISABLED) {
        ...
        try {
          ...
          throw catalogue::UserSpecifiedAnEmptyStringReasonWhenTapeStateNotActive(
            std::regex_replace(ex.getMessageValue(), std::regex(Tape::stateToString(Tape::REPACKING_PENDING)),
    ...
    case Tape::EXPORTED:
      try {
        ...
        throw catalogue::UserSpecifiedAnEmptyStringReasonWhenTapeStateNotActive(
          std::regex_replace(ex.getMessageValue(), std::regex(Tape::stateToString(Tape::EXPORTED_PENDING)),
    ...
    try {
      ...
      try {
        ...
        params.add("fileId", j->archiveFile.archiveFileID)
          .add("reportType", j->reportType())
          .add("exceptionMSG", ex.getMessageValue());
        lc.log(log::WARNING, "In Scheduler::reportArchiveJobsBatch(): failed to reportFailed the job because it does "
    ...
    try {
      ...
      try {
        ...
        params.add("fileId", current.archiveJob->archiveFile.archiveFileID)
          .add("reportType", current.archiveJob->reportType())
          .add("exceptionMSG", ex.getMessageValue());
        lc.log(log::WARNING, "In Scheduler::reportArchiveJobsBatch(): failed to reportFailed the current job because "
                             "it does not exist in the objectstore.");
      }
    }
  }
