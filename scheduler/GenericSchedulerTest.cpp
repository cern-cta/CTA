...
struct SchedulerTestParam {
  ...
  SchedulerTestParam(cta::SchedulerDatabaseFactory& dbFactory) : m_dbFactory(dbFactory) {}

  SchedulerTestParam(cta::SchedulerDatabaseFactory& dbFactory,
...
std::ostream& operator<<(std::ostream& os, const SchedulerTestParam& c) {
  if (!c.m_triggerTapeStateChangeBehaviour.has_value()) {
    ...
    return os << "{ "
              << "\"from\": "
              << "\"" << cta::common::dataStructures::Tape::stateToString(params.fromState) << "\""
              << ", "
              << "\"to\": "
              << "\"" << cta::common::dataStructures::Tape::stateToString(params.toState) << "\""
              << ", "
              << "\"expected_state\": "
              << "\"" << cta::common::dataStructures::Tape::stateToString(params.observedState) << "\""
              << ", "
              << "\"expected_exception\": "
              << "\"" << (params.changeRaisedException ? "yes" : "no") << "\""
              << ", "
              << "\"expected_cleanup\": "
              << "\"" << (params.cleanupFlagActivated ? "yes" : "no") << "\""
              << " }";
  }
}
...
public:
  ...
  public:
    const char* what() const noexcept { return "Failed to get scheduler db."; }
  };
  ...
  virtual void SetUp() {
    ...
    m_db = std::move(sdb);
    m_scheduler =
      std::make_unique<Scheduler>(*m_catalogue, *m_db, s_minFilesToWarrantAMount, s_minBytesToWarrantAMount);
  }
  ...
  void setupDefaultCatalogue() {
    ...
    catalogue.RequesterMountRule()->createRequesterMountRule(s_adminOnAdminHost, mountPolicyName, s_diskInstance,
                                                             s_userName, ruleComment);
    ...
    catalogue.TapePool()->createTapePool(s_adminOnAdminHost, s_tapePoolName, vo.name, nbPartialTapes,
...
TEST_P(SchedulerTest, archive_to_new_file) {
...

  {
    ...
    for (auto rqstItor = poolRqsts.cbegin(); rqstItor != poolRqsts.cend(); rqstItor++) {
...
TEST_P(SchedulerTest, archive_report_and_retrieve_new_file) {
  ...
  catalogue.LogicalLibrary()->createLogicalLibrary(s_adminOnAdminHost, s_libraryName, libraryIsDisabled,
                                                   libraryComment);
  {
  ...
  for (int mountPass = 0; mountPass < 2; ++mountPass) {
...

    {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        retrieveJobList.front()->transferFailed(
          "Retrieve failed (mount " + std::to_string(mountPass) + ", attempt " + std::to_string(i) + ")", lc);
      }
...
TEST_P(SchedulerTest, repack) {
  ...
  cta::SchedulerDatabase::QueueRepackRequest qrr(
    tape1, "file://" + tempDirectory.path(), common::dataStructures::RepackInfo::Type::MoveOnly,
  ...
  scheduler.promoteRepackRequestsToToExpand(lc);
  {
    auto repacks = scheduler.getRepacks();
    ASSERT_EQ(
      1, std::count_if(repacks.begin(), repacks.end(), [](RepackInfo& r) { return r.status == Status::ToExpand; }));
    ASSERT_EQ(1, repacks.size());
  }
}
...
TEST_P(SchedulerTest, getNextRepackRequestToExpand) {
  ...
  ASSERT_EQ(repackRequestToExpand2.get()->getRepackInfo().type,
            common::dataStructures::RepackInfo::Type::AddCopiesOnly);

  auto nullRepackRequest = scheduler.getNextRepackRequestToExpand();
  ASSERT_EQ(nullRepackRequest, nullptr);
}

TEST_P(SchedulerTest, DISABLED_expandRepackRequest) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTest, DISABLED_expandRepackRequestRetrieveFailed) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTest, DISABLED_expandRepackRequestArchiveSuccess) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTest, DISABLED_expandRepackRequestArchiveFailed) {
  ASSERT_EQ(0, 1);
}
...
TEST_P(SchedulerTest, expandRepackRequestRepackingTape) {
  ...
  {
    cta::SchedulerDatabase::QueueRepackRequest qrr(
      vid, "file://" + tempDirectory.path(), common::dataStructures::RepackInfo::Type::MoveOnly,
      common::dataStructures::MountPolicy::s_defaultMountPolicyForRepack, s_defaultRepackNoRecall);
...
TEST_P(SchedulerTest, DISABLED_archiveReportMultipleAndQueueRetrievesWithActivities) {
  ...
  enum ExpectedActivity { Unknown, A, B };
...
TEST_P(SchedulerTest, DISABLED_expandRepackRequestAddCopiesOnly) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTest, DISABLED_expandRepackRequestShouldFailIfArchiveRouteMissing) {
  ASSERT_EQ(0, 1);
}
...
TEST_P(SchedulerTest, DISABLED_cancelRepackRequest) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTest, DISABLED_getNextMountEmptyArchiveForRepackIfNbFilesQueuedIsLessThan2TimesMinFilesWarrantAMount) {
  ASSERT_EQ(0, 1);
}
...
TEST_P(SchedulerTest, getNextMountTapeStatesThatShouldNotReturnAMount) {
  ...
  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::BROKEN, std::nullopt,
                                    std::string("Test"));
  ASSERT_EQ(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));
  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::ACTIVE,
                                    common::dataStructures::Tape::BROKEN, std::nullopt);
  ASSERT_NE(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));

  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::EXPORTED, std::nullopt,
                                    std::string("Test"));
  ASSERT_EQ(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));
  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::ACTIVE,
                                    common::dataStructures::Tape::EXPORTED, std::nullopt);
  ASSERT_NE(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));

  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::REPACKING_DISABLED,
                                    std::nullopt, std::string("Test"));
  ASSERT_EQ(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));
  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::ACTIVE,
                                    common::dataStructures::Tape::REPACKING_DISABLED, std::nullopt);
  ASSERT_NE(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));

  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::DISABLED, std::nullopt,
                                    std::string("Test"));
  ASSERT_EQ(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));
  catalogue.Tape()->modifyTapeState(s_adminOnAdminHost, tape.vid, common::dataStructures::Tape::ACTIVE,
                                    common::dataStructures::Tape::DISABLED, std::nullopt);
  ASSERT_NE(nullptr, scheduler.getNextMount(s_libraryName, driveName, lc));
...
TEST_P(SchedulerTest, DISABLED_repackRetrieveRequestsFailToFetchDiskSystem) {
  ASSERT_EQ(0, 1);
}
...
TEST_P(SchedulerTest, expandRepackRequestShouldThrowIfUseBufferNotRecallButNoDirectoryCreated) {
  ...
  catalogue.DiskInstanceSpace()->createDiskInstanceSpace({"user", "host"}, "diskInstanceSpace", "diskInstance",
                                                         "eos:ctaeos:default", 10, "no comment");
  catalogue.DiskSystem()->createDiskSystem({"user", "host"}, "repackBuffer", "diskInstance", "diskInstanceSpace",
                                           tempDirectory.path(), 10L * 1000 * 1000 * 1000, 15 * 60, "no comment");
  ...
  cta::SchedulerDatabase::QueueRepackRequest qrr(
    s_vid, "file://DOES_NOT_EXIST", common::dataStructures::RepackInfo::Type::MoveOnly,
...
TEST_P(SchedulerTest, expandRepackRequestShouldNotThrowIfTapeDisabledButNoRecallFlagProvided) {
  ...
  tempDirectory.mkdir();
  cta::SchedulerDatabase::QueueRepackRequest qrr(
    s_vid, pathRepackBuffer, common::dataStructures::RepackInfo::Type::MoveOnly,
    common::dataStructures::MountPolicy::s_defaultMountPolicyForRepack, noRecall);
  scheduler.queueRepack(admin, qrr, lc);
  scheduler.waitSchedulerDbSubthreadsComplete();
...
TEST_P(SchedulerTest, DISABLED_retrieveArchiveAllTypesMaxDrivesVoInFlightChangeScheduleMount) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTest, DISABLED_getQueuesAndMountSummariesTest) {
  ASSERT_EQ(0, 1);
}
...
TEST_P(SchedulerTest,
       DISABLED_getNextMountWithArchiveForUserAndArchiveForRepackShouldReturnBothMountsArchiveMinRequestAge) {
  ASSERT_EQ(0, 1);
}

TEST_P(SchedulerTestTriggerTapeStateChangeBehaviour, DISABLED_triggerTapeStateChangeValidScenarios) {
  ASSERT_EQ(0, 1);
}
