...
CREATE TYPE REPACK_REQ_STATUS AS ENUM (
  ...
  'RRS_Complete',
  'RRS_Failed' );
/* CREATE TYPE JOB_QUEUE_TYPE AS ENUM (
  'JobsToTransfer',
  'FailedJobs',
  'JobsToReportToUser',
...
CREATE TABLE ARCHIVE_JOB_QUEUE(
  ...
  TAPE_POOL VARCHAR(100) CONSTRAINT AJQ_TPN_NN NOT NULL CONSTRAINT AJQ_TPN_NE CHECK(TAPE_POOL <> ''),
  MOUNT_POLICY VARCHAR(100) CONSTRAINT AJQ_MPN_NN NOT NULL CONSTRAINT AJQ_MPN_NE CHECK(MOUNT_POLICY <> ''),
...
CREATE VIEW RETRIEVE_JOB_SUMMARY(
  ...
  RETRIEVE_MIN_REQUEST_AGE) AS SELECT
    ...
    COUNT(*),
    SUM(SIZE_IN_BYTES),
    MIN(ACTIVE_STARTTIME),
    MAX(MP_RETRIEVE_PRIORITY),
    MIN(MP_RETRIEVE_MIN_REQ_AGE)
  FROM ARCHIVE_JOB_QUEUE GROUP BY
    MOUNT_ID,
    STATUS,
    VID,
