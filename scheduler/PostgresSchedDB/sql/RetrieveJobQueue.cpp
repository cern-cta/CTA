...
#include "scheduler/PostgresSchedDB/sql/RetrieveJobQueue.hpp"
...
void RetrieveJobQueueRow::updateMountId(Transaction& txn,
                                        const std::list<RetrieveJobQueueRow>& rowList,
                                        uint64_t mountId) {
  if (rowList.empty()) {
    return;
  }
  ...
  const char* const sql = "UPDATE RETRIEVE_JOB_QUEUE SET "
                          "MOUNT_ID = :MOUNT_ID "
                          "WHERE "
