...
struct RetrieveJobQueueRow {
   ...
   */
  RetrieveJobQueueRow(const rdbms::Rset& rset) { *this = rset; }
  ...
  RetrieveJobQueueRow& operator=(const rdbms::Rset& rset) {
    ...
    status = from_string<RetrieveJobStatus>(rset.columnString("STATUS"));
  ...
  void insert(Transaction& txn) const {
    ...
    const char* const sql = "INSERT INTO RETRIEVE_JOB_QUEUE("
                            "RETRIEVE_REQID,"
                            "STATUS,"
                            "VID,"
                            "ACTIVITY,"
