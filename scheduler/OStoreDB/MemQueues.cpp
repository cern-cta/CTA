...
void MemQueue<objectstore::ArchiveRequest, objectstore::ArchiveQueue>::specializedAddJobsToQueueAndCommit(
  ...
  for (auto& j : jobsToAdd) {
    jtal.push_back({j.job,
                    j.request.getAddressIfSet(),
                    j.request.getArchiveFile().archiveFileID,
                    j.request.getArchiveFile().fileSize,
                    j.request.getMountPolicy(),
                    j.request.getEntryLog().time});
  ...
  for (auto& jta : jobsToAdd) {
    ...
    for (auto& j : request.getArchiveFile().tapeFiles) {
      if (j.copyNb == job.copyNb) {
        ...
        jtal.emplace_back(j.copyNb,
                          j.fSeq,
                          request.getAddressIfSet(),
                          criteria.archiveFile.fileSize,
                          criteria.mountPolicy,
                          request.getEntryLog().time,
                          request.getActivity(),
...
void MemQueue<objectstore::RetrieveRequest, objectstore::RetrieveQueue>::specializedUpdateCachedQueueStats(
  ...
  objectstore::Helpers::updateRetrieveQueueStatisticsCache(queue.getVid(),
                                                           summary.jobs,
                                                           summary.bytes,
                                                           summary.priority,
                                                           lc);
