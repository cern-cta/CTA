...
void OStoreDB::delayIfNecessary(log::LogContext& lc) {
  ...
  if (delayInserted) {
    ...
    params.add("sleepDelay", sleepDelay).add("lockDelay", lockDelay).add("taskQueueSize", taskQueueSize);
...
std::list<SchedulerDatabase::RetrieveQueueCleanupInfo>
  ...
  for (auto& rqp : re.dumpRetrieveQueues(common::dataStructures::JobQueueType::JobsToTransferForUser)) {
    ...
    try {
      ...
      params.add("queueObject", rqp.address).add("tapeVid", rqp.vid).add("exceptionMessage", ex.getMessageValue());
      logContext.log(
        log::DEBUG,
...
void OStoreDB::fetchMountInfo(SchedulerDatabase::TapeMountDecisionInfo& tmdi,
  ...
  for (auto& aqp : re.dumpArchiveQueues(common::dataStructures::JobQueueType::JobsToTransferForUser)) {
    ...
    try {
      ...
      logContext.log(
        log::DEBUG,
        "WARNING: In OStoreDB::fetchMountInfo(): failed to lock/fetch an archive queue for user. Skipping it.");
      continue;
    }
    ...
    if (aqueueJobsSummary.jobs) {
      ...
      if (mountPolicies.size()) {
        ...
        if (mountPoliciesInQueueList.size()) {
          std::transform(
            mountPoliciesInQueueList.begin(), mountPoliciesInQueueList.end(),
    ...
    if (queueLockTime > 1 || queueFetchTime > 1) {
      logContext.log(
        log::WARNING,
        "In OStoreDB::fetchMountInfo(): fetched an archive for user queue and that lasted more than 1 second.");
    }
  }
  ...
  for (auto& aqp : re.dumpArchiveQueues(common::dataStructures::JobQueueType::JobsToTransferForRepack)) {
    ...
    try {
      ...
      logContext.log(
        log::DEBUG,
        "WARNING: In OStoreDB::fetchMountInfo(): failed to lock/fetch an archive queue for repack. Skipping it.");
      continue;
    }
    // If there are files queued, we create an entry for this tape pool in the
    // mount candidates list.
    ...
    if (aqueueRepackJobsSummary.jobs) {
      ...
      if (mountPolicies.size()) {
        //We get all the mount policies that are on the queue from the catalogue list
        auto mountPoliciesInQueueList =
          getMountPoliciesInQueue(mountPolicies, aqueueRepackJobsSummary.mountPolicyCountMap);
        m.mountPolicyNames = std::list<std::string>();
        //If an operator removed the queue mountPolicies from the catalogue, we will have no results...
        if (mountPoliciesInQueueList.size()) {
          std::transform(
    ...
    if (queueLockTime > 1 || queueFetchTime > 1) {
      logContext.log(
        log::WARNING,
        "In OStoreDB::fetchMountInfo(): fetched an archive for repack queue and that lasted more than 1 second.");
    }
  }
  ...
  for (auto& rqp : re.dumpRetrieveQueues(common::dataStructures::JobQueueType::JobsToTransferForUser)) {
    ...
    if (rqSummary.jobs) {
      ...
      if (mountPolicies.size()) {
        ...
        if (mountPoliciesInQueueList.size()) {
          std::transform(
            mountPoliciesInQueueList.begin(), mountPoliciesInQueueList.end(), std::back_inserter(queueMountPolicyNames),
            [](const cta::common::dataStructures::MountPolicy& policy) -> std::string { return policy.name; });
  ...
  for (const auto& driveState : driveStates) {
    if (activeDriveStatuses.count(static_cast<int>(driveState.driveStatus))) {
      ...
      tmdi.existingOrNextMounts.back().bytesTransferred =
        driveState.bytesTransferedInSession ? driveState.bytesTransferedInSession.value() : 0;
      tmdi.existingOrNextMounts.back().filesTransferred =
...
std::list<common::dataStructures::MountPolicy>
  ...
  std::copy_if(mountPoliciesInCatalogue.begin(), mountPoliciesInCatalogue.end(), std::back_inserter(mountPolicyRet),
...
common::dataStructures::MountPolicy
  ...
  common::dataStructures::MountPolicy bestMountPolicy = cta::utils::reduce(
    ++mountPolicies.begin(), mountPolicies.end(), mountPolicies.front(),
...
//------------------------------------------------------------------------------
std::string OStoreDB::getHighestPriorityArchiveMountPolicyName(
  const std::list<common::dataStructures::MountPolicy>& mountPolicies) {
  if (mountPolicies.empty()) {
    throw cta::exception::Exception(
      "In OStoreDB::getHighestPriorityArchiveMountPolicyName(), empty mount policy list.");
  }
  //We need to get the most advantageous mountPolicy
  //As the Init element of the reduce function is the first element of the list, we start the reduce with the second element (++mountPolicyInQueueList.begin())
  common::dataStructures::MountPolicy bestMountPolicy = cta::utils::reduce(
...
//------------------------------------------------------------------------------
std::string OStoreDB::getLowestRequestAgeArchiveMountPolicyName(
  const std::list<common::dataStructures::MountPolicy>& mountPolicies) {
  if (mountPolicies.empty()) {
    throw cta::exception::Exception(
      "In OStoreDB::getLowestRequestAgeArchiveMountPolicyName(), empty mount policy list.");
  }
  //We need to get the most advantageous mountPolicy
  //As the Init element of the reduce function is the first element of the list, we start the reduce with the second element (++mountPolicyInQueueList.begin())
  common::dataStructures::MountPolicy bestMountPolicy = cta::utils::reduce(
...
//------------------------------------------------------------------------------
std::string OStoreDB::getHighestPriorityRetrieveMountPolicyName(
  const std::list<common::dataStructures::MountPolicy>& mountPolicies) {
  if (mountPolicies.empty()) {
    throw cta::exception::Exception(
      "In OStoreDB::getHighestPriorityRetrieveMountPolicyName(), empty mount policy list.");
  }
  //We need to get the most advantageous mountPolicy
  //As the Init element of the reduce function is the first element of the list, we start the reduce with the second element (++mountPolicyInQueueList.begin())
  common::dataStructures::MountPolicy bestMountPolicy = cta::utils::reduce(
...
std::string OStoreDB::getLowestRequestAgeRetrieveMountPolicyName(
  ...
  if (mountPolicies.empty()) {
    throw cta::exception::Exception(
      "In OStoreDB::getLowestRequestAgeRetrieveMountPolicyName(), empty mount policy list.");
  }
  //We need to get the most advantageous mountPolicy
  //As the Init element of the reduce function is the first element of the list, we start the reduce with the second element (++mountPolicyInQueueList.begin())
  common::dataStructures::MountPolicy bestMountPolicy = cta::utils::reduce(
...
std::unique_ptr<SchedulerDatabase::TapeMountDecisionInfo> OStoreDB::getMountInfoNoLock(PurposeGetMountInfo purpose,
  ...
  {
    log::ScopedParamContainer params(logContext);
    params.add("rootFetchNoLockTime", rootFetchNoLockTime).add("fetchMountInfoTime", fetchMountInfoTime);
    ...
    try {
      ...
      for (auto& a : archiveQueueList) {
        ...
        if (!aq.getJobsSummary().jobs) {
          ...
          log::ScopedParamContainer params(lc);
          params.add("tapePool", a.tapePool).add("queueType", toString(queueType)).add("queueObject", a.address);
      ...
      for (auto& r : retrieveQueueList) {
        ...
        if (!rq.getJobsSummary().jobs) {
          ...
          log::ScopedParamContainer params(lc);
          params.add("tapeVid", r.vid).add("queueType", toString(queueType)).add("queueObject", r.address);
...
std::unique_ptr<SchedulerDatabase::ArchiveMount>
  OStoreDB::TapeMountDecisionInfoNoLock::createArchiveMount(const common::dataStructures::MountType& type,
...
std::string OStoreDB::queueArchive(const std::string& instanceName,
  ...
  for (auto& copy : criteria.copyToPoolMap) {
    ...
    aReq->addJob(copy.first, copy.second, m_agentReference->getAgentAddress(), hardcodedRetriesWithinMount,
                 hardcodedTotalRetries, hardcodedReportRetries);
  ...
  auto* et = new EnqueueingTask([aReqPtr, mutexForHelgrindAddr, this] {
    ...
    params.add("jobObject", aReq->getAddressIfSet())
      ...
      .add("totalTime", arRelockTime + arTotalQueueingTime + arTotalCommitTime + arTotalQueueUnlockTime +
                          arOwnerResetTime + arLockRelease + agOwnershipResetTime);
  ...
  logContext.log(log::INFO,
                 "In OStoreDB::queueArchive(): recorded request for queueing (enqueueing posted to thread pool).");
...
SchedulerDatabase::JobsFailedSummary OStoreDB::getArchiveJobsFailedSummary(log::LogContext& logContext) {
  ...
  for (auto& aj : queueList) {
    ...
    try {
      ...
      log::ScopedParamContainer params(logContext);
      params.add("queueObject", aj.address).add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::DEBUG,
                     "WARNING: In OStoreDB::getArchiveJobsFailedSummary(): failed to lock/fetch an archive queue.");
      continue;
    }
...
void OStoreDB::setArchiveJobBatchReported(std::list<cta::SchedulerDatabase::ArchiveJob*>& jobsBatch,
  ...
  if (completeJobsToDelete.size()) {
    ...
    while (completeJobsToDeleteItor != completeJobsToDelete.end()) {
      ...
      try {
        ...
        log::ScopedParamContainer params(lc);
        params.add("fileId", j->archiveFile.archiveFileID);
        lc.log(log::WARNING, "In OStoreDB::setArchiveJobBatchReported() failed to asyncDeleteRequest because it does "
                             "not exist in the objectstore.");
    ...
    for (auto& j : completeJobsToDelete) {
      try {
        ...
        log::ScopedParamContainer params(lc);
        params.add("fileId", j->archiveFile.archiveFileID).add("objectAddress", j->m_archiveRequest.getAddressIfSet());
        lc.log(log::INFO,
               "In OStoreDB::setArchiveJobBatchReported(): deleted ArchiveRequest after completion and reporting.");
        ...
        lc.log(
          log::ERR,
          "In OStoreDB::setArchiveJobBatchReported(): failed to delete ArchiveRequest after completion and reporting.");
  ...
  for (auto& queue : failedQueues) {
    ...
    for (auto& j : queue.second) {
      insertedElements.emplace_back(CaAQF::InsertedElement {&j.job->m_archiveRequest, j.job->tapeFile.copyNb,
                                                            j.job->archiveFile, std::nullopt,
    ...
    try {
      ...
      for (auto& failedAR : failure.failedElements) {
        try {
          ...
          lc.log(
            log::WARNING,
            "In OStoreDB::setArchiveJobBatchReported(), queueing impossible, job do not exist in the objectstore.");
        }
      }
    }
    ...
    switch (j->reportType) {
      case SchedulerDatabase::RetrieveJob::ReportType::FailureReport: {
        try {
          ...
          log::ScopedParamContainer params(lc);
          params.add("fileId", j->archiveFile.archiveFileID).add("exceptionMsg", ex.getMessageValue());
          lc.log(log::WARNING, "In OStoreDB::setRetrieveJobBatchReportedToUser(), fail to fail the job because it does "
                               "not exists in the objectstore.");
      ...
      default: {
        ...
        lc.log(log::ERR,
               "In OStoreDB::setRetrieveJobBatchReportedToUser(): unexpected job status. Leaving the job as-is.");
      }
    }
next:
    jobsBatchItor++;
  ...
  for (auto& queue : failedQueues) {
    ...
    for (auto& j : queue.second) {
      ...
      if (tf_it == j.job->archiveFile.tapeFiles.end()) {
        throw cta::exception::Exception("In OStoreDB::setRetrieveJobBatchReported(): tape copy not found");
      }
      insertedElements.emplace_back(
        CaRQF::InsertedElement {&j.job->m_retrieveRequest, tf_it->copyNb, tf_it->fSeq, j.job->archiveFile.fileSize,
                                common::dataStructures::MountPolicy(), j.job->m_activity, j.job->m_diskSystemName});
    }
    try {
...
jobFound : {
  ...
  logContext.log(log::INFO,
                 "In OStoreDB::queueRetrieve(): recorded request for queueing (enqueueing posted to thread pool).");
}
  return ret;
}
...
void OStoreDB::cancelRetrieve(const std::string& instanceName,
  ...
  try {
    ...
    log::ScopedParamContainer params(lc);
    params.add("archiveFileId", rqst.archiveFileID).add("retrieveRequestId", rqst.retrieveRequestId);
  ...
  if (rr.isFailed()) {
    log::ScopedParamContainer params(lc);
    params.add("ArchiveFileID", rqst.archiveFileID).add("RetrieveRequest", rqst.retrieveRequestId);
...
void OStoreDB::cancelArchive(const common::dataStructures::DeleteArchiveRequest& request, log::LogContext& lc) {
  ...
  try {
    ...
    log::ScopedParamContainer params(lc);
    params.add("archiveFileId", request.archiveFileID).add("archiveRequestId", request.address.value());
  ...
  if (ar.isFailed()) {
    log::ScopedParamContainer params(lc);
    params.add("ArchiveFileID", request.archiveFileID).add("archiveRequest", request.address.value());
...
void OStoreDB::requeueRetrieveRequestJobs(std::list<cta::SchedulerDatabase::RetrieveJob*>& jobs,
  ...
  for (auto& job : jobs) {
    ...
    rr->garbageCollectRetrieveRequest(m_agentReference->getAgentAddress(), *m_agentReference, logContext, m_catalogue,
                                      true);
  }
  locks.clear();
  rrlist.clear();
...
void OStoreDB::reserveRetrieveQueueForCleanup(const std::string& vid, std::optional<uint64_t> cleanupHeartBeatValue) {
  ...
  if (!rq.getQueueCleanupDoCleanup()) {
    throw RetrieveQueueNotReservedForCleanup(
      "Queue no longer has the cleanup flag enabled after fetching. Skipping it.");
  }
...
void OStoreDB::tickRetrieveQueueCleanupHeartbeat(const std::string& vid) {
  ...
  try {
    ...
    if (rq.getQueueCleanupAssignedAgent().has_value() &&
      ...
      throw RetrieveQueueNotReservedForCleanup("Another agent is alive and cleaning up the retrieve queue of tape " +
...
auto OStoreDB::RepackRequestPromotionStatistics::promotePendingRequestsForExpansion(size_t requestCount,
  ...
  RQPAlgo::PoppedElementsBatch poppedElements;
  {
    ...
    poppedElements = rqpAlgo.popNextBatchFromContainerAndSwitchStatus(
      m_pendingRepackRequestQueue, m_lockOnPendingRepackRequestsQueue, criteria, newStatus, lc);
...
void OStoreDB::setRetrieveQueueCleanupFlag(const std::string& vid, bool val, log::LogContext& logContext) {
...

  {
    ...
    try {
      ...
      qAddress = re.addOrGetRetrieveQueueAndCommit(vid, *m_agentReference,
                                                   common::dataStructures::JobQueueType::JobsToTransferForUser);
      rqueue.setAddress(qAddress);
  ...
  try {
    ...
    logContext.log(
      log::DEBUG,
      "WARNING: In OStoreDB::setRetrieveQueueCleanupFlag(): failed to set cleanup flag value on retrieve queue.");
  }
  ...
  if (queueLockTime > 1 || queueFetchTime > 1) {
    logContext.log(
      log::WARNING,
      "In OStoreDB::setRetrieveQueueCleanupFlag(): fetched a retrieve queue and that lasted more than 1 second.");
  }
}
  ...
  uint64_t nbTransformedSubrequest = 0;
  {
    ...
    for (auto& rr : m_subrequestList) {
      try {
        asyncTransformsAndReqs.push_back(
          {rr, std::unique_ptr<objectstore::RetrieveRequest::AsyncRetrieveToArchiveTransformer>(
        ...
        params.add("fileId", rr.archiveFile.archiveFileID)
          ...
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): failed to "
        ...
        lc.log(
          log::ERR,
          "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): failed to asyncTransformToArchiveRequest().");
      }
    }
    ...
    for (auto& atar : asyncTransformsAndReqs) {
      try {
        ...
        lc.log(log::INFO, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(), turned successful retrieve "
                          "request in archive request.");
        SorterArchiveRequest sorterArchiveRequest;
        for (auto& copyNbToArchive : atar.subrequestInfo.repackInfo.copyNbsToRearchive) {
          ...
          sorterArchiveJob.archiveRequest = std::make_shared<objectstore::ArchiveRequest>(
            atar.subrequestInfo.subrequest->getAddressIfSet(), m_oStoreDb.m_objectStore);
        ...
        successfullyTransformedSubrequests.push_back(
          SuccessfullyTranformedRequest {std::make_shared<objectstore::ArchiveRequest>(
                                           atar.subrequestInfo.subrequest->getAddressIfSet(), m_oStoreDb.m_objectStore),
                                         atar.subrequestInfo, sorterArchiveRequest});
      }
      catch (const cta::exception::NoSuchObject& ex) {
        log::ScopedParamContainer params(lc);
        params.add("fileId", atar.subrequestInfo.archiveFile.archiveFileID)
          .add("subrequestAddress", atar.subrequestInfo.subrequest->getAddressIfSet())
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): async transformation failed "
        ...
        params.add("fileId", atar.subrequestInfo.archiveFile.archiveFileID)
          ...
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::ERR,
               "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): async transformation failed on wait().");
      }
    }
    ...
    if (failedSubrequests.size()) {
      ...
      for (auto& fs : failedSubrequests) {
        ...
        try {
          ...
          params.add("fileId", fs->archiveFile.archiveFileID)
            ...
            .add("exceptionMsg", ex.getMessageValue());
          lc.log(log::WARNING, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): failed to asyncDelete() "
          ...
          lc.log(
            log::ERR,
            "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): failed to asyncDelete() retrieve request.");
        }
      }
      ...
      for (auto& adar : asyncDeleterAndReqs) {
        try {
          ...
          lc.log(log::INFO, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): deleted retrieve request after "
                            "failure to transform in archive request.");
        }
        catch (const cta::exception::NoSuchObject& ex) {
          // Log the failure to delete.
          log::ScopedParamContainer params(lc);
          params.add("fileId", adar.subrequestInfo.archiveFile.archiveFileID)
            .add("subrequestAddress", adar.subrequestInfo.subrequest->getAddressIfSet())
            .add("exceptionMsg", ex.getMessageValue());
          lc.log(log::WARNING, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): async deletion of retrieve "
                               "request failed on wait(). Object does not exist in the objectstore.");
        }
        catch (cta::exception::Exception& ex) {
          // Log the failure to delete.
          log::ScopedParamContainer params(lc);
          params.add("fileId", adar.subrequestInfo.archiveFile.archiveFileID)
            .add("subrequestAddress", adar.subrequestInfo.subrequest->getAddressIfSet())
            .add("exceptionMsg", ex.getMessageValue());
          lc.log(log::ERR, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): async deletion of retrieve "
                           "request failed on wait().");
        }
      }
      ...
      lc.log(log::INFO, "In OStoreDB::RepackRetrieveSuccessesReportBatch::report(): successfully removed retrieve "
                        "requests from the agent's ownership.");
    }
  }
...
void OStoreDB::RepackRetrieveFailureReportBatch::report(log::LogContext& lc) {
  ...
  // before the next step.
  {
    ...
    for (auto& fs : m_subrequestList) {
      ...
      try {
        ...
        params.add("fileId", fs.archiveFile.archiveFileID)
          .add("subrequestAddress", fs.subrequest->getAddressIfSet())
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RepackRetrieveFailureReportBatch::report(): failed to asyncDelete() "
                             "retrieve request. Object does not exist in the objectstore.");
      }
      catch (cta::exception::Exception& ex) {
        // Log the failure to delete.
    ...
    for (auto& adar : asyncDeleterAndReqs) {
      try {
        ...
        params.add("fileId", adar.subrequestInfo.archiveFile.archiveFileID)
          .add("subrequestAddress", adar.subrequestInfo.subrequest->getAddressIfSet())
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RepackRetrieveFailureReportBatch::report(): async deletion of retrieve "
                             "request failed on wait(). Object does not exist in the objectstore.");
      }
      catch (cta::exception::Exception& ex) {
        // Log the failure to delete.
        ...
        params.add("fileId", adar.subrequestInfo.archiveFile.archiveFileID)
          .add("subrequestAddress", adar.subrequestInfo.subrequest->getAddressIfSet())
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::ERR, "In OStoreDB::RepackRetrieveFailureReportBatch::report(): async deletion of retrieve request "
                         "failed on wait().");
      }
    }
    timingList.insertAndReset("asyncDeleteRetrieveWaitTime", t);
    ...
    timingList.addToLog(params);
    params.add("agentAddress", m_oStoreDb.m_agentReference->getAgentAddress());
    lc.log(log::INFO, "In OStoreDB::RepackRetrieveFailureReportBatch::report(): successfully removed retrieve requests "
      ...
      if (!subReqInfoMap.at(rsr.fSeq).subrequestDeleted) {
        ...
        try {
          ...
          for (auto& currentCopyNbTapePool : rRRepackInfo.archiveRouteMap) {
            int nbTapepool =
              std::count_if(rRRepackInfo.archiveRouteMap.begin(), rRRepackInfo.archiveRouteMap.end(),
        ...
        if (bestVid.empty()) {
          ...
          try {
            bestVid =
              Helpers::selectBestRetrieveQueue(candidateVids, m_oStoreDB.m_catalogue, m_oStoreDB.m_objectStore, true);
          }
          catch (Helpers::NoTapeAvailableForRetrieve&) {
            ...
            lc.log(log::ERR, "In OStoreDB::RepackRequest::addSubrequests(): could not queue a retrieve subrequest. "
                             "Subrequest failed. Maybe the tape to repack is disabled ?");
            goto nextSubrequest;
          }
        }
        for (auto& tc : rsr.archiveFile.tapeFiles) {
        ...
        }
        {
          ...
          lc.log(log::ERR, "In OStoreDB::RepackRequest::addSubrequests(): could not find the copyNb for the chosen "
                           "VID. Subrequest failed.");
          goto nextSubrequest;
        }
copyNbFound:;
        if (rsr.hasUserProvidedFile) {
        ...
        try {
          ...
          lc.log(log::ERR, "In OStoreDB::RepackRequest::addSubrequests(): could not asyncInsert the subrequest because "
                           "it already exists, continuing expansion");
          goto nextSubrequest;
        }
        catch (exception::Exception& ex) {
          ...
          params.add("fileId", rsr.archiveFile.archiveFileID)
            ...
            .add("bestVid", bestVid)
            .add("ExceptionMessage", ex.getMessageValue());
          lc.log(log::ERR, "In OStoreDB::RepackRequest::addSubrequests(): could not asyncInsert the subrequest.");
        }
      }
nextSubrequest:
      nbSubReqProcessed++;
    ...
    if (notCreatedSubrequests.size()) {
      ...
      m_repackRequest.commit();
      lc.log(log::ERR, "In OStoreDB::RepackRequest::addSubRequests(), reported the failed creation of Retrieve "
                       "Requests to the Repack request");
    }
...
SchedulerDatabase::JobsFailedSummary OStoreDB::getRetrieveJobsFailedSummary(log::LogContext& logContext) {
  ...
  for (auto& rj : queueList) {
    ...
    try {
      ...
      log::ScopedParamContainer params(logContext);
      params.add("queueObject", rj.address).add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::DEBUG,
...
//------------------------------------------------------------------------------
std::unique_ptr<SchedulerDatabase::ArchiveMount>
  OStoreDB::TapeMountDecisionInfo::createArchiveMount(const common::dataStructures::MountType& type,
                                                      const catalogue::TapeForWriting& tape,
                                                      const std::string& driveName,
                                                      const std::string& logicalLibrary,
                                                      const std::string& hostName) {
...
//------------------------------------------------------------------------------
std::list<std::unique_ptr<SchedulerDatabase::ArchiveJob>>
  OStoreDB::ArchiveMount::getNextJobBatch(uint64_t filesRequested,
...
void OStoreDB::RetrieveMount::putQueueToSleep(const std::string& diskSystemName,
  ...
  Helpers::getLockedAndFetchedJobQueue<RetrieveQueue>(rq, lock, *m_oStoreDB.m_agentReference, vid, queueType,
                                                      logContext);
...
bool OStoreDB::RetrieveMount::testReserveDiskSpace(const cta::DiskSpaceReservationRequest& diskSpaceReservationRequest,
  ...
  try {
    ...
    for (const auto& failedDiskSystem : ex.m_failedDiskSystems) {
      ...
      logContext.log(cta::log::ERR, "In OStoreDB::RetrieveMount::testReserveDiskSpace(): unable to request EOS free "
                                    "space for disk system, putting queue to sleep");
    ...
    logContext.log(cta::log::ERR, "In OStoreDB::RetrieveMount::testReserveDiskSpace(): got an exception from "
  ...
  for (const auto& ds : diskSystemNames) {
    ...
    if (diskSystemFreeSpace.at(ds).freeSpace < diskSpaceReservationRequest.at(ds) +
      ...
      logContext.log(cta::log::WARNING, "In OStoreDB::RetrieveMount::testReserveDiskSpace(): could not allocate disk "
                                        "space for job, applying backpressure");
...
bool OStoreDB::RetrieveMount::reserveDiskSpace(const cta::DiskSpaceReservationRequest& diskSpaceReservationRequest,
  ...
  try {
    ...
    for (const auto& failedDiskSystem : ex.m_failedDiskSystems) {
      ...
      params.add("diskSystemName", failedDiskSystem.first);
      params.add("failureReason", failedDiskSystem.second.getMessageValue());
      logContext.log(cta::log::ERR, "In OStoreDB::RetrieveMount::reserveDiskSpace(): unable to request EOS free space "
    ...
    cta::log::ScopedParamContainer params(logContext);
    params.add("exceptionWhat", ex.what());
    logContext.log(cta::log::ERR, "In OStoreDB::RetrieveMount::reserveDiskSpace(): got an exception from "
                                  "diskSystemFreeSpace.fetchDiskSystemFreeSpace().");
    throw;
  }
  ...
  for (const auto& ds : diskSystemNames) {
    ...
    if (diskSystemFreeSpace.at(ds).freeSpace < diskSpaceReservationRequest.at(ds) +
      ...
      logContext.log(
        cta::log::WARNING,
        "In OStoreDB::RetrieveMount::reservediskSpace(): could not allocate disk space for job, applying backpressure");
...
void OStoreDB::RetrieveMount::flushAsyncSuccessReports(std::list<cta::SchedulerDatabase::RetrieveJob*>& jobsBatch,
    ...
    if (osdbJob->isRepack) {
      try {
        ...
        params.add("fileId", osdbJob->archiveFile.archiveFileID)
          ...
          .add("exceptionMessage", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RetrieveMount::flushAsyncSuccessReports(): async status update failed, job "
        ...
        params.add("fileId", osdbJob->archiveFile.archiveFileID)
          .add("requestObject", osdbJob->m_retrieveRequest.getAddressIfSet())
          .add("exceptionMessage", ex.getMessageValue());
        lc.log(log::ERR, "In OStoreDB::RetrieveMount::flushAsyncSuccessReports(): async status update failed. "
      ...
      try {
        osdbJob->m_jobDelete->wait();
        {
          ...
          cta::log::ScopedParamContainer spc(lc);
          std::string vid = osdbJob->archiveFile.tapeFiles.at(osdbJob->selectedCopyNb).vid;
          spc.add("tapeVid", vid).add("mountType", "RetrieveForUser").add("fileId", osdbJob->archiveFile.archiveFileID);
          lc.log(cta::log::INFO, "In OStoreDB::RetrieveMount::flushAsyncSuccessReports(), retrieve job successful");
        }
        ...
        params.add("fileId", osdbJob->archiveFile.archiveFileID)
          .add("requestObject", osdbJob->m_retrieveRequest.getAddressIfSet())
          .add("exceptionMessage", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RetrieveMount::flushAsyncSuccessReports(): async deletion failed because "
        ...
        params.add("fileId", osdbJob->archiveFile.archiveFileID)
          .add("requestObject", osdbJob->m_retrieveRequest.getAddressIfSet())
          .add("exceptionMessage", ex.getMessageValue());
        lc.log(log::ERR, "In OStoreDB::RetrieveMount::flushAsyncSuccessReports(): async deletion failed. "
                         "Will leave job to garbage collection.");
      }
    }
  }
  ...
  for (auto& repackRequestQueue : jobsToRequeueForRepackMap) {
    ...
    for (auto& req : repackRequestQueue.second) {
      insertedRequests.push_back(RQTRTRFSAlgo::InsertedElement {
        &req->m_retrieveRequest, req->selectedCopyNb, req->archiveFile.tapeFiles.at(req->selectedCopyNb).fSeq,
        req->archiveFile.fileSize, mountPolicy, req->m_activity, req->m_diskSystemName});
...
void OStoreDB::ArchiveMount::setJobBatchTransferred(
  ...
  while (jobsBatchItor != jobsBatch.end()) {
    ...
    try {
      ...
      params.add("tapeVid", (*jobsBatchItor)->tapeFile.vid)
        ...
        .add("requestObject", castFromSchedDBJob(jobsBatchItor->get())->m_archiveRequest.getAddressIfSet())
        .add("exceptionMessage", ex.getMessageValue());
      lc.log(log::WARNING, "In OStoreDB::RetrieveMount::setJobBatchTransferred(): async succeed transfer failed, job "
                           "does not exist in the objectstore.");
    }
  }
  ...
  while (jobsBatchItor != jobsBatch.end()) {
    try {
      ...
      jobsBatch.erase(jobsBatchItor++);
      log::ScopedParamContainer params(lc);
      params.add("fileId", (*jobsBatchItor)->archiveFile.archiveFileID).add("exceptionMessage", ex.getMessageValue());
      lc.log(log::WARNING, "In OStoreDB::RetrieveMount::setJobBatchTransferred(): wait async succeed transfer failed, "
  ...
  if (jobsToQueueForReportingToUser.size()) {
    ...
    for (auto& list : insertedElementsLists) {
      try {
        ...
        log::ScopedParamContainer params(lc);
        params.add("tapeVid", list.first).add("jobs", list.second.size()).add("enqueueTime", t.secs());
        lc.log(
          log::INFO,
          "In OStoreDB::ArchiveMount::setJobBatchTransferred(): queued a batch of requests for reporting to user.");
      }
      catch (cta::exception::Exception& ex) {
        log::ScopedParamContainer params(lc);
        params.add("tapeVid", list.first).add("exceptionMSG", ex.getMessageValue());
        lc.log(log::ERR, "In OStoreDB::ArchiveMount::setJobBatchTransferred(): failed to queue a batch of requests for "
                         "reporting to user.");
        lc.logBacktrace(log::INFO, ex.backtrace());
      }
    }
  ...
  if (jobsToQueueForReportingToRepack.size()) {
    ...
    for (auto& j : jobsToQueueForReportingToRepack) {
      insertedElementsLists[j->getRepackInfoAfterAsyncSuccess().repackRequestAddress].emplace_back(
        AqtrtrCa::InsertedElement {&j->m_archiveRequest, j->tapeFile.copyNb, j->archiveFile, std::nullopt,
                                   std::nullopt});
      log::ScopedParamContainer params(lc);
      params.add("repackRequestAddress", j->getRepackInfoAfterAsyncSuccess().repackRequestAddress)
        ...
        .add("requestObject", j->m_archiveRequest.getAddressIfSet());
      lc.log(log::INFO,
             "In OStoreDB::ArchiveMount::setJobBatchTransferred(): will queue request for reporting to repack.");
    }
    for (auto& list : insertedElementsLists) {
retry:
      ...
      try {
        utils::Timer tLocal;
        aqtrtrCa.referenceAndSwitchOwnership(list.first, m_oStoreDB.m_agentReference->getAgentAddress(), list.second,
                                             lc);
        log::ScopedParamContainer params(lc);
        params.add("repackRequestAddress", list.first).add("jobs", list.second.size()).add("enqueueTime", t.secs());
        lc.log(
          log::INFO,
          "In OStoreDB::ArchiveMount::setJobBatchTransferred(): queued a batch of requests for reporting to repack.");
      }
      catch (cta::exception::NoSuchObject& ex) {
        log::ScopedParamContainer params(lc);
        params.add("tapeVid", list.first).add("exceptionMSG", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::ArchiveMount::setJobBatchTransferred(): failed to queue a batch of requests "
                             "for reporting to repack, jobs do not exist in the objectstore.");
      }
        ...
        params.add("tapeVid", list.first)
          ...
          .add("exceptionMSG", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::ArchiveMount::setJobBatchTransferred(): unable to queue some elements "
        ...
        list.second.remove_if([&ex](const AqtrtrCa::InsertedElement& elt) {
          ...
          return std::find_if(ex.failedElements.begin(), ex.failedElements.end(),
  ...
  }
  {
    ...
    lc.log(
      log::INFO,
      "In OStoreDB::ArchiveMount::setJobBatchTransferred(): set ArchiveRequests successful and queued for reporting.");
  }
}
...
void OStoreDB::ArchiveJob::failTransfer(const std::string& failureReason, log::LogContext& lc) {
  ...
  switch (enQueueingNextStep.nextStep) {
    case NextStep::Nothing: {
      ...
      lc.log(log::INFO, "In ArchiveJob::failTransfer(): left the request owned, to be garbage collected for retry at "
    ...
    case NextStep::EnqueueForReportForRepack: {
      ...
      params.add("fileId", archiveFile.archiveFileID)
        ...
        .add("totalRetries", retryStatus.totalRetries)
        .add("maxTotalRetries", retryStatus.maxTotalRetries);
      lc.log(log::INFO, "In ArchiveJob::failTransfer(): enqueued job for reporting for Repack");
      return;
    }
    case NextStep::EnqueueForTransferForUser: {
      ...
      params.add("fileId", archiveFile.archiveFileID)
        ...
        .add("totalRetries", retryStatus.totalRetries)
        .add("maxTotalRetries", retryStatus.maxTotalRetries);
      lc.log(log::INFO, "In ArchiveJob::failTransfer(): requeued job for (potentially in-mount) retry.");
      return;
    }
    case NextStep::EnqueueForTransferForRepack: {
      ...
      params.add("fileId", archiveFile.archiveFileID)
        ...
        .add("totalRetries", retryStatus.totalRetries)
        .add("maxTotalRetries", retryStatus.maxTotalRetries);
      lc.log(log::INFO, "In ArchiveJob::failTransfer(): requeued job for (potentially in-mount) retry (repack).");
      return;
    }
    case NextStep::StoreInFailedJobsContainer: {
      ...
      params.add("fileId", archiveFile.archiveFileID)
        ...
        .add("totalRetries", retryStatus.totalRetries)
        .add("maxTotalRetries", retryStatus.maxTotalRetries);
      lc.log(log::INFO, "In ArchiveJob::failTransfer(): stored job in failed container for operator handling.");
      return;
    }
  }
}
  ...
  switch (enQueueingNextStep.nextStep) {
    ...
    default: {
      ...
      if (enQueueingNextStep.nextStep == NextStep::StoreInFailedJobsContainer) {
        lc.log(log::INFO, "In ArchiveJob::failReport(): stored job in failed container for operator handling.");
      }
      else {
        lc.log(log::ERR, "In ArchiveJob::failReport(): stored job in failed container after unexpected next step.");
      }
      return;
    }
  }
...
void OStoreDB::RepackArchiveReportBatch::report(log::LogContext& lc) {
    ...
    if (moreJobsToDo) {
      try {
        if (ar.exists()) {
          jobOwnerUpdatersList.push_back(
            JobOwnerUpdaters {std::unique_ptr<objectstore::ArchiveRequest::AsyncJobOwnerUpdater>(ar.asyncUpdateJobOwner(
                                sri.archivedCopyNb, "", m_oStoreDb.m_agentReference->getAgentAddress(), newStatus)),
                              sri});
        }
      }
      catch (const cta::exception::NoSuchObject& ex) {
        ...
        params.add("fileId", sri.archiveFile.archiveFileID)
          ...
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RepackArchiveReportBatch::report(): failed to asyncUpdateJobOwner(), object "
                             "does not exist in the objectstore.");
      }
      catch (cta::exception::Exception& ex) {
        // Log the error
      ...
      try {
        ...
        params.add("fileId", sri.archiveFile.archiveFileID)
          .add("subrequestAddress", sri.subrequest->getAddressIfSet())
          .add("exceptionMsg", ex.getMessageValue());
        lc.log(log::WARNING, "In OStoreDB::RepackArchiveReportBatch::report(): failed to asyncDelete(), object does "
                             "not exist in the objectstore.");
      }
      catch (cta::exception::Exception& ex) {
        // Log the error
  ...
  for (auto& d : deletersList) {
    try {
      ...
      params.add("fileId", d.subrequestInfo.archiveFile.archiveFileID)
        .add("subrequestAddress", d.subrequestInfo.subrequest->getAddressIfSet())
        .add("exceptionMsg", ex.getMessageValue());
      lc.log(log::WARNING, "In OStoreDB::RepackArchiveReportBatch::report(): async deletion failed. Object does not "
  ...
  if (repackRequestStatus == objectstore::serializers::RepackRequestStatus::RRS_Complete ||
    ...
    if (repackRequestStatus == objectstore::serializers::RepackRequestStatus::RRS_Complete) {
      ...
      try {
        ...
        log::ScopedParamContainer params(lc);
        params.add("repackRequestAddress", m_repackRequest.getAddressIfSet()).add("exceptionMsg", ex.getMessageValue());
        lc.log(log::ERR,
  ...
  for (auto& jou : jobOwnerUpdatersList) {
    try {
      ...
      params.add("fileId", jou.subrequestInfo.archiveFile.archiveFileID)
        ...
        .add("copyNb", jou.subrequestInfo.archivedCopyNb)
        .add("exceptionMsg", ex.getMessageValue());
      lc.log(log::WARNING, "In OStoreDB::RepackArchiveReportBatch::report(): async job update failed. Object does not "
                           "exist in the objectstore.");
    }
    catch (cta::exception::Exception& ex) {
      // Log the error
  ...
  timingList.addToLog(params);
  params.add("archiveReportType", (newStatus == cta::objectstore::serializers::ArchiveJobStatus::AJS_Complete) ?
                                    "ArchiveSuccesses" :
                                    "ArchiveFailures");
  ...
  switch (enQueueingNextStep.nextStep) {
    case NextStep::Nothing: {
      ...
      params.add("fileId", archiveFile.archiveFileID)
        ...
        .add("totalRetries", retryStatus.totalRetries)
        .add("maxTotalRetries", retryStatus.maxTotalRetries);
      lc.log(log::INFO, "In RetrieveJob::failTransfer(): left the request owned, to be garbage collected for retry at "
                        "the end of the mount.");
      return;
    }
    ...
    case NextStep::EnqueueForReportForUser: {
      ...
      insertedElements.push_back(CaRqtr::InsertedElement {&m_retrieveRequest, tf.copyNb, tf.fSeq, af.fileSize,
                                                          rfqc.mountPolicy, m_activity, m_diskSystemName});
      m_retrieveRequest.commit();
      rel.release();
