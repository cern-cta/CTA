...
struct RepackRequestTrackingRow {
  ...
  void insert(rdbms::Conn& conn) const {
    ...
    conn.setDbQuerySummary(cta::semconv::attr::DbQuerySummary::kDbInsertRepack);
  ...
  static rdbms::Rset getRepackRequestStatistics(rdbms::Conn& conn) {
    ...
    auto stmt = conn.createStmt(sql);
    conn.setDbQuerySummary(cta::semconv::attr::DbQuerySummary::kDbSelectRepack);
  ...
  /**
    ...
    */
  static rdbms::Rset updateRepackRequestsProgress(Transaction& txn, const std::vector<RepackRequestProgress>& updates);
