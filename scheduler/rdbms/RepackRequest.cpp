  ...
  uint64_t RepackRequest::addSubrequestsAndUpdateStats(
    ...
    for (auto &r: repackSubrequests) {
      ...
      if (srmap.find(r.fSeq) == srmap.end()) {
        ...
        newSub.archiveCopyNbAccounted = r.copyNbsToRearchive;
        newSub.fSeq = r.fSeq;
        srmap[r.fSeq] = &newSub;
      }
    }
      ...
      if (!rrRowBunchToTransfer.empty()) {
        ...
        try {
          ...
          for (auto &row: rrRowBunchToTransfer) {
            ...
            failedArchiveReq += srmap[row.fSeq]->archiveCopyNbAccounted.size();
          }
        }
      }
...
//}
  ...
  RepackRequest &RepackRequest::operator=(const schedulerdb::postgres::RepackRequestTrackingRow &row) {
    ...
    //  // Suppose you have these variables from row or some config
    //  std::string_view policyName = row.mountPolicyName;
    //  uint64_t archivePriority = row.archivePriority;             // new DB column
    //  uint64_t archiveMinRequestAge = row.archiveMinRequestAge;   // new DB column
    //  uint64_t retrievePriority = row.retrievePriority;           // new DB column
    //  uint64_t retrieveMinRequestAge = row.retrieveMinRequestAge; // new DB column
//
    //  // Create MountPolicy
