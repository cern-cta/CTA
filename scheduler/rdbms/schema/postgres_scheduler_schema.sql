...
CREATE INDEX IDX_ARCHIVE_JOB_QUEUE_COMPOSITE ON ARCHIVE_JOB_QUEUE (TAPE_POOL, STATUS, MOUNT_ID, IN_DRIVE_QUEUE);
CREATE INDEX IDX_ARCHIVE_JOB_QUEUE_COMPOSITE_ORDERING ON ARCHIVE_JOB_QUEUE (PRIORITY, JOB_ID);
...
CREATE TABLE ARCHIVE_JOB_SUMMARY_NEW(
    STATUS                   ARCHIVE_JOB_STATUS CONSTRAINT AJQ_S_NN NOT NULL,
    TAPE_POOL                VARCHAR(100) CONSTRAINT AJQ_TPN_NN NOT NULL,
    MOUNT_POLICY             VARCHAR(100) CONSTRAINT AJQ_MPN_NN NOT NULL,
    JOBS_COUNT               BIGINT,
...
CREATE VIEW ARCHIVE_JOB_SUMMARY AS (SELECT MOUNT_ID,
    ...
    MAX(LAST_UPDATE_TIME) AS LAST_JOB_UPDATE_TIME,
        FROM ARCHIVE_JOB_QUEUE WHERE MOUNT_ID IS NULL
        AND IN_DRIVE_QUEUE IS FALSE GROUP BY STATUS, TAPE_POOL, MOUNT_POLICY
    );
