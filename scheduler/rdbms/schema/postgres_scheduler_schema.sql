...
CREATE VIEW RETRIEVE_JOB_SUMMARY AS (SELECT STATUS,
    VID,
    MOUNT_POLICY,
    MOUNT_ID,
    COUNT(*) AS JOBS_COUNT,
    ...
    MAX(LAST_UPDATE_TIME) AS LAST_JOB_UPDATE_TIME
        FROM RETRIEVE_INSERT_QUEUE WHERE MOUNT_ID IS NULL GROUP BY STATUS, VID, MOUNT_POLICY, MOUNT_ID
    );
