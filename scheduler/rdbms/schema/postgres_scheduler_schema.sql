...
/* REPACK_DEST_VID VARCHAR(20), */
  ...
  IN_DRIVE_QUEUE BOOLEAN DEFAULT FALSE,
/* ARCHIVE specific columns */
...
CREATE INDEX IDX_ARCHIVE_JOB_QUEUE_COMPOSITE ON ARCHIVE_JOB_QUEUE (TAPE_POOL, STATUS, MOUNT_ID, IN_DRIVE_QUEUE, PRIORITY, JOB_ID);
CREATE INDEX IDX_ARCHIVE_JOB_QUEUE_TAPE_POOL_PRIORITY_JOB_ID ON ARCHIVE_JOB_QUEUE (TAPE_POOL, PRIORITY, JOB_ID);
CREATE INDEX IDX_ARCHIVE_JOB_QUEUE_MOUNT_ID_IN_DRIVE_QUEUE_STATUS_LAST_UPDATE_TIME ON ARCHIVE_JOB_QUEUE (MOUNT_ID, IN_DRIVE_QUEUE, STATUS, LAST_UPDATE_TIME);
CREATE INDEX IDX_ARCHIVE_JOB_QUEUE_MOUNT_ID_IN_DRIVE_QUEUE_STATUS_LAST_UPDATE_TIME ON ARCHIVE_JOB_QUEUE (MOUNT_ID, IN_DRIVE_QUEUE);
...
/* From RetrieveJobPointer/RetrieveQueue */
  ...
  PRIORITY SMALLINT,
  MIN_RETRIEVE_REQUEST_AGE BIGINT,
  START_TIME BIGINT,
...
CREATE VIEW ARCHIVE_JOB_SUMMARY AS (SELECT MOUNT_ID,
    STATUS,
    IN_DRIVE_QUEUE,
    TAPE_POOL,
    MOUNT_POLICY,
    ...
    EXTRACT(EPOCH FROM NOW())::INTEGER AS LAST_SUMMARY_UPDATE_TIME FROM ARCHIVE_JOB_QUEUE GROUP BY MOUNT_ID,
    STATUS,
    IN_DRIVE_QUEUE,
    TAPE_POOL,
    MOUNT_POLICY);
CREATE VIEW RETRIEVE_JOB_SUMMARY AS (SELECT MOUNT_ID,
    STATUS,
    VID,
    MOUNT_POLICY,
    COUNT(*) AS JOBS_COUNT,
    SUM(SIZE_IN_BYTES) AS JOBS_TOTAL_SIZE,
    MIN(START_TIME) AS OLDEST_JOB_START_TIME,
    MAX(PRIORITY) AS RETRIEVE_PRIORITY,
    MIN(MIN_RETRIEVE_REQUEST_AGE) AS RETRIEVE_MIN_REQUEST_AGE,
    MAX(LAST_UPDATE_TIME) AS LAST_JOB_UPDATE_TIME,
    EXTRACT(EPOCH FROM NOW())::INTEGER AS LAST_UPDATE_TIME FROM RETRIEVE_JOB_QUEUE GROUP BY MOUNT_ID,
    STATUS,
    VID,
    MOUNT_POLICY);
