...
ALTER TABLE RETRIEVE_JOB_QUEUE ADD COLUMN LAST_UPDATE_TIME timestamp DEFAULT CURRENT_TIMESTAMP;
...
CREATE MATERIALIZED VIEW ARCHIVE_JOB_SUMMARY AS (SELECT MOUNT_ID,
    STATUS,
    TAPE_POOL,
    MOUNT_POLICY,
    COUNT(*) AS JOBS_COUNT,
    ...
    EXTRACT(EPOCH FROM NOW()) AS LAST_UPDATE_TIME FROM ARCHIVE_JOB_QUEUE GROUP BY MOUNT_ID,
    STATUS,
    TAPE_POOL,
    MOUNT_POLICY);
CREATE MATERIALIZED VIEW RETRIEVE_JOB_SUMMARY AS (SELECT MOUNT_ID,
    STATUS,
    VID,
    MOUNT_POLICY,
    COUNT(*) AS JOBS_COUNT,
    ...
    MAX(LAST_UPDATE_TIME) AS LAST_JOB_UPDATE_TIME,
    EXTRACT(EPOCH FROM NOW()) AS LAST_UPDATE_TIME FROM RETRIEVE_JOB_QUEUE GROUP BY MOUNT_ID,
    STATUS,
    VID,
    MOUNT_POLICY);
