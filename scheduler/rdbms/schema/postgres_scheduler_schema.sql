...
CREATE INDEX IDX_REPACK_ARCHIVE_ACTIVE_QUEUE_SINGLE_COPY ON REPACK_ARCHIVE_ACTIVE_QUEUE (JOB_ID) WHERE REQUEST_JOB_COUNT = 1;
CREATE INDEX IDX_REPACK_ARCHIVE_ACTIVE_QUEUE_MULTI_COPY ON REPACK_ARCHIVE_ACTIVE_QUEUE (JOB_ID) WHERE REQUEST_JOB_COUNT > 1;
...
CREATE TABLE SCHEDULER_MOUNT_WORK_LOCK (
  WORK_KEY VARCHAR(255) CONSTRAINT SMWL_WK_NN NOT NULL,
  LOCKED_BY_HOST VARCHAR(100) CONSTRAINT SMWL_LBH_NN NOT NULL,
  LOCKED_TIME INTEGER DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::INTEGER),
  LOCK_HEARTBEAT_TIME INTEGER DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::INTEGER),
  LOCK_EXPIRES_TIME INTEGER CONSTRAINT SMWL_LET_NN NOT NULL,
  CONSTRAINT SCHEDULER_MOUNT_WORK_LOCK_PK PRIMARY KEY(WORK_KEY)
);

CREATE INDEX IDX_SCHEDULER_MOUNT_WORK_LOCK_EXPIRY ON SCHEDULER_MOUNT_WORK_LOCK (LOCK_EXPIRES_TIME);

CREATE TABLE SCHEDULER_MOUNT_CANDIDATES (
  CANDIDATE_ID BIGSERIAL PRIMARY KEY,
  CANDIDATE_KEY VARCHAR(512) CONSTRAINT SMC_CK_NN NOT NULL,
/* Mount identity */
  MOUNT_TYPE VARCHAR(100) CONSTRAINT SMC_MT_NN NOT NULL,
  LOGICAL_LIBRARY VARCHAR(100) CONSTRAINT SMC_LL_NN NOT NULL,
  TAPE_POOL VARCHAR(100) CONSTRAINT SMC_TP_NN NOT NULL,
  VO VARCHAR(100) CONSTRAINT SMC_VO_NN NOT NULL,
  VID VARCHAR(20),
  ACTIVITY VARCHAR(100),
/* Precomputed scheduling and ordering data */
  PRIORITY SMALLINT CONSTRAINT SMC_PRIORITY_NN NOT NULL,
  MIN_REQUEST_AGE INTEGER CONSTRAINT SMC_MRA_NN NOT NULL,
  FILES_QUEUED BIGINT CONSTRAINT SMC_FQ_NN NOT NULL,
  BYTES_QUEUED BIGINT CONSTRAINT SMC_BQ_NN NOT NULL,
  OLDEST_JOB_START_TIME BIGINT CONSTRAINT SMC_OJST_NN NOT NULL,
  YOUNGEST_JOB_START_TIME BIGINT CONSTRAINT SMC_YJST_NN NOT NULL,
  RATIO_OF_MOUNT_QUOTA_USED DOUBLE PRECISION CONSTRAINT SMC_ROMQU_NN NOT NULL,
  CANDIDATE_SCORE BIGINT CONSTRAINT SMC_CS_NN NOT NULL,
/* Tape metadata */
  MEDIA_TYPE VARCHAR(100),
  LABEL_FORMAT SMALLINT,
  VENDOR VARCHAR(100),
  CAPACITY_IN_BYTES BIGINT,
  LAST_FSEQ BIGINT,
  ENCRYPTION_KEY_NAME VARCHAR(100),
/* Reservation by taped */
  STATE VARCHAR(100) DEFAULT 'Available' CONSTRAINT SMC_STATE_NN NOT NULL,
  STATE_REASON VARCHAR(1000),
  RESERVED_BY_HOST VARCHAR(100),
  RESERVED_BY_DRIVE VARCHAR(100),
  RESERVED_TIME INTEGER,
  RESERVATION_HEARTBEAT_TIME INTEGER,
/* Producer/debug metadata */
  CREATED_BY VARCHAR(100),
  CREATION_TIME INTEGER DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::INTEGER),
  LAST_UPDATE_TIME INTEGER DEFAULT (EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::INTEGER),
  LAST_SEEN_REFRESH_ID BIGINT CONSTRAINT SMC_LSRI_NN NOT NULL
);

CREATE SEQUENCE SCHEDULER_MOUNT_CANDIDATES_REFRESH_ID_SEQ
    INCREMENT BY 1
    START WITH 1
    NO MAXVALUE
    MINVALUE 1
...
CREATE UNIQUE INDEX IDX_SCHEDULER_MOUNT_CANDIDATES_KEY
  ON SCHEDULER_MOUNT_CANDIDATES (CANDIDATE_KEY);

CREATE INDEX IDX_SCHEDULER_MOUNT_CANDIDATES_PICK
  ON SCHEDULER_MOUNT_CANDIDATES (STATE, CANDIDATE_SCORE DESC, OLDEST_JOB_START_TIME, CANDIDATE_ID);

CREATE UNIQUE INDEX IDX_SCHEDULER_MOUNT_CANDIDATES_RESERVED_DRIVE
  ON SCHEDULER_MOUNT_CANDIDATES (RESERVED_BY_HOST, RESERVED_BY_DRIVE)
  WHERE STATE = 'Reserved';

CREATE UNIQUE INDEX IDX_SCHEDULER_MOUNT_CANDIDATES_RESERVED_VID
  ON SCHEDULER_MOUNT_CANDIDATES (VID)
  WHERE VID IS NOT NULL AND STATE = 'Reserved';
