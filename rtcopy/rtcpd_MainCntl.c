...
int rtcpd_MainCntl(SOCKET *accept_socket) {
  ...
  *client_socket = INVALID_SOCKET;
  ...
  if(tapeNeedsToBeReleasedAtEndOfSession) {
      int rlsFlags = TPRLS_ALL;
      int rls_rc = 0;
      char *const path = NULL;

      /* Log whether or not the rtcpd daemon is configured to reuse mounts */
      /* and set the TPRLS_UNLOAD bit of the flags for the Ctape_rls()     */
      /* function if the rtcpd daemon is not configured to reuse mounts    */
      if(rtcpdIsConfiguredToReuseMount()) {
          rtcp_log(LOG_INFO, "rtcpd is configured to reuse mount\n");
      } else {
          rtcp_log(LOG_INFO, "rtcpd is not configured to reuse mount\n");
          rlsFlags |= TPRLS_UNLOAD;
      }
