...
create_dg_s_list_from_conffile (gflag, Sflag, dev_group, server_name)
     int gflag;
     int Sflag;
     char dev_group[MAXLEN];
     char server_name[MAXSERVNAME];
...
create_tapeseq(tppositrec, timestamp, serv)
...
match_tapeseq(partial_completed_rec, timestamp, serv)
