 ...
 *
 * @(#)$RCSfile: rtcpcldCatalogueInterface.c,v $ $Revision: 1.125 $ $Release$ $Date: 2005/02/24 15:19:48 $ $Author: obarring $
 *
 * 
 *
 * @author Olof Barring
...
#ifndef lint
static char sccsid[] = "@(#)$RCSfile: rtcpcldCatalogueInterface.c,v $ $Revision: 1.125 $ $Release$ $Date: 2005/02/24 15:19:48 $ Olof Barring";
#endif /* not lint */
     ...
     tape_list_t *tape;
{
  ...
  if ( rc == -1 ) {
    save_serrno = serrno;
    LOG_DBCALLANDKEY_ERR("Cstager_IStagerSvc_segmentsForTape()",
                         Cstager_IStagerSvc_errorMsg(*stgsvc),
                         tape->dbRef->key);
    serrno = save_serrno;
    return(-1);
     ...
     file_list_t **file;
{
  ...
  if ( rc == -1 ) {
    ...
    LOG_DBCALLANDKEY_ERR("Cstager_IStagerSvc_bestTapeCopyForStream()",
                         Cstager_IStagerSvc_errorMsg(*stgsvc),
                         key);
   ...
   */
  fl->filereq.blocksize = 32760;
  if ( strstr(tape->tapereq.devtype,"3592") != NULL ) 
    fl->filereq.blocksize = 256*1024;
     ...
     tape_list_t *tape;
{
  ...
  if ( stream != NULL ) {
    ...
    if ( rc == -1 ) {
      save_serrno = serrno;
      LOG_DBCALLANDKEY_ERR("Cstager_IStagerSvc_resetStream()",
                           Cstager_IStagerSvc_errorMsg(*stgSvc),
                           key);
      serrno = save_serrno;
      return(-1);
