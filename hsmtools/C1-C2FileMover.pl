...
# *
# * @(#)$RCSfile: C1-C2FileMover.pl,v $ $Revision: 1.1 $ $Release$ $Date: 2006/03/24 16:33:19 $ $Author: itglp $
# *
# * Copies a list of files from a Castor1 stager to a Castor2 one.
...
# * Info about the source file is obtained by stage_qry. 
# *
...
# Get details about the file from stageqry --dump

my $fsize=`grep -C11 'Catalog entry' txt | tail -1 | awk '{ print $3}'`
my $origpath=`grep -C12 'Catalog entry' txt | tail -1 | awk '{ print $3}'`
my $origstatus=`grep -C19 'Catalog entry' txt | tail -1 | awk '{ print $6}'`
my $fcid=`grep -C26 'Catalog entry' txt | tail -1 | awk '{ print $3'`
# to get the fileclass name from the fileclass id we have to query the NS.
# TODO store all tuples once for ever...
my $fclass=`nslistclass | grep -C1 CLASS_ID | grep -C1 $fcid | tail -1 | awk '{ print $2 }'`
...
#     DROP PROCEDURE bestFileSystemForJob_DBI;
