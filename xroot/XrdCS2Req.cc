...
//          $Id: XrdCS2Req.cc,v 1.1 2008/02/25 15:15:46 apeters Exp $

const char *XrdCS2ReqCVSID = "$Id: XrdCS2Req.cc,v 1.1 2008/02/25 15:15:46 apeters Exp $";
...
void XrdCS2Req::Queue()
{
   ...
   DEBUG("Queue: try to Queue " << thePath);
...
// Add this request to the slot table
   ...
   if (!STab[hval]) STab[hval] = this;
            ...
            while(rp) {
	      DEBUG("Queue: loop addr = " << rp << " path =  " << rp->thePath);
	      // we check if the path is already in the active queue
	      if (!strcmp(rp->thePath,thePath)) {
		// if yes, we don't need to add an object to the queue because
		// the prepare command will be executed for that path, but we have
		// to append our object into the Same list! 

		DEBUG("Queue: found same " << myLock << " " << thePath);
		Same = rp->Same; rp->Same = this;
		prp = 0;
		break;
	      }
	      prp = rp; rp = rp->Next;
   ...
   if (post) {
     DEBUG("Queue: Post semaphore " << thePath);
...
//
     ...
     DEBUG("Queue: no Post [found same]" <<  thePath);
   }
...
XrdCS2Req *XrdCS2Req::Recycle()
{
   EPNAME("Recycle");
   XrdCS2Req *reqP = Same;
   DEBUG("in queue recycle this = " << this  << " Same = " << reqP << "Free = " << numFree << "Max Free = " << maxFree);
