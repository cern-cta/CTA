...
int XrdxCastor2Fs::Configure(XrdSysError &Eroute) 
{
  ...
  if (SubscribeCms) {
    // for the moment we don't use that since cmsd doesn't provide what we need for load balancing.
    Eroute.Say("=====> xcastor2.subscribe : true","","");  
    {
      Balancer = new XrdCmsFinderTRG(Eroute.logger(),
				      XrdCms::IsRedir | XrdCms::IsMeta, myPort, 0);
      printf("Calling Configure\n");
      if (!Balancer->Configure(ConfigFN)){
	Eroute.Emsg("Config","Cannot start the Finder!");
	printf("Deleting Balancer\n");
	delete Balancer; Balancer = 0;
      }

      //      if (!evrObject.Init(&Eroute, Balancer)) {
      //	Eroute.Emsg("Config","Cannot initialize event reader!");
      //	NoGo = 1;
      //      }	
  ...
  if (Procfilesystem !="") {
	...
	XrdOucString ps = PACKAGE_STRING; ps+="\n";
