...
void *XrdCS2OfsRateLimitStart(void *pp) {
  XrdCS2OfsRateLimit *gc = (XrdCS2OfsRateLimit*)pp;
  gc->Limit();
  return (void*)0;
}
...
/******************************************************************************/
bool
XrdCS2OfsRateLimit::Init(XrdSysError &ep) {
  pthread_t tid;
  int n, rc;
  eDest = &ep;
  eDest->Say("###### " ,"ratelimiter: starting thread ","");

  if ((rc = XrdSysThread::Run(&tid, XrdCS2OfsRateLimitStart, static_cast<void *>(this),
			      0, "Rate Limiter"))) {
