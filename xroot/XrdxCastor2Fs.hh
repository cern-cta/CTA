...
const   char       *FName() {return (const char *)fname;}
                    ...
                    XrdxCastor2FsDirectory(char *user=0) : XrdSfsDirectory(user)
                                {ateof = 0; fname = 0;
                                 dh    = (DIR *)0;
...
public:
        ...
        int            fctl(int, const char*, XrdOucErrInfo&) {return 0;}
                       ...
                       XrdxCastor2FsFile(char *user=0) : XrdSfsFile(user)
...
//
        XrdSfsDirectory *newDir(char *user=0)
                        {return (XrdSfsDirectory *)new XrdxCastor2FsDirectory(user);}

        XrdSfsFile      *newFile(char *user=0)
                        {return      (XrdSfsFile *)new XrdxCastor2FsFile(user);}
