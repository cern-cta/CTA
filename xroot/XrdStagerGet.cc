...
// XrdStagerGet                                                         //
//                                                                      //
// command line tool to stage files                                     // 
//////////////////////////////////////////////////////////////////////////

//       $Id: XrdStagerGet.cc,v 1.1 2008/09/15 10:04:02 apeters Exp $
...
void PrintUsage() {
   cerr << 
     "usage1: xrdstager_get [-d dbglevel] [-p] [-s] [-O info] xrootd_url1 ... xrootd_urlN " << endl <<
     ...
     "usage2: xrdstager_get [-d dbglevel] [-p] xrootd_url_dest -S xrootd_url_src" << endl <<
...
int main(int argc, char**argv) {
  ...
  dbglvl = -1;
   ...
   if (useprepare) {
	 ...
	 cout << "[PREPARE  ] path=" << u.File.c_str() << " status=FAILED" << endl;
	 continue;
       } else {
	 cout << "[PREPARE  ] path=" << u.File.c_str() << " status=OK" << endl;
       }
	 ...
	 if ( !oi.cli->IsOpen_inprogress() ) {
	     ...
	     if (oi.cli->IsOpen_wait() && oi.cli->Stat(&sti)) {
	       cout << "[STAGEOPEN] path=" << oi.origurl->GetUrl() << 
		 " size=" << sti.size << " status=ONLINE" << endl;
	     }
	     else {
	       cout << "[STAGEOPEN] path=" << oi.origurl->GetUrl() << " status=OFFLINE" << endl;
	     }
