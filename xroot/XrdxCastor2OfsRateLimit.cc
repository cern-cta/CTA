...
XrdxCastor2OfsRateLimit::Limit() {
  ...
  while(1) {
    ...
    readdesiredrate  = XrdOfsFS.ReadRateLimit;
    writedesiredrate = XrdOfsFS.WriteRateLimit;
    readratemissing  = (float) XrdOfsFS.ReadRateMissing;
    writeratemissing = (float) XrdOfsFS.WriteRateMissing;
    if (XrdOfsFS.ReadRateMissing) {
      readdesiredrate = (XrdOfsFS.ReadRateLimit*1.0 - XrdOfsFS.ReadRateMissing*1.0);
      //      printf("Missing Rate %d/%d -> %f\n",XrdOfsFS.ReadRateLimit,XrdOfsFS.ReadRateMissing, readdesiredrate);
      if (!(cnt % 4)) {
    ...
    if (XrdOfsFS.WriteRateMissing) {
      writedesiredrate = (XrdOfsFS.WriteRateLimit*1.0 - XrdOfsFS.WriteRateMissing*1.0);
      //      printf("Missing Rate %d/%d -> %f\n",XrdOfsFS.WriteRateLimit,XrdOfsFS.WriteRateMissing, writedesiredrate);
      if (!(cnt % 4)) {
	XrdOfsFS.RateMissingCnt++;
	XrdOfsFS.WriteRateMissing=0;
      }
    } 
    ...
    readstopbytes[cnt%RATEPERIOD]  = XrdOfsFS.TotalReadBytes;
    writestopbytes[cnt%RATEPERIOD]  = XrdOfsFS.TotalWriteBytes;

    // keep the proc values more or less uptodate
    if ( time(NULL) > procupdatetime ) {
      if (!XrdOfsFS.UpdateProc("*")) {
	...
	XrdOfsFS.ReadDelay = (unsigned int)(readsleeper/1000.0); // we need ms
	...
	XrdOfsFS.MeasuredReadRate = (int)rate;
      }
	...
	XrdOfsFS.WriteDelay = (unsigned int)(writesleeper/1000.0); // we need ms
	...
	XrdOfsFS.MeasuredWriteRate = (int)rate;
      }
    }
  }
}
