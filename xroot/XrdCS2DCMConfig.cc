...
//
   opterr = 0;
   if (argc > 1 && '-' == *argv[1]) 
      while ((c = getopt(argc,argv,"dl:s:p:q:S:P:")) && ((unsigned char)c != 0xff))
     { switch(c)
       {
       ...
       case 'l': if (logfn) free(logfn);
                 logfn = strdup(optarg);
                 break;
       case 's': if (Opts.service_class) free(Opts.service_class);
	         Opts.service_class = strdup(optarg);
                 break;
       ...
       case 'q': if (!(QLim = atoi(optarg)))
                 ...
                 break;
       case 'S': if (Opts.service_class) free(Opts.service_class);
	         Opts.stage_host = strdup(optarg);
	         break;
       case 'P': if (!(Opts.stage_port = atoi(optarg)))
	            {XrdLog.Emsg("Config", "Invalid -P value -",optarg);NoGo=1;}
                 break;
...
//
   if ((olbP = getenv("XRDOLBPATH")) && *olbP)
      ...
      olbdsock = socket(AF_UNIX, SOCK_DGRAM, 0);
      if (olbdsock < 0) { perror("opening datagram socket"); exit(1); }
      /* Construct name of socket to send to. */
      olbdsockname.sun_family = AF_UNIX;
      strcpy(olbdsockname.sun_path, buff);
      } else olbdsock=0;
...
//
   if (!theFile.Init(thePath))
	 ...
	 {
	   char buffer[4096];
	   sprintf(buffer,"gone %s\n", theFile.Pfn());

	   /* Send message. */
	   if (sendto(olbdsock, buffer, strlen(buffer), 0, (const sockaddr*)&olbdsockname, sizeof(struct sockaddr_un)) < 0) { perror("sending datagram messa\
ge"); }
	 }
      }
}
