...
void print_globstat _PROTO((time_t, time_t, int, int, int));
     ...
     char **argv;
{
  ...
  int num_t_rtcp_requests = 0;			/* number of direct tape requests generating fork of a stager */
  int num_h_rtcp_requests = 0;			/* number of tape requests generating fork of a stager through CASTOR files */
  ...
  Coptind = 1;
  Copterr = 1;
  while ((c = Cgetopt (argc, argv, "de:f:s:p:S:")) != -1) {
	switch (c) {
	case 'd':
      debug++;
      break;
  ...
  while (getacctrec (fd_acct, &accthdr, (char *) &rp, &swapped)) {
	...
	if (rp.subtype == STGSTART) {		/* stgdaemon started */
      ...
      nrecok++;
	} else if (rp.subtype == STGCMDS && rp.u2.s.t_or_d == 't') { /* direct tape access, stager started */
      num_t_rtcp_requests++;
      nrecok++;
    } else if (rp.subtype == STGCMDS && rp.u2.s.t_or_d == 'h') { /* CASTOR access, stager started */
      num_h_rtcp_requests++;
      nrecok++;
  ...
  printf("                                ... %10d (filtered)\n", nrecfiltered);
  printf("\n");
  print_globstat (starttime, endtime, num_t_rtcp_requests, num_h_rtcp_requests, num_multireqs);
...
void print_globstat (starttime, endtime, num_t_rtcp_requests, num_h_rtcp_requests, num_multireqs)
     time_t starttime;
     time_t endtime;
     int num_t_rtcp_requests;
     int num_h_rtcp_requests;
     int num_multireqs;
{
  ...
  printf ("stagein    %7d %6.2f %7d %7d %5d %6d %5d %7d %4d %4d %6d %7d %6d\n",
          nb_req[1], nb_req[1] > 0 ? (float) nb_apireq[1]/nb_req[1] : 0, rc[1][0], rc[1][7], rc[1][1], rc[1][2], rc[1][3],
          rc[1][4], rc[1][10], rc[1][11], rc[1][5], rc[1][6], rc[1][8]);
  printf ("stageout   %7d %6.2f %7d %7d %5d %6d %5d %7d %4d %4d %6d %7d %6d\n",
          nb_req[2], nb_req[2] > 0 ? (float) nb_apireq[2]/nb_req[2] : 0, rc[2][0], rc[2][7], rc[2][1], rc[2][2], rc[2][3],
          rc[2][4], rc[2][10], rc[2][11], rc[2][5], rc[2][6], rc[2][8]);
  printf ("stagewrt   %7d %6.2f %7d %7d %5d %6d %5d %7d %4d %4d %6d %7d %6d\n",
          nb_req[3], nb_req[3] > 0 ? (float) nb_apireq[3]/nb_req[3] : 0, rc[3][0], rc[3][7], rc[3][1], rc[3][2], rc[3][3],
          rc[3][4], rc[3][10], rc[3][11], rc[3][5], rc[3][6], rc[3][8]);
  printf ("stageput   %7d %6.2f %7d %7d %5d %6d %5d %7d %4d %4d %6d %7d %6d\n",
          nb_req[4], nb_req[4] > 0 ? (float) nb_apireq[4]/nb_req[4] : 0, rc[4][0], rc[4][7], rc[4][1], rc[4][2], rc[4][3],
          rc[4][4], rc[4][10], rc[4][11], rc[4][5], rc[4][6], rc[4][8]);
  printf ("stagealloc %7d %6.2f %7d %7d %5d %6d %5d %7d %4d %4d %6d %7d %6d\n",
          nb_req[11], nb_req[11] > 0 ? (float) nb_apireq[11]/nb_req[11] : 0, rc[11][0], rc[11][7], rc[11][1], rc[11][2], rc[11][3],
          rc[11][4], rc[11][10], rc[11][11], rc[11][5], rc[11][6], rc[11][8]);

  printf ("stageqry   %7d %6.2f\n", nb_req[5], nb_req[5] > 0 ? (float) nb_apireq[5]/nb_req[5] : 0);
  printf ("stageclr   %7d %6.2f\n", nb_req[6], nb_req[6] > 0 ? (float) nb_apireq[6]/nb_req[6] : 0);
  printf ("stagekill  %7d %6.2f\n", nb_req[7], nb_req[7] > 0 ? (float) nb_apireq[7]/nb_req[7] : 0);
  printf ("stageupdc  %7d %6.2f\n", nb_req[8], nb_req[8] > 0 ? (float) nb_apireq[8]/nb_req[8] : 0);
  printf ("stageinit  %7d %6.2f\n", nb_req[9], nb_req[9] > 0 ? (float) nb_apireq[9]/nb_req[9] : 0);
  printf ("stagecat   %7d %6.2f\t\n", nb_req[10], nb_req[10] > 0 ? (float) nb_apireq[10]/nb_req[10] : 0);
  printf ("stageget   %7d %6.2f\t\n", nb_req[12], nb_req[12] > 0 ? (float) nb_apireq[12]/nb_req[12] : 0);
  printf ("stagechng  %7d %6.2f\t\n", nb_req[14], nb_req[14] > 0 ? (float) nb_apireq[14]/nb_req[14] : 0);
  printf ("stageshutdown %4d %6.2f\t\n", nb_req[15], nb_req[15] > 0 ? (float) nb_apireq[15]/nb_req[15] : 0);
  printf ("stageping  %7d %6.2f\t\n", nb_req[16], nb_req[16] > 0 ? (float) nb_apireq[16]/nb_req[16] : 0);
  printf ("restart(s) %7d\t\n", nb_restart);
  printf ("\nStagein Statistics :\n");
  printf ("\tNumber of direct tape positionning requests\t\t\t%d\n", num_t_rtcp_requests);
  printf ("\tNumber of CASTOR tape positionning requests\t\t\t%d\n", num_h_rtcp_requests);
  printf ("\tNumber of tape (direct mode or CASTOR) remount requests (write mode)\t\t\t%d\n", num_wrt_remount_requests);
  printf ("\tNumber of tape (direct mode or CASTOR) remount requests (read  mode)\t\t\t%d\n", num_in_remount_requests);
  num_pos_requests_avoided = rc[1][0] - (num_t_rtcp_requests + num_h_rtcp_requests);
  if (num_pos_requests_avoided < 0 ) num_pos_requests_avoided = 0;
  printf ("\tNumber of tape (direct mode or CASTOR) positionning requests avoided by stager (read mode)\t%d\n", num_pos_requests_avoided);
     ...
     char *cmd;
{
  ...
  fprintf (stderr, "%s",
           "[-d][-e end_time][-f accounting_file][-s start_time][-p pool_name][-S <a or t>]\n");
}
