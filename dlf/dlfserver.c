...
#ifndef lint
static char sccsid[] = "@(#)$RCSfile: dlfserver.c,v $ $Revision: 1.1 $ $Date: 2003/08/20 13:07:50 $ CERN IT-ADC/CA Vitaly Motyakov";
#endif /* not lint */
...
struct main_args *main_args;
{
	...
	if (fgets (cfbuf, sizeof(cfbuf), cf) &&
		...
		strcpy (db_srvr, p_s);
	  	...
	  	dlflogit (func, "Configuration file %s incorrect", dlfconfigfile);
		return (CONFERR);
	}
	(void) fclose (cf);
...
void *arg;
{
	...
	if ((c = getreq (thip->s, &magic, &req_type, &req_data, &data_len, &clienthost)) == 0)
...
struct dlf_srv_thread_info *thip;
{
	...
	switch (req_type) {
        case DLF_SHUTDOWN:
                ...
                break;
	case DLF_STORE:
	        c = proc_entermsgreq (magic, req_type, req_data, data_len, clienthost, thip);
                break;
...
struct dlf_srv_thread_info *thip;
{
	...
	while (1) {
		if (req_type == DLF_STORE) {
		  if (c = dlf_srv_entermessage (mag, r_data, d_len, c_host, thip, &last_message)){
        	        return(c);
		  }
		}
		else {
		...
		if (rc = getreq (thip->s, &mag, &new_req_type, &r_data, &d_len, &c_host) != 0) {
			break;
		}
