...
/**
 * $Id: process.c,v 1.2 2006/06/19 06:52:55 waldron Exp $
 */
...
int sendrep(client_t *client, int type, int rc) {
	...
	if (netwrite_timeout(client->fd, repbuf, repsize, 5) != repsize) {
		...
		log(LOG_DEBUG, "sendrep() - failed to netwrite_timeout [client: %s] : %s\n", 
		    client->ip, neterror());
		return APP_FAILURE;
	}
	if (type == DLF_REP_RC) {
...
int parse_init(client_t *client, char *data, int len) {
	...
	for (i = 0; rbp < eob; i++) {
		...
		if (m == NULL) {
			log(LOG_ERR, "parse_init() - failed to malloc [client: %s] : %s\n", client->ip, strerror(errno)); 
...
int parse_log(client_t *client, char *data, int len) {
	...
	if (message == NULL) {
		log(LOG_ERR, "parse_log() - failed to malloc [client: %s] : %s\n", client->ip, strerror(errno)); 
		sendrep(client, DLF_REP_ERR, DLF_ERR_SYSERR);
		return APP_FAILURE;
	}
	message->plist = NULL;
...
int process_request(client_t *client) {
	...
	if (data == NULL) {
		log(LOG_ERR, "process_request() - failed to malloc [client: %s] : %s\n", client->ip, strerror(errno));
		sendrep(client, DLF_REP_ERR, DLF_ERR_SYSERR);
		return APP_FAILURE;
	}
