    ...
    case -1:  // poll() encountered an error
      ...
      if (pollErrno == EINTR) {
        const time_t remainingTime =
          timeout - (std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) - startTime);
...
int connectWithTimeout(const std::string& hostName, const unsigned short port, const int timeout) {
  try {
    ...
    return connectWithTimeout(AF_INET,
                              SOCK_STREAM,
                              protocol,
                              reinterpret_cast<struct sockaddr*>(&s_in),
                              length,
                              timeout);
    ...
    cta::exception::Exception ex;
    ex.getMessage() << "Failed to connect: Caught an unknown exception: hostName=" << hostName << ", port=" << port
                    << ", timeout=" << timeout;
    throw ex;
  }
}
