...
protected:
  ...
  void setUp() { unlink(m_listenSockPath); }

  void tearDown() { unlink(m_listenSockPath); }
  ...
  int createLocalListenSocket(const char* const listenSockPath) {
    ...
    // Bind the socket
    {
      ...
      strncpy(listenAddr.sun_path, listenSockPath, sizeof(listenAddr.sun_path) - 1);

      cta::exception::Errnum::throwOnNonZero(
        bind(smartListenSock.get(), (const struct sockaddr*) &listenAddr, sizeof(listenAddr)),
        "Call to bind() failed: ");
    }
...
TEST_F(cta_mediachanger_IoTest, connectWithTimeout) {
  ...
  ASSERT_NO_THROW(smartListenSock.reset(createLocalListenSocket(m_listenSockPath)));
  ...
  cta::SmartFd smartClientConnectionSock;
  {
    ...
    ASSERT_NO_THROW(smartClientConnectionSock.reset(cta::mediachanger::connectWithTimeout(
      sockDomain, sockType, sockProtocol, (const struct sockaddr*) &address, address_len, timeout)));
  }
  ...
  cta::SmartFd smartServerConnectionSock;
  {
    ...
    ASSERT_NO_THROW(
      smartServerConnectionSock.reset(cta::mediachanger::acceptConnection(smartListenSock.get(), acceptTimeout)));
  }

  // Send message from client to server and check it was sent
  char clientToServerMessageBuf[] = "Test message from client to server: HELLO";
  const size_t clientToServerMessageBufLen = sizeof(clientToServerMessageBuf);
  {
    const int timeout = 1;  // Time out is in seconds
    ASSERT_NO_THROW(cta::mediachanger::writeBytes(smartClientConnectionSock.get(), timeout, clientToServerMessageBufLen,
                                                  clientToServerMessageBuf));
  }
  {
    const int timeout = 1;  // Time out is in seconds
    char serverInputBuf[clientToServerMessageBufLen];
    memset(serverInputBuf, '\0', sizeof(serverInputBuf));
    ASSERT_NO_THROW(cta::mediachanger::readBytes(smartServerConnectionSock.get(), timeout, clientToServerMessageBufLen,
                                                 serverInputBuf));

    ASSERT_EQ(std::string(clientToServerMessageBuf), std::string(serverInputBuf));
  }

  // Send message from server to client and check it was sent
  char serverToClientMessageBuf[] = "Test message from server to client: BONJOUR";
  const size_t serverToClientMessageBufLen = sizeof(serverToClientMessageBuf);
  {
    const int timeout = 1;  // Time out is in seconds
    ASSERT_NO_THROW(cta::mediachanger::writeBytes(smartServerConnectionSock.get(), timeout, serverToClientMessageBufLen,
                                                  serverToClientMessageBuf));
  }
  {
    const int timeout = 1;  // Time out is in seconds
    char clientInputBuf[serverToClientMessageBufLen];
    memset(clientInputBuf, '\0', sizeof(clientInputBuf));
    ASSERT_NO_THROW(cta::mediachanger::readBytes(smartClientConnectionSock.get(), timeout, serverToClientMessageBufLen,
                                                 clientInputBuf));

    ASSERT_EQ(std::string(serverToClientMessageBuf), std::string(clientInputBuf));
  }
}
