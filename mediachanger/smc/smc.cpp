...
void smc_qlib_jsonPrint(const struct robot_info* const robot_info) {
  ...
  prodRevLvl[4] = '\0';
  printf("[");
  printf("{\"inquiry\":{\"vendor\":\"%s\",\"product\":\"%s\",\"revision\":\"%s\"},", T10Vendor, prodId, prodRevLvl);
  printf("\"transport\":{\"count\":%d,\"start\":%d},", robot_info->transport_count, robot_info->transport_start);
  printf("\"slot\":{\"count\":%d,\"start\":%d},", robot_info->slot_count, robot_info->slot_start);
  printf("\"port\":{\"count\":%d,\"start\":%d},", robot_info->port_count, robot_info->port_start);
  printf("\"device\":{\"count\":%d,\"start\":%d}", robot_info->device_count, robot_info->device_start);
  printf("}]");
}

static int smc_qlib(const struct robot_info* const robot_info, const int isJsonEnabled) {
  if (isJsonEnabled) {
...
void smc_qport_humanPrint(const struct smc_element_info* const element_info, const int numberOfElements) {
  ...
  for (i = 0; i < numberOfElements; i++) {
    ...
    printf("    %4d\t%s\t%s\n", (element_info + i)->element_address, (element_info + i)->name, pstatus);
  }
}

void smc_qport_jsonPrint(const struct smc_element_info* const element_info, const int numberOfElements) {
  const char* pstatus;
  int i;
  printf("[");
  for (i = 0; i < numberOfElements; i++) {
...
void smc_qslot_jsonPrint(const struct smc_element_info* element_info, const int numberOfElements) {
  int i;
  printf("[");
  for (i = 0; i < numberOfElements; i++) {
    if (0 != i) {
...
void smc_qvid_humanPrint(const struct smc_element_info* const element_info, const int numberOfElements) {
  ...
  for (i = 0; i < numberOfElements; i++) {
    ...
    printf("%s\t    %4d\t%s\n", (element_info + i)->name, (element_info + i)->element_address, ptype);
  }
}

void smc_qvid_jsonPrint(const struct smc_element_info* const element_info, const int numberOfElements) {
  int i;
  const char* ptype;
  char ptypes[5][6] = {"", "hand", "slot", "port", "drive"};
  printf("[");
    ...
    switch (c) {
      ...
      case 'q': /* query */
        if (req_type) {
          ...
          req_type = c;
          qry_type = *optarg;
          if (qry_type != 'D' && qry_type != 'L' && qry_type != 'P' && qry_type != 'S' && qry_type != 'V') {
