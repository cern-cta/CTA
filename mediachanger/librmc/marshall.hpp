...
#define bitalloc(size) (bitvct) malloc(size / BITSOFBYTE + ((size % BITSOFBYTE) ? 1 : 0))
...
#define marshall_SHORT(ptr, n)                      \
  {                                                 \
    SHORT n_ = htons((unsigned short) (n));         \
    (void) memcpy((ptr), SHORTADDR(n_), SHORTSIZE); \
    INC_PTR(ptr, SHORTSIZE);                        \
  }

#define unmarshall_SHORT(ptr, n)                                                                                       \
  {                                                                                                                    \
    SHORT n_ = 0;                                                                                                      \
    (void) memcpy(SHORTADDR(n_), (ptr), SHORTSIZE);                                                                    \
    n = ntohs((unsigned short) (n_));                                                                                  \
    if (BIT_ISSET(ptr, 0) && sizeof(SHORT) - SHORTSIZE > 0) (void) memset((char*) &n, 255, sizeof(SHORT) - SHORTSIZE); \
    INC_PTR(ptr, SHORTSIZE);                                                                                           \
  }
...
#define marshall_LONG(ptr, n)                     \
  {                                               \
    LONG n_ = htonl((unsigned long) (n));         \
    (void) memcpy((ptr), LONGADDR(n_), LONGSIZE); \
    INC_PTR(ptr, LONGSIZE);                       \
  }

#define unmarshall_LONG(ptr, n)                                                                                    \
  {                                                                                                                \
    LONG n_ = 0;                                                                                                   \
    (void) memcpy(LONGADDR(n_), (ptr), LONGSIZE);                                                                  \
    n = ntohl((unsigned long) (n_));                                                                               \
    if (BIT_ISSET(ptr, 0) && sizeof(LONG) - LONGSIZE > 0) (void) memset((char*) &n, 255, sizeof(LONG) - LONGSIZE); \
    INC_PTR(ptr, LONGSIZE);                                                                                        \
  }
...
#define marshall_STRING(ptr, str)                \
  {                                              \
    (void) strcpy((char*) (ptr), (char*) (str)); \
    INC_PTR(ptr, strlen(str) + 1);               \
  }

#define marshall_STRINGN(ptr, str, n)                \
  {                                                  \
    (void) strncpy((char*) (ptr), (char*) (str), n); \
    ((char*) (ptr))[n - 1] = 0;                      \
    if (strlen(str) + 1 > n)                         \
      INC_PTR(ptr, n);                               \
    else                                             \
...
#define unmarshall_STRING(ptr, str)              \
  {                                              \
...
#define unmarshall_HYPER(ptr, n)                                  \
  {                                                               \
    U_HYPER u_ = 0;                                               \
    LONG n_ = 0;                                                  \
    (void) memcpy(LONGADDR(n_), (ptr), LONGSIZE);                 \
    *((LONG*) ((char*) &(u_) + LONGSIZE)) = ntohl((U_LONG) (n_)); \
    INC_PTR(ptr, LONGSIZE);                                       \
    n_ = 0;                                                       \
    (void) memcpy(LONGADDR(n_), (ptr), LONGSIZE);                 \
...
#define marshall_OPAQUE(ptr, raw, n)  \
  {                                   \
    (void) memcpy((ptr), (raw), (n)); \
    INC_PTR(ptr, (n));                \
  }

#define unmarshall_OPAQUE(ptr, raw, n) \
  {                                    \
    (void) memcpy((raw), (ptr), (n));  \
    INC_PTR(ptr, (n));                 \
  }
