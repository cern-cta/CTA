...
struct hostent *Cgethostbyname(const char *name)
{
    ...
    rc = gethostbyname_r(name,reinterpret_cast<hostent*>(result),reinterpret_cast<char*>(buffer),bufsize,&hp,&h_errnoop);
    h_errno = h_errnoop;
    if ( rc == -1 ) hp = NULL;
    return(hp);
}
...
struct hostent *Cgethostbyaddr(const void *addr, size_t len, int type)
{
    ...
    rc = gethostbyaddr_r(addr,len,type,reinterpret_cast<hostent*>(result),reinterpret_cast<char*>(buffer),bufsize,&hp,&h_errnoop);
    h_errno = h_errnoop;
    if ( rc == -1 ) hp = NULL;
    return(hp);
}
