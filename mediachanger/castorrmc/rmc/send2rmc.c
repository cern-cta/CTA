	...
	const int user_repbuf_len)
{
	...
	if ((hp = Cgethostbyname (rmchost)) == NULL) {
		json_log(L_ERR, __FUNCTION__,RMC09, "Host unknown:", rmchost);
		serrno = ERMCUNREC;
		return (-1);
	}
	...
	if ((s = socket (AF_INET, SOCK_STREAM, 0)) < 0) {
		json_log(L_ERR, __FUNCTION__,RMC02, "socket", neterror());
		serrno = SECOMERR;
		return (-1);
	}
	...
	if (connect (s, (struct sockaddr *) &sin, sizeof(sin)) < 0) {
		if (errno == ECONNREFUSED) {
			json_log(L_ERR, __FUNCTION__,RMC00, rmchost);
			(void) close (s);
			serrno = ERMCNACT;
			return (-1);
		} else {
			json_log(L_ERR, __FUNCTION__,RMC02, "connect", neterror());
			(void) close (s);
			serrno = SECOMERR;
			return (-1);
		}
	...
	if ((n = netwrite (s, reqp, reql)) <= 0) {
		if (n == 0)
			json_log(L_ERR, __FUNCTION__,RMC02, "send", sys_serrlist[SERRNO]);
		else
			json_log(L_ERR, __FUNCTION__,RMC02, "send", neterror());
		(void) close (s);
		serrno = SECOMERR;
		return (-1);
	}
	...
	while (1) {
		if ((n = netread (s, repbuf, 3 * LONGSIZE)) <= 0) {
			if (n == 0)
				json_log(L_ERR, __FUNCTION__,RMC02, "recv", sys_serrlist[SERRNO]);
			else
				json_log(L_ERR, __FUNCTION__,RMC02, "recv", neterror());
			(void) close (s);
			serrno = SECOMERR;
			return (-1);
		}
