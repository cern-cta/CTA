         ...
         char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "D:deimN:q:S:V:j", longopts, NULL)) != EOF) {
		...
		case 'e':	/* export */
			if (req_type) {
				JSON_LOG(L_ERR, SR002, req_type, c);
				errflg++;
			} else
				req_type = c;
			break;
	...
	if (req_type && *rmchost == '\0') {
		JSON_LOG(L_ERR, "rmcserver must be specified\n");
		errflg++;
	}
	if (req_type == 'd' && drvord < 0) {
		JSON_LOG(L_ERR, SR006);
		errflg++;
	}
	if (req_type == 'e' && *vid =='\0') {
		JSON_LOG(L_ERR, SR011);
		errflg++;
	}
	if (req_type == 'm' && (*vid =='\0' || drvord < 0)) {
		JSON_LOG(L_ERR, SR007);
		errflg++;
	}
	...
	if (drvord >= robot_info.device_count) {
		JSON_LOG(L_ERR, SR008, robot_info.device_count);
		exit (USERR);
	}
	if (slotaddr > (robot_info.slot_count + robot_info.slot_start)) {
		JSON_LOG(L_ERR, SR016, robot_info.slot_count + robot_info.slot_start);
		exit (USERR);
	}
