/*
 * Copyright (C) 1990-1998 by CERN CN-PDP/CS
 * All rights reserved
 */

#if !defined(lint)
static char sccsid[] =  "@(#)socket.c	1.28 05/06/98  CERN CN-PDP/CS F. Hemmer";
#endif /* lint */
...
s_errmsg()                              /* return last error message    */
{
	...
	return(sys_errlist[errno]);
        ...
        if ((rc = sys$getmsg(vaxc$errno,&msglen,&msgbuf,0,0)) != SS$_NORMAL) {
                sprintf(msgbuf.buf,"%%system-f-vmserr, vms error code: 0x%x, unable to get vms message", vaxc$errno);
                return(msgbuf.buf);
        }
        else {
