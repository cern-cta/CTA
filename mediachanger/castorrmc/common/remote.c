 ...
 */
int isremote(struct in_addr from_host, char* host_name) {
  char* p;
  ...
  if ((p = getconfent("SIMULATION", "REMOTE", 1)) != NULL && (p = (char*) strtok_r(p, " \t", &last)) != NULL &&
      !strcmp(p, "YES")) {
    (*logfunc)(LOG_DEBUG, "isremote(): Client simulates remote behaviour\n");
    return 1;
  }
  if ((p = getconfent("ISREMOTE", "CALLS", 1)) != NULL && (p = (char*) strtok_r(p, " \t", &last)) != NULL &&
      !strcmp(p, "NO")) {
    ...
    if ((fs = fopen(rthfile, "r")) != NULL) {
      while (fgets(s, CA_MAXHOSTNAMELEN + 1, fs) != NULL) {
        if ((cp = strtok_r(s, " \n\t", &last)) != NULL) {
          ...
          if (isdigit(cp[0])) {
            strcpy(ent, cp);
            if (strtok_r(cp, ".", &last) == NULL || strtok_r(NULL, ".", &last) == NULL) {
...
int CDoubleDnsLookup(int s, char* host) {
  ...
  while (hp->h_addr_list[i] != NULL) {
    (*logfunc)(LOG_DEBUG,
               "CDoubleDnsLookup() comparing %s with %s\n",
               inet_ntoa(*(struct in_addr*) (hp->h_addr_list[i])),
               inet_ntoa(from.sin_addr));
    if (((struct in_addr*) (hp->h_addr_list[i++]))->s_addr == from.sin_addr.s_addr) {
      return (0);
    }
  }
  serrno = SENOSHOST;
...
int isadminhost(int s, char* peerhost) {
  ...
  if (admin_hosts == NULL) {
    admin_hosts = getenv("ADMIN_HOSTS");
  }
  if (admin_hosts == NULL) {
    admin_hosts = getconfent("ADMIN", "HOSTS", 1);
  }

  if ((admin_hosts != NULL) && ((admin_host = strstr(admin_hosts, peerhost)) != NULL)) {
    i = strlen(peerhost);
    if ((admin_host[i] == '\0' || admin_host[i] == ' ' || admin_host[i] == '\t' || admin_host[i] == ',') &&
        (admin_host == admin_hosts || admin_host[-1] == ' ' || admin_host[-1] == '\t' || admin_host[-1] == ',')) {
      return (0);
    }
  }
  serrno = SENOTADMIN;
