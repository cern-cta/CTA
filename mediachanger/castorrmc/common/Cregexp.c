...
Cregexp_t* Cregexp_comp(char* exp) {
  ...
  *_Cregexp_code = _Cregexp_dummy;
  _Cregexp_c((char) CREGEXP_MAGIC, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  if (_Cregexp_reg(0, &flags, _Cregexp_parse, _Cregexp_npar, _Cregexp_dummy, _Cregexp_code, _Cregexp_size) == NULL) {
    return (NULL);
  }
  ...
  *_Cregexp_code = r->program;
  ...
  if (OP(_Cregexp_next(scan, _Cregexp_dummy)) == END) { /* Only one top-level choice. */
    scan = OPERAND(scan);
    ...
    if (flags & SPSTART) {
      longest = NULL;
      len = 0;
      for (; scan != NULL; scan = _Cregexp_next(scan, _Cregexp_dummy)) {
...
char* _Cregexp_reg(int paren, /* Parenthesized? */
  ...
  *flagp = HASWIDTH; /* Tentatively. */
  ...
  if (paren) {
    ...
    parno = *_Cregexp_npar;
    (*_Cregexp_npar)++;
    ret = _Cregexp_node((char) (OPEN + parno), _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  } else {
    ret = NULL;
  }

  /* Pick up the branches, linking them together. */
  if ((br = _Cregexp_branch(&flags, _Cregexp_parse, _Cregexp_npar, _Cregexp_dummy, _Cregexp_code, _Cregexp_size)) ==
      NULL) {
    return (NULL);
  }
  if (ret != NULL) {
    _Cregexp_tail(ret, br, _Cregexp_dummy); /* OPEN -> first. */
  } else {
    ret = br;
  }
  if (!(flags & HASWIDTH)) {
  ...
  *flagp |= flags & SPSTART;
  while (*(*_Cregexp_parse) == '|') {
    ...
    _Cregexp_tail(ret, br, _Cregexp_dummy); /* BRANCH -> BRANCH. */
    if (!(flags & HASWIDTH)) {
      *flagp &= ~HASWIDTH;
    }
    *flagp |= flags & SPSTART;
  ...
  ender = _Cregexp_node((char) ((paren) ? CLOSE + parno : END), _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  _Cregexp_tail(ret, ender, _Cregexp_dummy);

  /* Hook the tails of the branches to the closing node. */
  for (br = ret; br != NULL; br = _Cregexp_next(br, _Cregexp_dummy)) {
    _Cregexp_optail(br, ender, _Cregexp_dummy);
  }
...
char* _Cregexp_branch(int* flagp,
  ...
  *flagp = WORST; /* Tentatively. */
  ...
  ret = _Cregexp_node((char) BRANCH, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  chain = NULL;
  while (*(*_Cregexp_parse) != '\0' && *(*_Cregexp_parse) != '|' && *(*_Cregexp_parse) != ')') {
    latest = _Cregexp_piece(&flags, _Cregexp_parse, _Cregexp_npar, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
    if (latest == NULL) {
      return (NULL);
    }
    *flagp |= flags & HASWIDTH;
    if (chain == NULL) { /* First piece. */
      *flagp |= flags & SPSTART;
      ...
      _Cregexp_tail(chain, latest, _Cregexp_dummy);
    }
    chain = latest;
  }
  if (chain == NULL) { /* Loop ran zero times. */
    _Cregexp_node((char) NOTHING, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  }
  return (ret);
}
...
char* _Cregexp_piece(int* flagp,
  ...
  ret = _Cregexp_atom(&flags, _Cregexp_parse, _Cregexp_npar, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  if (ret == NULL) {
    return (NULL);
  }
  ...
  *flagp = (op != '+') ? (WORST | SPSTART) : (WORST | HASWIDTH);
  ...
  if (op == '*' && (flags & SIMPLE)) {
    _Cregexp_insert(STAR, ret, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  } else if (op == '*') {
    /* Emit x* as (x&|), where & means "self". */
    _Cregexp_insert(BRANCH, ret, _Cregexp_dummy, _Cregexp_code, _Cregexp_size); /* Either x */
    _Cregexp_optail(ret,
                    _Cregexp_node((char) BACK, _Cregexp_dummy, _Cregexp_code, _Cregexp_size),
                    _Cregexp_dummy);           /* and loop */
    _Cregexp_optail(ret, ret, _Cregexp_dummy); /* back */
    _Cregexp_tail(ret,
                  _Cregexp_node((char) BRANCH, _Cregexp_dummy, _Cregexp_code, _Cregexp_size),
                  _Cregexp_dummy); /* or */
    _Cregexp_tail(ret,
                  _Cregexp_node((char) NOTHING, _Cregexp_dummy, _Cregexp_code, _Cregexp_size),
                  _Cregexp_dummy); /* null. */
  } else if (op == '+' && (flags & SIMPLE)) {
    _Cregexp_insert(PLUS, ret, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
  } else if (op == '+') {
    /* Emit x+ as x(&|), where & means "self". */
    next = _Cregexp_node((char) BRANCH, _Cregexp_dummy, _Cregexp_code, _Cregexp_size); /* Either */
    _Cregexp_tail(ret, next, _Cregexp_dummy);
    _Cregexp_tail(_Cregexp_node((char) BACK, _Cregexp_dummy, _Cregexp_code, _Cregexp_size),
                  ret,
    ...
    _Cregexp_tail(ret,
                  _Cregexp_node((char) BRANCH, _Cregexp_dummy, _Cregexp_code, _Cregexp_size),
                  _Cregexp_dummy);                                                      /* or */
    next = _Cregexp_node((char) NOTHING, _Cregexp_dummy, _Cregexp_code, _Cregexp_size); /* null. */
    _Cregexp_tail(ret, next, _Cregexp_dummy);
    _Cregexp_optail(ret, next, _Cregexp_dummy);
  }
  (*_Cregexp_parse)++;
...
char* _Cregexp_atom(int* flagp,
  ...
  *flagp = WORST; /* Tentatively. */
  ...
  switch (*(*_Cregexp_parse)++) {
    case '^':
      ret = _Cregexp_node((char) BOL, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      break;
    case '$':
      ret = _Cregexp_node((char) EOL, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      break;
    case '.':
      ret = _Cregexp_node((char) ANY, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
    ...
    case '[': {
      ...
      if (*(*_Cregexp_parse) == '^') { /* Complement of range. */
        ret = _Cregexp_node((char) ANYBUT, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
        (*_Cregexp_parse)++;
      } else {
        ret = _Cregexp_node((char) ANYOF, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      }
      if (*(*_Cregexp_parse) == ']' || *(*_Cregexp_parse) == '-') {
        _Cregexp_c((char) *(*_Cregexp_parse)++, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      }
      while (*(*_Cregexp_parse) != '\0' && *(*_Cregexp_parse) != ']') {
        if (*(*_Cregexp_parse) == '-') {
          (*_Cregexp_parse)++;
          if (*(*_Cregexp_parse) == ']' || *(*_Cregexp_parse) == '\0') {
            _Cregexp_c((char) '-', _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
          } else {
            ...
            for (; class <= classend; class ++) {
              _Cregexp_c((char) class, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
            }
            (*_Cregexp_parse)++;
          }
        } else {
      ...
      _Cregexp_c((char) '\0', _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      if (*(*_Cregexp_parse) != ']') {
        /* unmatched [] */
        serrno = EINVAL;
        return (NULL);
    ...
    case '(':
      ret = _Cregexp_reg(1, &flags, _Cregexp_parse, _Cregexp_npar, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      if (ret == NULL) {
        return (NULL);
      }
    ...
    case '\\':
      ...
      ret = _Cregexp_node((char) EXACTLY, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      _Cregexp_c((char) *(*_Cregexp_parse)++, _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      _Cregexp_c((char) '\0', _Cregexp_dummy, _Cregexp_code, _Cregexp_size);
      *flagp |= HASWIDTH | SIMPLE;
      break;
 ...
 */
void _Cregexp_tail(char* p, char* val, char* _Cregexp_dummy) {
  char* scan;
  char* temp;
  int offset;
  ...
  scan = p;
  for (;;) {
    temp = _Cregexp_next(scan, _Cregexp_dummy);
    if (temp == NULL) {
      break;
    }
    scan = temp;
 ...
 */
void _Cregexp_optail(char* p, char* val, char* _Cregexp_dummy) {
  /* "Operandless" and "op != BRANCH" are synonymous in practice. */
  if (p == NULL || p == _Cregexp_dummy || OP(p) != BRANCH) {
    return;
  }
  _Cregexp_tail(OPERAND(p), val, _Cregexp_dummy);
}
 ...
 */
int Cregexp_exec(Cregexp_t* prog, char* string) {
  char* s;
  char** _Cregexp_parse; /* Input-scan pointer. */
  int* _Cregexp_npar;    /* () count. */
  char* _Cregexp_dummy;
...
int _Cregexp_match(char* prog,
  ...
  scan = prog;
  while (scan != NULL) {
    next = _Cregexp_next(scan, _Cregexp_dummy);
    ...
    switch (OP(scan)) {
      ...
      case BRANCH: {
        ...
        if (OP(next) != BRANCH) { /* No choice. */
          ...
          do {
            ...
            *_Cregexp_input = save;
            scan = _Cregexp_next(scan, _Cregexp_dummy);
      ...
      case PLUS: {
        ...
        save = *_Cregexp_input;
        no = _Cregexp_repeat(OPERAND(scan), _Cregexp_input);
        while (no >= min) {
 ...
 */
int _Cregexp_repeat(char* p, char** _Cregexp_input) {
  int count = 0;
  char* scan;
  char* opnd;
 ...
 */
char* _Cregexp_next(char* p, char* _Cregexp_dummy) {
  int offset;
 ...
 */
int Cregexp_sub(Cregexp_t* prog, char* source, char* dest, size_t maxsize) {
  char* src;
  char* dst;
  char c;
  int no;
