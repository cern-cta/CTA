/*
 * $Id: Cregexp.c,v 1.1 2001/11/30 10:57:09 jdurand Exp $
 */

#ifndef lint
static char sccsid[] = "@(#)$RCSfile: Cregexp.c,v $ $Revision: 1.1 $ $Date: 2001/11/30 10:57:09 $ CERN/IT/PDP/DM Jean-Damien Durand";
#endif /* not lint */
...
/* =============================================================== */
#include "Cthread_api.h"      /* Cthread interface       */
...
/* Local variables */
/* =============== */
static int _Cregexp_parse_key;     /* TSD for _Cregexp_parse */
static int _Cregexp_npar_key;      /* TSD for _Cregexp_npar */
static int _Cregexp_dummy_key;     /* TSD for _Cregexp_dummy */
static int _Cregexp_code_key;      /* TSD for _Cregexp_code */
static int _Cregexp_size_key;      /* TSD for _Cregexp_size */
static int _Cregexp_input_key;     /* TSD for _Cregexp_input */
static int _Cregexp_bol_key;       /* TSD for _Cregexp_bol */
static int _Cregexp_startp_key;    /* TSD for _Cregexp_startp */
static int _Cregexp_endp_key;      /* TSD for _Cregexp_endp */
...
int _Cregexp_gettsd(_Cregexp_parse,
     ...
     char ****_Cregexp_endp;
{
  char **_Cregexp_parse_tsd;
  int  *_Cregexp_npar_tsd;
  char *_Cregexp_dummy_tsd;
  char **_Cregexp_code_tsd;
  long *_Cregexp_size_tsd;
  char **_Cregexp_input_tsd;
  char **_Cregexp_bol_tsd;
  char ***_Cregexp_startp_tsd;
  char ***_Cregexp_endp_tsd;

  /* Get TSD variables */
  if (Cthread_getspecific(&_Cregexp_parse_key, (void **) &_Cregexp_parse_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_npar_key,  (void **) &_Cregexp_npar_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_dummy_key, (void **) &_Cregexp_dummy_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_code_key,  (void **) &_Cregexp_code_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_size_key,  (void **) &_Cregexp_size_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_input_key, (void **) &_Cregexp_input_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_bol_key,  (void **) &_Cregexp_bol_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_startp_key, (void **) &_Cregexp_startp_tsd) == 0 &&
      Cthread_getspecific(&_Cregexp_endp_key,  (void **) &_Cregexp_endp_tsd) == 0) {
    if (_Cregexp_parse_tsd == NULL) {
      if ((_Cregexp_parse_tsd = (char **) malloc(sizeof(char *))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_parse_key,_Cregexp_parse_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_npar_tsd == NULL) {
      if ((_Cregexp_npar_tsd = (int *) malloc(sizeof(int))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_npar_key,_Cregexp_npar_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_dummy_tsd == NULL) {
      if ((_Cregexp_dummy_tsd = (char *) malloc(sizeof(char))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_dummy_key,_Cregexp_dummy_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_code_tsd == NULL) {
      if ((_Cregexp_code_tsd = (char **) malloc(sizeof(char *))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_code_key,_Cregexp_code_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_size_tsd == NULL) {
      if ((_Cregexp_size_tsd = (long *) malloc(sizeof(long))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_size_key,_Cregexp_size_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_input_tsd == NULL) {
      if ((_Cregexp_input_tsd = (char **) malloc(sizeof(char *))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_input_key,_Cregexp_input_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_bol_tsd == NULL) {
      if ((_Cregexp_bol_tsd = (char **) malloc(sizeof(char *))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_bol_key,_Cregexp_bol_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_startp_tsd == NULL) {
      if ((_Cregexp_startp_tsd = (char ***) malloc(sizeof(char **))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_startp_key,_Cregexp_startp_tsd) != 0) {
        return(-1);
      }
    }
    if (_Cregexp_endp_tsd == NULL) {
      if ((_Cregexp_endp_tsd = (char ***) malloc(sizeof(char **))) == NULL) {
        serrno = EDB_DS_MALLOC;
        return(-1);
      }
      if (Cthread_setspecific(&_Cregexp_endp_key,_Cregexp_endp_tsd) != 0) {
        return(-1);
      }
    }
  } else {
  ...
  /* OKAY */
  *_Cregexp_parse  = _Cregexp_parse_tsd;
  *_Cregexp_npar   = _Cregexp_npar_tsd;
  *_Cregexp_dummy  = _Cregexp_dummy_tsd;
  *_Cregexp_code   = _Cregexp_code_tsd;
  *_Cregexp_size   = _Cregexp_size_tsd;
  *_Cregexp_input  = _Cregexp_input_tsd;
  *_Cregexp_bol    = _Cregexp_bol_tsd;
  *_Cregexp_startp = _Cregexp_startp_tsd;
  *_Cregexp_endp   = _Cregexp_endp_tsd;
...
/*
 * Last Update: "Thursday 20 September, 2001 at 09:46:30 CEST by Jean-Damien Durand (<A HREF=mailto:Jean-Damien.Durand@cern.ch>Jean-Damien.Durand@cern.ch</A>)"
 */
