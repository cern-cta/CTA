...
Review the [release notes](../../release-notes.md) for the versions covered by your upgrade.
...
RPM upgrades preserve whether each packaged service is enabled and restart it only when it is already running. See [RPM Packages and Services](../deployment/installation/rpm-packages.md#package-lifecycle) for the complete package lifecycle contract.

## Test upgrade path

!!! danger
    Upgrading CTA can affect its disk-system integration: test the upgrade path with the disk system used by your deployment before upgrading production.
