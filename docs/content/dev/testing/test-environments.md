...
The chart dependency order is illustrated here; disk-system-specific setup is described below:
...
flowchart LR
  ...
  Start --> Catalogue
  Start --> Scheduler
  Authentication --> DiskBuffer["Selected Disk System"]
  Authentication --> CTA
  Catalogue --> CTA
  Scheduler --> CTA
  DiskBuffer --> Finish
...
The following sequence describes the EOS test deployment.
...
4. In the EOS deployment, once the `auth` chart is installed, it will start installing the `eos` chart, spawning components such as the MGM and FST.
