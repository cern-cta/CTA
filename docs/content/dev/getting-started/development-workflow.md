# Development Workflow with cta-dev
...
complete value with `--cta-version`. For example, this produces `local-test` for both the RPM version and the Docker
image tag:

```bash
cta-dev up --cta-version local-test
```
...
### PostgreSQL scheduler build

Use `cta-dev build --scheduler-type pgsched` to select the PostgreSQL scheduler backend. The build uses `CTA_USE_PGSCHED`, requires `postgresql-server` for unit tests and `postgresql-libs` at runtime, and excludes the `ctaobjectstore` library.
...
```bash
cta-dev deploy --eos-config my-eos-values.yaml
