...
The disk system requests a tape copy and supplies a destination in its buffer. CTA schedules the read, transfers the file from tape, and reports the result. Client request tracking, pinning, and eviction are disk-system responsibilities.
...
See [Data Integrity](data-integrity.md) for checksum verification and zero-length-file semantics.

## Retrieval ordering

See [Recommended Access Order](../tape/rao.md) for ordering reads within a retrieval batch.
