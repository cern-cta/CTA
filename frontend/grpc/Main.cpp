...
int main(const int argc, char* const* const argv) {
  ...
  if (jwtConfig.has_value()) {
    ...
    {
      lc.push({"jwks_uri", jwtConfig->m_jwksUri});
      lc.push({"total_timeout", std::to_string(jwtConfig->m_jwksTotalTimeout)});
      lc.push({"key_timeout", std::to_string(jwtConfig->m_pubkeyTimeout)});
      lc.push({"cache_refresh_interval", std::to_string(jwtConfig->m_cacheRefreshInterval)});
