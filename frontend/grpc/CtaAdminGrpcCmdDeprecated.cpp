...
#include "CtaAdminGrpcCmdDeprecated.hpp"
...
cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::CtaAdminGrpcCmdDeprecated(int argc, char** argv) : m_strExecname(argv[0]) {
  ...
  if(std::string(argv[argno]) == "--json") { CtaAdminGrpcCmdDeprecated::m_abIsJson = true; ++argno; }
...
void cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::exe(cta::log::Logger& log, const std::string& strSslRoot,
  ...
  switch(cmd_pair(m_request.admincmd().cmd(), m_request.admincmd().subcmd())) {
    case cmd_pair(cta::admin::AdminCmd::CMD_TAPE, cta::admin::AdminCmd::SUBCMD_LS):
      try {
        ...
        lc.log(cta::log::CRIT, "In cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::exe(): Problem with the execution of the tape ls command.");
        throw; // rethrow
      }
      break;
    default:
...
void cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::parseOptions(int start, int argc, char** argv, const cta::admin::cmd_val_t& options) {
...
void cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::addOption(const cta::admin::Option& option, const std::string& strValue) {
...
void cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::readListFromFile(cta::admin::OptionStrList &str_list, const std::string &filename)
{
...
void cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated::throwUsage(const std::string &strError) const {
...
int main(const int argc,  char** argv) {
  ...
  // Command parsing
  cta::frontend::grpc::client::CtaAdminGrpcCmdDeprecated CtaAdminGrpcCmdDeprecated(vArgvCMD.size(), vArgvCMD.data());
  ...
  // Execute the command
  try {
    CtaAdminGrpcCmdDeprecated.exe(*upLog, strSslRoot, strSslKey, strSslCert, strGrpcHost, uiGrpcPort);
  } catch(const cta::exception::Exception& e) {
