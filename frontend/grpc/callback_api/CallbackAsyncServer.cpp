...
#include "CallbackAsyncServer.hpp"
...
cta::frontend::grpc::server::CallbackAsyncServer::CallbackAsyncServer(cta::log::Logger& log,
...
cta::frontend::grpc::server::CallbackAsyncServer::~CallbackAsyncServer() {
  if (m_upServer) {
    m_upServer->Shutdown();
  }
...
cta::frontend::grpc::request::IHandler& cta::frontend::grpc::server::CallbackAsyncServer::getHandler(const cta::frontend::grpc::request::Tag tag) {
  ...
  if(itorFind == m_umapHandlers.end()) {
    ...
    osExMsg << "In grpc::CallbackAsyncServer::getHandler(): Handler " << tag << " is not registered.";
...
void cta::frontend::grpc::server::CallbackAsyncServer::releaseHandler(const cta::frontend::grpc::request::Tag tag) {
  std::lock_guard<std::mutex> lck(m_mtxLockHandler);
  
  {
    ...
    lc.log(cta::log::DEBUG, "In grpc::CallbackAsyncServer::releaseHandler(): Release handler.");
  }
  /*
   * Check if the handler is registered;
   */
  ...
  if(itorFind == m_umapHandlers.end()) {
    std::ostringstream osExMsg;
    osExMsg << "In grpc::CallbackAsyncServer::releaseHandler(): Handler " << tag << " is not registered.";
    throw cta::exception::Exception(osExMsg.str());
  }
  m_umapHandlers.erase(itorFind);
}

void cta::frontend::grpc::server::CallbackAsyncServer::run(const std::shared_ptr<::grpc::ServerCredentials>& spServerCredentials, const std::shared_ptr<::grpc::AuthMetadataProcessor>& spAuthProcessor) {
  log::LogContext lc(m_log);
  m_upCompletionQueue = m_upServerBuilder->AddCompletionQueue();
  // 
  if(!spServerCredentials) {
    throw cta::exception::Exception("In grpc::CallbackAsyncServer::run(): Incorrect server credentials.");
  }
  if(!spAuthProcessor) {
    std::ostringstream osExMsg;
    throw cta::exception::Exception("In grpc::CallbackAsyncServer::run(): Incorrect authorization processor.");
  }
  /*
   * Set auth prosess
   */
  ...
  lc.log(cta::log::INFO, "In grpc::CallbackAsyncServer::run(): Server is starting.");
  /*
  ...
  for (std::thread& worker : m_vThreads) {
    std::thread t(&CallbackAsyncServer::process, this, uiThreadId);
    worker.swap(t);
    uiThreadId++;
  }
  {
    ...
    lc.log(cta::log::INFO, "In grpc::CallbackAsyncServer::run(): Server is listening.");
  }
  for (std::thread& worker : m_vThreads) {
    worker.join();
  }
...
void cta::frontend::grpc::server::CallbackAsyncServer::process(unsigned int uiId) {
  ...
  while(true) {
    ...
    if(!m_upCompletionQueue->Next(&pTag, &bOk)) {
      ...
      lc.log(cta::log::ERR, "In grpc::CallbackAsyncServer::process(): The completion queue has been shutdown.");
      break;
    }
    if(!pTag) {
      log::ScopedParamContainer params(lc);
      params.add("thread", uiId);
      lc.log(cta::log::ERR, "In grpc::CallbackAsyncServer::process(): Invalid tag delivered by notification queue.");
      break;
    }
    ...
    try {
      ...
      lc.log(log::ERR, "grpc::CallbackAsyncServer::process(): Got an exception.");
    }
  }
