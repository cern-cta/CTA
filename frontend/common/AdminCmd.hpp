/*
 * SPDX-FileCopyrightText: 2023 CERN
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

#pragma once

#include "AdminCmdOptions.hpp"
#include "catalogue/Catalogue.hpp"
#include "frontend/common/FrontendService.hpp"

#include "cta_frontend.pb.h"

namespace cta::frontend {

enum class AdminCmdStatus { SUCCESS, USER_ERROR, EXCEPTION };

class AdminCmd : public AdminCmdOptions {
public:
  AdminCmd(const frontend::FrontendService& frontendService,
           const common::dataStructures::SecurityIdentity& clientIdentity,
           const admin::AdminCmd& adminCmd);

  ~AdminCmd() override = default;

  /*!
   * Process the admin command
   *
   * @return Protobuf to return to the client
   */
  xrd::Response process();

  /*!
   * @return       The missing tape file copies minimum age.
   */
  uint64_t getMissingFileCopiesMinAgeSecs() const { return m_missingFileCopiesMinAgeSecs; }

protected:
  /*!
   * Convert AdminCmd <Cmd, SubCmd> pair to an integer so that it can be used in a switch statement
   */
  static constexpr unsigned int cmd_pair(admin::AdminCmd::Cmd cmd, admin::AdminCmd::SubCmd subcmd) {
    return (cmd << 16) + subcmd;
  }

  /*!
   * Log an admin command
   *
   * @param[in]    status      Status of the executed admin command
   * @param[in]    reason      Reason for this log message (error message)
   * @param[in]    t           Timer
   */
  void logAdminCmd(const AdminCmdStatus status, const std::string& reason, utils::Timer& t);

  /*!
   * Drive state enum
   */
  enum DriveState { Up, Down };

  /*!
   * Changes state for the drives by a given regex.
   *
   * @param[in]    regex                The regex to match drive name(s) to change
   * @param[in]    desiredDriveState    Desired drive state (up, forceDown, reason, comment)
   *
   * @return       The result of the operation, to return to the client
   */
  std::string setDriveState(const std::string& regex,
                            const common::dataStructures::DesiredDriveState& desiredDriveState);

  inline static const std::string c_disabledAdminCmdMsg = "Admin command disabled on this frontend.";

  const admin::AdminCmd m_adminCmd;   //!< Administrator Command protocol buffer
  catalogue::Catalogue& m_catalogue;  //!< Reference to CTA Catalogue
  cta::Scheduler& m_scheduler;        //!< Reference to CTA Scheduler
  log::LogContext m_lc;               //!< CTA Log Context

private:
  /*!
   * Process admin commands
   *
   * @param[out]    response    CTA Admin Command response message
   */
  void processAdmin_Add(xrd::Response& response) const;
  void processAdmin_Ch(xrd::Response& response) const;
  void processAdmin_Rm(xrd::Response& response) const;
  void processArchiveRoute_Add(xrd::Response& response) const;
  void processArchiveRoute_Ch(xrd::Response& response) const;
  void processArchiveRoute_Rm(xrd::Response& response) const;
  void processDrive_Up(xrd::Response& response);
  void processDrive_Down(xrd::Response& response);
  void processDrive_Ch(xrd::Response& response);
  void processDrive_Rm(xrd::Response& response);
  void processFailedRequest_Rm(xrd::Response& response);
  void processGroupMountRule_Add(xrd::Response& response) const;
  void processGroupMountRule_Ch(xrd::Response& response) const;
  void processGroupMountRule_Rm(xrd::Response& response) const;
  void processLogicalLibrary_Add(xrd::Response& response) const;
  void processLogicalLibrary_Ch(xrd::Response& response) const;
  void processLogicalLibrary_Rm(xrd::Response& response) const;
  void processMediaType_Add(xrd::Response& response) const;
  void processMediaType_Ch(xrd::Response& response) const;
  void processMediaType_Rm(xrd::Response& response) const;
  void processMountPolicy_Add(xrd::Response& response) const;
  void processMountPolicy_Ch(xrd::Response& response) const;
  void processMountPolicy_Rm(xrd::Response& response) const;
  void processRepack_Add(xrd::Response& response);
  void processRepack_Rm(xrd::Response& response);
  void processRepack_Err(xrd::Response& response) const;
  void processRequesterMountRule_Add(xrd::Response& response) const;
  void processRequesterMountRule_Ch(xrd::Response& response) const;
  void processRequesterMountRule_Rm(xrd::Response& response) const;
  void processActivityMountRule_Add(xrd::Response& response) const;
  void processActivityMountRule_Ch(xrd::Response& response) const;
  void processActivityMountRule_Rm(xrd::Response& response) const;
  void processStorageClass_Add(xrd::Response& response) const;
  void processStorageClass_Ch(xrd::Response& response) const;
  void processStorageClass_Rm(xrd::Response& response) const;
  void processTape_Add(xrd::Response& response) const;
  void processTape_Ch(xrd::Response& response);
  void processTape_Rm(xrd::Response& response);
  void processTape_Reclaim(xrd::Response& response);
  void processTape_Label(xrd::Response& response);
  void processTapeFile_Rm(xrd::Response& response) const;
  void processTapePool_Add(xrd::Response& response) const;
  void processTapePool_Ch(xrd::Response& response) const;
  void processTapePool_Rm(xrd::Response& response) const;
  void processDiskSystem_Add(xrd::Response& response) const;
  void processDiskSystem_Ch(xrd::Response& response) const;
  void processDiskSystem_Rm(xrd::Response& response) const;
  void processDiskInstance_Add(xrd::Response& response) const;
  void processDiskInstance_Ch(xrd::Response& response) const;
  void processDiskInstance_Rm(xrd::Response& response) const;
  void processDiskInstanceSpace_Add(xrd::Response& response) const;
  void processDiskInstanceSpace_Ch(xrd::Response& response) const;
  void processDiskInstanceSpace_Rm(xrd::Response& response) const;
  void processVirtualOrganization_Add(xrd::Response& response) const;
  void processVirtualOrganization_Ch(xrd::Response& response);
  void processVirtualOrganization_Rm(xrd::Response& response);
  void processPhysicalLibrary_Add(xrd::Response& response) const;
  void processPhysicalLibrary_Ch(xrd::Response& response) const;
  void processPhysicalLibrary_Rm(xrd::Response& response) const;
  void processRecycleTapeFile_Restore(xrd::Response& response) const;
  void processArchiveFile_Ch(xrd::Response& response) const;

  void triggerTapeStateChange(const common::dataStructures::SecurityIdentity& admin,
                              const std::string& vid,
                              const common::dataStructures::Tape::State& new_state,
                              const std::optional<std::string>& stateReason,
                              log::LogContext& logContext);

  common::dataStructures::SecurityIdentity m_cliIdentity;       //!< Client identity: username, host, authentication
  const uint64_t m_archiveFileMaxSize;                          //!< Maximum allowed file size for archive requests
  const std::optional<std::string> m_repackBufferURL;           //!< Repack buffer URL
  const std::optional<std::uint64_t> m_repackMaxFilesToSelect;  //!< Repack max files to expand
  const uint64_t m_missingFileCopiesMinAgeSecs;                 //!< Missing tape file copies minimum age
  const uint64_t m_deletionReclaimDelayDays;                    //!< Minimum quarantine period before tape reclaim
  const std::optional<std::string> m_schedulerBackendName;      //!< Name of the Scheduler DB to which Frontend connects
  const common::AdminCmdMode m_adminCommandMode;  //!< Option to select which admin command mode will be used
};

}  // namespace cta::frontend
