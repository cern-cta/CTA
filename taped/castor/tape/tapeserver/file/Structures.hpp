...
public:
  ...
  inline unsigned char getLBPMethod() const {
    ...
    try {
      ...
      throw cta::exception::InvalidArgument(std::string("In VOL1::getLBPMethod(): syntax error for numeric value: ")
                                            + LBPMethod);
    } catch (std::out_of_range&) {
      throw cta::exception::InvalidArgument(std::string("In VOL1::getLBPMethod(): out of range value: ") + LBPMethod);
    }
    switch (hexValue) {
      ...
      case SCSI::logicBlockProtectionMethod::ReedSolomon:
        return hexValue;
      default:
        throw cta::exception::InvalidArgument(std::string("In VOL1::getLBPMethod(): unexpected value: ") + LBPMethod);
    }
  }
...
public:
  ...
  /**
       ...
       */
  inline std::string getfSeq() const { return toString(m_fSeq); }

  /**
       * @return The number of block written on tape per file (only valid in trailer, fairly useless)
       */
  inline std::string getBlockCount() const { return toString(m_blockCount); }
};
...
public:
  /**
       ...
       */
  inline std::string getBlockLength() const { return toString(m_blockLength); }
};
...
public:
  /**
       ...
       */
  inline std::string getBlockSize() const { return toString(m_actualBlockSize); }

  /**
       * @return The file sequence number
       */
  inline std::string getfSeq() const { return toString(m_actualfSeq); }
};
...
inline void setDate(char (&t)[6]) {
  ...
  buff << (localTime.tm_year / 100 ? '0' : ' ') << (localTime.tm_year % 100) << std::setfill('0') << std::setw(3)
       << (localTime.tm_yday + 1);
