...
public:
  ...
  static bool_t xdr_chunk(XDR* xdrs, Chunk* objp) {
    ...
    if (!xdr_bytes(xdrs,
                   reinterpret_cast<char**>(&objp->m_data.m_pcDataVal),
                   reinterpret_cast<uint32_t*>(&objp->m_data.m_uiDataLen),
                   32768)) {
      return (FALSE);
    }
    return (TRUE);
...
private:
  ...
  static bool_t _decode(XDR* xdrs, Record* objp) {
    ...
    if ((objp->bFree && objp->m_RChunk.m_pChunk == nullptr)
        || !xdr_array(xdrs,
                      reinterpret_cast<char**>(&objp->m_RChunk.m_pChunk),
                      reinterpret_cast<uint32_t*>(&objp->m_RChunk.m_ulChunkLen),
                      2048,
                      sizeof(Chunk),
