  ...
  while (itor != m_allOptions.end() && !found) {
    ...
    if (keyValue.at(0) == name) {
      try {
        ...
        std::string errorMsg =
          "The RAO configuration option parameter named " + name + " does not contain any boolean value";
  ...
  while (itor != m_allOptions.end() && !found) {
    ...
    if (keyValue.at(0) == name) {
      try {
        ...
        std::string errorMsg =
          "The RAO configuration option parameter named " + name + " does not contain any string value";
        throw cta::exception::Exception(errorMsg);
      }
    }
  }
...
RAOOptions::CostHeuristicType RAOOptions::getCostHeuristicType() const {
  try {
    ...
    throw cta::exception::Exception("In RAOOptions::getCostHeuristic(), unable to find the cost heuristic to use from "
                                    "the RAOAlgorithmOptions configuration line ("
                                    + m_options + ")");
  }
}
