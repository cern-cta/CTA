...
void WriteSession::setHostName() {
  ...
  m_hostName = m_hostName.substr(0, m_hostName.find("."));  // cppcheck-suppress uselessCallsSubstr
  ...
  try {
    ...
    while (std::getline(resolv, buf)) {
      if (buf.substr(0, 7) == toFind) {
        ...
        m_siteName = m_siteName.substr(0, m_siteName.find("."));  // cppcheck-suppress uselessCallsSubstr
