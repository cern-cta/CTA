...
static_assert(sizeof(RawUHL2) == sizeof(cta::tape::tapeFile::UHL2));
...
void setVol1LabelStandard(cta::tape::tapeFile::VOL1& vol1, const char labelStandard) {
...
cta::tape::drive::deviceInfo makeDeviceInfo() {
...
void writeVol1(cta::tape::drive::FakeDrive& drive, const std::string& vid, const char labelStandard) {
...
void writeEnstoreTape(cta::tape::drive::FakeDrive& drive, const std::string& vid, const std::string& payload) {
...
std::vector<char> makeEnstoreLargeHeaderBlock(const std::string& vid, const size_t payloadSize) {
  ...
  cta::tape::tapeFile::HDR1 hdr1;
  hdr1.fill("1", vid, 1);

  cta::tape::tapeFile::HDR2 hdr2;
  hdr2.fill(ENSTORE_LARGE_BLOCK_SIZE, true);
  setRecordFormat(hdr2, 'D');

  cta::tape::tapeFile::UHL1 uhl1;
...
std::vector<char> makeEnstoreLargeTrailerBlock(const std::string& vid) {
  const auto deviceInfo = makeDeviceInfo();
  cta::tape::tapeFile::EOF1 eof1;
  eof1.fill("1", vid, 1, 1);

  cta::tape::tapeFile::EOF2 eof2;
  eof2.fill(ENSTORE_LARGE_BLOCK_SIZE, true);
  setRecordFormat(eof2, 'D');

  cta::tape::tapeFile::UTL1 utl1;
...
void writeEnstoreLargeTape(cta::tape::drive::FakeDrive& drive, const std::string& vid, const std::string& payload) {
...
protected:
  void readAndVerifyPayload(const cta::common::dataStructures::Label::Format labelFormat,
    ...
    const auto readSession = cta::tape::tapeFile::ReadSessionFactory::create(m_drive, volInfo, false);
    ...
    ASSERT_THROW(reader->readNextDataBlock(data.data(), blockSize), cta::tape::tapeFile::EndOfFile);
  }
