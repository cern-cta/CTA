...
/**
   ...
   */
std::string driverStatusSuggestionsToString(const unsigned short int driverStatus);
...
public:
  enum { securityProtocolInformation = 0x00, tapeDataEncrytpion = 0x20 };
};

class spoutSecurityProtocolPages {
public:
  enum { tapeDataEncryption = 0x20 };
};
...
public:
  enum {
    ...
    tapeAlert = 0x2e,
    dataCompression32h =
      0x32,  // for LTO, SDLT. We have Data Compression 1Bh for LTO5,6 and DataComppression 0Fh in SSC-3
...
public:
  enum { controlDataProtection = 0x0A, deviceConfiguration = 0x10 };
};
...
public:
  enum { subpageCode = 0xF0 };
};

class inquiryVPDPages {
public:
  enum { unitSerialNumber = 0x80 };
};

class writeErrorsDevicePage {
public:
  enum { totalCorrectedErrors = 0x0003, totalProcessed = 0x0005, totalUncorrectedErrors = 0x0006 };
};
...
public:
  enum { readPerformanceEfficiency = 0x03d0, writePerformanceEfficiency = 0x04d0 };
};
