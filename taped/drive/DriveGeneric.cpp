...
std::unique_ptr<DriveInterface> createDrive(const SCSI::DeviceInfo& di, System::virtualWrapper& sw) {
  ...
  if (std::string::npos != di.product.find("MHVTL") || std::string::npos != serialNumber.find("MHVTL")) {
    ...
    if (ret) {
      return std::unique_ptr<DriveInterface>(ret);
    } else {
...
drive::DriveGeneric::DriveGeneric(const SCSI::DeviceInfo& di, System::virtualWrapper& sw)
   ...
   */
  cta::exception::Errnum::throwOnMinusOne(m_tapeFD = m_sysWrapper.open(m_SCSIInfo.nst_dev.c_str(), O_RDWR | O_NONBLOCK),
...
//------------------------------------------------------------------------------
bool drive::DriveGeneric::tapeAlertsCriticalForWrite(const std::vector<uint16_t>& codes) {
...
void drive::DriveGeneric::setDensityAndCompression(bool compression, unsigned char densityCode) {
  ...
  {  // set parameters and we use filled structure devConfig from the previous SCSI call
    ...
    if (0 != densityCode) {
      devConfig.blockDescriptor.densityCode = densityCode;
    }
    if (compression) {
      devConfig.modePage.selectDataComprAlgorithm = 1;
    } else {
      devConfig.modePage.selectDataComprAlgorithm = 0;
    }
...
//------------------------------------------------------------------------------
void drive::DriveGeneric::setLogicalBlockProtection(const unsigned char method,
                                                    const unsigned char methodLength,
                                                    const bool enableLBPforRead,
                                                    const bool enableLBPforWrite) {
...
drive::LBPInfo drive::DriveGeneric::getLBPInfo() {
  ...
  cta::exception::Errnum::throwOnMinusOne(m_sysWrapper.ioctl(m_tapeFD, SG_IO, &sgh), "Failed SG_IO ioctl");
  SCSI::ExceptionLauncher(sgh,
                          std::string("SCSI error fetching data in getLBPInfo: ") + SCSI::statusToString(sgh.status));
...
void drive::DriveGeneric::generateRAO(std::list<SCSI::Structures::RAO::blockLims>& files, int maxSupported) {
  ...
  for (int i = 0; i < udSize; ++i) {
    ...
    strncpy(reinterpret_cast<char*>(ud.get()[i].udsName),
            reinterpret_cast<char*>(it->fseq),
  ...
  switch (m_lbpToUse) {
    ...
    case lbpToUse::crc32cReadOnly: {
      ...
      if (cta::verifyCrc32cForMemoryBlockWithCrc32c(SCSI::logicBlockProtectionMethod::CRC32CSeed,
        ...
        throw cta::exception::Exception("In DriveGeneric::readBlock: Failed checksum verification");
      }
    }
    case lbpToUse::disabled: {
      ssize_t res = m_sysWrapper.read(m_tapeFD, data, count);
      cta::exception::Errnum::throwOnMinusOne(res, "In DriveGeneric::readBlock: Failed ST read");
      return res;
    }
    default:
...
void drive::DriveGeneric::readFileMark(const std::string& context) {
  ...
  // Generic handling of other errors
  cta::exception::Errnum::throwOnMinusOne(res, context + ": Failed ST read in DriveGeneric::readFileMark");
...
void drive::DriveGeneric::SCSI_inquiry() {
  ...
  std::cout << "INQUIRY result: " << std::endl
            << "sgh.dxfer_len=" << sgh.dxfer_len << " sgh.sb_len_wr=" << ((int) sgh.sb_len_wr)
            << " sgh.status=" << ((int) sgh.status) << " sgh.info=" << ((int) sgh.info) << std::endl;
  std::cout << SCSI::Structures::hexDump(dataBuff)
            << SCSI::Structures::toString(*reinterpret_cast<SCSI::Structures::inquiryData_t*>(dataBuff));
}
...
drive::compressionStats drive::DriveT10000::getCompressionStats() {
  ...
  SCSI::Structures::logSenseLogPageHeader_t& logPageHeader =
    *reinterpret_cast<SCSI::Structures::logSenseLogPageHeader_t*>(dataBuff);

  const unsigned char* endPage = dataBuff + SCSI::Structures::toU16(logPageHeader.pageLength) + sizeof(logPageHeader);
  ...
  while (logParameter < endPage) { /* values in bytes  */
    SCSI::Structures::logSenseParameter_t& logPageParam =
      *reinterpret_cast<SCSI::Structures::logSenseParameter_t*>(logParameter);
...
void drive::DriveMHVTL::enableCRC32CLogicalBlockProtectionReadWrite() {
  throw cta::exception::Exception("In DriveMHVTL::enableCRC32CLogicalBlockProtectionReadWrite(): not supported");
}
...
void drive::DriveGeneric::waitUntilReady(const uint32_t timeoutSecond) {
  ...
  if (GMT_ONLINE(mtInfo.mt_gstat) == 0) {
    ...
    ex.getMessage() << "Tape drive empty after waiting " << timeoutSecond << " seconds.";
    throw ex;
  }
}
...
void drive::DriveGeneric::waitTestUnitReady(const uint32_t timeoutSecond) {
  ...
  cta::exception::TimeOut ex;
  ex.getMessage() << "Failed to test unit ready after waiting " << timeoutSecond
                  << " seconds: " << lastTestUnitReadyExceptionMsg;
  throw ex;
}
