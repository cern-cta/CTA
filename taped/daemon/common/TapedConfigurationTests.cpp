...
TEST(cta_Daemon, TapedConfigurationFull) {
  ...
  ASSERT_EQ(300, completeConfig.archiveFetchUnderfillLimits.value().underfillWatchPeriodSecs);
  ASSERT_EQ(5, completeConfig.archiveFetchUnderfillLimits.value().underfillMinSamples);
  ASSERT_EQ(35, completeConfig.archiveFetchUnderfillLimits.value().underfillStartThreshold);
  ASSERT_EQ(75, completeConfig.archiveFetchUnderfillLimits.value().underfillRecoveryThreshold);
