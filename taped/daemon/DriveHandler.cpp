...
#include "telemetry/OtelInit.hpp"
...
DriveHandler::DriveHandler(const TapedConfig& tapedConfig,
    ...
    : SubprocessHandler(std::string("drive:") + driveInfo.driveName),
      ...
      m_lc(m_processManager.logContext()) {
...
SubprocessHandler::ProcessingStatus DriveHandler::fork() {
  ...
  try {
    ...
    cta::telemetry::cleanupOpenTelemetry(m_lc);
    // and fork
    m_pid = ::fork();
    ...
    if (!m_pid) {
      ...
      // cta::telemetry::reinitOpenTelemetry(m_lc);
      ...
      if (!m_catalogue) {
        const std::string catalogueHandlerName = "DriveHandlerParent-" + m_driveInfo.driveName;
    ...
    params.add("tapeDrive", m_driveInfo.driveName).add(semconv::log::exceptionMessage, ex.getMessageValue());
...
void DriveHandler::kill() {
  ...
  if (m_pid != -1) {
    ...
    try {
      ...
      scoped.add("tapeDrive", m_driveInfo.driveName);
...
SubprocessHandler::ProcessingStatus DriveHandler::processEvent() {
  ...
  try {
    ...
    if (auto datagram = m_socketPair->receive(server::SocketPair::Side::child); !message.ParseFromString(datagram)) {
    ...
    if (message.reportingstate()) {
      DriveHandlerStateReporter stateReporter(m_driveInfo.driveName,
                                              &m_processingStatus,
                                              &m_sessionVid,
                                              &m_lastDataMovementTime,
                                              &m_lc);
      m_lastStateChangeTime =
        stateReporter.processState(message, &m_sessionState, &m_previousState, &m_sessionType, &m_previousType);
    }
...
SubprocessHandler::ProcessingStatus DriveHandler::processSigChild() {
  ...
  try {
    ...
    scoped.add("subprocessPid", m_pid)
      .add("tapeDrive", m_driveInfo.driveName)
  ...
  if (rc) {
    ...
    if (WIFEXITED(processStatus)) {
      ...
      // If we are shutting down, we should not request a new session.
      if (m_sessionState != SessionState::Shutdown) {
        m_lc.log(log::CRIT, "Drive subprocess crashed. Will spawn a new one.");
        m_processingStatus.forkRequested = true;
      } else {
        m_lc.log(log::CRIT, "Drive subprocess crashed. Will not spawn new one as we are shutting down.");
        m_processingStatus.forkRequested = false;
      }
      log::ScopedParamContainer scoped(m_lc);
      scoped.add("Error_sessionKilled", 1);
...
int DriveHandler::runChild() {
  ...
  driveInfo.driveName = m_driveInfo.driveName;
  ...
  if (m_previousSession == PreviousSession::Initiating) {
    ...
    try {
      ...
      scheduler->createTapeDriveStatus(driveInfo,
                                       driveState,
                                       cta::common::dataStructures::MountType::NoMount,
...
void DriveHandler::resetLogParams(cta::tape::daemon::TapedProxy* driveHandlerProxy) {
  m_lc.log(log::DEBUG,
           "In DriveHandler::resetLogParams(): Will clear watchdog log parameters in child and parent processes.");
...
void DriveHandler::setDriveDownForShutdown(const std::string& reason) {
  ...
  driveCatalogue.reportDriveStatus(driveInfo,
                                   cta::common::dataStructures::MountType::NoMount,
                                   cta::common::dataStructures::DriveStatus::Down,
                                   std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()),
                                   m_lc);
  driveCatalogue.setDesiredDriveState(m_driveInfo.driveName, desiredDriveState, m_lc);
}
...
void DriveHandler::puttingDriveDown(IScheduler* scheduler,
...
std::shared_ptr<cta::IScheduler> DriveHandler::createScheduler(const std::string& prefixProcessName,
  ...
  try {
    processName = prefixProcessName + m_driveInfo.driveName;
    log::ScopedParamContainer params(m_lc);
    params.add("processName", processName);
#ifdef CTA_PGSCHED
    m_sched_db_init =
