...
void RecallReportPacker::WorkerThread::run() {
  ...
  while (true) {
    ...
    try {
      ...
      params.add(cta::semconv::log::exceptionMessage, e.getMessageValue()).add("exceptionType", typeid(e).name());
      m_parent.m_lc.log(
        cta::log::ERR,
      ...
      //either from the catch a few lines above or directly from rep->execute
      cta::log::ScopedParamContainer params(m_parent.m_lc);
      params.add(cta::semconv::log::exceptionMessage, e.what()).add("exceptionType", typeid(e).name());
  ...
  try {
    ...
    if (m_parent.isDiskDone()) {
      ...
      m_parent.m_lc.log(cta::log::DEBUG,
                        "In RecallReportPacker::WorkerThread::run(): all disk threads are finished, telling the mount "
                        "that Disk threads are complete");
