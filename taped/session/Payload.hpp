...
public:
  ...
  size_t remainingFreeSpace() const { return m_totalCapacity - m_size; }
  ...
  bool append(castor::tape::tapeFile::FileReader& from) {
    if (from.getBlockSize() > remainingFreeSpace()) {
      ...
      err << "Trying to read a tape file block with too little space left: BlockSize=" << from.getBlockSize()
          << " remainingFreeSpace=" << remainingFreeSpace() << " (totalSize=" << m_totalCapacity << ")";
   ...
   */
  static unsigned long zeroAdler32() { return ::adler32(0L, Z_NULL, 0); }
