...
public:
   ...
   */
  TapeReadTask(cta::RetrieveJob* retrieveJob, DataConsumer& destination, RecallMemoryManager& mm)
  ...
  void execute(tapeFile::ReadSession& rs,
    ...
    try {
      ...
      // This is an error case. Log and signal to the disk write task
      {
        ...
        cta::log::LogContext::ScopedParam sp1(lc, Param(semconv::log::exceptionMessage, ex.getMessageValue()));
...
private:
   ...
   */
  std::unique_ptr<tapeFile::FileReader> openFileReader(tapeFile::ReadSession& session, cta::log::LogContext& lc) {
    using cta::log::Param;
    ...
    try {
      ...
      // Log the error
      ScopedParam sp0(lc, Param(semconv::log::exceptionMessage, ex.getMessageValue()));
