    ...
    try {
      ...
      lc.log(cta::log::WARNING,
             "Timed out while scheduling new mount. Could not acquire global scheduler lock in "
               + std::to_string(m_dataTransferConfig.wdGetNextMountMaxSecs) + " seconds.");
    ...
    if (!tapeMount) {
      ...
      if (!nextMountTimeout) {
        lc.log(cta::log::DEBUG,
               "No new mount found. (sleeping " + std::to_string(m_dataTransferConfig.wdIdleSessionTimer)
                 + " seconds)");
...
castor::tape::tapeserver::daemon::DataTransferSession::executeRead(cta::log::LogContext& logContext,
  ...
  // We can now start instantiating all the components of the data path
  {
    ...
    if (fetchResult && reservationResult) {
      ...
      cta::log::Param errorMessageParam(semconv::log::exceptionMessage, "Aborted: empty recall mount");
      ...
      try {
        ...
        cta::log::LogContext::ScopedParam sp11(
          logContext,
          cta::log::Param(semconv::log::exceptionMessage, "Aborted: empty recall mount"));
...
castor::tape::tapeserver::daemon::DataTransferSession::executeWrite(cta::log::LogContext& logContext,
  ...
  // Once we got hold of the drive, we can run the session
  {
    ...
    if (taskInjector.synchronousInjection(noFilesToMigrate)) {
      ...
      cta::log::Param errorMessageParam(semconv::log::exceptionMessage, "Aborted: empty migration mount");
      cta::log::Param mountIdParam("mountId", mountId);
      cta::log::Param mountTypeParam("mountType", mountType);
      cta::log::Param statusParam("status", status);
      cta::log::Param mountAttemptedParam("mountAttempted", 0);
...
void castor::tape::tapeserver::daemon::DataTransferSession::putDriveDown(const std::string& headerErrMsg,
  ...
  cta::log::ScopedParamContainer params(logContext);
  params.add("devFilename", m_driveConfig.devFilename).add(semconv::log::exceptionMessage, headerErrMsg);
