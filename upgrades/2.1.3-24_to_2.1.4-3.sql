 ...
 *
 * @(#)$RCSfile: 2.1.3-24_to_2.1.4-3.sql,v $ $Release: 1.2 $ $Release$ $Date: 2007/09/10 06:53:42 $ $Author: waldron $
 *
 * This script upgrades a CASTOR v2.1.3-24 database into v2.1.4-3
 *
...
DECLARE
  unused VARCHAR(100);
BEGIN
  SELECT release INTO unused FROM CastorVersion WHERE release = '2_1_3_24';
EXCEPTION WHEN NO_DATA_FOUND THEN
  -- Error, we can't apply this script
  raise_application_error(-20000, 'PL/SQL release mismatch. Please run previous upgrade scripts before this one.');
END;

UPDATE CastorVersion SET schemaVersion = '2_1_4_3', release = '2_1_4_3';
COMMIT;
...
ALTER TABLE Disk2DiskCopyDoneRequest ADD (sourceDiskCopyId INTEGER);
...
/* new indexes, useful for reqIdStageQuery */
...
BEGIN
  FOR tab IN (SELECT object_name FROM user_objects
              WHERE  object_type = 'TABLE') LOOP
      EXECUTE IMMEDIATE 'ALTER TABLE '||tab.object_name||' ENABLE ROW MOVEMENT';
  END LOOP;
END;

/* Rename the ID2Type index */
BEGIN
  FOR a IN (SELECT constraint_name
              FROM user_constraints 
             WHERE table_name = 'ID2TYPE' 
               AND constraint_name LIKE 'SYS_%'
	       AND index_name <> 'I_ID2TYPE_ID'
               AND constraint_type = 'P')
  LOOP
    EXECUTE IMMEDIATE 'ALTER INDEX '|| a.constraint_name ||' RENAME TO I_Id2Type_id';
  END LOOP;
END;
