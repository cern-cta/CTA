 ...
 *
 * This script upgrades a CASTOR v2.1.13-5 CNS database to XXXXXX
 *
...
BEGIN
  ...
  UPDATE UpgradeLog
   ...
   WHERE schemaVersion = '2_1_13_X'
     AND release = '2_1_13_X'
     AND state != 'COMPLETE';
  COMMIT;
END;
/
...
VALUES ('2_1_13_X', '2_1_13_X', 'NON TRANSPARENT');
COMMIT;
...
ALTER TABLE Cns_file_metadata ADD (stagerTime NUMBER NOT NULL DEFAULT 0);

UPDATE (
  SELECT * FROM Cns_file_metadata f, Cns_seg_metadata s
	 WHERE f.fileid = s.s_fileid
	)
	SET s.gid = f.gid,
	    s.creationTime = f.mtime,
	    s.lastModificationTime = 0;
