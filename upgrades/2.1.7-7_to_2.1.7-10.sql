 ...
 *
 * @(#)$RCSfile: 2.1.7-7_to_2.1.7-10.sql,v $ $Release: 1.2 $ $Release$ $Date: 2008/07/10 13:52:45 $ $Author: waldron $
 *
 * This script upgrades a CASTOR v2.1.7-7 database into v2.1.7-10
 *
...
UPDATE CastorVersion SET schemaVersion = '2_1_7_10', release = '2_1_7_10';
COMMIT;
...
BEGIN
  SELECT count(*) INTO cnt
   ...
   WHERE job_name = 'TABLESHRINKJOB';
  IF cnt <> 0 THEN
    dbms_scheduler.drop_job('TABLESHRINKJOB');
  END IF;
END;
...
BEGIN
  ...
  SELECT count(*) INTO cnt
    FROM user_indexes
   WHERE index_name = 'I_SUBREQUEST_LASTMODTIME';
  IF cnt = 0 THEN
    EXECUTE IMMEDIATE 'CREATE INDEX I_SubRequest_LastModTime on SubRequest (lastModificationTime) LOCAL';
  END IF;
END;
...
/* Modify the diskcopy table */
CREATE TABLE TempDiskCopy AS SELECT * FROM DiskCopy WHERE ROWNUM < 1;
ALTER TABLE TempDiskCopy ADD (diskCopySize INTEGER, nbCopyAccesses NUMBER DEFAULT 0);

INSERT /*+ append */ 
  INTO TempDiskCopy (path, gcWeight, creationTime, id, filesystem, castorfile,
                     status, lastAccessTime, diskCopySize, nbCopyAccesses) 
SELECT dc.path, dc.gcWeight, dc.creationtime, dc.id, dc.filesystem, dc.castorfile, dc.status, 
       dc.lastAccessTime, cf.filesize, 0
...
DROP TABLE DiskCopy;
ALTER TABLE TempDiskCopy RENAME TO DiskCopy;

/* Recreate indexes on DiskCopy table */
ALTER TABLE DiskCopy ADD CONSTRAINT I_DiskCopy_Id PRIMARY KEY (id);
...
/* Enable row movement on DiskCopy table */
...
/* Enable row movement on tables */
BEGIN
  FOR tab IN (SELECT table_name
                FROM user_tables
               WHERE table_name IN ('BASEADDRESS', 'CLIENT',
                                    'DISK2DISKCOPYDONEREQUEST', 'GETUPDATEDONE',
                                    'GETUPDATEFAILED', 'PUTFAILED',
                                    'FILES2DELETE', 'FILESDELETED',
                                    'FILESDELETIONFAILED', 'GCFILE',
                                    'GCLOCALFILE', 'MOVERCLOSEREQUEST',
                                    'PUTSTARTREQUEST', 'PUTDONESTART',
                                    'GETUPDATESTARTREQUEST', 'QUERYPARAMETER',
                                    'STAGEPREPARETOGETREQUEST', 'STAGEPREPARETOPUTREQUEST',
                                    'STAGEPREPARETOUPDATEREQUEST', 'STAGEPUTREQUEST',
                                    'STAGEUPDATEREQUEST', 'STAGERMREQUEST',
                                    'STAGEPUTDONEREQUEST', 'STAGEFILEQUERYREQUEST',
                                    'STAGEREQUESTQUERYREQUEST', 'STAGEFINDREQUESTREQUEST',
                                    'SUBREQUEST', 'STAGERELEASEFILESREQUEST',
                                    'STAGEABORTREQUEST', 'STAGEGETNEXTREQUEST',
                                    'STAGEPUTNEXTREQUEST', 'STAGEUPDATENEXTREQUEST',
                                    'TAPE', 'SEGMENT', 'TAPEPOOL', 'TAPECOPY',
                                    'CASTORFILE', 'DISKCOPY', 'FILESYSTEM', 'SVCCLASS',
                                    'DISKPOOL', 'STREAM', 'FILECLASS', 'DISKSERVER',
                                    'SETFILEGCWEIGHT', 'STAGEREPACKREQUEST',
                                    'STAGEDISKCOPYREPLICAREQUEST', 'NSFILESDELETED',
                                    'DISK2DISKCOPYSTARTREQUEST', 'FIRSTBYTEWRITTEN',
                                    'STAGEGETREQUEST', 'STGFILESDELETED',
                                    'DISKPOOLQUERY', 'VERSIONQUERY',
                                    'CHANGEPRIVILEGE', 'BWUSER',
                                    'REQUESTTYPE', 'LISTPRIVILEGES')
                 AND row_movement = 'DISABLED')
  LOOP
    EXECUTE IMMEDIATE 'ALTER TABLE '||tab.table_name||' ENABLE ROW MOVEMENT';
  END LOOP;
END;
