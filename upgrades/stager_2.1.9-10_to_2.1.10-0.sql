...
/* SQL statement to add replicaFileSize to Disk2DiskCopyDoneRequest */
ALTER TABLE Disk2DiskCopyDoneRequest ADD (replicaFileSize INTEGER);

/* SQL statement to add queryType to DiskPoolQuery */
ALTER TABLE DiskPoolQuery ADD (queryType INTEGER);

/* SQL statement to add comments field to DrainingFileSystem */
ALTER TABLE DrainingFileSystem ADD (comments VARCHAR2(50) DEFAULT 'N/A' CONSTRAINT NN_DrainingFs_Comments NOT NULL);
...
DROP FUNCTION GetAvailFreeSpace;
...
/* Add missing primary key on the DiskPool2SvcClass table */
DECLARE
  unused VARCHAR2(30);
BEGIN
  -- Drop the old incorrectly named PK if exists.
  BEGIN
    SELECT constraint_name INTO unused
      FROM user_constraints
     WHERE table_name = 'DISKPOOL2SVCCLASS'
       AND constraint_name = 'PK_DISKPOOL2SVCCLASS';
    EXECUTE IMMEDIATE 'ALTER TABLE DiskPool2SvcClass DROP CONSTRAINT PK_DiskPool2SvcClass';
  EXCEPTION WHEN NO_DATA_FOUND THEN
    NULL;
  END;
  -- Drop the old incorrectly named PK associated index if exists.
  BEGIN
    SELECT index_name INTO unused
      FROM user_indexes
     WHERE table_name = 'DISKPOOL2SVCCLASS'
       AND index_name = 'PK_DISKPOOL2SVCCLASS';
    EXECUTE IMMEDIATE 'DROP INDEX PK_DISKPOOL2SVCCLASS';
  EXCEPTION WHEN NO_DATA_FOUND THEN
    NULL;
  END;
  -- Create the new PK.
  BEGIN
    SELECT constraint_name INTO unused
      FROM user_constraints
     WHERE table_name = 'DISKPOOL2SVCCLASS'
       AND constraint_name = 'PK_DISKPOOL2SVCCLASS_PC';
  EXCEPTION WHEN NO_DATA_FOUND THEN
    EXECUTE IMMEDIATE 'ALTER TABLE DiskPool2SvcClass ADD CONSTRAINT PK_DiskPool2SvcClass_PC PRIMARY KEY (parent, child)';
  END;
END;
/
