...
DECLARE
  ...
  PRAGMA EXCEPTION_INIT (UnknownColumn, -904);
BEGIN
  EXECUTE IMMEDIATE 'ALTER TABLE Tape DROP COLUMN Full';
EXCEPTION WHEN UnknownColumn THEN
  -- Ignore missing full column. This only means we are upgrading a 2.1.11-8
  NULL;
END;
/
...
DROP TABLE TapeGatewayRequest CASCADE CONSTRAINTS PURGE;
...
DROP INDEX I_Segment_Tape;
DROP INDEX I_Segment_TapeStatus;
...
BEGIN
  ...
  dropProcedureSafe('STARTREPACKMIGRATION');
  dropFunctionSafe('RTCPCLIENTDISRUNNING');
  dropFunctionSafe('TAPEGATEWAYDISRUNNING');
  dropFunctionSafe('TAPEINTERFACEDAEMONISRUNNING');
  dropProcedureSafe('ANYSEGMENTSFORTAPE');
  dropProcedureSafe('CHECKFILEFORREPACK');
  dropProcedureSafe('FAILEDSEGMENTS');
  dropProcedureSafe('LOCKCASTORFILEBYID');
  dropProcedureSafe('RESTARTSTUCKRECALLS');
  dropProcedureSafe('SEGMENTSFORTAPE');
  dropProcedureSafe('TG_CHECKCONFIGURATION');
END;
/
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      -- ignore, so that we continue compilng the other invalid items
      NULL;
    END;
  END LOOP;
END;
