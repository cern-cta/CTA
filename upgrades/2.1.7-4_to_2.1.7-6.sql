 ...
 *
 * @(#)$RCSfile: 2.1.7-4_to_2.1.7-6.sql,v $ $Release: 1.2 $ $Release$ $Date: 2008/05/05 09:05:32 $ $Author: waldron $
 *
...
DECLARE
  unused VARCHAR(100);
BEGIN
  SELECT release INTO unused FROM CastorVersion WHERE release LIKE '2_1_7_4%';
...
ALTER TABLE StageDiskCopyReplicaRequest ADD sourceSvcClass INTEGER;
ALTER TABLE StageDiskCopyReplicaRequest RENAME COLUMN destDiskCopyId TO destDiskCopy;
ALTER TABLE StageDiskCopyReplicaRequest RENAME COLUMN sourceDiskCopyId TO sourceDiskCopy;

/* Set the source service class for all pending diskcopy replication requests */
BEGIN
  FOR a IN (SELECT StageDiskCopyReplicaRequest.id, max(SvcClass.id) svcclass
              FROM StageDiskCopyReplicaRequest, DiskCopy dest, DiskCopy src, 
                   FileSystem, DiskPool2SvcClass, SvcClass
             WHERE StageDiskCopyReplicaRequest.destDiskCopy = dest.id
               AND dest.status = 1
               AND dest.fileSystem = 0
               AND StageDiskCopyReplicaRequest.sourceDiskCopy = src.id
               AND src.fileSystem = FileSystem.id
               AND FileSystem.diskPool = DiskPool2SvcClass.parent
               AND DiskPool2SvcClass.child = SvcClass.id
          GROUP BY StageDiskCopyReplicaRequest.id)
  LOOP
    UPDATE StageDiskCopyReplicaRequest 
       SET sourceSvcClass = a.svcclass
     WHERE id = a.id;
  END LOOP;
END;
COMMIT;
...
BEGIN
  -- Insert an entry to allow everyone to issue DiskPoolQuery requests
  -- If no restricts are previously listed for this request type
