 ...
 *
 * This script upgrades a CASTOR v2.1.8-11 MONITORING database into v2.1.9-0
 *
...
DECLARE
  unused VARCHAR(100);
BEGIN
  SELECT release INTO unused FROM CastorVersion WHERE release LIKE '2_1_8_11%';
...
UPDATE CastorVersion SET release = '2_1_9_0', schemaVersion = '2_1_9_0';
COMMIT;
...
/* Drop the DLFStats view */
DROP VIEW DLFStats;

/* Remove the runMaxTime column from the configSchema table */
DECLARE
  unused VARCHAR2(30);
BEGIN
  SELECT table_name INTO unused
    FROM user_tab_cols
   WHERE table_name = 'CONFIGSCHEMA'
     AND column_name = 'RUNMAXTIME';
  EXECUTE IMMEDIATE 'ALTER TABLE ConfigSchema DROP COLUMN runMaxTime';
EXCEPTION WHEN NO_DATA_FOUND THEN
  NULL; -- Nothing
END;
/
