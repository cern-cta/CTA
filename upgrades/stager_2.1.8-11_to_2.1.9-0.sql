 ...
 *
 * This script upgrades a CASTOR v2.1.8-11 STAGER database into v2.1.9-0
 *
...
/* Modifications to the Tape table */
ALTER TABLE Tape ADD dgn VARCHAR2(2048);
ALTER TABLE Tape ADD label VARCHAR2(2048);
ALTER TABLE Tape ADD density VARCHAR2(2048);
ALTER TABLE Tape ADD devtype VARCHAR2(2048);

/* Modifications to the TapeCopy table */
ALTER TABLE TapeCopy ADD nbRetry NUMBER;
ALTER TABLE TapeCopy ADD errorCode NUMBER;

/* Remove deleted types from the WhiteList */
DELETE FROM WhiteList
 WHERE reqType NOT IN (SELECT type FROM Type2Obj);

COMMIT;

/* Drop the TapeRequestState table */
DECLARE
  unused VARCHAR2(30);
BEGIN
  SELECT table_name INTO unused
    FROM user_tables
   WHERE table_name = 'TAPEREQUESTSTATE';  
  EXECUTE IMMEDIATE 'DROP TABLE TAPEREQUESTSTATE';
EXCEPTION WHEN NO_DATA_FOUND THEN
  NULL; -- Nothing
END;
/
