 ...
 *
 * @(#)$RCSfile: 2.1.7-19_to_2.1.8-2.sql,v $ $Release: 1.2 $ $Release$ $Date: 2008/10/07 18:42:01 $ $Author: waldron $
 *
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    -- Schedule the start date of the job to 2 hours from now. This
    -- basically pauses the job for 2 hours so that the upgrade can
...
UPDATE SvcClass SET disk1Behavior = 0;
...
UPDATE DiskCopy SET gcType = 0 WHERE status = 9;

/* Accounting modifications */
CREATE TABLE Accounting (euid INTEGER, svcClass INTEGER, nbBytes INTEGER);

/* Initial population of the accounting table */
INSERT INTO Accounting (euid, SvcClass, nbBytes) 
SELECT DiskCopy.owneruid, SvcClass.id, sum(CastorFile.filesize) bytes
  FROM DiskCopy, CastorFile, FileSystem, DiskPool2SvcClass, SvcClass
 WHERE DiskCopy.filesystem = FileSystem.id
   AND FileSystem.diskpool = DiskPool2SvcClass.parent
   AND DiskPool2SvcClass.child = SvcClass.id
   AND DiskCopy.castorfile = CastorFile.id
   AND DiskCopy.status IN (0, 10)
 GROUP BY DiskCopy.owneruid, SvcClass.id;

/* Checksum -> NS support */
ALTER TABLE MoverCloseRequest ADD (csumType VARCHAR2(2048), csumValue VARCHAR2(2048));
...
/* Removed unneeded column from the Disk2DiskCopyStatRequest table */
ALTER TABLE Disk2DiskCopyStartRequest DROP COLUMN destSvcClass;
