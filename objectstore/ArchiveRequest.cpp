...
void ArchiveRequest::commit() {
  ...
  for (auto& job : m_payload.jobs()) {
    int nbTapepool = std::count_if(m_payload.jobs().begin(),
                                   m_payload.jobs().end(),
  ...
  for (auto j = jl->begin(); j != jl->end(); j++) {
    ...
    if (c_statusesImplyingQueueing.count(status) && owner == presumedOwner) {
      ...
      try {
        ...
        Helpers::getLockedAndFetchedJobQueue<ArchiveQueue>(aq,
                                                           aql,
                                                           agentReference,
                                                           containerId,
                                                           getQueueType(status),
        ...
        jta.push_back({jd,
                       getAddressIfSet(),
                       getArchiveFile().archiveFileID,
                       getArchiveFile().fileSize,
                       getMountPolicy(),
                       getEntryLog().time});
        ...
        commit();
        lc.log(log::ERR,
               "In ArchiveRequest::garbageCollect(): failed to requeue the job and failed it. Internal error: the job "
               "is now orphaned.");
      }
    }
  }
...
std::string ArchiveRequest::getJobOwner(uint32_t copyNumber) {
  ...
  auto j =
    std::find_if(jl.begin(), jl.end(), [&copyNumber](decltype(*jl.begin())& j2) { return j2.copynb() == copyNumber; });
  if (jl.end() == j) {
...
//------------------------------------------------------------------------------
auto ArchiveRequest::determineNextStep(uint32_t copyNumberUpdated,
                                       JobEvent jobEvent,
                                       log::LogContext& lc) -> EnqueueingNextStep {
  checkPayloadWritable();
