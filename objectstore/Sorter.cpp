...
void Sorter::executeRetrieveAlgorithm(const std::string& vid,
  ...
  for (auto& jobToAdd : jobs) {
    ...
    succeededJobs[job.jobDump.copyNb] = jobToAdd;
    previousOwner = job.previousOwner->getAgentAddress();
    jobsToAdd.push_back({job.retrieveRequest.get(),
                         job.jobDump.copyNb,
                         job.fSeq,
                         job.fileSize,
                         job.mountPolicy,
...
void Sorter::insertRetrieveRequest(RetrieveRequestInfosAccessorInterface& accessor,
  ...
  if (copyNb == std::nullopt) {
    ...
    if (candidateVidsToTransfer.empty()) {
      throw cta::exception::Exception("In Sorter::insertRetrieveRequest(): there are no ToTransfer jobs in the "
                                      "RetrieveRequest and copyNb was not provided.");
    }

    std::string bestVid;
    try {
      bestVid = Helpers::selectBestRetrieveQueue(candidateVidsToTransfer, m_catalogue, m_objectstore, lc, isRepack);
    } catch (Helpers::NoTapeAvailableForRetrieve&) {
      std::stringstream err;
      err << "In Sorter::insertRetrieveRequest(): no vid available. archiveId="
          << accessor.getArchiveFile().archiveFileID;
      throw RetrieveRequestHasNoCopies(err.str());
    }

    const auto& tapeFileList = accessor.getArchiveFile().tapeFiles;
    if (auto vid_it =
          std::find_if(begin(tapeFileList),
                       end(tapeFileList),
    ...
    try {
      ...
      threading::MutexLocker mapLocker(m_mutex);
      m_retrieveQueuesAndRequests[std::make_tuple(bestVid, common::dataStructures::JobQueueType::JobsToTransferForUser)]
        .emplace_back(rjqi);
