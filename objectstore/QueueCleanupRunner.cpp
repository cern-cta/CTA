    ...
    for (const auto& vid : queueVidSet) {
      if (vidToTapesMap.count(vid) == 0) {
        ...
        logContext.log(log::ERR,
                       "In QueueCleanupRunner::runOnePass(): failed to find the tape " + vid +
                         " in the database. Skipping it.");
      }
    }
  } else {
    logContext.log(log::DEBUG, "In QueueCleanupRunner::runOnePass(): no queues requested a cleanup.");
    return;
  }
  ...
  for (const auto& [queueVid, tapeData] : vidToTapesMap) {
    ...
    if (tapeData.state != common::dataStructures::Tape::REPACKING_PENDING &&
      ...
      logContext.log(log::INFO,
                     "In QueueCleanupRunner::runOnePass(): Queue has the cleanup flag enabled but is not in the "
    ...
    try {
      ...
      m_db.reserveRetrieveQueueForCleanup(queueVid,
                                          prevHeartbeatExists ? std::optional(m_heartbeatCheck[queueVid].heartbeat) :
                                                                std::nullopt);
      ...
      logContext.log(log::DEBUG,
                     "In QueueCleanupRunner::runOnePass(): Unable to find the retrieve queue for cleanup. Queue may "
                     "have already been deleted. Skipping it.");
      continue;
    } catch (OStoreDB::RetrieveQueueNotReservedForCleanup& ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::DEBUG,
                     "In QueueCleanupRunner::runOnePass(): Unable to reserve the retrieve queue due to it not being "
                     "available for cleanup. Skipping it.");
      continue;
    } catch (cta::exception::Exception& ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::WARNING,
                     "In QueueCleanupRunner::runOnePass(): Unable to reserve the retrieve queue for cleanup for "
    ...
    while (true) {
      ...
      try {
        ...
        paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
        logContext.log(log::WARNING,
                       "In QueueCleanupRunner::runOnePass(): Unable to update heartbeat of retrieve queue cleanup, due "
                       "to it not being reserved by agent. Aborting cleanup.");
        break;
      } catch (cta::exception::Exception& ex) {
        log::ScopedParamContainer paramsExcMsg(logContext);
        paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
        logContext.log(log::WARNING,
                       "In QueueCleanupRunner::runOnePass(): Unable to update heartbeat of retrieve queue cleanup for "
                       "unknown reasons. Aborting cleanup.");
        break;
      }
    }
