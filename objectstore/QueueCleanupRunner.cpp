...
QueueCleanupRunner::QueueCleanupRunner(AgentReference& agentReference,
                                       SchedulerDatabase& oStoreDb,
                                       catalogue::Catalogue& catalogue,
                                       std::optional<double> heartBeatTimeout,
                                       std::optional<int> batchSize) :
m_catalogue(catalogue),
m_db(oStoreDb),
m_batchSize(batchSize.value_or(DEFAULT_BATCH_SIZE)),
m_heartBeatTimeout(heartBeatTimeout.value_or(DEFAULT_HEARTBEAT_TIMEOUT)) {}
...
void QueueCleanupRunner::runOnePass(log::LogContext& logContext) {
  ...
  if (!queueVidSet.empty()) {
    ...
    for (const auto& vid : queueVidSet) {
      if (vidToTapesMap.count(vid) == 0) {
        ...
        logContext.log(log::ERR, "ERROR: In QueueCleanupRunner::runOnePass(): failed to find the tape " + vid +
                                   " in the database. Skipping it.");
      }
    }
  }
  else {
    logContext.log(log::DEBUG, "DEBUG: In QueueCleanupRunner::runOnePass(): no queues requested a cleanup.");
    return;
  }
  ...
  for (const auto& [queueVid, tapeData] : vidToTapesMap) {
    ...
    if (tapeData.state != common::dataStructures::Tape::REPACKING_PENDING &&
      ...
      logContext.log(log::INFO, "In QueueCleanupRunner::runOnePass(): Queue is has cleanup flag enabled but is not in "
                                "the expected PENDING state. Skipping it.");
      continue;
    }
    ...
    try {
      ...
      m_db.reserveRetrieveQueueForCleanup(
        queueVid, prevHeartbeatExists ? std::optional(m_heartbeatCheck[queueVid].heartbeat) : std::nullopt);
    }
      ...
      logContext.log(log::DEBUG, "In QueueCleanupRunner::runOnePass(): Unable to find the retrieve queue for cleanup. "
                                 "Queue may have already been deleted. Skipping it.");
      continue;
    }
    catch (OStoreDB::RetrieveQueueNotReservedForCleanup& ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::DEBUG, "In QueueCleanupRunner::runOnePass(): Unable to reserve the retrieve queue due to it "
                                 "not being available for cleanup. Skipping it.");
      continue;
    }
    catch (cta::exception::Exception& ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::WARNING, "In QueueCleanupRunner::runOnePass(): Unable to reserve the retrieve queue for "
                                   "cleanup for unknown reasons. Skipping it.");
      continue;
    }
    ...
    while (true) {
      ...
      paramsLoopMsg.add("numberOfJobsMoved", dbRet.size()).add("jobMovingTime", jobMovingTime).add("tapeVid", queueVid);
      ...
      try {
        ...
        log::ScopedParamContainer paramsExcMsg(logContext);
        paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
        logContext.log(log::WARNING, "In QueueCleanupRunner::runOnePass(): Unable to update heartbeat of retrieve "
                                     "queue cleanup, due to it not being reserved by agent. Aborting cleanup.");
        break;
      }
      catch (cta::exception::Exception& ex) {
        log::ScopedParamContainer paramsExcMsg(logContext);
        paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
        logContext.log(log::WARNING, "In QueueCleanupRunner::runOnePass(): Unable to update heartbeat of retrieve "
                                     "queue cleanup for unknown reasons. Aborting cleanup.");
        break;
      }
    }
    ...
    // Finally, update the tape state out of PENDING
    {
      ...
      try {
        ...
        logContext.log(log::WARNING, "WARNING: In QueueCleanupRunner::runOnePass(): Failed to find a tape in the "
                                     "database. Unable to update tape state.");
        continue;  // Ignore queue
      }
      ...
      switch (tapeDataRefreshed.state) {
        case common::dataStructures::Tape::REPACKING_PENDING:
          m_catalogue.Tape()->modifyTapeState(
            admin, queueVid, common::dataStructures::Tape::REPACKING, common::dataStructures::Tape::REPACKING_PENDING,
        ...
        case common::dataStructures::Tape::EXPORTED_PENDING:
          m_catalogue.Tape()->modifyTapeState(
            admin, queueVid, common::dataStructures::Tape::EXPORTED, common::dataStructures::Tape::EXPORTED_PENDING,
        ...
        default:
          ...
          logContext.log(log::WARNING, "WARNING: In QueueCleanupRunner::runOnePass(): Cleaned up tape is not in a "
                                       "PENDING state. Unable to change it to its corresponding final state.");
          break;
      }
    }
  }
