...
struct QueueCleanupRunnerConcurrentTestParams {
  ...
  QueueCleanupRunnerConcurrentTestParams(cta::SchedulerDatabaseFactory& dbFactory,
                                         std::list<RetrieveRequestSetup>& retrieveRequestSetupList,
                                         std::list<ConcurrentTapeQueueTransition>& tapeQueueTransitionList,
                                         double cleanupTimeout) :
  dbFactory(dbFactory),
 ...
 */
class QueueCleanupRunnerConcurrentTest : public ::testing::TestWithParam<QueueCleanupRunnerConcurrentTestParams> {
public:
...
public:
  ...
  public:
    const char* what() const noexcept override { return "Triggered exception"; }
  };
...
TEST_P(QueueCleanupRunnerConcurrentTest, CleanupRunnerParameterizedTest) {
  ...
  for (auto& retrieveRequestSetupList : GetParam().retrieveRequestSetupList) {
    ...
    unitTests::fillRetrieveRequestsForCleanupRunner(requests, retrieveRequestSetupList.numberOfRequests, requestsPtrs,
                                                    allVids, activeVid, be,
  ...
  {
    cta::objectstore::QueueCleanupRunner qCleanupRunnerBroken(agentForCleanupRef, brokenOStore, catalogue,
                                                              GetParam().cleanupTimeout);
    cta::objectstore::QueueCleanupRunner qCleanupRunnerOk(agentForCleanupRef, oKOStore, catalogue,
                                                          GetParam().cleanupTimeout);
    ...
    ASSERT_NO_THROW(qCleanupRunnerOk.runOnePass(
      lc));  // Two passes are needed for the other cleanup runner to be able to track the heartbeats
...
static std::list<ConcurrentTapeQueueTransition> Test_tapeQueueTransitionList_Completed = {
  {"Tape0", Tape::ACTIVE, Tape::REPACKING, Tape::REPACKING, {10, 0}, {0, 10}},
};
static std::list<ConcurrentTapeQueueTransition> Test_tapeQueueTransitionList_Failed = {
  {"Tape0", Tape::ACTIVE, Tape::REPACKING, Tape::REPACKING_PENDING, {10, 0}, {10, 0}},
};
...
INSTANTIATE_TEST_CASE_P(
  ...
  ::testing::Values(
    ...
    QueueCleanupRunnerConcurrentTestParams(OStoreDBFactoryVFS,
