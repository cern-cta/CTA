...
template<>
const std::string ContainerTraits<ArchiveQueue, ArchiveQueueToReportToRepackForSuccess>::c_containerTypeName =
...
template<>
auto ContainerTraits<ArchiveQueue, ArchiveQueueToReportToRepackForSuccess>::getContainerSummary(Container& cont)
  -> ContainerSummary {
...
template<>
auto ContainerTraits<ArchiveQueue, ArchiveQueueToReportToRepackForSuccess>::getPoppingElementsCandidates(
  Container& cont,
  ...
  auto candidateJobsFromQueue =
    cont.getCandidateList(std::numeric_limits<uint64_t>::max(), unfulfilledCriteria.files, elemtsToSkip);
