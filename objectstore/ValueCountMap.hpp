...
void ValueCountMap<Type, Key>::decCount(const Key& value) {
  ...
  auto counter = std::find_if(m_valueCountMap.begin(),
                              m_valueCountMap.end(),
                              // https://trac.cppcheck.net/ticket/10739
                              // cppcheck-suppress internalAstError
                              [&value](decltype(*m_valueCountMap.begin()) pair) { return pair.value() == value; });
  ...
  if (!counter->count()) {
    ...
    auto counter2 = std::find_if(m_valueCountMap.begin(),
                                 m_valueCountMap.end(),
                                 [&value](decltype(*m_valueCountMap.begin()) pair) { return pair.value() == value; });
...
typedef ValueCountMap<google::protobuf::RepeatedPtrField<serializers::StringCountPair>, std::string>
  ValueCountMapString;
