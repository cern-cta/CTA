...
public:
  ...
  explicit ScopedSharedLock(ObjectOpsBase& oo) { lock(oo); }

  void setObjectLocked(ObjectOpsBase* objectOps) final { objectOps->m_locksCount++; }

  void setObjectUnlocked(ObjectOpsBase* objectOps) final { objectOps->m_locksCount--; }
...
public:
  ...
  explicit ScopedExclusiveLock(ObjectOpsBase& oo, uint64_t timeout_us = 0) { lock(oo, timeout_us); }
  ...
  virtual void getHeaderFromObjectData(const std::string& objData) {
    if (!m_header.ParseFromString(objData)) {
      ...
      throw cta::exception::Exception(std::string("In ObjectOps<") + typeid(PayloadType).name()
                                      + ">::getHeaderFromObjectData(): could not parse header: "
                                      + m_header.InitializationErrorString() + " size=" + std::to_string(objData.size())
                                      + " data(b64)=\"" + objDataBase64 + "\"");
    }
