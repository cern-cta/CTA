...
struct ContainerTraits<RetrieveQueue, C> {
  ...
  struct PoppedElementsSummary {
    ...
    bool operator==(const PoppedElementsSummary& pes) const { return files == pes.files; }

    bool operator<(const PopCriteria& pc) const { return files < pc.files; }
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::ContainerSummary::addDeltaToLog(const ContainerSummary& previous,
...
template<typename C>
auto ContainerTraits<RetrieveQueue, C>::PopCriteria::operator-=(const PoppedElementsSummary& pes) -> PopCriteria& {
  files -= pes.files;
  return *this;
}

template<typename C>
void ContainerTraits<RetrieveQueue, C>::PoppedElementsSummary::addDeltaToLog(const PoppedElementsSummary& previous,
                                                                             log::ScopedParamContainer& params) const {
  params.add("filesAdded", files - previous.files).add("filesBefore", previous.files).add("filesAfter", files);
}

template<typename C>
void ContainerTraits<RetrieveQueue, C>::PoppedElementsList::insertBack(PoppedElementsList&& insertedList) {
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::PoppedElementsBatch::addToLog(log::ScopedParamContainer& params) const {
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::getLockedAndFetched(Container& cont,
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::getLockedAndFetchedNoCreate(Container& cont,
                                                                    ScopedExclusiveLock& contLock,
                                                                    const ContainerIdentifier& cId,
                                                                    log::LogContext& lc) {
...
retry:
  ...
  if (rqAddress.empty()) {
    throw NoSuchContainer("In ContainerTraits<RetrieveQueue,C>::getLockedAndFetchedNoCreate(): no such retrieve queue");
  }
  ...
  try {
    ...
    try {
      ...
      params.add("tapeVid", cId).add("queueObject", cont.getAddressIfSet());
      lc.log(log::INFO, "In ContainerTraits<RetrieveQueue,C>::getLockedAndFetchedNoCreate(): dereferenced missing "
                        "queue from root entry");
    }
    catch (RootEntry::RetrieveQueueNotEmpty& ex) {
      log::ScopedParamContainer params(lc);
      params.add("tapeVid", cId).add("queueObject", cont.getAddressIfSet()).add("Message", ex.getMessageValue());
      lc.log(log::INFO, "In ContainerTraits<RetrieveQueue,C>::getLockedAndFetchedNoCreate(): could not dereference "
                        "missing queue from root entry");
    }
    catch (RootEntry::NoSuchRetrieveQueue& ex) {
      // Somebody removed the queue in the meantime. Barely worth mentioning.
      log::ScopedParamContainer params(lc);
      params.add("tapeVid", cId).add("queueObject", cont.getAddressIfSet());
      lc.log(log::DEBUG, "In ContainerTraits<RetrieveQueue,C>::getLockedAndFetchedNoCreate(): could not dereference "
                         "missing queue from root entry: already done.");
    }
    attemptCount++;
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::addReferencesAndCommit(Container& cont,
                                                               typename InsertedElement::list& elemMemCont,
                                                               AgentReference& agentRef,
                                                               log::LogContext& lc) {
  ...
  for (auto& e : elemMemCont) {
    ...
    jobsToAdd.push_back(
      {e.copyNb, e.fSeq, rr.getAddressIfSet(), e.filesize, e.policy, ::time(nullptr), e.activity, e.diskSystemName});
  }
  cont.addJobsAndCommit(jobsToAdd, agentRef, lc);
}

template<typename C>
void ContainerTraits<RetrieveQueue, C>::addReferencesIfNecessaryAndCommit(Container& cont,
                                                                          typename InsertedElement::list& elemMemCont,
                                                                          AgentReference& agentRef,
                                                                          log::LogContext& lc) {
  std::list<RetrieveQueue::JobToAdd> jobsToAdd;
  for (auto& e : elemMemCont) {
    RetrieveRequest& rr = *e.retrieveRequest;
    jobsToAdd.push_back({e.copyNb, e.fSeq, rr.getAddressIfSet(), e.filesize, e.policy, rr.getCreationTime(), e.activity,
                         e.diskSystemName});
  }
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::removeReferencesAndCommit(
  Container& cont,
...
template<typename C>
void ContainerTraits<RetrieveQueue, C>::removeReferencesAndCommit(Container& cont,
                                                                  std::list<ElementAddress>& elementAddressList) {
  cont.removeJobsAndCommit(elementAddressList);
}

template<typename C>
auto ContainerTraits<RetrieveQueue, C>::getContainerSummary(Container& cont) -> ContainerSummary {
  ContainerSummary ret(cont.getJobsSummary());
  return ret;
}

template<typename C>
auto ContainerTraits<RetrieveQueue, C>::switchElementsOwnership(typename InsertedElement::list& elemMemCont,
                                                                const ContainerAddress& contAddress,
                                                                const ContainerAddress& previousOwnerAddress,
                                                                log::TimingList& timingList,
                                                                utils::Timer& t,
...
template<typename C>
auto ContainerTraits<RetrieveQueue, C>::getElementSummary(const PoppedElement& poppedElement) -> PoppedElementsSummary {
...
template<typename C>
auto ContainerTraits<RetrieveQueue, C>::switchElementsOwnership(PoppedElementsBatch& poppedElementBatch,
                                                                const ContainerAddress& contAddress,
                                                                const ContainerAddress& previousOwnerAddress,
                                                                log::TimingList& timingList,
                                                                utils::Timer& t,
...
template<typename C>
bool ContainerTraits<RetrieveQueue, C>::trimContainerIfNeeded(Container& cont,
                                                              ScopedExclusiveLock& contLock,
                                                              const ContainerIdentifier& cId,
                                                              log::LogContext& lc) {
  if (!cont.isEmpty()) {
...
struct ContainerTraits<RetrieveQueue, RetrieveQueueToTransfer>::PoppedElementsSummary {
  ...
  bool operator==(const PoppedElementsSummary& pes) const { return bytes == pes.bytes && files == pes.files; }
...
template<typename C>
auto ContainerTraits<RetrieveQueue, C>::getPoppingElementsCandidates(Container& cont,
                                                                     PopCriteria& unfulfilledCriteria,
  ...
  auto candidateJobsFromQueue =
    cont.getCandidateList(std::numeric_limits<uint64_t>::max(), unfulfilledCriteria.files, elementsToSkip,
