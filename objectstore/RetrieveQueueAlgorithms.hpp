...
struct ContainerTraits<RetrieveQueue, C> {
  struct ContainerSummary : public RetrieveQueue::JobsSummary {
    ...
    explicit ContainerSummary(const RetrieveQueue::JobsSummary& c)
        : RetrieveQueue::JobsSummary({c.jobs,
                                      c.bytes,
                                      c.oldestJobStartTime,
                                      c.youngestJobStartTime,
                                      c.priority,
                                      c.minRetrieveRequestAge,
                                      c.mountPolicyCountMap,
                                      c.activityCounts,
                                      std::nullopt}) {}
...
retry:
  ...
  try {
    ...
    try {
      ...
      lc.log(log::INFO,
             "In ContainerTraits<RetrieveQueue,C>::getLockedAndFetchedNoCreate(): dereferenced missing queue from root "
             "entry");
...
auto ContainerTraits<RetrieveQueue, C>::getPoppingElementsCandidates(Container& cont,
  ...
  auto candidateJobsFromQueue =
    cont.getCandidateList(std::numeric_limits<uint64_t>::max(),
