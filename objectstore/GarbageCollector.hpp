...
public:
  ...
  struct OwnedObjectSorter {
    ...
    std::map<std::tuple<std::string, common::dataStructures::JobQueueType, std::string>,
  ...
  private:
    std::string dispatchArchiveAlgorithms(std::list<std::shared_ptr<ArchiveRequest>>& jobs,
    ...
    void executeArchiveAlgorithm(std::list<std::shared_ptr<ArchiveRequest>>& jobs,
