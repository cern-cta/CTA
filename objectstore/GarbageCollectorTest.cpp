...
TEST_F(ObjectStore, GarbageCollectorBasicFunctionality) {
  ...
  {
    cta::objectstore::GarbageCollector gc(lc, be, gcAgentRef, catalogue);
    gc.runOnePass();
    gc.runOnePass();
  }
...
TEST_F(ObjectStore, GarbageCollectorArchiveRequest) {
  ...
  while (true) {
    ...
    // - Referenced in the first tape pool
    {
      ...
      jta.push_back({jd,
                     ar.getAddressIfSet(),
                     ar.getArchiveFile().archiveFileID,
                     1000U + pass,
                     policy,
                     std::chrono::system_clock::to_time_t(std::chrono::system_clock::now())});
...
TEST_F(ObjectStore, GarbageCollectorRetrieveRequest) {
  ...
  while (true) {
    ...
    // - Reference job in the first tape
    {
      ...
      jta.push_back({1,
                     rqc.archiveFile.tapeFiles.front().fSeq,
                     rr.getAddressIfSet(),
                     rqc.archiveFile.fileSize,
                     rqc.mountPolicy,
                     sReq.creationLog.time,
                     std::nullopt,
                     std::nullopt});
...
// We will need a log object
  ...
  agent.insertAndRegisterSelf(lc);
  {
    ...
    repackRequest.setCreationLog(
      cta::common::dataStructures::EntryLog("test",
                                            "test",
                                            std::chrono::system_clock::to_time_t(std::chrono::system_clock::now())));
...
TEST_F(ObjectStore, GarbageCollectorRepackRequestRunningExpandFinished) {
  ...
  std::string agentGarbageCollectingRepackRequestAddress;
  {
    ...
    {
      cta::objectstore::GarbageCollector gc(lc2, be, gcAgentRef, catalogue);
      gc.runOnePass();
    }
  }
  {
...
// We will need a log object
...

  {
    ...
    cta::objectstore::RetrieveQueue rqToReportToUser(
      re.getRetrieveQueueAddress("Tape0", JobQueueType::JobsToReportToUser),
      be);
...

  {
    ...
    cta::objectstore::RetrieveQueue rqToReportToRepackForSuccess(
      re.getRetrieveQueueAddress(ri.repackRequestAddress, JobQueueType::JobsToReportToRepackForSuccess),
      be);
...

  {
    ...
    cta::objectstore::Agent agentToReportToRepackForSuccessJobAutoGc(
      agentRefToReportToRepackForSuccessJobAutoGc.getAgentAddress(),
      be);
...

  {
    ...
    cta::objectstore::RetrieveQueue rqToReportToRepackForFailure(
      re.getRetrieveQueueAddress(ri.repackRequestAddress, JobQueueType::JobsToReportToRepackForFailure),
      be);
...

  {
    ...
    cta::objectstore::Agent agentToReportToRepackForFailureJobAutoGc(
      agentRefToReportToRepackForFailureJobAutoGc.getAgentAddress(),
      be);
...
// We will need a log object
...

  {
    ...
    cta::objectstore::Agent agentToReportToRepackForFailureJobAutoGc(
      agentRefToTransferRepackingTapeAutoGc.getAgentAddress(),
      be);
    agentToReportToRepackForFailureJobAutoGc.initialize();
    agentToReportToRepackForFailureJobAutoGc.setTimeout_us(0);
    agentToReportToRepackForFailureJobAutoGc.insertAndRegisterSelf(lc);
    ...
    cta::objectstore::RetrieveQueue rqToTransferForUser(
      re.getRetrieveQueueAddress("Tape0", JobQueueType::JobsToTransferForUser),
      be);
...
TEST_F(ObjectStore, GarbageCollectorArchiveAllStatusesAndQueues) {
  ...
  }
  {
...

    {
      ...
      cta::objectstore::ArchiveQueue aqToTransferForUser(
        re.getArchiveQueueAddress(tapePool, JobQueueType::JobsToTransferForUser),
        be);
  ...
  }
  {
    ...
    //The Archive Request should be queued in the ArchiveQueueToReportForUser
    {
      ...
      cta::objectstore::ArchiveQueue aqToReportToUserForFailure(
        re.getArchiveQueueAddress(tapePool, JobQueueType::JobsToReportToUser),
        be);
  ...
  }
  {
    ...
    cta::objectstore::Agent agentToReportToUserForFailureAutoGC(
      agentRefToReportToUserForFailureAutoGC.getAgentAddress(),
      be);
  ...
  }
  {
    ...
    cta::objectstore::Agent agentToReportToUserForTransferAutoGC(
      agentRefToReportToUserForTransferAutoGC.getAgentAddress(),
      be);
...

  {
    ...
    //The Archive Request should be queued in the ArchiveQueueToReportToRepackForSuccess
    {
      ...
      cta::objectstore::ArchiveQueue aqToReportToRepackForSuccess(
        re.getArchiveQueueAddress(ri.repackRequestAddress, JobQueueType::JobsToReportToRepackForSuccess),
        be);
...

  {
    ...
    cta::objectstore::Agent agentToReportToRepackForSuccessAutoGC(
      agentRefToReportToRepackForSuccessAutoGC.getAgentAddress(),
      be);
...

  {
    ...
    //The Archive Request should be queued in the ArchiveQueueToReportToRepackForFailure
    {
      ...
      cta::objectstore::ArchiveQueue aqToReportToRepackForFailure(
        re.getArchiveQueueAddress(ri.repackRequestAddress, JobQueueType::JobsToReportToRepackForFailure),
        be);
  ...
  }
  {
    ...
    cta::objectstore::Agent agentToReportToRepackForFailureAutoGC(
      agentRefToReportToRepackForFailureAutoGC.getAgentAddress(),
      be);
