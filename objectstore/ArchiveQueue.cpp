...
ArchiveQueue::ArchiveQueue(Backend& os) : ObjectOps<serializers::ArchiveQueue, serializers::ArchiveQueue_t>(os) {}
...
bool ArchiveQueue::checkMapsAndShardsCoherency() {
  ...
  if (uint64_t totalJobs = m_payload.archivejobscount();
      totalBytes != bytesFromShardPointers || totalJobs != jobsExpectedFromShardsPointers) {
    return false;
  }
    ...
    }
    {
      ...
      }
      {
        ...
        throw exception::Exception(
          std::string("In ArchiveQueue::rebuild(): failed to record summary for shard " + s->getAddressIfSet()));
      }
shardUpdated:;
  ...
  while (nextJob != jobsToAdd.end()) {
    ...
    if (uint64_t shardCount = m_payload.archivequeueshards_size();
      ...
      if (auto shardSummary = aqs.getJobsSummary();
          shardPointer.shardbytescount() != shardSummary.bytes || shardPointer.shardjobscount() != shardSummary.jobs) {
        log::ScopedParamContainer params(lc);
        params.add("archiveQueueObject", getAddressIfSet())
          .add("shardNumber", shardCount - 1)
          .add("shardObject", shardPointer.address())
...
void ArchiveQueue::removeJobsAndCommit(const std::list<std::string>& jobsToRemove, log::LogContext& lc) {
  ...
  while (localJobsToRemove.size() && shardIndex < mutableArchiveQueueShards->size()) {
    ...
    localJobsToRemove.remove_if([&removalResult](const std::string& ja) {
      return std::count_if(removalResult.removedJobs.begin(),
                           removalResult.removedJobs.end(),
                           [&ja](ArchiveQueueShard::JobInfo& j) { return j.address == ja; });
    });  // end of remove_if
...
auto ArchiveQueue::getCandidateList(uint64_t maxBytes,
                                    uint64_t maxFiles,
                                    const std::set<std::string>& archiveRequestsToSkip,
                                    log::LogContext& lc) -> CandidateJobList {
  checkPayloadReadable();
  ...
  for (auto& aqsp : m_payload.archivequeueshards()) {
    ...
    if (ret.candidateBytes < maxBytes && ret.candidateFiles < maxFiles) {
      ...
      auto shardCandidates =
        aqs.getCandidateJobList(maxBytes - ret.candidateBytes, maxFiles - ret.candidateFiles, archiveRequestsToSkip);
