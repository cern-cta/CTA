...
void RetrieveQueue::addJobsAndCommit(std::list<common::dataStructures::RetrieveJobToAdd>& jobsToAdd,
...
auto RetrieveQueue::getCandidateList(uint64_t maxBytes,
  ...
  for (auto& rqsp : m_payload.retrievequeueshards()) {
    ...
    if (ret.candidateBytes < maxBytes && ret.candidateFiles < maxFiles) {
      ...
      auto shardCandidates = rqs.getCandidateJobList(maxBytes - ret.candidateBytes,
                                                     maxFiles - ret.candidateFiles,
...
void RetrieveQueue::removeJobsAndCommit(const std::list<std::string>& jobsToRemove, log::LogContext& lc) {
  ...
  while (localJobsToRemove.size() && shardIndex < mutableRetrieveQueueShards->size()) {
    ...
    if (removalResult.jobsAfter) {
      ...
      if (!needToRebuild
          && (shardPointer->shardbytescount() != removalResult.bytesAfter
...
void RetrieveQueue::garbageCollect(const std::string& presumedOwner,
  ...
  if (!getQueueCleanupDoCleanup()) {
    lc.log(log::WARNING,
           "In RetrieveQueue::garbageCollect(): trying to gabarbage collect Retrieve Queue not marked for cleanup. "
           "Ignoring it.");
  }

  // It might happen that the queue was added to ownership but the
  // server crashed before changing the reservation value of the queue.
  // Check assigned agent is the same as the one we are cleaning up.
  if (getQueueCleanupAssignedAgent().has_value() && getQueueCleanupAssignedAgent().value() == presumedOwner) {
