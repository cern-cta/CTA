...
BackendRados::BackendRados(log::Logger& logger,
                           const std::string& userId,
                           const std::string& pool,
                           const std::string& radosNameSpace) :
m_user(userId),
m_pool(pool),
m_namespace(radosNameSpace),
m_cluster(),
m_radosCtxPool() {
  log::LogContext lc(logger);
  cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
    [&]() { return -m_cluster.init(userId.c_str()); }, "In BackendRados::BackendRados, failed to m_cluster.init");
  try {
    RadosTimeoutLogger rtl;
    cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
      [&]() { return -m_cluster.conf_read_file(nullptr); },
    ...
    rtl.reset();
    cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
      [&]() { return -m_cluster.conf_parse_env(nullptr); },
    ...
    rtl.reset();
    cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
      [&]() { return -m_cluster.connect(); }, "In BackendRados::BackendRados, failed to m_cluster.connect");
    ...
    for (size_t i = 0; i < std::thread::hardware_concurrency(); i++) {
      ...
      rtl.reset();
      cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
        [&]() { return -m_cluster.ioctx_create(pool.c_str(), m_radosCtxPool.back()); },
      ...
      cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
        [&]() {
          ...
          rtl.logIfNeeded("In BackendRados::BackendRados(): m_radosCtxPool.back().read()",
...
void BackendRados::create(const std::string& name, const std::string& content) {
  if (content.empty()) {
    throw exception::Exception("In BackendRados::create: trying to create an empty object.");
  }
...
restat:;
        ...
        if (-ENOENT == statRet) {
          ...
          if (size) {
            ...
            en.getMessage() << " Object is still here after 10s. statRet=" << statRet << " size=" << size
                            << " time=" << time;
            throw en;
          }
          else {
...
void BackendRados::ScopedLock::releaseNotify() {
  ...
  cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
    ...
    std::string("In cta::objectstore::ScopedLock::release, failed unlock: ") + m_oid);
  NOTIFYRELEASED();
  ...
  cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
    [&]() { return -m_context.aio_notify(m_oid, completion, bl, 10000, nullptr); },
  ...
  switch (-rc) {
    ...
    default:
      cta::exception::Errnum::throwOnReturnedErrno(
        -rc, std::string("In cta::objectstore::ScopedLock::releaseBackoff, failed unlock: ") + m_oid);
      break;
  }
  NOTIFYRELEASED();
...
void BackendRados::ScopedLock::set(const std::string& oid, const std::string& clientId, LockType lockType) {
  m_oid = oid;
  m_clientId = clientId;
  m_lockType = lockType;
  m_lockSet = true;
...
BackendRados::LockWatcher::LockWatcher(librados::IoCtx& context, const std::string& name) : m_context(context) {
  ...
  RadosTimeoutLogger rtl;
  cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
    [&]() { return -m_context.watch2(name, &m_watchHandle, m_internal.get()); },
...
void BackendRados::LockWatcher::Internal::handle_notify(uint64_t notify_id,
                                                        uint64_t cookie,
...
void BackendRados::LockWatcher::Internal::deleter(librados::completion_t cb, void* i) {
  ...
  internal->m_radosTimeoutLogger.logIfNeeded("BackendRados::LockWatcher::Internal::deleter(): aio_unwatch callback",
                                             internal->m_name);
}
...
void BackendRados::lockNotify(const std::string& name,
                              uint64_t timeout_us,
                              LockType lockType,
  ...
  while (true) {
    ...
    TIMESTAMPEDPRINT(lockType == LockType::Shared ? "Post-watch-setup/Pre-relock (shared)" :
                                                    "Post-watch-setup/Pre-relock (exclusive)");
    ...
    if (!rc) {
      TIMESTAMPEDPRINT(lockType == LockType::Shared ? "Post-relock (shared) (got it)" :
                                                      "Post-relock (exclusive) (got it)");
      NOTIFYLOCKED();
    }
    else {
    ...
    LockWatcher::durationUs watchTimeout =
    ...
    if (timeout_us) {
      watchTimeout =
        std::min(watchTimeout, LockWatcher::durationUs(timeout_us) - LockWatcher::durationUs(timeoutTimer.usecs()));
  ...
  time_t date;
  cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
    [&]() { return -radosCtx.stat(name, &size, &date); },
    std::string("In BackendRados::lock, failed to librados::IoCtx::stat: ") + name + "/" + "lock" + "/" + clientId +
      "//");
  if (!size) {
    // The object has a zero size: we probably created it by attempting the locking.
    cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
      [&]() { return -radosCtx.remove(name); },
      std::string("In BackendRados::lock, failed to librados::IoCtx::remove: ") + name + "//");
...
void BackendRados::RadosLockTimingLogger::Measurements::addAttempt(double latency,
                                                                   double waitTime,
...
void BackendRados::RadosLockTimingLogger::addMeasurements(const Measurements& measurements) {
  ...
  if (measurements.waitCount) {
    if (m_measurements.waitCount) {
      ...
      m_measurements.minLatencyMultiplier =
        std::min(measurements.minLatencyMultiplier, m_measurements.minLatencyMultiplier);
      m_measurements.maxLatencyMultiplier =
        std::max(measurements.maxLatencyMultiplier, m_measurements.maxLatencyMultiplier);
    }
    else {
  ...
  if (m_measurements.totalCalls) {
    {
      ...
      logFile << date << " pid=" << ::getpid() << " progname=" << program_invocation_name
              ...
              << " waitCount=" << m_measurements.waitCount << " minAttempts=" << m_measurements.minAttempts
              << " maxAttempts=" << m_measurements.maxAttempts
              << " averageTotalTime=" << m_measurements.totalTime / m_measurements.totalCalls
              << " minTotalTime=" << m_measurements.minTotalTime << " maxTotalTime=" << m_measurements.maxTotalTime
              << " averageLatency="
              << (m_measurements.attempts ? m_measurements.totalLatency / m_measurements.attempts : 0)
              << " minLatency=" << m_measurements.minLatency << " maxLatency=" << m_measurements.maxLatency
              << " averageWaitTime="
              << (m_measurements.waitCount ? m_measurements.totalWaitTime / m_measurements.waitCount : 0)
              << " minWaitTime=" << m_measurements.minWaitTime << " maxWaitTime=" << m_measurements.maxWaitTime
              << " averageLatencyMultiplier="
              << (m_measurements.waitCount ? m_measurements.totalLatencyMultiplier / m_measurements.waitCount : 0)
...
void BackendRados::lockBackoff(const std::string& name,
                               uint64_t timeout_us,
                               LockType lockType,
                               const std::string& clientId,
                               librados::IoCtx& radosCtx) {
  ...
  while (true) {
    ...
    if (timeout_us && (timeoutTimer.usecs() > (int64_t) timeout_us)) {
      throw exception::TimeoutException(
        "In BackendRados::lockBackoff(): timeout : timeout set = " + std::to_string(timeout_us) +
        " usec, time to lock the object : " + std::to_string(timeoutTimer.usecs()) +
        " usec, number of tries to lock = " + std::to_string(nbTriesToLock) + " object: " + name);
    }
    timespec ts;
    auto latencyUsecs = t.usecs();
    auto wait = latencyUsecs * backoff++ / c_backoffFraction;
    wait = std::min(wait, c_maxWait);
    if (backoff > c_maxBackoff) {
      backoff = 1;
    }
  ...
  cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
    [&]() { return -radosCtx.stat(name, &size, &date); },
    std::string("In BackendRados::lockBackoff, failed to librados::IoCtx::stat: ") + name + "/" + "lock" + "/" +
      clientId + "//");
  if (!size) {
    // The object has a zero size: we probably created it by attempting the locking.
    cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
      [&]() { return -radosCtx.remove(name); },
      std::string("In BackendRados::lockBackoff, failed to librados::IoCtx::remove: ") + name + "//");
...
m_jobFuture(m_job.get_future()) {
  try {
    ...
    aioc->release();
    if (rc) {
      cta::exception::Errnum errnum(
        -rc, std::string("In BackendRados::AsyncCreator::AsyncCreator(): failed to launch aio_operate(): ") + m_name);
...
void BackendRados::AsyncCreator::createExclusiveCallback(librados::completion_t completion, void* pThis) {
  ...
  ac.m_radosTimeoutLogger.logIfNeeded("In BackendRados::AsyncCreator::createExclusiveCallback(): aio_operate callback",
                                      ac.m_name);
  try {
    ...
    if (rados_aio_get_return_value(completion)) {
      if (EEXIST == -rados_aio_get_return_value(completion)) {
        ...
        if (!ac.m_retryTimer) {
          ac.m_retryTimer.reset(new utils::Timer());
        }
        RadosTimeoutLogger rtl;
        int rc;
        ...
        rtl.logIfNeeded("In BackendRados::AsyncCreator::createExclusiveCallback(): m_radosCtx.aio_operate() call",
                        ac.m_name);
      }
      else {
        cta::exception::Errnum errnum(
          -rados_aio_get_return_value(completion),
          std::string("In BackendRados::AsyncCreator::createExclusiveCallback(): could not create object: ") +
            ac.m_name);
        throw Backend::CouldNotCreate(errnum.getMessageValue());
      }
    }
...
m_jobFuture(m_job.get_future()) {
  ...
  try {
    ...
    m_lockAsync.reset(new std::future<void>(std::async(std::launch::async, [this]() {
      try {
        ...
        cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
          [&]() {
            rc = m_backend.getRadosCtx().aio_read(m_name, aioc, &m_radosBufferList, std::numeric_limits<int32_t>::max(),
                                                  0);
            return 0;
          },
        ...
        if (rc) {
          cta::exception::Errnum errnum(
            -rc,
            std::string("In BackendRados::AsyncUpdater::AsyncUpdater::lock_lambda(): failed to launch aio_read(): ") +
              m_name);
...
void BackendRados::AsyncUpdater::deleteEmptyCallback(librados::completion_t completion, void* pThis) {
  ...
  try {
    ...
    throw cta::exception::NoSuchObject(
      std::string("In BackendRados::AsyncUpdater::deleteEmptyCallback(): no such object: ") + au.m_name);
  }
  catch (...) {
...
void BackendRados::AsyncUpdater::UpdateJob::execute() {
  ...
  try {
    ...
    if (!au.m_radosBufferList.length()) {
      std::string errorMsg =
        "In BackendRados::AsyncUpdater::UpdateJob::execute(): considering empty object (name=" + au.m_name +
        ")  as non-existing";
      throw cta::exception::NoSuchObject(errorMsg);
    }
    std::string value;
    ...
    if (updateWithDelete) {
      ...
      rtl.logIfNeeded("In BackendRados::AsyncUpdater::fetchCallback::update_lambda(): m_radosCtx.aio_write_full() call",
                      au.m_name);
      aioc->release();
      if (rc) {
        cta::exception::Errnum errnum(
          -rc, "In BackendRados::AsyncUpdater::fetchCallback::update_lambda(): failed to launch aio_write_full()" +
                 au.m_name);
...
void BackendRados::AsyncUpdater::commitCallback(librados::completion_t completion, void* pThis) {
  AsyncUpdater& au = *((AsyncUpdater*) pThis);
  au.m_radosTimeoutLogger.logIfNeeded("In BackendRados::AsyncUpdater::commitCallback(): aio_write_full() callback",
                                      au.m_name);
  try {
    ...
    if (rc) {
      cta::exception::Errnum errnum(
        -rc, std::string("In BackendRados::AsyncUpdater::commitCallback(): failed to launch aio_unlock()") + au.m_name);
...
void BackendRados::AsyncUpdater::unlockCallback(librados::completion_t completion, void* pThis) {
  AsyncUpdater& au = *((AsyncUpdater*) pThis);
  au.m_radosTimeoutLogger.logIfNeeded("In BackendRados::AsyncUpdater::unlockCallback(): aio_unlock() callback",
                                      au.m_name);
  try {
    ...
    m_lockAsync.reset(new std::future<void>(std::async(std::launch::async, [this]() {
      try {
        ...
        if (rc) {
          cta::exception::Errnum errnum(
            -rc,
            std::string("In BackendRados::AsyncDeleter::AsyncDeleter(): failed to launch aio_remove(): ") + m_name);
          throw Backend::CouldNotDelete(errnum.getMessageValue());
        }
      }
      catch (...) {
...
void BackendRados::AsyncDeleter::deleteCallback(librados::completion_t completion, void* pThis) {
  ...
  au.m_radosTimeoutLogger.logIfNeeded("In BackendRados::AsyncDeleter::deleteCallback(): aio_remove() callback",
                                      au.m_name);
  try {
    // Check that the object could be deleted.
    if (rados_aio_get_return_value(completion)) {
...
void BackendRados::AsyncLockfreeFetcher::AioReadPoster::execute() {
  ...
  try {
    ...
    cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
      [&]() {
        rc = au.m_backend.getRadosCtx().aio_read(au.m_name, aioc, &au.m_radosBufferList,
    ...
    if (rc) {
      cta::exception::Errnum errnum(
        -rc,
        std::string("In BackendRados::AsyncLockfreeFetcher::AsyncLockfreeFetcher(): failed to launch aio_read(): ") +
          au.m_name);
      throw cta::exception::NoSuchObject(errnum.getMessageValue());
    }
  }
...
void BackendRados::AsyncLockfreeFetcher::fetchCallback(librados::completion_t completion, void* pThis) {
  ...
  au.m_radosTimeoutLogger.logIfNeeded("In BackendRados::AsyncLockfreeFetcher::fetchCallback(): aio_read callback",
                                      au.m_name);
  try {
    // Check that the object could be read.
    if (rados_aio_get_return_value(completion) < 0) {
    ...
    if (!au.m_radosBufferList.length()) {
      std::string errorMsg =
        "In BackendRados::AsyncLockfreeFetcher::fetchCallback(): considering empty object (name=" + au.m_name +
        ") as non-existing";
      throw cta::exception::NoSuchObject(errorMsg);
    }
    std::string value;
