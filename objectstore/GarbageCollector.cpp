...
void GarbageCollector::cleanupDeadAgent(const std::string& address,
                                        const std::list<log::Param>& agentDetails,
                                        log::LogContext& lc) {
...
void GarbageCollector::OwnedObjectSorter::sortFetchedObjects(Agent& agent,
  ...
  for (auto& obj : fetchedObjects) {
    ...
    if (obj->getOwner() != agent.getAddressIfSet()) {
      ...
      lc.log(log::WARNING,
             "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): skipping object which is not owned by this "
             "agent");
      agent.removeFromOwnership(obj->getAddressIfSet());
      ownershipUdated = true;
      continue;
        ...
        for (auto& j : ar->dumpJobs()) {
          if ((j.owner == agent.getAddressIfSet() && ar->c_statusesImplyingQueueing.count(j.status))) {
            ...
            try {
              ...
              lc.log(log::INFO,
                     "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): Selected archive request for "
                     "requeueing to the corresponding queue");
              jobRequeued = true;
              ...
              params3.add("tapePool", j.tapePool)
                ...
                .add("fileId", ar->getArchiveFile().archiveFileID);
              lc.log(log::WARNING,
                     "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): Job garbage collected with a "
                     "status not queueable. Leaving it as is.");
            }
          }
        }
        if (!jobRequeued) {
          ...
          lc.log(log::INFO,
                 "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): No active archive job to requeue "
                 "found. Request will remain as-is.");
        }
        break;
      }
      case serializers::RetrieveRequest_t: {
        ...
        if (candidateVids.empty()) {
          ...
          try {
            ...
            lc.log(log::ERR,
                   "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): Failed to determine destination "
        ...
        try {
          ...
          lc.log(log::INFO,
                 "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): No available tape found. Marking "
                 "request for normal GC (and probably deletion).");
          otherObjects.emplace_back(new GenericObject(rr->getAddressIfSet(), objectStore));
          break;
        }
        ...
        params3.add("fileId", rr->getArchiveFile().archiveFileID)
          ...
          .add("fSeq", fSeq);
        lc.log(log::INFO,
               "In GarbageCollector::OwnedObjectSorter::sortFetchedObject(): Selected vid to be requeued for retrieve "
               "request.");
      } break;
      default:
...
std::string
GarbageCollector::OwnedObjectSorter::dispatchArchiveAlgorithms(std::list<std::shared_ptr<ArchiveRequest>>& jobs,
    ...
    while (requestsList.size()) {
      ...
      for (auto& ar : currentJobBatch) {
        if (ar.use_count() == 1 && !jobsIndividuallyGCed.count(ar->getAddressIfSet())) {
          ...
          lc.log(log::DEBUG,
                 "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): Removed AR from agent "
                 "ownership.");
        } else {
          log::ScopedParamContainer params(lc);
          params.add("archiveRequestObject", ar->getAddressIfSet())
          ...
          lc.log(log::DEBUG,
                 "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): Did not remove AR from "
                 "agent ownership.");
        }
      }
      if (ownershipUpdated) {
