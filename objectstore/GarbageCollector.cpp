...
GarbageCollector::GarbageCollector(cta::log::LogContext& lc,
...
void GarbageCollector::trimGoneTargets() {
  ...
  for (auto wa = m_watchedAgents.begin(); wa != m_watchedAgents.end();) {
    if (agentList.end() == std::find(agentList.begin(), agentList.end(), wa->first)) {
      ...
      m_lc.log(log::DEBUG, "In GarbageCollector::trimGoneTargets(): removed now gone agent.");
    } else {
      ++wa;
    }
  }
...
void GarbageCollector::acquireTargets() {
  m_agentRegister.fetchNoLock();
  ...
  for (auto& c : candidatesList) {
    ...
    if (c != m_ourAgentReference.getAgentAddress() && !alreadyTrackedAgents.count(c)) {
      ...
      m_lc.log(log::DEBUG, "In GarbageCollector::acquireTargets(): started tracking an untracked agent");
...
void GarbageCollector::cleanupDeadAgent(const std::string& address, const std::list<log::Param>& agentDetails) {
  ...
  try {
    ...
    m_lc.log(log::DEBUG,
             "In GarbageCollector::cleanupDeadAgent(): agent already deleted when trying to lock it. Skipping it.");
    return;
  }
  agent.fetch();
  log::ScopedParamContainer params(m_lc);
  ...
  if (agent.getOwner() != m_agentRegister.getAddressIfSet()) {
    ...
    m_lc.log(log::DEBUG,
             "In GarbageCollector::cleanupDeadAgent(): skipping agent which is not owned by agent register anymore.");
  ...
  arl.release();
  {
    ...
    m_lc.log(log::DEBUG, "In GarbageCollector::cleanupDeadAgent(): will cleanup dead agent.");
  }
...
void GarbageCollector::OwnedObjectSorter::fetchOwnedObjects(
  ...
  for (auto& obj : ownedObjects) {
    ...
    try {
      ...
      lc.log(log::INFO,
             "In GarbageCollector::OwnedObjectSorter::fetchOwnedObjects(): skipping garbage collection of now "
             "gone object.");
...
void GarbageCollector::OwnedObjectSorter::sortFetchedObjects(
  objectstore::Agent& agent,
  std::list<std::shared_ptr<objectstore::GenericObject>>& fetchedObjects,
  objectstore::Backend& objectStore,
  ...
  for (auto& obj : fetchedObjects) {
    ...
    if (obj->getOwner() != agent.getAddressIfSet()) {
      ...
      lc.log(log::WARNING,
             "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): skipping object which is not owned "
             "by this agent");
      agent.removeFromOwnership(obj->getAddressIfSet());
      ownershipUdated = true;
      continue;
    ...
    switch (obj->type()) {
      case objectstore::serializers::ArchiveRequest_t: {
        ...
        for (auto& j : ar->dumpJobs()) {
          if ((j.owner == agent.getAddressIfSet() && ar->c_statusesImplyingQueueing.count(j.status))) {
            ...
            try {
              ...
              lc.log(log::INFO,
                     "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): Selected archive request "
              ...
              params3.add("tapePool", j.tapePool)
                ...
                .add("fileId", ar->getArchiveFile().archiveFileID);
              lc.log(log::WARNING,
                     "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): Job garbage collected with "
        ...
        if (!jobRequeued) {
          ...
          lc.log(log::INFO,
                 "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): No active archive job to "
      ...
      case objectstore::serializers::RetrieveRequest_t: {
        ...
        try {
          ...
          lc.log(log::INFO,
                 "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): No available tape found. "
        ...
        params3.add("fileId", rr->getArchiveFile().archiveFileID)
          ...
          .add("fSeq", fSeq);
        lc.log(log::INFO,
               "In GarbageCollector::OwnedObjectSorter::sortFetchedObject(): Selected vid to be requeued for "
               "retrieve request.");
      } break;
      default:
...
void GarbageCollector::OwnedObjectSorter::executeArchiveAlgorithm(
...
std::string GarbageCollector::OwnedObjectSorter::dispatchArchiveAlgorithms(
  const std::list<std::shared_ptr<objectstore::ArchiveRequest>>& jobs,
...
void GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(objectstore::Agent& agent,
                                                                        objectstore::AgentReference& agentReference,
                                                                        objectstore::Backend& objectStore,
                                                                        cta::log::LogContext& lc) {
    ...
    while (requestsList.size()) {
      ...
      for (auto& ar : currentJobBatch) {
        if (ar.use_count() == 1 && !jobsIndividuallyGCed.count(ar->getAddressIfSet())) {
          ...
          lc.log(log::DEBUG,
                 "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): Removed AR from "
                 "agent ownership.");
        } else {
          log::ScopedParamContainer params(lc);
          params.add("archiveRequestObject", ar->getAddressIfSet())
          ...
          lc.log(log::DEBUG,
                 "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): Did not remove AR "
...
void GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateRetrieveJobs(objectstore::Agent& agent,
                                                                         objectstore::AgentReference& agentReference,
                                                                         objectstore::Backend& objectStore,
                                                                         cta::log::LogContext& lc) {
...
void GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(objectstore::Agent& agent,
                                                                         objectstore::AgentReference& agentReference,
                                                                         objectstore::Backend& objectStore,
                                                                         cta::catalogue::Catalogue& catalogue,
                                                                         cta::log::LogContext& lc) const {
  ...
  for (auto& go : otherObjects) {
    ...
    if (go->exists()) {
      ...
      lc.log(log::INFO,
             "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(): garbage collected owned "
             "object.");
      ++garbageCollectedOtherObjects;
    } else {
      lc.log(log::INFO,
             "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(): "
