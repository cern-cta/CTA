...
void GarbageCollector::trimGoneTargets(log::LogContext& lc) {
  ...
  for (std::map<std::string, AgentWatchdog*>::iterator wa = m_watchedAgents.begin(); wa != m_watchedAgents.end();) {
...
void GarbageCollector::cleanupDeadAgent(const std::string& address,
                                        std::list<log::Param> agentDetails,
                                        log::LogContext& lc) {
  ...
  arl.release();
  {
    ...
    for (auto p : agentDetails) {
      params2.add(p.getName(), p.getValue());
    }
...
void GarbageCollector::OwnedObjectSorter::fetchOwnedObjects(Agent& agent,
  ...
  for (auto& obj : ownedObjects) {
    ...
    try {
      ...
      lc.log(
        log::INFO,
        "In GarbageCollector::OwnedObjectSorter::fetchOwnedObjects(): skipping garbage collection of now gone object.");
...
void GarbageCollector::OwnedObjectSorter::sortFetchedObjects(Agent& agent,
  ...
  for (auto& obj : fetchedObjects) {
    ...
    if (obj->getOwner() != agent.getAddressIfSet()) {
      ...
      lc.log(log::WARNING, "In GarbageCollector::OwnedObjectSorter::sortFetchedObjects(): skipping object which is not "
                           "owned by this agent");
      agent.removeFromOwnership(obj->getAddressIfSet());
      ownershipUdated = true;
      continue;
...
doGCObject:
    switch (obj->type()) {
      ...
      case serializers::RetrieveRequest_t: {
        ...
        if (candidateVids.empty()) {
          ...
          try {
            ...
            if (rr->getQueueType() != common::dataStructures::JobQueueType::FailedJobs &&
              ...
              retrieveQueuesAndRequests[std::make_tuple(rr->getRepackInfo().repackRequestAddress, rr->getQueueType(),
                                                        vid)]
                .emplace_back(rr);
            }
            else {
            ...
            lc.log(log::ERR, "Failed to determine destination queue for retrieve request. Marking request for normal "
                             "GC (and probably deletion).");
        ...
        retrieveQueuesAndRequests[std::make_tuple(vid, common::dataStructures::JobQueueType::JobsToTransferForUser,
                                                  vid)]
          .emplace_back(rr);
        log::ScopedParamContainer params3(lc);
...
void GarbageCollector::OwnedObjectSorter::executeArchiveAlgorithm(std::list<std::shared_ptr<ArchiveRequest>>& jobs,
  ...
  try {
    aqcl.referenceAndSwitchOwnershipIfNecessary(containerIdentifier, agent.getAddressIfSet(), queueAddress, jobsToAdd,
                                                lc);
  }
    ...
    for (auto& failedAR : failure.failedElements) {
      try {
        ...
        if (typeid(e) == typeid(cta::exception::NoSuchObject) || typeid(e) == typeid(Backend::WrongPreviousOwner)) {
          ...
          lc.log(log::INFO, "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): "
  ...
  for (auto& arup : jobsToAdd) {
    if (!jobsNotRequeued.count(arup.archiveRequest->getAddressIfSet())) {
      ...
      lc.log(log::INFO,
             "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): requeued archive job.");
    }
  }
  jobsToAdd.clear();
}
...
std::string GarbageCollector::OwnedObjectSorter::dispatchArchiveAlgorithms(
  std::list<std::shared_ptr<ArchiveRequest>>& jobs,
  ...
  switch (jobQueueType) {
    case common::dataStructures::JobQueueType::JobsToTransferForUser:
      executeArchiveAlgorithm<ArchiveQueueToTransferForUser>(jobs, queueAddress, containerIdentifier, tapepool,
                                                             jobsIndividuallyGCed, agent, agentReference, objectstore,
                                                             lc);
      break;
    case common::dataStructures::JobQueueType::JobsToReportToUser:
      executeArchiveAlgorithm<ArchiveQueueToReportForUser>(jobs, queueAddress, containerIdentifier, tapepool,
                                                           jobsIndividuallyGCed, agent, agentReference, objectstore,
                                                           lc);
      break;
    case common::dataStructures::JobQueueType::JobsToTransferForRepack:
      executeArchiveAlgorithm<ArchiveQueueToTransferForRepack>(jobs, queueAddress, containerIdentifier, tapepool,
                                                               jobsIndividuallyGCed, agent, agentReference, objectstore,
                                                               lc);
      break;
    case common::dataStructures::JobQueueType::JobsToReportToRepackForSuccess:
      executeArchiveAlgorithm<ArchiveQueueToReportToRepackForSuccess>(jobs, queueAddress, containerIdentifier, tapepool,
                                                                      jobsIndividuallyGCed, agent, agentReference,
                                                                      objectstore, lc);
      break;
    case common::dataStructures::JobQueueType::JobsToReportToRepackForFailure:
      executeArchiveAlgorithm<ArchiveQueueToReportToRepackForFailure>(jobs, queueAddress, containerIdentifier, tapepool,
                                                                      jobsIndividuallyGCed, agent, agentReference,
                                                                      objectstore, lc);
      break;
    case common::dataStructures::JobQueueType::FailedJobs:
      executeArchiveAlgorithm<ArchiveQueueFailed>(jobs, queueAddress, containerIdentifier, tapepool,
                                                  jobsIndividuallyGCed, agent, agentReference, objectstore, lc);
      break;
    default:
      break;
  }
...
void GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(Agent& agent,
  ...
  for (auto& archiveQueueIdAndReqs : archiveQueuesAndRequests) {
    ...
    while (requestsList.size()) {
      ...
      dispatchArchiveAlgorithms(currentJobBatch, queueType, containerIdentifier, tapepool, jobsIndividuallyGCed, agent,
                                agentReference, objectStore, lc);
          ...
          try {
            ...
            if (typeid(e) == typeid(cta::exception::NoSuchObject) || typeid(e) == typeid(Backend::WrongPreviousOwner)) {
              ...
              params.add("retrieveRequestObject", rrup.retrieveRequest->getAddressIfSet())
                ...
                .add("exceptionMessage", e.getMessageValue());
              lc.log(log::ERR, "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateRetrieveJobs(): "
                               "failed to requeue retrieve job with unexpected error. Removing from queue and will "
                               "re-run individual garbage collection.");
              // We will re-run the individual GC for this one.
...
void GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(Agent& agent,
  ...
  for (auto& go : otherObjects) {
    ...
    if (go->exists()) {
      ...
      lc.log(
        log::INFO,
        "In GarbageCollector::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(): garbage collected owned object.");
    }
    else {
