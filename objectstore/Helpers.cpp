    ...
    try {
      ...
      if (i && typeid(ex) == typeid(cta::exception::NoSuchObject)) {
        ...
        try {
          ...
          lc.log(log::INFO, "In Helpers::getLockedAndFetchedQueue<ArchiveQueue>(): removed reference to gone archive "
      ...
      if (i && typeid(ex) == typeid(cta::exception::NoSuchObject)) {
        ...
        try {
          ...
          params.add("tapeVid", vid.value())
            .add("queueObject", retrieveQueue.getAddressIfSet())
            .add("exceptionMsg", ex.getMessageValue());
          lc.log(log::INFO, "In Helpers::getLockedAndFetchedQueue<RetrieveQueue>(): removed reference to gone retrieve "
                            "queue from root entry.");
        }
        catch (...) { /* Failing here is not fatal. We can get an exception if the queue was deleted in the meantime */
        }
...
//------------------------------------------------------------------------------
std::string Helpers::selectBestRetrieveQueue(const std::set<std::string>& candidateVids,
      ...
      if (g_retrieveQueueStatistics.at(v).updating) {
        ...
        if ((g_retrieveQueueStatistics.at(v).tapeStatus.state == common::dataStructures::Tape::ACTIVE && !isRepack) ||
          ...
          logUpdateCacheIfNeeded(
            false, g_retrieveQueueStatistics.at(v),
  ...
  if (candidateVidsStats.empty()) {
    ...
    candidateVidsStats.insert(candidateVidsStats.end(), candidateVidsStatsFallback.begin(),
...
void Helpers::logUpdateCacheIfNeeded(const bool entryCreation,
  ...
  logFile << date << " pid=" << ::getpid() << " tid=" << syscall(SYS_gettid) << " message=" << message
          << " entryCreation=" << entryCreation << " vid=" << tapeStatistic.tapeStatus.vid
