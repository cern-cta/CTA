...
void RepackRequest::updateRepackDestinationInfos(const cta::common::dataStructures::ArchiveFile& archiveFile,
  ...
  for (auto rdiIter = m_payload.mutable_destination_infos()->begin();
       !rdiFound && rdiIter != m_payload.mutable_destination_infos()->end();
       ++rdiIter) {
...
//------------------------------------------------------------------------------
auto RepackRequest::getOrPrepareSubrequestInfo(std::set<uint64_t> fSeqs,
                                               AgentReference& agentRef) -> SubrequestInfo::set {
  checkPayloadWritable();
