...
void RetrieveQueueShard::garbageCollect(const std::string& presumedOwner,
...
RetrieveQueue::CandidateJobList
RetrieveQueueShard::getCandidateJobList(uint64_t maxBytes,
                                        uint64_t maxFiles,
...
std::list<common::dataStructures::RetrieveJobToAdd> RetrieveQueueShard::dumpJobsToAdd() {
  ...
  for (auto& j : m_payload.retrievejobs()) {
    ret.emplace_back(
      ...
      j.address(),
      j.size(),
      common::dataStructures::MountPolicy(j.mountpolicyname(), 0, 0, j.priority(), j.minretrieverequestage()),
      j.starttime(),
      j.has_activity() ? std::optional<std::string> {j.activity()} : std::nullopt,
      j.has_destination_disk_system_name() ? std::optional<std::string> {j.destination_disk_system_name()} :
                                             std::nullopt);
  }
  return ret;
}
...
void RetrieveQueueShard::addJobsBatch(JobsToAddSet& jobsToAdd) {
  ...
  if (0 == N) {
    ...
    Np = m_payload.retrievejobs(0).fseq() <= fSeqLimit ? 0 : 1;
  } else {
    size_t iminmin = 0;
    size_t iminmax = N - 1;
    size_t imin = N / 2;
...
void RetrieveQueueShard::addJob(const common::dataStructures::RetrieveJobToAdd& jobToAdd) {
  ...
  if (jobToAdd.diskSystemName) {
    j->set_destination_disk_system_name(jobToAdd.diskSystemName.value());
  }
...
void RetrieveQueueShard::addJobsThroughCopy(JobsToAddSet& jobsToAdd) {
  ...
  for (auto& jobToAdd : jobsToAdd) {
    ...
    if (jobToAdd.diskSystemName) {
      rjp.set_destination_disk_system_name(jobToAdd.diskSystemName.value());
    }
