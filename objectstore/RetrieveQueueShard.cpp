...
auto RetrieveQueueShard::dumpJobs() -> std::list<JobInfo> {
  ...
  for (auto& j : m_payload.retrievejobs()) {
    ret.emplace_back(JobInfo {j.size(), j.address(), (uint16_t) j.copynb(), j.priority(), j.minretrieverequestage(),
                              (time_t) j.starttime(), j.fseq(), j.mountpolicyname(), std::nullopt, std::nullopt});
    if (j.has_activity()) {
