...
public:
  ...
  void referenceAndSwitchOwnership(const typename ContainerTraits<Q, C>::ContainerIdentifier& contId,
    ...
    auto failedOwnershipSwitchElements = ContainerTraits<Q, C>::switchElementsOwnership(elements,
    ...
    if (failedOwnershipSwitchElements.empty()) {
      ...
      lc.log(log::WARNING,
             "In ContainerAlgorithms::referenceAndSwitchOwnership(): "
  ...
  void referenceAndSwitchOwnershipIfNecessary(const typename ContainerTraits<Q, C>::ContainerIdentifier& contId,
    ...
    if (failedOwnershipSwitchElements.empty()) {
      ...
      params.add("errorCount", failedOwnershipSwitchElements.size());
      lc.log(log::WARNING,
             "In ContainerAlgorithms::referenceAndSwitchOwnershipIfNecessary(): "
             "Encountered problems while requeuing a batch of elements");
      throw failureEx;
    }
  }
  ...
  popNextBatchFromContainerAndSwitchStatus(C& cont,
    ...
    contSummaryBefore = ContainerTraits<Q, C>::getContainerSummary(cont);
    {
      ...
      {
        auto failedOwnershipSwitchElements =
          ContainerTraits<Q, C>::switchElementsOwnershipAndStatus(candidateElements,
    ...
    }
    {
      ...
      lc.log(
        log::INFO,
        "In ContainerTraits<Q,C>::popNextBatchFromContainerAndSwitchStatus(): did one round of elements retrieval.");
    }
logAndReturn: {
  log::ScopedParamContainer params(lc);
  params.add("C", ContainerTraits<Q, C>::c_containerTypeName);
  ...
  lc.log(log::INFO,
         "In ContainerTraits<Q,C>::popNextBatchFromContainerAndSwitchStatus(): elements retrieval complete.");
}
    return ret;
  }
  ...
  PoppedElementsBatch popNextBatch(const typename ContainerTraits<Q, C>::ContainerIdentifier& contId,
    ...
    while (!unexpectedException && ret.summary < popCriteria && !didTrim) {
      ...
      {
        auto failedOwnershipSwitchElements =
          ContainerTraits<Q, C>::switchElementsOwnership(candidateElements,
                                                         m_agentReference.getAgentAddress(),
                                                         cont.getAddressIfSet(),
                                                         localTimingList,
                                                         t,
