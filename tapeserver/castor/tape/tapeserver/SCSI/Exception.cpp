...
cta::exception::Exception("") {
  ...
  if (SCSI::Status::CHECK_CONDITION == status) {
    ...
    try {
      ...
      w << ": In addition, failed to get Sense Key string: " << ex.getMessage().str();
    }
    try {
      w << ": " << sense->getACSString();
    }
    catch (Exception& ex) {
      w << ": In addition, failed to get ACS string: " << ex.getMessage().str();
    }
  }
  setWhat(w.str());
}
...
cta::exception::Exception("") {
  ...
  w << context << (context.size() ? " " : "")
    << "SCSI command failed with host_status: " << SCSI::hostStatusToString(host_status);
  setWhat(w.str());
}
...
cta::exception::Exception("") {
  ...
  w << context << (context.size() ? " " : "")
    << "SCSI command failed with driver_status: " << SCSI::driverStatusToString(driver_status)
    << (driverSuggestions.size() ? ": Driver suggestions:" : "") << driverSuggestions;
