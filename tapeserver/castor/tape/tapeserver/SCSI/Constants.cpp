...
std::string castor::tape::SCSI::statusToString(unsigned char status) {
  switch (status) {
    ...
    default:
      std::stringstream ret;
      ret << "Reserved of obsolete code: " << std::hex << std::nouppercase << std::showbase << status;
      return ret.str();
  }
}
...
std::string castor::tape::SCSI::hostStatusToString(const unsigned short int hostStatus) {
  switch (hostStatus) {
    ...
    default:
      std::stringstream ret;
      ret << "Unknown host status code: " << std::hex << std::nouppercase << std::showbase << hostStatus;
      return ret.str();
  }
}
...
std::string castor::tape::SCSI::driverStatusToString(const unsigned short int driverStatus) {
  switch (driverStatus & castor::tape::SCSI::DriverStatus::MASK) {
    ...
    default:
      std::stringstream ret;
      ret << "Unknown driver status code: " << std::hex << std::nouppercase << std::showbase << driverStatus;
      return ret.str();
  }
}
...
std::string castor::tape::SCSI::senseConstants::getASCString(uint8_t asc, uint8_t ascq) {
  ...
  for (int i = 0; ascRangesStrings[i].text; i++) {
    if (ascRangesStrings[i].asc == asc && ascRangesStrings[i].ascq_min <= ascq &&
