...
SCSI::DeviceVector::DeviceVector(System::virtualWrapper& sysWrapper) : m_sysWrapper(sysWrapper) {
  ...
  DIR* dirp;
  cta::exception::Errnum::throwOnNull(dirp = m_sysWrapper.opendir(sysDevsPath.c_str()),
  ...
  while (struct dirent* dent = m_sysWrapper.readdir(dirp)) {
    ...
    if (0 == checkResult.size()) {
      continue; /* we only use digital names like 6:0:3:0 */
    }
    std::string fullpath = sysDevsPath + "/" + std::string(dent->d_name);
    /* We expect only symbolic links in this directory, */
    char rp[PATH_MAX];
    cta::exception::Errnum::throwOnNull(m_sysWrapper.realpath(fullpath.c_str(), rp),
...
SCSI::DeviceInfo& SCSI::DeviceVector::findBySymlink(const std::string& path) {
  struct stat sbuff;
  cta::exception::Errnum::throwOnMinusOne(m_sysWrapper.stat(path.c_str(), &sbuff),
  ...
  throw SCSI::DeviceVector::NotFound(std::string("Could not find tape device pointed to by ") + path);
}

std::string SCSI::DeviceVector::readfile(const std::string& path) {
  int fd;
  cta::exception::Errnum::throwOnMinusOne(fd = m_sysWrapper.open(path.c_str(), 0),
  ...
  while (ssize_t sread = m_sysWrapper.read(fd, buf, readfileBlockSize)) {
    cta::exception::Errnum::throwOnMinusOne(sread, std::string("Could not read from open file ") + path);
    ret.append(buf, sread);
  }
  cta::exception::Errnum::throwOnNonZero(m_sysWrapper.close(fd), std::string("Error closing file ") + path);
  return ret;
}

SCSI::DeviceInfo::DeviceFile SCSI::DeviceVector::readDeviceFile(const std::string& path) {
  DeviceInfo::DeviceFile ret;
  if (std::string file = readfile(path); !::sscanf(file.c_str(), "%u:%u\n", &ret.major, &ret.minor)) {
...
SCSI::DeviceInfo::DeviceFile SCSI::DeviceVector::statDeviceFile(const std::string& path) {
  struct stat sbuf;
  cta::exception::Errnum::throwOnNonZero(m_sysWrapper.stat(path.c_str(), &sbuf),
...
void SCSI::DeviceVector::getTapeInfo(DeviceInfo& devinfo) {
  ...
  if (!dirp) {
    ...
    cta::exception::Errnum::throwOnNull(dirp = m_sysWrapper.opendir(devinfo.sysfs_entry.c_str()),
                                        std::string("Error opening tape device directory ") + devinfo.sysfs_entry
  ...
  while (struct dirent* dent = m_sysWrapper.readdir(dirp)) {
    ...
    if (res.size()) {
      ...
      devinfo.st = readDeviceFile(devinfo.sysfs_entry + tapeDir + "/" + std::string(dent->d_name) + "/dev");
      /* Check the actual device file */
      DeviceInfo::DeviceFile realFile = statDeviceFile(devinfo.st_dev);
      if (devinfo.st != realFile) {
        std::stringstream err;
        err << "Mismatch between sysfs info and actual device file: " << devinfo.sysfs_entry + "/" + dent->d_name
            << " indicates " << devinfo.st.major << ":" << devinfo.st.minor << " while " << devinfo.st_dev
            << " is: " << realFile.major << ":" << realFile.minor;
    ...
    if (res.size()) {
      ...
      /* Read the major and major number */
      devinfo.nst = readDeviceFile(devinfo.sysfs_entry + tapeDir + "/" + std::string(dent->d_name) + "/dev");
      /* Check the actual device file */
      DeviceInfo::DeviceFile realFile = statDeviceFile(devinfo.nst_dev);
      if (devinfo.nst != realFile) {
        std::stringstream err;
        err << "Mismatch between sysfs info and actual device file: " << devinfo.sysfs_entry + "/" + dent->d_name
            << " indicates " << devinfo.nst.major << ":" << devinfo.nst.minor << " while " << devinfo.st_dev
            << " is: " << realFile.major << ":" << realFile.minor;
        throw cta::exception::Exception(err.str());
      }
    }
...
SCSI::DeviceInfo SCSI::DeviceVector::getDeviceInfo(const char* path) {
  ...
  /* Get name of sg device */
  {
    ...
    ssize_t rlSize;
    cta::exception::Errnum::throwOnMinusOne(rlSize = m_sysWrapper.readlink(lp.c_str(), rl, sizeof(rl) - 1),
                                            std::string("Could not read link ") + lp);
    rl[rlSize] = '\0';
    std::string gl(rl);
    size_t pos = gl.find_last_of("/");
    if (pos == std::string::npos) {
      throw cta::exception::Exception(std::string("Could not find last / in link: ") + gl + " read from "
                                      + ret.sysfs_entry + "/generic");
    }
  ...
  if (DeviceInfo::DeviceFile realFile = statDeviceFile(ret.sg_dev); ret.sg != realFile) {
    std::stringstream err;
    err << "Mismatch between sysfs info and actual device file: " << ret.sysfs_entry + "/generic/dev" << " indicates "
        << ret.sg.major << ":" << ret.sg.minor << " while " << ret.sg_dev << " is: " << realFile.major << ":"
        << realFile.minor;
    throw cta::exception::Exception(err.str());
  }
