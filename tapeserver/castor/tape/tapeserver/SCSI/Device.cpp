...
SCSI::DeviceInfo SCSI::DeviceVector::getDeviceInfo(const char* path) {
  ...
  if (ret.sg != realFile) {
    std::stringstream err;
    err << "Mismatch between sysfs info and actual device file: " << ret.sysfs_entry + "/generic/dev"
        << " indicates " << ret.sg.major << ":" << ret.sg.minor << " while " << ret.sg_dev << " is: " << realFile.major
        << ":" << realFile.minor;
    throw cta::exception::Exception(err.str());
  }
