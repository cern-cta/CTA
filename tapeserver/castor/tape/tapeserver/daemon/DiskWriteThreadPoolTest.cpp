...
namespace unitTests{
  ...
  class TestingDatabaseRetrieveMount: public cta::SchedulerDatabase::RetrieveMount {
    ...
    std::list<std::unique_ptr<cta::SchedulerDatabase::RetrieveJob> > getNextJobBatch(uint64_t filesRequested, uint64_t bytesRequested, cta::log::LogContext& logContext) override { throw cta::exception::NotImplementedException();}
