 ...
 */
void castor::tape::tapeserver::daemon::DataTransferSession::setProcessCapabilities(const std::string& capabilities) {
  cta::log::LogContext lc(m_log);
  try {
    m_capUtils.setProcText(capabilities);
    cta::log::LogContext::ScopedParam sp(lc, cta::log::Param("capabilities", m_capUtils.getProcText()));
    lc.log(cta::log::INFO, "Set process capabilities for using tape");
  }
  catch (const cta::exception::Exception& ne) {
    lc.log(cta::log::ERR, "Failed to set process capabilities for using the tape ");
  }
}
...
castor::tape::tapeserver::daemon::Session::EndOfSessionAction
  ...
  while (true) {
    ...
    try {
      ...
      lc.log(cta::log::WARNING, "Global lock timeout while getting new mount (" +
                                  std::to_string(m_dataTransferConfig.wdGlobalLockAcqMaxSecs) +
                                  " seconds reached). "
                                  "Total time trying to get new mount is " +
                                  std::to_string(t.secs()) + " seconds.");
    ...
    if (!tapeMount) {
      ...
      m_scheduler.reportDriveStatus(m_driveInfo, cta::common::dataStructures::MountType::NoMount,
                                    cta::common::dataStructures::DriveStatus::Up, lc);
      if (!globalLockTimeout) {
        lc.log(cta::log::DEBUG, "No new mount found. (sleeping " +
                                  std::to_string(m_dataTransferConfig.wdIdleSessionTimer) + " seconds)");
  ...
  params.add("tapeVid", m_volInfo.vid).add("mountId", m_volInfo.mountId);
  {
...
castor::tape::tapeserver::daemon::Session::EndOfSessionAction
  ...
  // We can now start instantiating all the components of the data path
  {
    ...
    TapeReadSingleThread readSingleThread(
      *drive, m_mediaChanger, reporter, m_volInfo, m_dataTransferConfig.bulkRequestRecallMaxFiles, m_capUtils, watchDog,
      logContext, reportPacker, m_dataTransferConfig.useLbp, m_dataTransferConfig.useRAO,
      m_dataTransferConfig.useEncryption, m_dataTransferConfig.externalEncryptionKeyScript, *retrieveMount,
      m_dataTransferConfig.tapeLoadTimeout, m_scheduler.getCatalogue());

    DiskWriteThreadPool threadPool(m_dataTransferConfig.nbDiskThreads, reportPacker, watchDog, logContext,
                                   m_dataTransferConfig.xrootPrivateKey, m_dataTransferConfig.xrootTimeout);
    ...
    if (m_dataTransferConfig.useRAO) {
      castor::tape::tapeserver::rao::RAOParams raoDataConfig(
        m_dataTransferConfig.useRAO, m_dataTransferConfig.raoLtoAlgorithm, m_dataTransferConfig.raoLtoAlgorithmOptions,
        m_volInfo.vid);
...
castor::tape::tapeserver::daemon::Session::EndOfSessionAction
  ...
  // Once we got hold of the drive, we can run the session
  {
    ...
    MigrationWatchDog watchDog(15, m_dataTransferConfig.wdNoBlockMoveMaxSecs, m_initialProcess, *archiveMount,
                               m_driveConfig.unitName, logContext);
    TapeWriteSingleThread writeSingleThread(
      *drive, m_mediaChanger, reporter, watchDog, m_volInfo, logContext, reportPacker, m_capUtils,
      m_dataTransferConfig.maxFilesBeforeFlush, m_dataTransferConfig.maxBytesBeforeFlush, m_dataTransferConfig.useLbp,
      m_dataTransferConfig.useEncryption, m_dataTransferConfig.externalEncryptionKeyScript, *archiveMount,
      m_dataTransferConfig.tapeLoadTimeout, m_scheduler.getCatalogue());

    DiskReadThreadPool threadPool(m_dataTransferConfig.nbDiskThreads, m_dataTransferConfig.bulkRequestMigrationMaxFiles,
                                  m_dataTransferConfig.bulkRequestMigrationMaxBytes, watchDog, logContext,
                                  m_dataTransferConfig.xrootPrivateKey, m_dataTransferConfig.xrootTimeout);
...
void castor::tape::tapeserver::daemon::DataTransferSession::putDriveDown(const std::string& headerErrMsg,
  ...
  params.add("devFilename", m_driveConfig.devFilename).add("errorMessage", headerErrMsg);
