...
m_tapeThreadComplete(false),
m_diskThreadComplete(false) {}
...
void RecallReportPacker::ReportError::execute(RecallReportPacker& reportPacker) {
  ...
  try {
    ...
    reportPacker.m_lc.log(cta::log::WARNING, "In RecallReportPacker::ReportError::execute(): call to "
                                             "m_failedRetrieveJob->failed() , job does not exist in the objectstore.");
  }
...
//------------------------------------------------------------------------------
RecallReportPacker::WorkerThread::WorkerThread(RecallReportPacker& parent) : m_parent(parent) {}
...
void RecallReportPacker::WorkerThread::run() {
  ...
  while (true) {
    ...
    try {
      ...
      m_parent.m_lc.log(cta::log::ERR, "In RecallReportPacker::WorkerThread::run(): Received a standard exception "
                                       "while reporting retrieve mount results.");
      if (m_parent.m_watchdog) {
        m_parent.m_watchdog->addToErrorCount("Error_reporting");
        m_parent.m_watchdog->addParameter(cta::log::Param("status", "failure"));
      }
      ...
      //we get there because to tried to close the connection and it failed
      //either from the catch a few lines above or directly from rep->execute
      m_parent.m_lc.log(cta::log::ERR, "In RecallReportPacker::WorkerThread::run(): Received an unknown exception "
                                       "while reporting retrieve mount results.");
      if (m_parent.m_watchdog) {
        m_parent.m_watchdog->addToErrorCount("Error_reporting");
        m_parent.m_watchdog->addParameter(cta::log::Param("status", "failure"));
  ...
  try {
    ...
    if (m_parent.isDiskDone()) {
      ...
      m_parent.m_lc.log(cta::log::DEBUG, "In RecallReportPacker::WorkerThread::run(): all disk threads are finished, "
                                         "telling the mount that Disk threads are complete");
    ...
    cta::log::ScopedParamContainer params(m_parent.m_lc);
    params.add("exceptionWhat", e.what()).add("exceptionType", typeid(e).name());
    m_parent.m_lc.log(cta::log::ERR, "In RecallReportPacker::WorkerThread::run(): Received a standard exception while "
                                     "reporting retrieve mount results.");
    if (m_parent.m_watchdog) {
      m_parent.m_watchdog->addToErrorCount("Error_reporting");
      m_parent.m_watchdog->addParameter(cta::log::Param("status", "failure"));
    }
    ...
    m_parent.m_lc.log(cta::log::ERR, "In RecallReportPacker::WorkerThread::run(): Received an unknown exception while "
                                     "reporting retrieve mount results.");
    if (m_parent.m_watchdog) {
      m_parent.m_watchdog->addToErrorCount("Error_reporting");
      m_parent.m_watchdog->addParameter(cta::log::Param("status", "failure"));
