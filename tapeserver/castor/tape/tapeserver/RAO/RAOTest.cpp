...
public:
  ...
  static std::unique_ptr<cta::RetrieveJob> createRetrieveJobForRAOTests(const uint64_t blockId,
...
TEST_F(RAOTest, InterpolationFilePositionEstimatorWrap0Test) {
  ...
  rao::InterpolationFilePositionEstimator estimator(eowPositions, mediaType);
  {
    std::unique_ptr<cta::RetrieveJob> retrieveJob =
      RAOTestEnvironment::createRetrieveJobForRAOTests(0, 1, 1, 100000000);
    ...
    uint64_t endPositionBlockId =
      jobTapeFile.blockId + (jobTapeFile.fileSize / rao::InterpolationFilePositionEstimator::c_blockSize) + 1;
    uint64_t expectedEndPositionLPos =
      mediaType.minLPos.value() +
      endPositionBlockId * (mediaType.maxLPos.value() - mediaType.minLPos.value()) / eowPositions.at(0).blockId;
...
TEST_F(RAOTest, InterpolationFilePositionEstimatorWrap1Test) {
...

  {
    ...
    std::unique_ptr<cta::RetrieveJob> retrieveJob =
      RAOTestEnvironment::createRetrieveJobForRAOTests(210000, 1, 1, 100000000);
    rao::FilePositionInfos positionFile = estimator.getFilePosition(*retrieveJob);
    ...
    uint64_t expectedLPos =
      mediaType.maxLPos.value() - fileBlockId * (mediaType.maxLPos.value() - mediaType.minLPos.value()) / b_max;
    ...
    uint64_t endPositionBlockId = jobTapeFile.blockId +
                                  (jobTapeFile.fileSize / rao::InterpolationFilePositionEstimator::c_blockSize) + 1 -
                                  eowPositions.at(0).blockId;
    uint64_t expectedEndPositionLPos =
      mediaType.maxLPos.value() - endPositionBlockId * (mediaType.maxLPos.value() - mediaType.minLPos.value()) / b_max;
    ASSERT_EQ(expectedEndPositionLPos, endPositionFile.getLPos());
...
TEST_F(RAOTest, InterpolationFilePositionEstimatorThrowsExceptionIfBlockIdGreaterThanLastEOWP) {
  ...
  {
    std::unique_ptr<cta::RetrieveJob> retrieveJob =
      RAOTestEnvironment::createRetrieveJobForRAOTests(100000000, 1, 3, 30);
...
TEST_F(RAOTest, RAOSLTFAlgorithm) {
  ...
  filePositionEstimator.reset(new rao::InterpolationFilePositionEstimator(
    RAOTestEnvironment::getLTO7MEndOfWrapPositions(), RAOTestEnvironment::getLTO7MMediaType()));
  costHeuristic.reset(new rao::CTACostHeuristic());
  std::unique_ptr<rao::SLTFRAOAlgorithm> sltfRAOAlgorithm =
    std::make_unique<rao::SLTFRAOAlgorithm>(filePositionEstimator, costHeuristic);
