...
m_endOfWrapPositions(endOfWrapPositions),
...
FilePositionInfos InterpolationFilePositionEstimator::getFilePosition(const cta::RetrieveJob& job) const {
  ...
  ret.setBeginningLandingZone(RAOHelpers::determineLandingZone(m_mediaType.minLPos.value(), m_mediaType.maxLPos.value(),
                                                               beginningPosition.getLPos()));
  ret.setEndLandingZone(
    RAOHelpers::determineLandingZone(m_mediaType.minLPos.value(), m_mediaType.maxLPos.value(), endPosition.getLPos()));
  return ret;
}
...
uint64_t InterpolationFilePositionEstimator::determineWrapNb(const uint64_t blockId) const {
  if (m_endOfWrapPositions.empty()) {
    std::string errorMsg =
      "In InterpolationFilePositionEstimator::determineWrapNb(), unable to find the wrap number of the blockId " +
      std::to_string(blockId) + " because no EOWP informations have been found.";
  ...
  if (eowpItor == m_endOfWrapPositions.end()) {
    eowpItor--;
    std::string errorMsg = "In InterpolationFilePositionEstimator::determineWrapNb(), the blockId " +
                           std::to_string(blockId) + " is greater than the last wrap EOWP blockId (" +
                           std::to_string(eowpItor->blockId) + ")";
    throw cta::exception::Exception(errorMsg);
  }
...
uint64_t
  InterpolationFilePositionEstimator::determineEndBlockId(const cta::common::dataStructures::TapeFile& file) const {
...
void InterpolationFilePositionEstimator::checkMediaTypeConsistency() {
  if (!m_mediaType.minLPos || !m_mediaType.maxLPos) {
    std::string errorMsg = "In InterpolationFilePositionEstimator::checkMediaTypeConsistency(), the media type (" +
                           m_mediaType.name +
                           ") associated to the tape does not give informations about the minLPos and maxLPos.";
    throw cta::exception::Exception(errorMsg);
  }
  if (!m_mediaType.nbWraps) {
    std::string errorMsg =
      "In InterpolationFilePositionEstimator::checkMediaTypeConsistency(), the media type (" + m_mediaType.name +
      ") associated to the tape mounted does not give informations about the number of wraps the media contains.";
    throw cta::exception::Exception(errorMsg);
  }
}
