...
void RAOHelpers::improveEndOfLastWrapPositionIfPossible(std::vector<drive::endOfWrapPosition>& endOfWrapPositions) {
  ...
  endOfWrapPositions[nbEndOfWrapPositions - 1].blockId =
    endOfWrapPositions.at(nbEndOfWrapPositions - 2).blockId + meanNbBlocksPerWrap;
}
...
uint8_t RAOHelpers::determineBand(uint32_t nbWrapsOnTape, uint32_t wrapNumber) {
  ...
  if (wrapNumber > nbWrapsOnTape - 1) {
    std::string errorMsg = "In RAOHelpers::determineBand(), the wrapNumber (" + std::to_string(wrapNumber) +
                           ") of the file is greater than the number of wraps the tape contains (" +
                           std::to_string(nbWrapsOnTape) + ").";
...
bool RAOHelpers::doesWrapChange(const FilePositionInfos& file1PositionInfos,
                                const FilePositionInfos& file2PositionInfos) {
  return (file1PositionInfos.getEndPosition().getWrap() != file2PositionInfos.getBeginningPosition().getWrap());
}

bool RAOHelpers::doesBandChange(const FilePositionInfos& file1PositionInfos,
                                const FilePositionInfos& file2PositionInfos) {
  return (file1PositionInfos.getEndBand() != file2PositionInfos.getBeginningBand());
}

bool RAOHelpers::doesLandingZoneChange(const FilePositionInfos& file1PositionInfos,
                                       const FilePositionInfos& file2PositionInfos) {
  return (file1PositionInfos.getEndLandingZone() != file2PositionInfos.getBeginningLandingZone());
}

bool RAOHelpers::doesDirectionChange(const FilePositionInfos& file1PositionInfos,
                                     const FilePositionInfos& file2PositionInfos) {
  return ((file1PositionInfos.getEndPosition().getWrap() % 2) !=
          (file2PositionInfos.getBeginningPosition().getWrap() % 2));
}

bool RAOHelpers::doesStepBack(const FilePositionInfos& file1PositionInfos,
                              const FilePositionInfos& file2PositionInfos) {
...
uint64_t RAOHelpers::computeLongitudinalDistance(const FilePositionInfos& file1PositionInfos,
                                                 const FilePositionInfos& file2PositionInfos) {
  ...
  return endOfFile1Lpos > beginningOfFile2Lpos ? endOfFile1Lpos - beginningOfFile2Lpos :
