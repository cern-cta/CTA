...
SLTFRAOAlgorithm::SLTFRAOAlgorithm(std::unique_ptr<FilePositionEstimator>& filePositionEstimator,
  ...
  switch (filePositionType) {
    case RAOOptions::FilePositionEstimatorType::interpolation: {
      if (m_catalogue != nullptr && m_drive != nullptr) {
        m_algorithm->m_filePositionEstimator = FilePositionEstimatorFactory::createInterpolationFilePositionEstimator(
          m_raoParams.getMountedVid(), m_catalogue, m_drive, m_algorithm->m_raoTimings);
      }
      else {
    ...
    default:
      std::string errorMsg = "In SLTFRAOAlgorithm::Builder::initializeFilePositionEstimator() unable to instanciate an "
                             "estimator to estimate the position of the files on tape "
                             "because the type given in parameter is unknown (" +
                             std::to_string(filePositionType) + ").";
