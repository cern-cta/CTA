...
void drive::DriveGeneric::setLogicalBlockProtection(const unsigned char method,
  ...
  SCSI::Structures::modeSenseControlDataProtection_t controlDataProtection;
  {
    ...
    SCSI::ExceptionLauncher(sgh, std::string("SCSI error fetching data in setLogicalBlockProtection: ") +
...

  {
    ...
    /* Manage both system error and SCSI errors. */
    cta::exception::Errnum::throwOnMinusOne(m_sysWrapper.ioctl(m_tapeFD, SG_IO, &sgh), "Failed SG_IO ioctl");
    SCSI::ExceptionLauncher(sgh, std::string("SCSI error setting data in setDataProtection : ") +
                                   SCSI::statusToString(sgh.status));
  }
}
...
void drive::DriveGeneric::enableCRC32CLogicalBlockProtectionReadOnly() {
  ...
  setLogicalBlockProtection(SCSI::logicBlockProtectionMethod::CRC32C, SCSI::logicBlockProtectionMethod::CRC32CLength,
                            true, false);
}
...
void drive::DriveGeneric::receiveRAO(std::list<SCSI::Structures::RAO::blockLims>& files) {
  ...
  while (files_no > 0) {
    ...
    strcpy(reinterpret_cast<char*>(bl.fseq), reinterpret_cast<const char*>(ud->udsName));
...
void drive::DriveGeneric::writeBlock(const void* data, size_t count) {
  switch (m_lbpToUse) {
    case lbpToUse::crc32cReadWrite: {
      uint8_t* dataWithCrc32c = (new (std::nothrow) uint8_t[count + SCSI::logicBlockProtectionMethod::CRC32CLength]);
      ...
      const size_t countWithCrc32c =
        cta::addCrc32cToMemoryBlock(SCSI::logicBlockProtectionMethod::CRC32CSeed, count, dataWithCrc32c);
      if (countWithCrc32c != count + SCSI::logicBlockProtectionMethod::CRC32CLength) {
        delete[] dataWithCrc32c;
        cta::exception::Errnum::throwOnMinusOne(-1, "Failed in DriveGeneric::writeBlock: incorrect length for block"
                                                    " with crc32c");
      }
      if (-1 == m_sysWrapper.write(m_tapeFD, dataWithCrc32c, countWithCrc32c)) {
        delete[] dataWithCrc32c;
...
ssize_t drive::DriveGeneric::readBlock(void* data, size_t count) {
  switch (m_lbpToUse) {
    ...
    case lbpToUse::crc32cReadOnly: {
      ...
      const ssize_t res =
        m_sysWrapper.read(m_tapeFD, dataWithCrc32c, count + SCSI::logicBlockProtectionMethod::CRC32CLength);
      if (-1 == res) {
        delete[] dataWithCrc32c;
      ...
      const size_t dataLenWithoutCrc32c = res - SCSI::logicBlockProtectionMethod::CRC32CLength;
      ...
      if (cta::verifyCrc32cForMemoryBlockWithCrc32c(SCSI::logicBlockProtectionMethod::CRC32CSeed, res,
                                                    dataWithCrc32c)) {
  ...
  switch (m_lbpToUse) {
    ...
    case lbpToUse::crc32cReadOnly: {
      ...
      if (-1 == res && ENOMEM == errno) {
        delete[] dataWithCrc32c;
        throw cta::exception::Errnum(errno, context + ": Failed ST read in DriveGeneric::readExactBlock. Tape volume "
                                                      "label size not be in the CTA/CASTOR format.");
      }
      // Generic handling of other errors
      if (-1 == res) {
        delete[] dataWithCrc32c;
        cta::exception::Errnum::throwOnMinusOne(res, context +
