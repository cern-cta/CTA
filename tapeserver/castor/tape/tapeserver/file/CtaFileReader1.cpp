...
void CtaFileReader1::positionByFseq(const cta::RetrieveJob& fileToRecall) {
...
void CtaFileReader1::positionByBlockID(const cta::RetrieveJob& fileToRecall) {
  // Make sure the session state is advanced to cover our failures
  // and allow next call to position to discover we failed half way
  m_session.setCurrentFilePart(PartOfFile::HeaderProcessing);
...
void CtaFileReader1::moveToFirstHeaderBlock() {
...
void CtaFileReader1::checkTrailers() {
...
size_t CtaFileReader1::readNextDataBlock(void* data, const size_t size) {
...
void CtaFileReader1::moveReaderByFSeqDelta(const int64_t fSeq_delta) {
  if (fSeq_delta == 0) {
...
void CtaFileReader1::useBlockID(const cta::RetrieveJob& fileToRecall) {
...
void CtaFileReader1::setBlockSize(const UHL1& uhl1) {
...
void CtaFileReader1::checkHeaders(const cta::RetrieveJob& fileToRecall) {
