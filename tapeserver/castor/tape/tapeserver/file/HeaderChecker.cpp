...
void HeaderChecker::checkOSM(const osm::LABEL& osmLabel, const std::string& volId) {
  if (osmLabel.name().compare(volId) != 0) {
    ...
    ex_str << "[OsmReadSession::OsmReadSession()] - VSN of tape (" << osmLabel.name() << ") is not the one requested ("
           << volId << ")";
    throw TapeFormatError(ex_str.str());
  }
}
...
void HeaderChecker::checkHDR1(const HDR1& hdr1,
  ...
  if (!checkHeaderNumericalField(hdr1.getFileId(), filetoRecall.retrieveRequest.archiveFileID,
                                 HeaderBase::hexadecimal)) {
    // the nsfileid stored in HDR1 as an hexadecimal string . The one in
    // filetoRecall is numeric
    std::ostringstream ex_str;
    ex_str << "[HeaderChecker::checkHDR1] - Invalid fileid detected: (0x)\"" << hdr1.getFileId() << "\". Wanted: 0x"
           << std::hex << filetoRecall.retrieveRequest.archiveFileID << std::endl;
    throw TapeFormatError(ex_str.str());
  }
  ...
  if (hdr1.getVSN().compare(volId)) {
    std::ostringstream ex_str;
    ex_str << "[HeaderChecker::checkHDR1] - Wrong volume ID info found in hdr1: " << hdr1.getVSN()
           << ". Wanted: " << volId;
    throw TapeFormatError(ex_str.str());
  }
}

void HeaderChecker::checkUHL1(const UHL1& uhl1, const cta::RetrieveJob& fileToRecall) {
  if (!checkHeaderNumericalField(uhl1.getfSeq(), fileToRecall.selectedTapeFile().fSeq, HeaderBase::decimal)) {
    std::ostringstream ex_str;
    ex_str << "[HeaderChecker::checkUHL1] - Invalid fseq detected in uhl1: \"" << uhl1.getfSeq()
           << "\". Wanted: " << fileToRecall.selectedTapeFile().fSeq;
    throw TapeFormatError(ex_str.str());
  }
}
...
void HeaderChecker::checkUTL1(const UTL1& utl1, const uint32_t fSeq) {
  if (!checkHeaderNumericalField(utl1.getfSeq(), static_cast<uint64_t>(fSeq), HeaderBase::decimal)) {
    std::ostringstream ex_str;
    ex_str << "[HeaderChecker::checkUTL1] - Invalid fseq detected in utl1: \"" << utl1.getfSeq()
           << "\". Wanted: " << fSeq;
    throw TapeFormatError(ex_str.str());
  }
}
