...
size_t castor::tape::tapeFile::CPIO::decode(const uint8_t* puiData, const size_t uiSize) {
  if (uiSize < (CPIO::HEADER + CPIO::PATHLEN)) {
    ...
    ex_str << "[CPIO::decode] - Invalid data block size: " << uiSize << " "
           << "the data block size is smaller then CPIO header";
  ...
  */
  ...
  if (puiData[65] == 'H') {
    ...
    sscanf(reinterpret_cast<const char*>(puiData), strFormat.str().c_str(), &m_strMagic[0], &m_uiDev, &m_uiIno,
           &m_uiMode, &m_uiUid, &m_uiGid, &m_uiNlink, &m_uiRdev, reinterpret_cast<uint64_t*>(&m_ulMtime), &m_uiNameSize,
           &m_ui64FileSize, &m_strFid[0]);
  }
  else {
