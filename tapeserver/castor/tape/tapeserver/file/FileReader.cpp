...
void FileReader::positionByFseq(const cta::RetrieveJob &fileToRecall) {
  ...
  if (fileToRecall.selectedTapeFile().fSeq == 1) {
    ...
    m_session->m_drive.spaceFileMarksBackwards(static_cast<uint32_t>(abs(fSeq_delta)) * 3 + 1);
    m_session->m_drive.readFileMark(
      "[FileReader::position] Reading file mark right before the header of the file we want to read");
  }
}

void FileReader::positionByBlockID(const cta::RetrieveJob &fileToRecall) {
...
void FileReader::setBlockSize(const UHL1 &uhl1)  {
...
size_t FileReader::read(void *data, const size_t size)  {
