...
bool Tape::operator==(const Tape& rhs) const {
  return vid == rhs.vid && lastFSeq == rhs.lastFSeq && logicalLibraryName == rhs.logicalLibraryName
         && tapePoolName == rhs.tapePoolName && capacityInBytes == rhs.capacityInBytes
         && dataOnTapeInBytes == rhs.dataOnTapeInBytes && encryptionKeyName == rhs.encryptionKeyName
         && purchaseOrder == rhs.purchaseOrder && full == rhs.full && state == rhs.state
         && creationLog == rhs.creationLog && lastModificationLog == rhs.lastModificationLog && comment == rhs.comment
         && labelLog == rhs.labelLog && lastWriteLog == rhs.lastWriteLog && lastReadLog == rhs.lastReadLog;
}
...
Tape::State Tape::stringToState(const std::string& stateStr, bool hidePendingStates) {
  ...
  try {
    ...
    throw cta::exception::UserError(std::string("The state given (") + stateUpperCase
                                    + ") does not exist. Possible values are "
                                    + Tape::getAllPossibleStates(hidePendingStates));
  }
}
...
std::ostream& operator<<(std::ostream& os, const Tape& obj) {
  ...
  os << "(vid=" << obj.vid << " lastFSeq=" << obj.lastFSeq << " logicalLibraryName=" << obj.logicalLibraryName
     << " tapePoolName=" << obj.tapePoolName << " capacityInBytes=" << obj.capacityInBytes
     << " dataOnTapeInBytes=" << obj.dataOnTapeInBytes
     << " encryptionKeyName=" << (obj.encryptionKeyName ? obj.encryptionKeyName.value() : "null")
     << " full=" << obj.full << " creationLog=" << obj.creationLog << " lastModificationLog=" << obj.lastModificationLog
     << " comment=" << obj.comment << " state=" << stateStr
     << " stateReason=" << (obj.stateReason ? obj.stateReason.value() : "null")
     << " stateUpdateTime=" << obj.stateUpdateTime << " stateModifiedBy=" << obj.stateModifiedBy
     << " labelLog=" << obj.labelLog << " lastWriteLog=" << obj.lastWriteLog << " lastReadLog=" << obj.lastReadLog
     << ")";
  return os;
}
