...
RuntimeDir::RuntimeDir() : m_dirPath("/run/cta/" + std::to_string(getpid())) {
  ...
  std::error_code ec;
  std::filesystem::create_directories(m_dirPath, ec);
  if (ec) {
    throw exception::Exception("Failed to create runtime directory '" + m_dirPath + "': " + ec.message());
  }
...
void RuntimeDir::restrictDirPermissions(const std::string& path) const {
  ...
  std::filesystem::permissions(path,
                               ...
                               ec);
  if (ec) {
    throw exception::Exception("Failed to set runtime dir permissions for '" + m_dirPath + "': " + ec.message());
  }
}
...
void RuntimeDir::deleteRuntimeDir() const {
  ...
  const auto ss = std::filesystem::symlink_status(m_dirPath);
  if (std::filesystem::is_symlink(ss)) {
