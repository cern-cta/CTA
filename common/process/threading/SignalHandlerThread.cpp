...
// Constructor
//------------------------------------------------------------------------------
cta::threading::SignalHandlerThread::SignalHandlerThread(const cta::log::LogContext& logContext)
    : m_sigFd(-1),
      m_logContext(logContext) {
  // Main thread should have already blocked all signals. 
  //if () {
  //  throw;
  //}
  // So lets read them all.
