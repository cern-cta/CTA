...
TEST_F(LoggerParamTypeTest, TestParams_int64_t) {
  ...
  for (auto& [param, expected_val] : params_results) {
    ASSERT_TRUE(std::holds_alternative<int64_t>(param.getValueVariant().value()))
      << "Failed testing Param type of '" << param.getName() << "'";
    ASSERT_EQ(std::get<int64_t>(param.getValueVariant().value()), expected_val)
      << "Failed testing Param value of '" << param.getName() << "'";
  }
}
...
TEST_F(LoggerParamTypeTest, TestParams_uint64_t) {
  ...
  for (auto& [param, expected_val] : params_results) {
    ASSERT_TRUE(std::holds_alternative<uint64_t>(param.getValueVariant().value()))
      << "Failed testing Param type of '" << param.getName() << "'";
    ASSERT_EQ(std::get<uint64_t>(param.getValueVariant().value()), expected_val)
      << "Failed testing Param value of '" << param.getName() << "'";
  }
}
...
TEST_F(LoggerParamTypeTest, TestParams_float) {
  ...
  for (auto& [param, expected_val] : params_results) {
    ASSERT_TRUE(std::holds_alternative<float>(param.getValueVariant().value()))
      << "Failed testing Param type of '" << param.getName() << "'";
    ASSERT_FLOAT_EQ(std::get<float>(param.getValueVariant().value()), expected_val)
      << "Failed testing Param value of '" << param.getName() << "'";
  }
}
...
TEST_F(LoggerParamTypeTest, TestParams_double) {
  ...
  for (auto& [param, expected_val] : params_results) {
    ASSERT_TRUE(std::holds_alternative<double>(param.getValueVariant().value()))
      << "Failed testing Param type of '" << param.getName() << "'";
    ASSERT_DOUBLE_EQ(std::get<double>(param.getValueVariant().value()), expected_val)
      << "Failed testing Param value of '" << param.getName() << "'";
  }
}
...
TEST_F(LoggerParamTypeTest, TestParams_bool) {
  ...
  for (auto& [param, expected_val] : params_results) {
    ASSERT_TRUE(std::holds_alternative<bool>(param.getValueVariant().value()))
      << "Failed testing Param type of '" << param.getName() << "'";
    ASSERT_EQ(std::get<bool>(param.getValueVariant().value()), expected_val)
      << "Failed testing Param value of '" << param.getName() << "'";
  }
}
...
TEST_F(LoggerParamTypeTest, TestParams_string) {
  ...
  // Test optional values too
  params_results.push_back(
    {Param("std::optional<std::string>", std::optional<std::string>("optional_str")), "optional_str"});

  for (auto& [param, expected_val] : params_results) {
    ASSERT_TRUE(std::holds_alternative<std::string>(param.getValueVariant().value()))
      << "Failed testing Param type of '" << param.getName() << "'";
    ASSERT_EQ(std::get<std::string>(param.getValueVariant().value()), expected_val)
      << "Failed testing Param value of '" << param.getName() << "'";
  }
}
...
TEST_F(LoggerParamTypeTest, TestParams_nullopt) {
  ...
  for (auto& param : params) {
    ASSERT_FALSE(param.getValueVariant().has_value())
      << "Failed testing Param type of empty '" << param.getName() << "'";
  }
}
