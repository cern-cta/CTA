...
void RWLock::rdlock()  {
  ...
  if(0 != rc) {
    throw exception::Exception("Failed to take read lock on underlying pthread read-write lock: " + utils::errnoToString(rc));
  }
}
...
void RWLock::wrlock()  {
  ...
  if(0 != rc) {
    throw exception::Exception("Failed to take write lock on underlying pthread read-write lock: " + utils::errnoToString(rc));
  }
}
