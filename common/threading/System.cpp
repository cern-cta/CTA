...
void cta::System::setUserAndGroup(const std::string &userName, const std::string &groupName) {
  ...
  int getpwnamRet = getpwnam_r(userName.c_str(), &pwd_buf, buffer, sizeof(buffer), &pwd);
  ...
  int getgrgidRet = getgrgid_r(pwd->pw_gid, &grp_buf, buffer, sizeof(buffer), &grp);
  ...
  int getgrnam = getgrnam_r(groupName.c_str(), &grp_buf, buffer, sizeof(buffer), &grp);
