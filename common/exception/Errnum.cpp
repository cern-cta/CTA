...
Errnum::Errnum(int err, std::string_view what) : Exception(), m_errnum(err), m_strerror(utils::errnoToString(err)) {
  std::stringstream whatWithErrnum;
  whatWithErrnum << what << (what.empty() ? "" : " ") << "Errno=" << m_errnum << ": " << m_strerror;
