...
TEST(cta_exceptions, Errnum_throwers) {
  ...
  ASSERT_THROW(cta::exception::Errnum::throwOnReturnedErrnoOrThrownStdException(
                 []() {
                   throw std::error_code(ENOSPC, std::system_category());
                   return 0;
                 },
                 "Context"),
               cta::exception::Errnum);
  ...
  ASSERT_THROW(cta::exception::Errnum::throwOnNegative(-1, "Context"), cta::exception::Errnum);
  ...
  ASSERT_THROW(cta::exception::Errnum::throwOnZero(0, "Context"), cta::exception::Errnum);
}
}  // namespace unitTests
