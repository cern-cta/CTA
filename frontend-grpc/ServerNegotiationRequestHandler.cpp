...
void cta::frontend::grpc::server::NegotiationRequestHandler::acquireCreds(const std::string& strService,
  ...
  gssMajStat = gss_acquire_cred(&gssMinStat, gssServerName, 0, gssMechs, GSS_C_ACCEPT, pGssServerCreds, NULL, NULL);
...
bool cta::frontend::grpc::server::NegotiationRequestHandler::next(const bool bOk) {
  ...
  if (!bOk) {
    switch (m_streamState) {
      case StreamState::PROCESSING: {
        log::ScopedParamContainer params(lc);
        params.add("tag", m_tag);
        lc.log(cta::log::ERR, "In grpc::server::NegotiationRequestHandler::next(): Server has been shut down before "
                              "receiving a matching request.");
      } break;
      default: {
        log::ScopedParamContainer params(lc);
        params.add("tag", m_tag);
        lc.log(cta::log::ERR, "In grpc::server::NegotiationRequestHandler::next(): Request processing aborted: call is "
  ...
  switch (m_streamState) {
    case StreamState::NEW:
      m_ctaNegotiationSvc.RequestNegotiate(&m_ctx, &m_rwNegotiation, &m_asyncServer.completionQueue(),
                                           &m_asyncServer.completionQueue(), m_tag);
      m_streamState = StreamState::PROCESSING;
      break;
    case StreamState::PROCESSING:
      m_asyncServer
        .registerHandler<cta::frontend::grpc::server::NegotiationRequestHandler>(m_ctaNegotiationSvc, m_strKeytab,
                                                                                 m_strService)
        .next(bOk);
    ...
    case StreamState::WRITE: {
      ...
      gssMajStat =
        gss_accept_sec_context(&gssAccSecMinStat, &m_gssCtx, m_serverCreds, &gssRecvToken, GSS_C_NO_CHANNEL_BINDINGS,
                               &gssSrcName, &gssOidMechType, &gssSendToken, &gssRetFlags, NULL, /* time_rec */
      ...
      switch (gssMajStat) {
        ...
        case GSS_S_COMPLETE:
          ...
          m_asyncServer.tokenStorage().store(
