...
cta::frontend::grpc::client::CtaAdminGrpcCmd::CtaAdminGrpcCmd(int argc, char** argv) : m_strExecname(argv[0]) {
  ...
  if (std::string(argv[argno]) == "--json") {
    CtaAdminGrpcCmd::m_abIsJson = true;
    ++argno;
  }
...
void cta::frontend::grpc::client::CtaAdminGrpcCmd::exe(cta::log::Logger& log,
  ...
  std::shared_ptr<::grpc::ChannelCredentials> spChannelCreds =
    ::grpc::SslCredentials({strSslRoot, strSslKey, strSslCert});
  ...
  try {
     ...
     */
    lc.log(cta::log::CRIT,
           "In cta::frontend::grpc::client::CtaAdminGrpcCmd::exe(): Problem with the negotiation service.");
    throw;  // rethrow
  }
  /*
  ...
  switch (cmd_pair(m_request.admincmd().cmd(), m_request.admincmd().subcmd())) {
    case cmd_pair(cta::admin::AdminCmd::CMD_TAPE, cta::admin::AdminCmd::SUBCMD_LS):
      try {
         ...
         */
        std::shared_ptr<::grpc::CallCredentials> spCallCredentials = ::grpc::MetadataCredentialsFromPlugin(
          std::unique_ptr<::grpc::MetadataCredentialsPlugin>(new KerberosAuthenticator(strEncodedToken)));
        std::shared_ptr<::grpc::ChannelCredentials> spCompositeCredentials =
          ::grpc::CompositeChannelCredentials(spChannelCreds, spCallCredentials);
        ...
        lc.log(
          cta::log::CRIT,
          "In cta::frontend::grpc::client::CtaAdminGrpcCmd::exe(): Problem with the execution of the tape ls command.");
        throw;  // rethrow
      }
      break;
    default:
      throw XrdSsiPb::PbException("Admin command pair <" + cta::admin::AdminCmd_Cmd_Name(m_request.admincmd().cmd()) +
                                  ", " + cta::admin::AdminCmd_SubCmd_Name(m_request.admincmd().subcmd()) +
                                  "> is not implemented.");
  }
}

void cta::frontend::grpc::client::CtaAdminGrpcCmd::parseOptions(int start,
                                                                int argc,
                                                                char** argv,
                                                                const cta::admin::cmd_val_t& options) {
  for (int i = start; i < argc; ++i) {
    ...
    for (opt_it = options.begin(); opt_it != options.end(); ++opt_it) {
      ...
      if (opt_num-- == 0 && opt_it->get_type() == cta::admin::Option::OPT_CMD) {
        break;
      }
...
void cta::frontend::grpc::client::CtaAdminGrpcCmd::addOption(const cta::admin::Option& option,
  ...
  switch (option.get_type()) {
    ...
    case cta::admin::Option::OPT_UINT:
      try {
...
void cta::frontend::grpc::client::CtaAdminGrpcCmd::readListFromFile(cta::admin::OptionStrList& str_list,
...
void cta::frontend::grpc::client::CtaAdminGrpcCmd::throwUsage(const std::string& strError) const {
  ...
  if (admincmd == cta::admin::AdminCmd::CMD_NONE) {
    ...
    help << "CTA Administration Tool" << std::endl
         << std::endl
         << "Usage: " << m_strExecname << " [--json] <command> [<subcommand> [<option>...]]" << std::endl
         << "       " << m_strExecname << " <command> help" << std::endl
         << std::endl
         ...
         << std::endl
         << "detailed help on the options of each subcommand, type: " << m_strExecname << " <command> help" << std::endl
         << std::endl;
...
int main(const int argc, char** argv) {
  ...
  while ((c = getopt_long(vArgvSSLgRPC.size(), vArgvSSLgRPC.data(), ":r:k:c:s:p:h", longOpt, &iOptIdx)) != -1) {
    switch (c) {
      ...
      case 'p':
        ...
        break;
      case 'h':
        std::cout << "CTA Admin gRPC" << std::endl
                  << std::endl
                  ...
                  << "  -s, --host <STRING>  gRPC server host name/adress"
                  << std::endl
                  // << "  -p, --port<NUMBER>  gRPC server TCP port (default: 17017)"  << std::endl
                  << "  -p, --port <NUMBER>  gRPC server TCP port"
                  << std::endl
                  // << "  -v, --version           print version and exit" << std::endl
                  << "  -h, --help          print this help and exit" << std::endl;
        return EXIT_SUCCESS;
      case ':':  // Missing parameter
        std::cerr << strExecName << ": command line error: option: '-" << (char) optopt << "' requires an argument"
                  << std::endl
  ...
  std::unique_ptr<cta::log::Logger> upLog =
    std::make_unique<cta::log::StdoutLogger>(strShortHostname, "grpc-streamserver");
