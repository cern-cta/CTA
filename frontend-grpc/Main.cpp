...
int main(const int argc, char* const* const argv) {
  ...
  auto catalogueFactory =
    catalogue::CatalogueFactoryFactory::create(logger, catalogueLogin, 10, nbArchiveFileListingConns);
  auto catalogue = catalogueFactory->create();
  try {
    catalogue->Schema()->ping();
    lc.log(log::INFO,
           "Connected to catalog " + catalogue->Schema()->getSchemaVersion().getSchemaVersion<std::string>());
  }
