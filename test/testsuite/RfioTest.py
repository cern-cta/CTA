...
class RfioPreRequisitesCase(unittest.TestCase):
    def mainScenariumSetUp(self):
        ...
        try:
            # get needed params
            global castorDir, inputFile, localDir, stageMap, castorConf, castorEnv, rfioRemoteDirOk
            ...
            myOut=UtilityForCastorTest.runOnShell(cmd,myScen)[0]
            rfioRemoteDirOk=1
            if myOut.find("Permission denied") != -1:
...
class RfioRfcpSimpleCase(unittest.TestCase):
    def localToLocal(self):
        ...
        assert re.search('through local \(in\) and local \(out\)', buffOut) != None, "rfio doesn't work local to local"
    ...
    def remoteToLocal(self):
        ...
        assert re.search('through eth[0-1] \(in\) and local \(out\)', buffOut) != None, "rfio doesn't work from remote to local"
    ...
    def remoteToRemote(self):
        ...
        assert re.search('through eth[0-1] \(in\) and eth[0-1] \(out\)',buffOut) != None, "rfio doesn't work from remote to remote"
...
class RfioOtherCmdSimpleCase(unittest.TestCase):
    ...
    def remoteRfcat(self):
        ...
        if not rfioRemoteDirOk:
            assert re.search('Permission denied', buffOut) != None, "rfcat is not working for remote files, expected Permission Denied"
            return

        fi=open(inputFile,"r")
        buffOut2=fi.read()
        fi.close()
        assert buffOut2.find(buffOut) != -1, "rfcat is not working for remote files"
    ...
    def remoteRfchmod(self):
        ...
        if rfioRemoteDirOk:
            ...
            assert re.search('Permission denied', buffOut) != None, "rfchmod is not working for remote files, expected Permission Denied"
            return
    ...
    def remoteRfdir(self):
        ...
        if rfioRemoteDirOk:
            ...
            assert re.search('Permission denied', buffOut) != None, "rfdir is not working for remote files, expected Permission Denied"
    ...
    def remoteRfrename(self):
        ...
        if rfioRemoteDirOk:
            assert buffOut.find("No such file or directory")!= -1, "rfrename doesn't work with remote files"
        else:
            assert re.search('Permission denied', buffOut) != None, "rfrename is not working for remote files, expected Permission Denied"
            return
    ...
    def remoteRfrm(self):
        ...
        if rfioRemoteDirOk:
            ...
            assert re.search('Permission denied', buffOut) != None, "rfrm is not working for remote files, expected Permission Denied"
            return
    ...
    def remoteRfstat(self):
        ...
        if rfioRemoteDirOk:
            ...
            assert re.search('Permission denied', buffOut) != None, "rfstat is not working for remote files, expected Permission Denied"
...
class RfioRfcpCastor(unittest.TestCase):
    def localToCastor(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioLocalToCastor"+myTag+ticket]
    ...
    def castorToLocal(self):
        ...
        assert UtilityForCastorTest.checkRfcpOutput(buffOut, 1) or buffOut.rfind(" bytes ready for migration") != -1, "rfio doesn't work from castor to local"
    ...
    def remoteToCastor(self):
        cmd=["rfcp "+inputFile+" "+remoteDir+"fileRfioRemoteToCastor"+myTag+ticket,"rfcp "+remoteDir+"fileRfioRemoteToCastor"+myTag+ticket+" "+self.protocol+myTurl+"fileRfioRemoteToCastorCopy"+myTag+ticket,"rfcp "+self.protocol+myTurl+"fileRfioRemoteToCastorCopy"+myTag+ticket+" "+localDir+"fileRfioRemoteToCastorCopyCopy"+myTag+ticket,"diff "+inputFile+" "+localDir+"fileRfioRemoteToCastorCopyCopy"+myTag+ticket]
        ...
        assert re.search('through eth[0-1] \(in\) and eth[0-1] \(out\)', buffOut) != None or buffOut.rfind(" bytes ready for migration") != -1, "rfio doesn't work from remote to castor"

        fi=open(localDir+"RfioRfcpRemoteToCastor"+myTag+"2","r")
        buffOut=fi.read()
        fi.close()
        assert UtilityForCastorTest.checkRfcpOutput(buffOut, 0) or buffOut.rfind(" bytes ready for migration") != -1, "rfio doesn't work from remote to castor"
    ...
    def castorToRemote(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioCastorToRemote"+myTag+ticket,"rfcp "+self.protocol+myTurl+"fileRfioCastorToRemote"+myTag+ticket+" "+remoteDir+"fileRfioCastorToRemoteCopy"+myTag+ticket,"rfcp "+remoteDir+"fileRfioCastorToRemoteCopy"+myTag+ticket+" "+localDir+"fileRfioCastorToRemoteCopyCopy"+myTag+ticket,"diff "+inputFile+" "+localDir+"fileRfioCastorToRemoteCopyCopy"+myTag+ticket]
        ...
        assert UtilityForCastorTest.checkRfcpOutput(buffOut, 1) or buffOut.rfind(" bytes ready for migration") != -1 , "rfio doesn't work from castor to remote"
        ...
        assert re.search('through eth[0-1] \(in\) and local \(out\)',buffOut) != None or buffOut.rfind(" bytes ready for migration") != -1 , "rfio doesn't work from castor to remote"
    ...
    def castorToCastor(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioCastorToCastor"+myTag+ticket,"rfcp "+self.protocol+myTurl+"fileRfioCastorToCastor"+myTag+ticket+" "+self.protocol+myTurl+"fileRfioCastorToCastorCopy"+myTag+ticket,"rfcp "+self.protocol+myTurl+"fileRfioCastorToCastorCopy"+myTag+ticket+" "+localDir+"fileRfioCastorToCastorCopyCopy"+myTag+ticket,"diff "+inputFile+" "+localDir+"fileRfioCastorToCastorCopyCopy"+myTag+ticket]
        ...
        assert re.search('through eth[0-1] \(in\) and eth[0-1] \(out\)',buffOut) != None or buffOut.rfind(" bytes ready for migration") != -1, "rfio doesn't work from castor to castor"

        fi=open(localDir+"RfioRfcpCastorToCastor"+myTag+"2","r")
        buffOut=fi.read()
        fi.close()
        assert UtilityForCastorTest.checkRfcpOutput(buffOut, 0) != None or buffOut.rfind(" bytes ready for migration") != -1, "rfio doesn't work from castor to castor"
...
class RfioOtherCmdCastor(unittest.TestCase):
    def castorRfcat(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioRfcatCastor"+myTag+ticket,"rfcat "+self.protocol+myTurl+"fileRfioRfcatCastor"+myTag+ticket]
    ...
    def castorRfchmod(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioRfchmodCastor"+myTag+ticket,"nsls -l "+castorDir+"fileRfioRfchmodCastor"+myTag+ticket,"rfchmod 777 "+self.protocol+myTurl+"fileRfioRfchmodCastor"+myTag+ticket,"nsls -l "+castorDir+"fileRfioRfchmodCastor"+myTag+ticket,"rfchmod 600 "+self.protocol+myTurl+"fileRfioRfchmodCastor"+myTag+ticket,"nsls -l "+castorDir+"fileRfioRfchmodCastor"+myTag+ticket]
    ...
    def castorRfdir(self):
        cmd=["rfdir "+self.protocol+myTurl,"nsls -l"+castorDir]
    ...
    def castorRfmkdir(self):
        cmd=["rfmkdir "+self.protocol+myTurl+"fileRfioCastorRfmkdir"+myTag+ticket,"nsls "+castorDir+"fileRfioCastorRfmkdir"+myTag+ticket]
    ...
    def castorRfrename(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioCastorRfrename"+myTag+ticket,"rfrename "+self.protocol+myTurl+"fileRfioCastorRfrename"+myTag+ticket+"  "+self.protocol+myTurl+"fileRfioCastorRfrename2"+myTag+ticket,"nsls  "+castorDir+"fileRfioCastorRfrename"+myTag+ticket,"nsls "+castorDir+"fileRfioCastorRfrename2"+myTag+ticket]
    ...
    def castorRfrm(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioCastorRfrm"+myTag+ticket,"nsls "+castorDir+"fileRfioCastorRfrm"+myTag+ticket,"rfrm "+self.protocol+myTurl+"fileRfioCastorRfrm"+myTag+ticket,"nsls "+castorDir+"fileRfioCastorRfrm"+myTag+ticket]
    ...
    def castorRfstat(self):
        cmd=["rfcp "+inputFile+" "+self.protocol+myTurl+"fileRfioCastorRfstat"+myTag+ticket,"rfstat "+self.protocol+myTurl+"fileRfioCastorRfstat"+myTag+ticket]
...
class RfioRfcpEnvSuite(unittest.TestSuite):
    def __init__(self):
        ...
        if testDefaultEnv:
            ...
            tests.append(RfioRfcpEnv4Suite())
            tests.append(RfioRfcpEnv5Suite())
        else:
            tests.append(RfioRfcpEnv4Suite())
            tests.append(RfioRfcpEnv5Suite())
...
class RfioOtherCmdEnvSuite(unittest.TestSuite):
    def __init__(self):
        ...
        if testDefaultEnv:
            ...
            tests.append(RfioOtherCmdEnv4Suite())
            tests.append(RfioOtherCmdEnv5Suite())
        else:
            tests.append(RfioOtherCmdEnv4Suite())
            tests.append(RfioOtherCmdEnv5Suite())
        unittest.TestSuite.__init__(self,tests)
