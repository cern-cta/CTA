...
int main(int argc, char **argv) {
  ...
  if(vmgr_querytape_byte_u64(vid, side, &vmgrTapeInfo, dgn)) {
    fprintf(stderr, "vmgr_querytape_byte_u64 failed: vid=\"%s\": %s\n", vid,
      sstrerror(serrno));
    exit(-1);
  }

  printf("vmgr_querytape_byte_u64 succeeded: vid=\"%s\" dgn=\"%s\"\n", vid,
    dgn);
