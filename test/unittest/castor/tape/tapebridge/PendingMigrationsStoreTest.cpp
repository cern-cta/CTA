...
public:
  ...
  void testMigrateToFSeqZero() {
    ...
    try {
      ...
      CPPUNIT_ASSERT_MESSAGE(
        "Checking errorMessage of session error is not empty",
        !error.getErrorMessage().empty());

      CPPUNIT_ASSERT_MESSAGE(
        "Checking errorMessage of session error is not UNKNOWN",
        error.getErrorMessage() != "UNKNOWN");

      CPPUNIT_ASSERT_MESSAGE(
        "Checking exception message contains error message",
        se.getMessage().str().find(error.getErrorMessage()) !=
          std::string::npos);
