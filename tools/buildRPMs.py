...
os.mkdir(workDir)
os.chdir(workDir)
os.popen('mkdir BUILD RPMS SOURCES SPECS SRPMS RPMS/i386 RPMS/x86_64')

# build RPMs
print 'Building RPMs for ' + targetOs + '/' + targetArch + ' ...'
rpmOutput = os.popen('RPM_BUILD_ROOT=`pwd` ORACLE_HOME=/afs/cern.ch/project/oracle/@sys/10203 LD_LIBRARY_PATH=$ORACLE_HOME/lib PATH=$ORACLE_HOME/bin:/usr/X11R6/bin:$PATH rpmbuild -ta ' + tarball)
m = false
...
if targetOs = 'SLC3' && targetArch = 'i386':
    ...
    # this is temporary needed until we get rid of GridFTPv1, which is only built for 32 bit  
    shutil.copyfile(rpmDir + '/castor-lib-' + fullVersion + '.rpm', intReleaseDir + '/SLC4/x86_64')
