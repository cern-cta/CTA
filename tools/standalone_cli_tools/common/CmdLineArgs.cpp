...
CmdLineArgs::CmdLineArgs(const int& argc, char* const* const& argv, const StandaloneCliTool& standaloneCliTool)
  ...
  while ((opt = getopt_long(argc, argv, shortopts[m_standaloneCliTool], longopts[m_standaloneCliTool], &opt_index))
         != -1) {
    switch (opt) {
      case 'c': {
        int64_t copyNumber = std::stol(std::string(optarg));
        if (copyNumber < 0) {
          throw std::out_of_range("copy number value cannot be negative");
        }
      ...
      default: {
        exception::CommandLineNotParsed ex("", false);
        ex.getMessage() << "getopt_long returned the following unknown value: 0x" << std::hex
                        << static_cast<int>(optopt);
        throw ex;
      }
    }
  }
...
void CmdLineArgs::printUsage(std::ostream& os) const {
  switch (m_standaloneCliTool) {
    ...
    case StandaloneCliTool::CTA_SEND_EVENT:
      ...
      break;
    case StandaloneCliTool::CTA_VERIFY_FILE:
      os << "cta-verify-file --vid/-v <vid> [--id/-I <archiveFileID,...>] [--filename/-F <filename>] [--instance/-i "
            "<instance>] [--request.user/-u <user>] [request.group/-g <group>]"
         << std::endl
         << std::endl
         << "  Submit a verification request for a given list of archive file IDs on a given tape." << std::endl
         << "   * If the --filename option is used, each line in the provided file should contain exactly one "
            "<archiveFileID>."
         << std::endl
         << std::endl;
      break;
    case StandaloneCliTool::CTA_CHANGE_STORAGE_CLASS:
      os << "    Usage:" << std::endl
         << "    cta-change-storage-class --id/-I <archiveFileID> | --json/-j <path> --storageclassname/-n "
            "<storageClassName> [--frequency/-t <eosRequestFrequency>]"
         << std::endl
         << std::endl;
      break;
    case StandaloneCliTool::EOS_NAMESPACE_INJECTION:
      os << "    Usage:" << std::endl << "    cta-eos-namespace-inject --json/--j <jsonPath> [--help/--h] \n";
      break;
    default:
      break;
  }
