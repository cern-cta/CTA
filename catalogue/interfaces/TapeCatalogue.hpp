...
public:
   ...
   * Returns the tapes with the specified volume identifiers.
   *
   * If ignoreMissingVids is false, this method will throw an exception when it cannot find ALL
   * of the specified tapes.
   *
   * @param vids The tape volume identifiers (VIDs).
  ...
  getVidToLogicalLibrary(const std::set<std::string, std::less<>>& vids) const = 0;
  ...
  virtual void modifyPurchaseOrder(const common::dataStructures::SecurityIdentity& admin,
                                   const std::string& vid,
                                   const std::string& purchaseOrder) = 0;
   ...
   */
  virtual void
  setTapeFull(const common::dataStructures::SecurityIdentity& admin, const std::string& vid, const bool fullValue) = 0;
