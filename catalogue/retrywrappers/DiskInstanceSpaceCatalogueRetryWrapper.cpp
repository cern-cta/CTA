...
void DiskInstanceSpaceCatalogueRetryWrapper::deleteDiskInstanceSpace(const std::string &name, const std::string &diskInstance) {
  return retryOnLostConnection(m_log, [this,&name,&diskInstance] {
...
void DiskInstanceSpaceCatalogueRetryWrapper::createDiskInstanceSpace(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin, name,&diskInstance,&freeSpaceQueryURL,&refreshInterval,&comment] {
    return m_catalogue->DiskInstanceSpace()->createDiskInstanceSpace(admin, name, diskInstance, freeSpaceQueryURL, refreshInterval, comment);
  }, m_maxTriesToConnect);
}
...
std::list<common::dataStructures::DiskInstanceSpace> DiskInstanceSpaceCatalogueRetryWrapper::getAllDiskInstanceSpaces() const {
  return retryOnLostConnection(m_log, [this] {
    return m_catalogue->DiskInstanceSpace()->getAllDiskInstanceSpaces();
  }, m_maxTriesToConnect);
}
...
void DiskInstanceSpaceCatalogueRetryWrapper::modifyDiskInstanceSpaceComment(
  ...
  const std::string &comment) {
  return retryOnLostConnection(m_log, [this,&admin,&name,&diskInstance,&comment] {
...
void DiskInstanceSpaceCatalogueRetryWrapper::modifyDiskInstanceSpaceRefreshInterval(
  const common::dataStructures::SecurityIdentity &admin, const std::string &name, const std::string &diskInstance,
  const uint64_t refreshInterval) {
  return retryOnLostConnection(m_log, [this,&admin,&name,&diskInstance,&refreshInterval] {
    return m_catalogue->DiskInstanceSpace()->modifyDiskInstanceSpaceRefreshInterval(admin, name, diskInstance, refreshInterval);
  }, m_maxTriesToConnect);
}
...
void DiskInstanceSpaceCatalogueRetryWrapper::modifyDiskInstanceSpaceFreeSpace(const std::string &name,
  ...
  return retryOnLostConnection(m_log, [this,&name,&diskInstance,&freeSpace] {
...
void DiskInstanceSpaceCatalogueRetryWrapper::modifyDiskInstanceSpaceQueryURL(
  const common::dataStructures::SecurityIdentity &admin, const std::string &name, const std::string &diskInstance,
  const std::string &freeSpaceQueryURL) {
  return retryOnLostConnection(m_log, [this,&admin,&name,&diskInstance,&freeSpaceQueryURL] {
    return m_catalogue->DiskInstanceSpace()->modifyDiskInstanceSpaceQueryURL(admin, name, diskInstance, freeSpaceQueryURL);
  }, m_maxTriesToConnect);
}
