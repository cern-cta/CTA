...
uint64_t ArchiveFileCatalogueRetryWrapper::checkAndGetNextArchiveFileId(
...
ArchiveFileItor
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->getArchiveFilesItor(searchCriteria); }, m_maxTriesToConnect);
}

common::dataStructures::ArchiveFile
  ArchiveFileCatalogueRetryWrapper::getArchiveFileForDeletion(const TapeFileSearchCriteria& searchCriteria) const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->getArchiveFileForDeletion(searchCriteria); }, m_maxTriesToConnect);
}
...
std::list<common::dataStructures::ArchiveFile>
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->getFilesForRepack(vid, startFSeq, maxNbFiles); },
    m_maxTriesToConnect);
}
...
ArchiveFileItor ArchiveFileCatalogueRetryWrapper::getArchiveFilesForRepackItor(const std::string& vid,
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->getArchiveFilesForRepackItor(vid, startFSeq); },
    m_maxTriesToConnect);
}
...
common::dataStructures::ArchiveFileSummary
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->getTapeFileSummary(searchCriteria); }, m_maxTriesToConnect);
}

common::dataStructures::ArchiveFile ArchiveFileCatalogueRetryWrapper::getArchiveFileById(const uint64_t id) const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->getArchiveFileById(id); }, m_maxTriesToConnect);
}
...
void ArchiveFileCatalogueRetryWrapper::moveArchiveFileToRecycleLog(
  ...
  log::LogContext& lc) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveFile()->moveArchiveFileToRecycleLog(request, lc); }, m_maxTriesToConnect);
}
