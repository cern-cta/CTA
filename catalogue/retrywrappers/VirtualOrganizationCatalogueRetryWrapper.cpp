...
void VirtualOrganizationCatalogueRetryWrapper::createVirtualOrganization(
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->createVirtualOrganization(admin, vo); }, m_maxTriesToConnect);
}

void VirtualOrganizationCatalogueRetryWrapper::deleteVirtualOrganization(const std::string& voName) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->deleteVirtualOrganization(voName); }, m_maxTriesToConnect);
}

std::list<common::dataStructures::VirtualOrganization>
  VirtualOrganizationCatalogueRetryWrapper::getVirtualOrganizations() const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->getVirtualOrganizations(); }, m_maxTriesToConnect);
}
...
common::dataStructures::VirtualOrganization
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->getVirtualOrganizationOfTapepool(tapepoolName); }, m_maxTriesToConnect);
}

common::dataStructures::VirtualOrganization
  VirtualOrganizationCatalogueRetryWrapper::getCachedVirtualOrganizationOfTapepool(
    const std::string& tapepoolName) const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->getCachedVirtualOrganizationOfTapepool(tapepoolName); },
    m_maxTriesToConnect);
}
...
void VirtualOrganizationCatalogueRetryWrapper::modifyVirtualOrganizationReadMaxDrives(
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->modifyVirtualOrganizationReadMaxDrives(admin, voName, readMaxDrives); },
    m_maxTriesToConnect);
}
...
void VirtualOrganizationCatalogueRetryWrapper::modifyVirtualOrganizationWriteMaxDrives(
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->modifyVirtualOrganizationWriteMaxDrives(admin, voName, writeMaxDrives); },
    m_maxTriesToConnect);
}
...
void VirtualOrganizationCatalogueRetryWrapper::modifyVirtualOrganizationMaxFileSize(
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->modifyVirtualOrganizationMaxFileSize(admin, voName, maxFileSize); },
    m_maxTriesToConnect);
}
...
void VirtualOrganizationCatalogueRetryWrapper::modifyVirtualOrganizationComment(
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->modifyVirtualOrganizationComment(admin, voName, comment); },
    m_maxTriesToConnect);
}
...
void VirtualOrganizationCatalogueRetryWrapper::modifyVirtualOrganizationDiskInstanceName(
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->VO()->modifyVirtualOrganizationDiskInstanceName(admin, voName, diskInstance); },
    m_maxTriesToConnect);
}
