...
SchemaVersion SchemaCatalogueRetryWrapper::getSchemaVersion() const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->Schema()->getSchemaVersion(); }, m_maxTriesToConnect);
}

void SchemaCatalogueRetryWrapper::verifySchemaVersion() {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->Schema()->verifySchemaVersion(); }, m_maxTriesToConnect);
}

void SchemaCatalogueRetryWrapper::ping() {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->Schema()->ping(); }, m_maxTriesToConnect);
}
