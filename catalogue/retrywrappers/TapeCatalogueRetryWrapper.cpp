...
void TapeCatalogueRetryWrapper::createTape(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&tape] {
    return m_catalogue->Tape()->createTape(admin, tape);
  }, m_maxTriesToConnect);
}

void TapeCatalogueRetryWrapper::deleteTape(const std::string &vid) {
  return retryOnLostConnection(m_log, [this,&vid] {
    return m_catalogue->Tape()->deleteTape(vid);
  }, m_maxTriesToConnect);
}
...
void TapeCatalogueRetryWrapper::reclaimTape(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&lc] {
    return m_catalogue->Tape()->reclaimTape(admin, vid, lc);
  }, m_maxTriesToConnect);
}

void TapeCatalogueRetryWrapper::checkTapeForLabel(const std::string &vid) {
  return retryOnLostConnection(m_log, [this,&vid] {
    return m_catalogue->Tape()->checkTapeForLabel(vid);
  }, m_maxTriesToConnect);
}

void TapeCatalogueRetryWrapper::tapeLabelled(const std::string &vid, const std::string &drive) {
  return retryOnLostConnection(m_log, [this,&vid,&drive] {
    return m_catalogue->Tape()->tapeLabelled(vid, drive);
  }, m_maxTriesToConnect);
}

uint64_t TapeCatalogueRetryWrapper::getNbFilesOnTape(const std::string &vid) const {
  return retryOnLostConnection(m_log, [this,&vid] {
    return m_catalogue->Tape()->getNbFilesOnTape(vid);
  }, m_maxTriesToConnect);
}
...
void TapeCatalogueRetryWrapper::modifyTapeMediaType(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&mediaType] {
    return m_catalogue->Tape()->modifyTapeMediaType(admin, vid, mediaType);
  }, m_maxTriesToConnect);
}
...
void TapeCatalogueRetryWrapper::modifyTapeVendor(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&vendor] {
    return m_catalogue->Tape()->modifyTapeVendor(admin, vid, vendor);
  },
    m_maxTriesToConnect);
}
...
void TapeCatalogueRetryWrapper::modifyTapeLogicalLibraryName(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&logicalLibraryName] {
...
void TapeCatalogueRetryWrapper::modifyTapeTapePoolName(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&tapePoolName] {
...
void TapeCatalogueRetryWrapper::modifyTapeEncryptionKeyName(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&encryptionKeyName] {
...
void TapeCatalogueRetryWrapper::modifyPurchaseOrder(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&purchaseOrder] {
...
void TapeCatalogueRetryWrapper::modifyTapeVerificationStatus(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&verificationStatus] {
...
void TapeCatalogueRetryWrapper::modifyTapeState(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&state,&prev_state,&stateReason] {
...
bool TapeCatalogueRetryWrapper::tapeExists(const std::string &vid) const {
  return retryOnLostConnection(m_log, [this,&vid] {
    return m_catalogue->Tape()->tapeExists(vid);
  }, m_maxTriesToConnect);
}
...
void TapeCatalogueRetryWrapper::setTapeFull(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&fullValue] {
    return m_catalogue->Tape()->setTapeFull(admin, vid, fullValue);
  }, m_maxTriesToConnect);
}
...
void TapeCatalogueRetryWrapper::setTapeDirty(const common::dataStructures::SecurityIdentity &admin, const std::string &vid,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&vid,&dirtyValue] {
    return m_catalogue->Tape()->setTapeDirty(admin, vid, dirtyValue);
  }, m_maxTriesToConnect);
}

void TapeCatalogueRetryWrapper::setTapeIsFromCastorInUnitTests(const std::string &vid) {
  return retryOnLostConnection(m_log, [this,&vid] {
    return m_catalogue->Tape()->setTapeIsFromCastorInUnitTests(vid);
  }, m_maxTriesToConnect);
}

void TapeCatalogueRetryWrapper::setTapeDirty(const std::string & vid) {
  return retryOnLostConnection(m_log, [this,&vid] {
    return m_catalogue->Tape()->setTapeDirty(vid);
  }, m_maxTriesToConnect);
}
