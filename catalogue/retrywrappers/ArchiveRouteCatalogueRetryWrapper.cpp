...
void ArchiveRouteCatalogueRetryWrapper::deleteArchiveRoute(const std::string& storageClassName, const uint32_t copyNb) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveRoute()->deleteArchiveRoute(storageClassName, copyNb); },
    m_maxTriesToConnect);
}

std::list<common::dataStructures::ArchiveRoute> ArchiveRouteCatalogueRetryWrapper::getArchiveRoutes() const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveRoute()->getArchiveRoutes(); }, m_maxTriesToConnect);
}
...
std::list<common::dataStructures::ArchiveRoute>
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->ArchiveRoute()->getArchiveRoutes(storageClassName, tapePoolName); },
    m_maxTriesToConnect);
}
