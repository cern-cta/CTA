...
void DiskSystemCatalogueRetryWrapper::createDiskSystem(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&name,&diskInstanceName,&diskInstanceSpaceName,&fileRegexp,&targetedFreeSpace,&sleepTime,&comment] {
    return m_catalogue->DiskSystem()->createDiskSystem(admin, name, diskInstanceName, diskInstanceSpaceName, fileRegexp, targetedFreeSpace, sleepTime, comment);
  }, m_maxTriesToConnect);
}

void DiskSystemCatalogueRetryWrapper::deleteDiskSystem(const std::string &name) {
  return retryOnLostConnection(m_log, [this,&name] {
    return m_catalogue->DiskSystem()->deleteDiskSystem(name);
  }, m_maxTriesToConnect);
}

disk::DiskSystemList DiskSystemCatalogueRetryWrapper::getAllDiskSystems() const {
  return retryOnLostConnection(m_log, [this] {
    return m_catalogue->DiskSystem()->getAllDiskSystems();
  }, m_maxTriesToConnect);
}
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemFileRegexp(const common::dataStructures::SecurityIdentity &admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&name,&fileRegexp] {
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemTargetedFreeSpace(
  ...
  return retryOnLostConnection(m_log, [this,&admin,&name,&targetedFreeSpace] {
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemSleepTime(const common::dataStructures::SecurityIdentity& admin,
  ...
  return retryOnLostConnection(m_log, [this,&admin,&name,&sleepTime] {
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemDiskInstanceName(
  ...
  return retryOnLostConnection(m_log, [this,&admin,&name,&diskInstanceName] {
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemDiskInstanceSpaceName(
  ...
  return retryOnLostConnection(m_log, [this,&admin,&name,&diskInstanceSpaceName] {
...
bool DiskSystemCatalogueRetryWrapper::diskSystemExists(const std::string &name) const {
  return retryOnLostConnection(m_log, [this,&name] {
    return m_catalogue->DiskSystem()->diskSystemExists(name);
  }, m_maxTriesToConnect);
}
