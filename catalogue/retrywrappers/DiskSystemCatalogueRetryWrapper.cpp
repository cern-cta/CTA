...
void DiskSystemCatalogueRetryWrapper::createDiskSystem(const common::dataStructures::SecurityIdentity& admin,
  ...
  return retryOnLostConnection(
    m_log,
    [&] {
      return m_catalogue->DiskSystem()->createDiskSystem(admin, name, diskInstanceName, diskInstanceSpaceName,
                                                         fileRegexp, targetedFreeSpace, sleepTime, comment);
    },
    m_maxTriesToConnect);
}
...
void DiskSystemCatalogueRetryWrapper::deleteDiskSystem(const std::string& name) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->deleteDiskSystem(name); }, m_maxTriesToConnect);
}

disk::DiskSystemList DiskSystemCatalogueRetryWrapper::getAllDiskSystems() const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->getAllDiskSystems(); }, m_maxTriesToConnect);
}
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemFileRegexp(const common::dataStructures::SecurityIdentity& admin,
                                                                 ...
                                                                 const std::string& fileRegexp) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->modifyDiskSystemFileRegexp(admin, name, fileRegexp); },
    m_maxTriesToConnect);
}
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemComment(const common::dataStructures::SecurityIdentity& admin,
                                                              ...
                                                              const std::string& comment) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->modifyDiskSystemComment(admin, name, comment); },
    m_maxTriesToConnect);
}
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemSleepTime(const common::dataStructures::SecurityIdentity& admin,
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->modifyDiskSystemSleepTime(admin, name, sleepTime); },
    m_maxTriesToConnect);
}
...
void DiskSystemCatalogueRetryWrapper::modifyDiskSystemDiskInstanceName(
  ...
  const std::string& diskInstanceName) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->modifyDiskSystemDiskInstanceName(admin, name, diskInstanceName); },
    m_maxTriesToConnect);
}
...
bool DiskSystemCatalogueRetryWrapper::diskSystemExists(const std::string& name) const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DiskSystem()->diskSystemExists(name); }, m_maxTriesToConnect);
}
