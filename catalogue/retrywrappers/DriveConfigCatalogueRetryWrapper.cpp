...
std::list<cta::catalogue::DriveConfigCatalogue::DriveConfig>
  ...
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DriveConfig()->getTapeDriveConfigs(); }, m_maxTriesToConnect);
}

std::list<std::pair<std::string, std::string>>
  DriveConfigCatalogueRetryWrapper::getTapeDriveConfigNamesAndKeys() const {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DriveConfig()->getTapeDriveConfigNamesAndKeys(); }, m_maxTriesToConnect);
}
...
void DriveConfigCatalogueRetryWrapper::deleteTapeDriveConfig(const std::string& tapeDriveName,
                                                             const std::string& keyName) {
  return retryOnLostConnection(
    m_log, [&] { return m_catalogue->DriveConfig()->deleteTapeDriveConfig(tapeDriveName, keyName); },
    m_maxTriesToConnect);
}
