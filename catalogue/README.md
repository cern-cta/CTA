...
**WARNING**
    The changing of the *SCHEMA_VERSION_MAJOR* number has to be modified **ONLY** if the changes made to the schema are [not backward-compatible](https://cta.docs.cern.ch/latest/dev/architecture/components/catalogue/modifications/backward-incompatible-upgrades/) with the currently running version of CTA.
...
6. If everything works, let's define a [upgrade strategy](https://cta.docs.cern.ch/latest/dev/architecture/components/catalogue/modifications/create-liquibase-changelog-file/).
