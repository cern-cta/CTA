...
TEST_P(cta_catalogue_FileRecycleLogTest, filesArePutInTheFileRecycleLogInsteadOfBeingSuperseded) {
  ...
  std::list<common::dataStructures::FileRecycleLog> fileRecycleLogs;
  {
    ...
    for (const auto& fileRecycleLog : fileRecycleLogs) {
      ...
      auto tapeFilesWrittenCopy1Itor =
        std::find_if(tapeFilesWrittenCopy1.begin(),
                     tapeFilesWrittenCopy1.end(),
...
TEST_P(cta_catalogue_FileRecycleLogTest, RestoreTapeFileCopyDeletedTwice) {
  ...
  m_catalogue->VO()->createVirtualOrganization(m_admin, m_vo);
  m_catalogue->TapePool()
    ->createTapePool(m_admin, tapePoolNameA, m_vo.name, nbPartialTapes, encryptionKeyName, supply, "Create tape pool");
  m_catalogue->TapePool()
    ->createTapePool(m_admin, tapePoolNameB, m_vo.name, nbPartialTapes, encryptionKeyName, supply, "Create tape pool");
  m_catalogue->StorageClass()->createStorageClass(m_admin, m_storageClassDualCopy);
...
TEST_P(cta_catalogue_FileRecycleLogTest, RestoreVariousDeletedTapeFileCopies) {
  ...
  m_catalogue->TapePool()
    ->createTapePool(m_admin, tapePoolName2, m_vo.name, nbPartialTapes, encryptionKeyName, supply, "Create tape pool");
  m_catalogue->TapePool()
    ->createTapePool(m_admin, tapePoolName3, m_vo.name, nbPartialTapes, encryptionKeyName, supply, "Create tape pool");
...
TEST_P(cta_catalogue_FileRecycleLogTest, RestoreArchiveFileAndCopy) {
...

  {
    ...
    m_catalogue->FileRecycleLog()->restoreFileInRecycleLog(searchCriteria,
                                                           std::to_string(12345678));  //previous fid + 1
