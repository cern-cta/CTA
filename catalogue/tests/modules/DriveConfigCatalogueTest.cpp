...
TEST_P(cta_catalogue_DriveConfigTest, getTapeDriveConfig) {
  ...
  m_catalogue->DriveConfig()->createTapeDriveConfig(tapeDriveName,
                                                    daemonUserName.category(),
                                                    daemonUserName.key(),
                                                    daemonUserName.value(),
...
TEST_P(cta_catalogue_DriveConfigTest, getAllDrivesConfigs) {
  ...
  for (size_t i = 0; i < 100; i++) {
    ...
    cta::SourcedParameter<std::string> daemonUserName {"taped", "DaemonUserName", "cta", "Compile time default"};
    m_catalogue->DriveConfig()->createTapeDriveConfig(ss.str(),
                                                      daemonUserName.category(),
                                                      daemonUserName.key(),
                                                      daemonUserName.value(),
                                                      daemonUserName.source());
    ...
    cta::SourcedParameter<std::string> defaultConfig {"taped",
                                                      "defaultConfig",
                                                      "cta",
...
TEST_P(cta_catalogue_DriveConfigTest, setSourcedParameterWithEmptyValue) {
  ...
  m_catalogue->DriveConfig()->createTapeDriveConfig(tapeDriveName,
                                                    raoLtoOptions.category(),
                                                    raoLtoOptions.key(),
                                                    raoLtoOptions.value(),
                                                    raoLtoOptions.source());
...
TEST_P(cta_catalogue_DriveConfigTest, multipleDriveConfig) {
  ...
  m_catalogue->DriveConfig()->createTapeDriveConfig(tapeDriveName1,
                                                    daemonUserName.category(),
                                                    daemonUserName.key(),
                                                    daemonUserName.value(),
                                                    daemonUserName.source());
  m_catalogue->DriveConfig()->createTapeDriveConfig(tapeDriveName1,
                                                    daemonGroupName.category(),
                                                    daemonGroupName.key(),
                                                    daemonGroupName.value(),
                                                    daemonGroupName.source());
  m_catalogue->DriveConfig()->createTapeDriveConfig(tapeDriveName2,
                                                    daemonUserName.category(),
                                                    daemonUserName.key(),
                                                    daemonUserName.value(),
                                                    daemonUserName.source());
...
TEST_P(cta_catalogue_DriveConfigTest, modifyTapeDriveConfig) {
  ...
  m_catalogue->DriveConfig()->createTapeDriveConfig(tapeDriveName,
                                                    daemonUserName1.category(),
                                                    daemonUserName1.key(),
                                                    daemonUserName1.value(),
                                                    daemonUserName1.source());
  ...
  m_catalogue->DriveConfig()->modifyTapeDriveConfig(tapeDriveName,
                                                    daemonUserName2.category(),
                                                    daemonUserName2.key(),
                                                    daemonUserName2.value(),
                                                    daemonUserName2.source());
