...
TEST_P(cta_catalogue_TapeTest, modifyTapeState_noReasonWhenNotActive) {
  ...
  std::string reason = "";
  ASSERT_THROW(m_catalogue->Tape()->modifyTapeState(
                 m_admin, m_tape1.vid, cta::common::dataStructures::Tape::State::BROKEN, std::nullopt, reason),
               cta::catalogue::UserSpecifiedAnEmptyStringReasonWhenTapeStateNotActive);

  ASSERT_THROW(m_catalogue->Tape()->modifyTapeState(
                 m_admin, m_tape1.vid, cta::common::dataStructures::Tape::State::DISABLED, std::nullopt, std::nullopt),
               cta::catalogue::UserSpecifiedAnEmptyStringReasonWhenTapeStateNotActive);
}
...
TEST_P(cta_catalogue_TapeTest, getTapesSearchCriteriaByState) {
  ...
  std::string reason = "Broken tape";
  ASSERT_NO_THROW(m_catalogue->Tape()->modifyTapeState(
    m_admin, vidTape1, cta::common::dataStructures::Tape::State::BROKEN, std::nullopt, reason));
