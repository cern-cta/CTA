...
TEST_P(cta_catalogue_TapeFileTest, moveFilesToRecycleLog) {
  ...
  for (uint64_t i = 1; i <= nbArchiveFiles; i++) {
    ...
    ASSERT_EQ(cta::catalogue::InsertFileRecycleLog::getDeletionReasonLog(m_admin.username, diskInstance),
              deletedArchiveFile.reasonLog);
...
TEST_P(cta_catalogue_TapeFileTest, prepareToRetrieveFileUsingArchiveFileId_repackingTapes) {
...

  {
    const cta::common::dataStructures::RetrieveFileQueueCriteria queueCriteria =
      m_catalogue->TapeFile()->prepareToRetrieveFile(diskInstanceName1,
                                                     archiveFileId,
                                                     requesterIdentity,
                                                     std::nullopt,
                                                     dummyLc);
  ...
  m_catalogue->Tape()->modifyTapeState(m_admin,
                                       m_tape1.vid,
                                       cta::common::dataStructures::Tape::State::REPACKING,
                                       std::nullopt,
                                       repackingReason);
  ...
  ASSERT_THROW(m_catalogue->TapeFile()
                 ->prepareToRetrieveFile(diskInstanceName1, archiveFileId, requesterIdentity, std::nullopt, dummyLc),
...
TEST_P(cta_catalogue_TapeFileTest, prepareToRetrieveFileUsingArchiveFileId) {
  ...
  ASSERT_THROW(m_catalogue->TapeFile()
                 ->prepareToRetrieveFile(diskInstanceName2, archiveFileId, requesterIdentity, std::nullopt, dummyLc),
               cta::exception::UserError);
}
...
TEST_P(cta_catalogue_TapeFileTest, prepareToRetrieveFileUsingArchiveFileId_ActivityMountPolicy) {
  ...
  m_catalogue->RequesterActivityMountRule()->createRequesterActivityMountRule(m_admin,
                                                                              mountPolicyToAdd1.name,
                                                                              diskInstanceName1,
                                                                              requesterName,
                                                                              activityRegex,
                                                                              comment);
  ...
  ASSERT_THROW(m_catalogue->TapeFile()
                 ->prepareToRetrieveFile(diskInstanceName2, archiveFileId, requesterIdentity, requestActivity, dummyLc),
               cta::exception::UserError);
}
