...
TEST_P(cta_catalogue_TapeFileTest, prepareToRetrieveFileUsingArchiveFileId_repackingTapes) {
...

  {
    const cta::common::dataStructures::RetrieveFileQueueCriteria queueCriteria =
      m_catalogue->TapeFile()->prepareToRetrieveFile(diskInstanceName1, archiveFileId, requesterIdentity, std::nullopt,
                                                     dummyLc);
  ...
  ASSERT_THROW(m_catalogue->TapeFile()->prepareToRetrieveFile(diskInstanceName1, archiveFileId, requesterIdentity,
                                                              std::nullopt, dummyLc),
...
TEST_P(cta_catalogue_TapeFileTest, prepareToRetrieveFileUsingArchiveFileId_disabledTapes) {
  ...
  m_catalogue->Tape()->modifyTapeState(m_admin, m_tape1.vid, cta::common::dataStructures::Tape::State::DISABLED,
                                       std::nullopt, disabledReason);
...
TEST_P(cta_catalogue_TapeFileTest, prepareToRetrieveFileUsingArchiveFileId_ActivityMountPolicy) {
  ...
  m_catalogue->RequesterActivityMountRule()->createRequesterActivityMountRule(
    m_admin, mountPolicyToAdd1.name, diskInstanceName1, requesterName, activityRegex, comment);

  const std::string secondActivityRegex = "^activity_specific$";
  m_catalogue->RequesterActivityMountRule()->createRequesterActivityMountRule(
    m_admin, mountPolicyToAdd2.name, diskInstanceName1, requesterName, secondActivityRegex, comment);
  {
