...
void RdbmsDiskSystemCatalogue::createDiskSystem(const common::dataStructures::SecurityIdentity& admin,
  ...
  try {
    ...
    if (fileRegexp.empty()) {
      throw UserSpecifiedAnEmptyStringFileRegexp(
        "Cannot create disk system because the file regexp is an empty string");
    }
    ...
    const char* const sql = "INSERT INTO DISK_SYSTEM("
                            "DISK_SYSTEM_NAME,"
                            "DISK_INSTANCE_NAME,"
...
void RdbmsDiskSystemCatalogue::modifyDiskSystemFileRegexp(const common::dataStructures::SecurityIdentity& admin,
  ...
  try {
    ...
    const time_t now = time(nullptr);
    const char* const sql = "UPDATE DISK_SYSTEM SET "
