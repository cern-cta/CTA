...
std::list<common::dataStructures::Tape> RdbmsTapeCatalogue::getTapes(rdbms::Conn& conn,
  ...
  if (searchCriteria.vid || searchCriteria.mediaType || searchCriteria.vendor || searchCriteria.logicalLibrary ||
      ...
      searchCriteria.diskFileIds || searchCriteria.state || searchCriteria.fromCastor || searchCriteria.purchaseOrder ||
      searchCriteria.physicalLibraryName || searchCriteria.checkMissingFileCopies) {
    sql += R"SQL( WHERE )SQL";
  }
