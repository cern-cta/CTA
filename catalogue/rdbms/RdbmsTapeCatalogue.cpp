...
void RdbmsTapeCatalogue::createTape(const common::dataStructures::SecurityIdentity& admin,
  ...
  try {
    ...
    const char* const sql = "INSERT INTO TAPE("
                            "VID, "
                            "MEDIA_TYPE_ID, "
                            "VENDOR, "
                            "LOGICAL_LIBRARY_ID, "
...
common::dataStructures::VidToTapeMap RdbmsTapeCatalogue::getTapesByVid(const std::set<std::string>& vids,
...
std::map<std::string, std::string> RdbmsTapeCatalogue::getVidToLogicalLibrary(const std::set<std::string>& vids) const {
  try {
    ...
    if (vids.size() != vidToLogicalLibrary.size()) {
      exception::Exception ex;
      ex.getMessage() << "Not all tapes were found: expected=" << vids.size()
                      << " actual=" << vidToLogicalLibrary.size();
      throw ex;
    }
...
void RdbmsTapeCatalogue::modifyTapeVendor(const common::dataStructures::SecurityIdentity& admin,
  ...
  try {
    const time_t now = time(nullptr);
    const char* const sql = "UPDATE TAPE SET "
                            "VENDOR = :VENDOR,"
                            "LAST_UPDATE_USER_NAME = :LAST_UPDATE_USER_NAME,"
                            "LAST_UPDATE_HOST_NAME = :LAST_UPDATE_HOST_NAME,"
                            "LAST_UPDATE_TIME = :LAST_UPDATE_TIME "
...
void RdbmsTapeCatalogue::modifyTapeState(const common::dataStructures::SecurityIdentity& admin,
  ...
  try {
    ...
    if (state != Tape::State::ACTIVE) {
      if (!trimmedReason) {
        throw UserSpecifiedAnEmptyStringReasonWhenTapeStateNotActive(
          std::string("Cannot modify the state of the tape ") + vid + " to " + stateStr +
          " because the reason has not been provided.");
      }
    }

    std::string sql = "UPDATE TAPE SET "
    ...
    if (0 == stmt.getNbAffectedRows()) {
      throw UserSpecifiedANonExistentTape(
        std::string("Cannot modify the state of the tape ") + vid +
...
std::list<common::dataStructures::Tape> RdbmsTapeCatalogue::getTapes(rdbms::Conn& conn,
  ...
  try {
    ...
    if (searchCriteria.vid || searchCriteria.mediaType || searchCriteria.vendor || searchCriteria.logicalLibrary ||
        searchCriteria.tapePool || searchCriteria.vo || searchCriteria.capacityInBytes || searchCriteria.full ||
        searchCriteria.diskFileIds || searchCriteria.state || searchCriteria.fromCastor) {
      sql += " WHERE";
    }
...
void RdbmsTapeCatalogue::deleteTapeFiles(rdbms::Conn& conn, const std::string& vid) const {
  try {
    const char* const sql = "DELETE FROM TAPE_FILE WHERE VID = :VID";
    auto stmt = conn.createStmt(sql);
    stmt.bindString(":VID", vid);
    stmt.executeNonQuery();
    RdbmsCatalogueUtils::setTapeDirty(conn, vid);
...
uint64_t RdbmsTapeCatalogue::getNbFilesOnTape(rdbms::Conn& conn, const std::string& vid) const {
  try {
    const char* const sql = "SELECT COUNT(*) AS NB_FILES FROM TAPE_FILE "
                            "WHERE VID = :VID ";
...
void RdbmsTapeCatalogue::executeGetTapesByVidStmtAndCollectResults(
  ...
  while (rset.next()) {
    ...
    tape.labelFormat = common::dataStructures::Label::validateFormat(
      rset.columnOptionalUint8("LABEL_FORMAT"), "[RdbmsCatalogue::executeGetTapesByVidsStmtAndCollectResults()]");
