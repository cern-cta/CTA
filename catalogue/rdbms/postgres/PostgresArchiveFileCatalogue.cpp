...
uint64_t PostgresArchiveFileCatalogue::getNextArchiveFileId(rdbms::Conn& conn) {
  try {
    const char* const sql = "select NEXTVAL('ARCHIVE_FILE_ID_SEQ') AS ARCHIVE_FILE_ID";
    auto stmt = conn.createStmt(sql);
...
void PostgresArchiveFileCatalogue::copyArchiveFileToFileRecyleLogAndDelete(
  rdbms::Conn& conn,
  ...
  try {
    ...
    tl.addToLog(spc);
    lc.log(log::INFO,
           "In PostgresCatalogue::copyArchiveFileToRecycleBinAndDelete: ArchiveFile moved to the recycle-bin.");
  }
  catch (exception::UserError&) {
    throw;
  }
