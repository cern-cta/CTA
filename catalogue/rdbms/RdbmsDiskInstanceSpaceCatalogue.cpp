  ...
  try {
    ...
    if (comment.empty()) {
      throw UserSpecifiedAnEmptyStringComment(
        "Cannot create disk instance space because the comment is an empty string");
    }
    ...
    const char* const sql = "INSERT INTO DISK_INSTANCE_SPACE("
                            "DISK_INSTANCE_NAME,"
...
void RdbmsDiskInstanceSpaceCatalogue::modifyDiskInstanceSpaceComment(
  ...
  try {
    ...
    const auto trimmedComment = RdbmsCatalogueUtils::checkCommentOrReasonMaxLength(comment, &m_log);
    const time_t now = time(nullptr);
    const char* const sql = "UPDATE DISK_INSTANCE_SPACE SET "
