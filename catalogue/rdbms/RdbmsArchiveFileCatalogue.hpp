...
protected:
  RdbmsArchiveFileCatalogue(log::Logger& log,
...
private:
   ...
   * class.
   */
  common::dataStructures::TapeCopyToPoolMap getCachedTapeCopyToPoolMap(const catalogue::StorageClass& storageClass,
   ...
   */
  uint64_t getCachedExpectedNbArchiveRoutes(const catalogue::StorageClass& storageClass,
                                            bool useRepackArchiveRoute) const;
  ...
  getCachedRequesterMountPolicy(const User& user) const;
   ...
   */
  ValueAndTimeBasedCacheInfo<std::optional<common::dataStructures::MountPolicy>>
  getCachedRequesterGroupMountPolicy(const Group& group) const;
  ...
  getTapeCopyToPoolMap(rdbms::Conn& conn, const catalogue::StorageClass& storageClass, bool isRepack) const;
   ...
   * @return The expected number of archive routes.
   */
  uint64_t getExpectedNbArchiveRoutes(rdbms::Conn& conn,
  ...
  getArchiveFileToRetrieveByArchiveFileId(rdbms::Conn& conn, const uint64_t archiveFileId) const;
