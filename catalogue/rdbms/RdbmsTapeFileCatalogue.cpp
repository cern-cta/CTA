...
void RdbmsTapeFileCatalogue::checkTapeFileWrittenFieldsAreSet(const std::string& callingFunc,
                                                              const TapeFileWritten& event) const {
  try {
    ...
    if (event.checksumBlob.length() == 0) {
...
void RdbmsTapeFileCatalogue::insertTapeFile(rdbms::Conn& conn,
  ...
  try {
      ...
      fileRecycleLogCatalogue->insertOldCopiesOfFilesIfAnyOnFileRecycleLog(conn, tapeFile, archiveFileId);
    {
      ...
      const char* const sql = "INSERT INTO TAPE_FILE("
                              "VID,"
                              "FSEQ,"
                              "BLOCK_ID,"
...
void RdbmsTapeFileCatalogue::deleteTapeFiles(rdbms::Conn& conn,
  ...
  try {
    ...
    const char* const deleteTapeFilesSql = "DELETE FROM "
                                           "TAPE_FILE "
    ...
    common::dataStructures::RetrieveFileQueueCriteria criteria;
    {
      ...
      if (nullptr == archiveFile.get()) {
        ...
        const auto nonBrokenState = std::find_if(
          std::begin(tapeFileStateList), std::end(tapeFileStateList), [](std::pair<std::string, std::string> state) {
            return (state.second != "BROKEN") && (state.second != "BROKEN_PENDING") && (state.second != "EXPORTED") &&
        ...
        if (nonBrokenState != std::end(tapeFileStateList)) {
          ex.getMessage() << "WARNING: File with archive file ID " << archiveFileId
                          << " exits in CTA namespace but is temporarily unavailable on " << nonBrokenState->second
                          << " tape " << nonBrokenState->first;
          throw ex;
        }
        ...
        ex.getMessage() << "ERROR: File with archive file ID " << archiveFileId
                        << " exits in CTA namespace but is permanently unavailable on " << brokenState.second
                        << " tape " << brokenState.first;
        throw ex;
      }
      if (mountPolicyName.has_value() && !mountPolicyName.value().empty()) {
        const auto mountPolicyCatalogue =
          static_cast<RdbmsMountPolicyCatalogue*>(m_rdbmsCatalogue->MountPolicy().get());
      ...
      if (!mountPolicies.requesterActivityMountPolicies.empty()) {
        ...
        mountPolicy = *std::max_element(
          mountPolicies.requesterActivityMountPolicies.begin(), mountPolicies.requesterActivityMountPolicies.end(),
