...
common::dataStructures::ArchiveFile
  RdbmsArchiveFileCatalogue::getArchiveFileForDeletion(const TapeFileSearchCriteria& criteria) const {
  ...
  try {
    ...
    while (rset.next()) {
      ...
      archiveFile.checksumBlob.deserializeOrSetAdler32(rset.columnBlob("CHECKSUM_BLOB"),
                                                       rset.columnUint64("CHECKSUM_ADLER32"));
...
common::dataStructures::TapeCopyToPoolMap
  RdbmsArchiveFileCatalogue::getCachedTapeCopyToPoolMap(const catalogue::StorageClass& storageClass) const {
  try {
...
uint64_t
  RdbmsArchiveFileCatalogue::getCachedExpectedNbArchiveRoutes(const catalogue::StorageClass& storageClass) const {
  try {
    auto getNonCachedValue = [&] {
      auto conn = m_connPool->getConn();
...
void RdbmsArchiveFileCatalogue::modifyArchiveFileStorageClassId(const uint64_t archiveFileId,
  ...
  try {
    ...
    if (!RdbmsCatalogueUtils::storageClassExists(conn, newStorageClassName)) {
      exception::UserError ue;
      ue.getMessage() << "Cannot modify archive file "
                      << ": " << archiveFileId << " because storage class "
                      << ":" << newStorageClassName << " does not exist";
      throw ue;
    }

    const char* const sql = "UPDATE ARCHIVE_FILE   "
...
void RdbmsArchiveFileCatalogue::modifyArchiveFileFxIdAndDiskInstance(const uint64_t archiveId,
  ...
  const char* const sql = "UPDATE ARCHIVE_FILE SET "
                          "DISK_FILE_ID = :FXID,"
...
void RdbmsArchiveFileCatalogue::moveArchiveFileToRecycleLog(const common::dataStructures::DeleteArchiveRequest& request,
  ...
  try {
    ...
    for (auto it = archiveFile.tapeFiles.begin(); it != archiveFile.tapeFiles.end(); it++) {
      ...
      tapeCopyLogStream << "copy number: " << static_cast<int>(it->copyNb) << " vid: " << it->vid
                        << " fSeq: " << it->fSeq << " blockId: " << it->blockId << " creationTime: " << it->creationTime
...
void RdbmsArchiveFileCatalogue::deleteArchiveFile(rdbms::Conn& conn,
  ...
  try {
    const char* const deleteArchiveFileSql = "DELETE FROM "
                                             "ARCHIVE_FILE "
...
const std::list<std::pair<std::string, std::string>>
  RdbmsArchiveFileCatalogue::getTapeFileStateListForArchiveFileId(rdbms::Conn& conn,
                                                                  const uint64_t archiveFileId) const {
  try {
    const char* const sql = "SELECT "
                            "TAPE_FILE.VID AS VID,"
