...
void OracleArchiveFileCatalogue::DO_NOT_USE_deleteArchiveFile_DO_NOT_USE(const std::string& diskInstanceName,
  ...
  try {
    ...
    while (selectRset.next()) {
      if (nullptr == archiveFile.get()) {
        ...
        archiveFile->checksumBlob.deserializeOrSetAdler32(selectRset.columnBlob("CHECKSUM_BLOB"),
                                                          selectRset.columnUint64("CHECKSUM_ADLER32"));
    ...
    if (diskInstanceName != archiveFile->diskInstance) {
      ...
      for (auto it = archiveFile->tapeFiles.begin(); it != archiveFile->tapeFiles.end(); it++) {
        ...
        tapeCopyLogStream << "copy number: " << static_cast<int>(it->copyNb) << " vid: " << it->vid
                          << " fSeq: " << it->fSeq << " blockId: " << it->blockId
                          << " creationTime: " << it->creationTime << " fileSize: " << it->fileSize
    ...
    for (auto it = archiveFile->tapeFiles.begin(); it != archiveFile->tapeFiles.end(); it++) {
      std::stringstream tapeCopyLogStream;
      tapeCopyLogStream << "copy number: " << it->copyNb << " vid: " << it->vid << " fSeq: " << it->fSeq
                        << " blockId: " << it->blockId << " creationTime: " << it->creationTime
    ...
    msg << __FUNCTION__ << ": diskInstanceName=" << diskInstanceName << ",archiveFileId=" << archiveFileId << ": "
        << ex.getMessage().str();
...
void OracleArchiveFileCatalogue::copyArchiveFileToFileRecyleLogAndDelete(
  ...
  try {
    ...
    tl.addToLog(spc);
    lc.log(log::INFO,
           "In OracleCatalogue::copyArchiveFileToRecycleBinAndDelete: ArchiveFile moved to the recycle-bin.");
  }
  catch (exception::UserError&) {
    throw;
  }
