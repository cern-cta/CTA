...
--preconditions onFail:HALT onError:HALT
--precondition-sql-check expectedResult:"12.0" SELECT CONCAT(CONCAT(CAST(SCHEMA_VERSION_MAJOR as VARCHAR(10)),'.'), CAST(SCHEMA_VERSION_MINOR AS VARCHAR(10))) AS CATALOGUE_VERSION FROM CTA_CATALOGUE;
ALTER TABLE TAPE CONSTRAINT TAPE_VID_CK CHECK(VID = UPPER(VID));
...
ALTER TABLE TAPE_FILE CONSTRAINT TAPE_FILE_IA_BOOL_CK CHECK(IS_ACCESSIBLE IN ('0', '1'));
--rollback ALTER TABLE TAPE_FILE ADD IS_ACCESSIBLE CHAR(1) DEFAULT '1' CONSTRAINT TAPE_FILE_IA_NN NOT NULL;
--rollback ALTER TABLE TAPE_FILE CONSTRAINT TAPE_FILE_IA_BOOL_CK CHECK(IS_ACCESSIBLE IN ('0', '1'));
