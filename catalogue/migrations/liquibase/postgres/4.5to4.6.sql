...
--changeset mvelosob:5 failOnError:true dbms:postgresql
--preconditions onFail:HALT onError:HALT
--precondition-sql-check expectedResult:"4.5" SELECT CONCAT(CONCAT(CAST(SCHEMA_VERSION_MAJOR as VARCHAR(10)),'.'), CAST(SCHEMA_VERSION_MINOR AS VARCHAR(10))) AS CATALOGUE_VERSION FROM CTA_CATALOGUE;
ALTER TABLE DRIVE_STATE ALTER COLUMN DISK_SYSTEM_NAME DROP NOT NULL;
ALTER TABLE DRIVE_STATE ALTER COLUMN RESERVED_BYTES DROP NOT NULL;
--rollback ALTER TABLE DRIVE_STATE ALTER COLUMN DISK_SYSTEM_NAME SET NOT NULL;
--rollback ALTER TABLE DRIVE_STATE ALTER COLUMN RESERVED_BYTES SET NOT NULL;

--changeset mvelosob:6 failOnError:true dbms:postgresql
--preconditions onFail:HALT onError:HALT
--precondition-sql-check expectedResult:"4.5" SELECT CONCAT(CONCAT(CAST(SCHEMA_VERSION_MAJOR as VARCHAR(10)),'.'), CAST(SCHEMA_VERSION_MINOR AS VARCHAR(10))) AS CATALOGUE_VERSION FROM CTA_CATALOGUE;
ALTER TABLE VIRTUAL_ORGANIZATION ADD DISK_INSTANCE_NAME VARCHAR(100);
