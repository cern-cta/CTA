...
#
#
# The Metrics Analysis Engine project is a "framework" for computing
# metrics.It can be applied to whatever type of key-value messages.
# For more informations,check out the project page on:
# https://twiki.cern.ch/twiki/bin/view/DSSGroup/MetricsAnalysisEngine
#
#
# Performance test:
#
...
# 
# a) With metric matching and Timestamp to EPOCH cnversion:  14 sec
# b) With metric matching messages EPOCH                  :  10 sec
# c) Without metric matching messages EPOCH               :   8 sec
#
# The code now uses approach b) if possible, so if there is a EPOCH field
# in the message then it's used. If not, it swithces to computing the
# epoch time fromt he TIMESTAMP using approach a). The EPOCH field is
# generated by Scribe Injector.
...
class Counter(int):
    """
    Count how many times it is called. The argoument can be whatever keyword and even empty.
    """
