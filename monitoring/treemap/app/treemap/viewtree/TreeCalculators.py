...
labelsize - height of the rectangle label, in px
...
class SquaredTreemapCalculator(object):
    ...
    def __init__(self, treemap_props):
        ...
        self.labelsize = self.treemap_props['labelsize']
    ...
    def calculate(self, optimizefortxt = False, sorted = True, bigdifftreshold = 1.5, squareoverflowdecision = False):
        ...
        if treemap_props['label']:
            vnode.setProperty('labelsize', self.labelsize)
        else:
            vnode.setProperty('labelsize', 0.0)
        ...
        if treemap_props['label']: y = y + self.labelsize
        width = width - self.paddingsize
        height = height - self.paddingsize
        if treemap_props['label']: height = height - self.labelsize
        ...
        self.calculateRecursion(x, y, width ,height , viewtree, self.paddingsize, self.minpaddingsize, self.labelsize, optimizefortxt, sorted, bigdifftreshold, squareoverflowdecision)
    ...
    def calculateRecursion(self, startx, starty, width ,height, viewtree, paddingsize, minpaddingsize, labelsize, optimizefortxt, sorted, bigdifftreshold, squareoverflowdecision):
...
#                
#                self.calculateRecursion(parentvnode.getProperty('x'), parentvnode.getProperty('y'), parentvnode.getProperty('width') ,parentvnode.getProperty('height'), viewtree, parentvnode.getProperty('paddingsize'), minpaddingsize, labelsize, optimizefortxt, sorted, bigdifftreshold, squareoverflowdecision)
#                
...
#                return
        ...
        if treemap_props['label']:
            parentcsize = viewtree.getCurrentObject().getProperty('labelsize')
            if parentcsize <= 0.0:
                return
...
#                if (max_value_in_strip*len(line_collection)) > 0: bigdifftrigger = (linelen * min_value_in_strip*(len(line_collection)))/(max_value_in_strip*(len(line_collection))+min_value_in_strip) < 40.0
            ...
            if stopdecision:
                ...
                for ch in line_collection: #calculate the witdth of each child
                    ...
                    vn = ViewNode()
                    if (2*labelsize > (chheight-2.0)) or not treemap_props['label']:
                        vn.setProperty('labelsize', 0.0)
                    else:
                        vn.setProperty('labelsize', labelsize)
        ...
        if line_collection:
            ...
            for ch in line_collection:
                ...
                #store the calculated values
                vn = ViewNode()
                if (2.0 * labelsize > (chheight-2.0)) or not treemap_props['label']:
                    vn.setProperty('labelsize', 0.0)
                else:
                    vn.setProperty('labelsize', labelsize)
        ...
        for i in range(len(totalchildnodes)):
            ...
            #(startx, starty, width ,height, viewtree, paddingsize, minpaddingsize, labelsize, optimizefortxt, sorted, bigdifftreshold, squareoverflowdecision)
            csize = totalviewnodes[i].getProperty('labelsize')
            if csize <= 0.0:
                self.calculateRecursion.__dict__['notextcount'] = self.calculateRecursion.__dict__['notextcount'] + 1
                self.calculateRecursion(totalviewnodes[i].getProperty('x')+paddingsize/2.0, totalviewnodes[i].getProperty('y')+paddingsize/2.0, totalviewnodes[i].getProperty('width')-paddingsize ,totalviewnodes[i].getProperty('height')-paddingsize, viewtree, paddingsize, minpaddingsize, labelsize, optimizefortxt, sorted, bigdifftreshold, squareoverflowdecision)
            else:
