...
#        colmn = 'name'
#        searchstring = 'bla'
#        theobj = Dirs.objects.get(pk=self.fileid)
#        w = Wrapper(theobj, colmn)
#        print self.name 
##        printSquaredTreemapCalculator w.evaluate(searchstring)
#        t = ObjectTree() 
#        t.setRoot(theobj, None)
        ch = self.getDirs()
        if not ch: return
#        for child in ch:
#            t.addChild(child, colmn, searchstring)
#        for i in range(t.countChildren()):
#            print t.getChild(i).getObject().__str__() + ' value: ' + t.getChild(i).evaluate().__str__()
#        t.traverseInto(childindex = 0)
#        t.traverseBack()
...
#        print 'now: 'QuerySet + start.__str__()
...
#        if ch[0].__str__() == "/castor/cern.ch/user/c/cvsadmin":
#            print 'profiling'
#            profile.runctx('tree = tc.calculate(500,600)', globals(), {'tc':tc, 'tree': tree})
#        else:
...
#        drawer.drawRect(20.0, 20.0, 100.0, 100.0, 5.0)
        drawer.drawTreemap(id(ch).__str__() + "test_tree.png")
        print ''
