...
while (OCIFetch($parsed_incoming_reqs)) {
	...
	$data['incoming_get'][$time] = $incoming_get;
	$data['incoming_prep_get'][$time] = $incoming_prep_get;
	$i++;
}
...
$graph->title->Set("Read File Requests (Last 4 Hours)");
...
$plot2 = new LinePlot(array_values($data['incoming_get']));
$plot2->SetColor("green");
$plot2 -> SetLegend("Direct Get Req. (#)");
$graph->AddY(0,$plot2);
$graph->ynaxis[0]->SetColor('green');

$plot3 = new LinePlot(array_values($data['incoming_prep_get']));
$plot3->SetColor("red");
$plot3 -> SetLegend("Prepare To Get Req. (#)");
$graph->AddY(1,$plot3);
