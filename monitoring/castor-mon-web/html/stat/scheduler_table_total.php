...
if ($timewindow == "10/1440")
	...
	$period = 30;
if ($qn == 1) 
	$query1 = "select svcclass, requesttype, sum(dispatched)
...
while(OCIFetch($parsed1)) {
	...
	$dispatched = OCIResult($parsed1,3); 
	if ($min == NULL) $min = $dispatched;
	if ($dispatched >= $max) $max = $dispatched;
	if ($dispatched <= $min) $min = $dispatched;
	$svcc[$svcclass] = $svcclass;
	$t[$type] = $type;
	$result[$svcclass][$type]['total'] = $dispatched;
}
	...
	<tr><td aling = left>
		...
		foreach($t as $type) { ?>
		  ...
		  <?php foreach($svcc as $svcclass          ) {
		        $temp = $result[$svcclass][$type]['total'];
		       	if ($temp == NULL) $temp = 0;
