...
while(OCIFetch($parsed1)) {
	...
	$maxtime = round((float)OCIResult($parsed1,3),3); 
	if ($min == NULL) $min = $maxtime;
	if ($maxtime >= $max) $max = $maxtime;
	if ($maxtime <= $min) $min = $maxtime;
	$svcc[$svcclass] = $svcclass;
	$t[$type] = $type;
	$result[$svcclass][$type]['max'] = $maxtime;
}
