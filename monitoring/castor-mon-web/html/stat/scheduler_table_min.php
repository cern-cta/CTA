...
if ($timewindow == "10/1440")
	...
	$period = 30;
if ($qn == 1) 
	$query1 = "select svcclass, requesttype, min(minqueuetime)
...
while(OCIFetch($parsed1)) {
	...
	$mintime = round((float)OCIResult($parsed1,3),3); 
	if ($min == NULL) $min = $mintime;
	if ($mintime >= $max) $max = $mintime;
	if ($mintime <= $min) $min = $mintime;
	$svcc[$svcclass] = $svcclass;
	$t[$type] = $type;
	$result[$svcclass][$type]['min'] = $mintime;
}
