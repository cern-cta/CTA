CREATE OR REPLACE PROCEDURE Proc_RecallStall AS
    ...
    execute immediate 'truncate table castor_stager.monitoring_RecallStall';
    ...
    SELECT * BULK COLLECT INTO s, a, b, c, d, e, f
      FROM (
         ...
         where dc.status = 2 and dc.castorfile=cf.id and cf.svcclass=svc.id group by svc.name
      );
    

      forall marker in s.first..s.last
        insert into CASTOR_STAGER.monitoring_RecallStall values(mytime, s(marker), a(marker), b(marker), c(marker), d(marker), e(marker), f(marker));
      
END Proc_RecallStall;
/ 
