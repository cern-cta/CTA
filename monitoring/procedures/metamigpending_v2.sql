CREATE OR REPLACE PROCEDURE Proc_MetaMigPending AS
    ...
    BEGIN
      ...
      execute immediate 'truncate table castor_stager.monitoring_MetaMigPending';
	...
	select b.name,
	 ...
	 group by svc.name order by svc.name) a,
      ...
      forall f in a.first..a.last
        insert into CASTOR_STAGER.monitoring_MetaMigPending values(mytime, a(f), b(f), c(f), d(f), e(f));

END Proc_MetaMigPending;
