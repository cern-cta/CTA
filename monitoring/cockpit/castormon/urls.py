...
urlpatterns = patterns('',
    ...
    url(r'^data/metric/(?P<metric_name>[a-zA-Z0-9_\-]+)$', 'castormon.cockpit.views.get_metric_data'), # used by ajax to get data
    url(r'^data/metric/(?P<metric_name>[a-zA-Z0-9_\-]+)/(?P<format_type>\w+)$', 'castormon.cockpit.views.get_metric_data'), # used by ajax to get data
    url(r'^data/metric/(?P<metric_name>[a-zA-Z0-9_\-]+)/(?P<timestamp_from>\d+)/(?P<format_type>\w+)$', 'castormon.cockpit.views.get_metric_data'), # used by ajax to get data
    url(r'^data/metric/(?P<metric_name>[a-zA-Z0-9_\-]+)/(?P<timestamp_from>\d+)/(?P<timestamp_to>\d+)/(?P<format_type>\w+)$', 'castormon.cockpit.views.get_metric_data'), # used by ajax to get data
