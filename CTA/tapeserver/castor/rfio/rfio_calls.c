      ...
      if (need_user_check && ((status=check_user_perm(&uid,&gid,host,&rcode,(((ntohopnflg(flags)) & (O_WRONLY|O_RDWR)) != 0) ? "WTRUST" : "RTRUST")) < 0) &&
      ...
      else
        {
          ...
          if (forced_filename!=NULL || !check_path_whitelist(host, filename, perm_array, ofilename, sizeof(ofilename),1)) {
            fd = ceph_posix_open(CORRECT_FILENAME(ofilename), ntohopnflg(flags),
          ...
          if (fd < 0) {
             ...
             */
            status= ceph_posix_lseek(fd,0L,SEEK_CUR);
            ...
            int     fd)
{
  ...
  if ( how != -1 ) {
    ...
    infop->seekop++;
    if ( (status= ceph_posix_lseek(fd,offset,how)) == -1 ) {
                ...
                int     fd)
{
   ...
   * Issuing the fstat()
   */
  status= ceph_posix_fstat(fd, &statbuf);
  rcode= errno;
            ...
            int     fd)
{
  ...
  status = ceph_posix_lseek(fd, offset, how);
  rcode= ( status < 0 ) ? errno : 0;
...
void *produce_thread(int *ptr)
{
  ...
  while ((! error) && (byte_read != 0)) {
    ...
    byte_read = ceph_posix_read(fd,array[produced % daemonv3_rdmt_nbuf].p,daemonv3_rdmt_bufsize);
