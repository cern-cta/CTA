...
QueueCleanupRunner::QueueCleanupRunner(objectstore::AgentReference &agentReference, SchedulerDatabase & oStoreDb, catalogue::Catalogue &catalogue, int batchSize) :
        m_catalogue(catalogue), m_db(oStoreDb),
        m_batchSize(batchSize) {
}

void QueueCleanupRunner::executeRunner(cta::log::LogContext &logContext) {
  ...
  if (!queueVidSet.empty()){
    try {
      ...
      logContext.log(log::ERR,
                     "In QueueCleanupRunner::executeRunner(): failed to read set of tapes from the database. Aborting cleanup.");
    ...
    for (const auto &vid: queueVidSet) {
      if (vidToTapesMap.count(vid) == 0) {
        ...
        logContext.log(log::ERR,
                       "In QueueCleanupRunner::executeRunner(): failed to find the tape " + vid + " in the database. Skipping it.");
      }
    }
  } else {
    logContext.log(log::DEBUG,
                   "In QueueCleanupRunner::executeRunner(): no queues requested a cleanup.");
    return;
  }
  ...
  for (const auto &[queueVid, tapeData]: vidToTapesMap) {
    ...
    if (tapeData.state != common::dataStructures::Tape::REPACKING_PENDING
      ...
      logContext.log(
              log::INFO,
              "In QueueCleanupRunner::executeRunner(): Queue has the cleanup flag enabled but is not in the expected PENDING state. Skipping it.");
      continue;
    }
    ...
    logContext.log(log::DEBUG, "In QueueCleanupRunner::executeRunner(): Will try to reserve retrieve queue.");
    try {
      ...
      logContext.log(log::DEBUG,
                     "In QueueCleanupRunner::executeRunner(): Unable to find the retrieve queue for cleanup. Queue may have already been deleted. Skipping it.");
      continue;
    } catch (OStoreDB::RetrieveQueueNotReservedForCleanup & ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::DEBUG,
                     "In QueueCleanupRunner::executeRunner(): Unable to reserve the retrieve queue due to it not being available for cleanup. Skipping it.");
      continue;
    } catch (OStoreDB::RetrieveQueueAlreadyReserved &ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::DEBUG,
                     "In QueueCleanupRunner::executeRunner(): Queue already reserved. Skipping it.");
      continue;
    } catch (cta::exception::Exception & ex) {
      log::ScopedParamContainer paramsExcMsg(logContext);
      paramsExcMsg.add("exceptionMessage", ex.getMessageValue());
      logContext.log(log::WARNING,
                     "In QueueCleanupRunner::executeRunner(): Unable to reserve the retrieve queue for cleanup for unknown reasons. Skipping it.");
      continue;
    }
    ...
    while (true) {
      ...
      logContext.log(cta::log::INFO,"In QueueCleanupRunner::executeRunner(): Queue jobs moved.");
    }
    ...
    // Finally, update the tape state out of PENDING
    {
      ...
      try {
        ...
        logContext.log(
          log::WARNING,
          "In QueueCleanupRunner::executeRunner(): Failed to find a tape in the database. Unable to update tape state.");
      ...
      try {
        switch (tapeDataRefreshed.state) {
          ...
          default:
            ...
            logContext.log(log::WARNING,
                           "In QueueCleanupRunner::executeRunner(): Cleaned up tape is not in a PENDING state. Unable to "
        ...
        if ((tapeDataRefreshed.state == Tape::REPACKING_PENDING && tapeDataRefreshedUpdated.state == Tape::REPACKING) ||
          ...
          logContext.log(log::WARNING,
                         "In QueueCleanupRunner::executeRunner(): Tape already moved into it's final state, probably by "
                         "another agent.");
        } else {
          logContext.log(log::ERR,
                         "In QueueCleanupRunner::executeRunner(): Tape moved into an unexpected final state, probably by "
                         "another agent.");
        }
      }
    }
