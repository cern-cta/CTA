...
Maintenance::Maintenance(cta::log::LogContext &lc, const cta::common::Config &config): m_lc(lc) {
  ...
  if (config.getOptionValueBool("cta.experimental.telemetry.enabled").value_or("false")) {
    try {
      ...
      if (cta::telemetry::stringToMetricsBackend(metricsBackend) == cta::telemetry::MetricsBackend::STDOUT) {
        m_lc.log(log::WARNING, "OpenTelemetry backend STDOUT is not supported for cta-taped. Using NOOP backend instead...");
        metricsBackend = cta::telemetry::metricsBackendToString(cta::telemetry::MetricsBackend::NOOP);
      }

      std::string otlpBasicAuthFile = config.getOptionValueStr("cta.telemetry.metrics.export.otlp.basic_auth_file").value();
      ...
      cta::telemetry::TelemetryConfig telemetryConfig =
        cta::telemetry::TelemetryConfigBuilder()
          ...
          .retainInstanceIdOnRestart(config.getOptionValueBool("cta.telemetry.retain_instance_id_on_restart").value_or(false))
          .resourceAttribute(cta::semconv::attr::kSchedulerNamespace, config.getOptionValueStr("cta.scheduler_backend_name").value())
          .metricsBackend(metricsBackend)
          .metricsExportInterval(std::chrono::milliseconds(config.getOptionValueInt("cta.telemetry.metrics.export.interval").value_or(15000)))
  ...
  m_schedDbInit = std::make_unique<SchedulerDBInit_t>("Maintenance", config.getOptionValueStr("cta.objectstore.backendpath").value(), m_lc.logger());

  if(!config.getOptionValueStr("cta.catalogue.config_file").has_value()) throw InvalidConfiguration("Could not find config entry 'cta.catalogue.config_file'");
  const rdbms::Login catalogueLogin = rdbms::Login::parseFile(config.getOptionValueStr("cta.catalogue.config_file").value());
  ...
  m_scheddb = std::move(m_schedDbInit->getSchedDB(
      *m_catalogue,
      m_lc.logger()));

  // Set Scheduler DB cache timeouts
  SchedulerDatabase::StatisticsCacheConfig statisticsCacheConfig;
  statisticsCacheConfig.tapeCacheMaxAgeSecs = config.getOptionValueInt("cta.schedulerdb.tape_cache_max_age_secs").value_or(600);
  statisticsCacheConfig.retrieveQueueCacheMaxAgeSecs = config.getOptionValueInt("cta.schedulerdb.retrieve_queue_cache_max_age_secs").value_or(10);
  m_scheddb->setStatisticsCacheConfig(statisticsCacheConfig);

  if (!config.getOptionValueStr("cta.scheduler_backend_name").has_value()) throw InvalidConfiguration("Could not find config entry 'cta.scheduler_backend_name'");
  m_scheduler = std::make_unique<cta::Scheduler>(*m_catalogue, *m_scheddb, config.getOptionValueStr("cta.scheduler_backend_name").value());

  // Add Disk Reporter
  if (config.getOptionValueBool("cta.disk_reporter.enabled").value_or(true)) {
    m_maintenanceRunners.emplace_back(
      std::make_unique<DiskReportRunner>(
        *m_scheduler,
  ...
  if (config.getOptionValueBool("cta.queue_cleanup.enabled").value_or(true)) {
    m_maintenanceRunners.push_back(std::move(m_schedDbInit->getQueueCleanupRunner(*m_catalogue, *m_scheddb,
  ...
  if (config.getOptionValueBool("cta.repack.enabled").value_or(true)) {
    m_maintenanceRunners.emplace_back(std::make_unique<RepackRequestManager>(
                                        *m_scheduler,
                                        config.getOptionValueInt(
                                          "cta.repack.request_manager.max_to_toexpand").value_or(2),
                                        config.getOptionValueInt(
                                          "cta.repack.reporter.soft_timeout").value_or(30)));
    log::ScopedParamContainer params(m_lc);
    params.add("maxRequestsToToExpand", config.getOptionValueInt("cta.reoack.request_manager.max_to_toexpand").value_or(2));
  ...
  if (m_maintenanceRunners.empty()) throw InvalidConfiguration("ERROR: No runner is enabled.");
}
