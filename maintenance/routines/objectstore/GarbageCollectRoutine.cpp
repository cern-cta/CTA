...
GarbageCollectRoutine::GarbageCollectRoutine(cta::log::LogContext& lc,
    ...
    : m_lc(lc),
      ...
      m_ourAgentReference(agentReference),
      m_agentRegister(os) {
  ...
  reLock.release();
  objectstore::ScopedSharedLock arLock(m_agentRegister);
  m_agentRegister.fetch();
  m_lc.log(cta::log::INFO, "Created GarbageCollectRoutine");
}

GarbageCollectRoutine::~GarbageCollectRoutine() {
...
void GarbageCollectRoutine::trimGoneTargets() {
  ...
  for (auto wa = m_watchedAgents.begin(); wa != m_watchedAgents.end();) {
    if (agentList.end() == std::find(agentList.begin(), agentList.end(), wa->first)) {
      ...
      m_lc.log(log::DEBUG, "In GarbageCollectRoutine::trimGoneTargets(): removed now gone agent.");
    } else {
      ++wa;
    }
  }
...
void GarbageCollectRoutine::acquireTargets() {
  m_agentRegister.fetchNoLock();
  ...
  for (auto& c : candidatesList) {
    ...
    if (c != m_ourAgentReference.getAgentAddress() && !alreadyTrackedAgents.count(c)) {
      ...
      objectstore::Agent ag(c, m_objectStore);
      try {
        ag.fetchNoLock();
      ...
      log::ScopedParamContainer params(m_lc);
      params.add("agentAddress", ag.getAddressIfSet()).add("gcAgentAddress", m_ourAgentReference.getAgentAddress());
      m_lc.log(log::DEBUG, "In GarbageCollectRoutine::acquireTargets(): started tracking an untracked agent");
      ...
      try {
        m_watchedAgents[c] = new objectstore::AgentWatchdog(c, m_objectStore);
...
void GarbageCollectRoutine::checkHeartbeats() {
...
void GarbageCollectRoutine::cleanupDeadAgent(const std::string& address, const std::list<log::Param>& agentDetails) {
  ...
  // deal with them.
  objectstore::Agent agent(address, m_objectStore);
  objectstore::ScopedExclusiveLock agLock;
  try {
    // The agent could be gone while we try to lock it.
    agLock.lock(agent);
  } catch (cta::exception::NoSuchObject& ex) {
    log::ScopedParamContainer params(m_lc);
    params.add("agentAddress", agent.getAddressIfSet()).add("gcAgentAddress", m_ourAgentReference.getAgentAddress());
    m_lc.log(
      log::INFO,
      "In GarbageCollectRoutine::cleanupDeadAgent(): agent already deleted when trying to lock it. Skipping it.");
    return;
  }
  agent.fetch();
  log::ScopedParamContainer params(m_lc);
  ...
  agent.commit();
  // Update the register
  objectstore::ScopedExclusiveLock arl(m_agentRegister);
  m_agentRegister.fetch();
  m_agentRegister.trackAgent(address);
  m_agentRegister.commit();
  arl.release();
  {
    log::ScopedParamContainer params2(m_lc);
  ...
  std::list<std::shared_ptr<objectstore::GenericObject>> fetchedObjects;
  ownedObjectSorter.fetchOwnedObjects(agent, fetchedObjects, m_objectStore, m_lc);
  ownedObjectSorter.sortFetchedObjects(agent, fetchedObjects, m_objectStore, m_catalogue, m_lc);
...
void GarbageCollectRoutine::OwnedObjectSorter::fetchOwnedObjects(
  ...
  std::list<std::shared_ptr<objectstore::GenericObject>> ownedObjects;
  std::map<objectstore::GenericObject*, std::unique_ptr<objectstore::GenericObject::AsyncLockfreeFetcher>>
    ownedObjectsFetchers;
  ...
  for (auto& obj : ownedObjectAddresses) {
    ...
    ownedObjects.emplace_back(new objectstore::GenericObject(obj, objectStore));
    try {
      ...
      lc.log(log::ERR,
               "In GarbageCollectRoutine::OwnedObjectSorter::fetchOwnedObjects(): "
  ...
  for (auto& obj : ownedObjects) {
    ...
    try {
      ...
      lc.log(log::INFO,
               "In GarbageCollectRoutine::OwnedObjectSorter::fetchOwnedObjects(): skipping garbage collection of now "
               "gone object.");
...
void GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObjects(
  objectstore::Agent& agent,
  std::list<std::shared_ptr<objectstore::GenericObject>>& fetchedObjects,
  objectstore::Backend& objectStore,
  ...
  bool ownershipUdated = false;
  using objectstore::serializers::ArchiveJobStatus;
  using objectstore::serializers::RetrieveJobStatus;
  for (auto& obj : fetchedObjects) {
    ...
    if (obj->getOwner() != agent.getAddressIfSet()) {
      ...
      if (obj->type() == objectstore::serializers::ArchiveRequest_t) {
        objectstore::ArchiveRequest ar(*obj);
      ...
      lc.log(log::WARNING,
               "In GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObjects(): skipping object which is not owned "
               "by this agent");
      agent.removeFromOwnership(obj->getAddressIfSet());
      ownershipUdated = true;
      continue;
...
doGCObject:
    switch (obj->type()) {
      case objectstore::serializers::ArchiveRequest_t: {
        ...
        auto ar = std::make_shared<objectstore::ArchiveRequest>(*obj);
        obj.reset();
        ...
        for (auto& j : ar->dumpJobs()) {
          if ((j.owner == agent.getAddressIfSet() && ar->c_statusesImplyingQueueing.count(j.status))) {
            ...
            try {
              ...
              archiveQueuesAndRequests[std::make_tuple(containerIdentifier, ar->getJobQueueType(j.copyNb), j.tapePool)]
                .emplace_back(ar);
              ...
              lc.log(log::INFO,
                       "In GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObjects(): Selected archive request "
                       "for requeueing to the corresponding queue");
              jobRequeued = true;
            } catch (objectstore::ArchiveRequest::JobNotQueueable&) {
              log::ScopedParamContainer params3(lc);
              params3.add("tapePool", j.tapePool)
                .add("containerIdentifier", containerIdentifier)
                .add("copynb", j.copyNb)
                .add("status", objectstore::ArchiveRequest::statusToString(j.status))
                .add("fileId", ar->getArchiveFile().archiveFileID);
              lc.log(log::WARNING,
                       "In GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObjects(): Job garbage collected with "
                       "a status not queueable. Leaving it as is.");
            }
          }
        }
        if (!jobRequeued) {
          ...
          lc.log(log::INFO,
                   "In GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObjects(): No active archive job to "
                   "requeue found. Request will remain as-is.");
        }
        break;
      }
      case objectstore::serializers::RetrieveRequest_t: {
        ...
        auto rr = std::make_shared<objectstore::RetrieveRequest>(*obj);
        obj.reset();
        ...
        if (candidateVids.empty()) {
          ...
          try {
            ...
            otherObjects.emplace_back(new objectstore::GenericObject(rr->getAddressIfSet(), objectStore));
            break;
          }
        }
        ...
        try {
          ...
          lc.log(log::INFO,
                   "In GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObjects(): No available tape found. "
                   "Marking request for normal GC (and probably deletion).");
          otherObjects.emplace_back(new objectstore::GenericObject(rr->getAddressIfSet(), objectStore));
          break;
        }
        retrieveQueuesAndRequests[std::make_tuple(vid,
        ...
        for (auto& tc : rr->getArchiveFile().tapeFiles) {
          if (tc.vid == vid) {
            copyNb = tc.copyNb;
            fSeq = tc.fSeq;
          }
        ...
        params3.add("fileId", rr->getArchiveFile().archiveFileID)
          ...
          .add("fSeq", fSeq);
        lc.log(log::INFO,
                 "In GarbageCollectRoutine::OwnedObjectSorter::sortFetchedObject(): Selected vid to be requeued for "
                 "retrieve request.");
      } break;
      default:
...
void GarbageCollectRoutine::OwnedObjectSorter::executeArchiveAlgorithm(
  const std::list<std::shared_ptr<objectstore::ArchiveRequest>>& jobs,
  ...
  try {
    aqcl.referenceAndSwitchOwnershipIfNecessary(containerIdentifier,
    ...
    for (auto& failedAR : failure.failedElements) {
      try {
        ...
        if (typeid(e) == typeid(cta::exception::NoSuchObject) ||
            typeid(e) == typeid(objectstore::Backend::WrongPreviousOwner)) {
          ...
          lc.log(log::INFO,
                   "In GarbageCollectRoutine::OwnedObjectSorter::executeArchiveAlgorithm(): "
          ...
          otherObjects.emplace_back(
            new objectstore::GenericObject(arup.archiveRequest->getAddressIfSet(), objectStore));
        }
      }
    }
  }
  ...
  for (auto& arup : jobsToAdd) {
    if (!jobsNotRequeued.count(arup.archiveRequest->getAddressIfSet())) {
      ...
      lc.log(log::INFO,
               "In GarbageCollectRoutine::OwnedObjectSorter::executeArchiveAlgorithm(): requeued archive job.");
    }
  }
  jobsToAdd.clear();
}

std::string GarbageCollectRoutine::OwnedObjectSorter::dispatchArchiveAlgorithms(
  const std::list<std::shared_ptr<objectstore::ArchiveRequest>>& jobs,
  ...
  switch (jobQueueType) {
    case common::dataStructures::JobQueueType::JobsToTransferForUser:
      executeArchiveAlgorithm<objectstore::ArchiveQueueToTransferForUser>(jobs,
                                                                          queueAddress,
                                                                          containerIdentifier,
                                                                          tapepool,
                                                                          jobsIndividuallyGCed,
                                                                          agent,
                                                                          agentReference,
                                                                          objectstore,
                                                                          lc);
    ...
    case common::dataStructures::JobQueueType::JobsToReportToUser:
      executeArchiveAlgorithm<objectstore::ArchiveQueueToReportForUser>(jobs,
                                                                        queueAddress,
                                                                        containerIdentifier,
                                                                        tapepool,
                                                                        jobsIndividuallyGCed,
    ...
    case common::dataStructures::JobQueueType::JobsToTransferForRepack:
      executeArchiveAlgorithm<objectstore::ArchiveQueueToTransferForRepack>(jobs,
                                                                            queueAddress,
                                                                            containerIdentifier,
                                                                            tapepool,
                                                                            jobsIndividuallyGCed,
    ...
    case common::dataStructures::JobQueueType::JobsToReportToRepackForSuccess:
      executeArchiveAlgorithm<objectstore::ArchiveQueueToReportToRepackForSuccess>(jobs,
                                                                                   queueAddress,
                                                                                   containerIdentifier,
                                                                                   tapepool,
                                                                                   jobsIndividuallyGCed,
    ...
    case common::dataStructures::JobQueueType::JobsToReportToRepackForFailure:
      executeArchiveAlgorithm<objectstore::ArchiveQueueToReportToRepackForFailure>(jobs,
                                                                                   queueAddress,
                                                                                   containerIdentifier,
                                                                                   tapepool,
                                                                                   jobsIndividuallyGCed,
    ...
    case common::dataStructures::JobQueueType::FailedJobs:
      executeArchiveAlgorithm<objectstore::ArchiveQueueFailed>(jobs,
                                                               queueAddress,
                                                               containerIdentifier,
                                                               tapepool,
                                                               jobsIndividuallyGCed,
...
void GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(
  objectstore::Agent& agent,
  objectstore::AgentReference& agentReference,
  objectstore::Backend& objectStore,
  cta::log::LogContext& lc) {
  ...
  for (auto& archiveQueueIdAndReqs : archiveQueuesAndRequests) {
    ...
    while (requestsList.size()) {
      ...
      dispatchArchiveAlgorithms(currentJobBatch,
                                queueType,
                                containerIdentifier,
                                tapepool,
                                jobsIndividuallyGCed,
      ...
      for (auto& ar : currentJobBatch) {
        if (ar.use_count() == 1 && !jobsIndividuallyGCed.count(ar->getAddressIfSet())) {
          ...
          lc.log(log::DEBUG,
                   "In GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): Removed AR from "
                   "agent ownership.");
        } else {
          log::ScopedParamContainer params(lc);
          params.add("archiveRequestObject", ar->getAddressIfSet())
          ...
          lc.log(log::DEBUG,
                   "In GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateArchiveJobs(): Did not remove AR "
...
void GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateRetrieveJobs(
  objectstore::Agent& agent,
  objectstore::AgentReference& agentReference,
  objectstore::Backend& objectStore,
  cta::log::LogContext& lc) {
  ...
  for (auto& retriveQueueIdAndReqs : retrieveQueuesAndRequests) {
    ...
    while (requestsList.size()) {
      ...
      objectstore::RetrieveQueue rq(objectStore);
      objectstore::ScopedExclusiveLock rql;
      objectstore::Helpers::getLockedAndFetchedJobQueue<objectstore::RetrieveQueue>(rq,
                                                                                    rql,
                                                                                    agentReference,
                                                                                    containerIdentifier,
                                                                                    queueType,
      ...
      for (auto& rr : currentJobBatch) {
        ...
        for (auto& tf : rr->getArchiveFile().tapeFiles) {
          if (tf.vid == vid) {
            jta.emplace_back(tf.copyNb,
                             tf.fSeq,
                             rr->getAddressIfSet(),
                             rr->getArchiveFile().fileSize,
                             rr->getRetrieveFileQueueCriteria().mountPolicy,
                             rr->getEntryLog().time,
                             rr->getActivity(),
                             rr->getDiskSystemName());
          }
        }
      }
      ...
      struct RRUpdatedParams {
        std::unique_ptr<objectstore::RetrieveRequest::AsyncJobOwnerUpdater> updater;
        std::shared_ptr<objectstore::RetrieveRequest> retrieveRequest;
        uint32_t copyNb;
      };

      {
        ...
        for (auto& rr : currentJobBatch) {
          for (auto& tf : rr->getArchiveFile().tapeFiles) {
            if (tf.vid == vid) {
              ...
              rrUpdatersParams.back().updater.reset(
                rr->asyncUpdateJobOwner(tf.copyNb, rq.getAddressIfSet(), agent.getAddressIfSet()));
            }
          }
        }
        ...
        for (auto& rrup : rrUpdatersParams) {
          try {
            ...
            lc.log(
              log::INFO,
              "In GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateRetrieveJobs(): requeued retrieve job.");
          } catch (cta::exception::Exception& e) {
            // Update did not go through. It could be benign
            std::string debugType = typeid(e).name();
            if (typeid(e) == typeid(cta::exception::NoSuchObject) ||
              ...
              params.add("retrieveRequestObject", rrup.retrieveRequest->getAddressIfSet())
                ...
                .add("exceptionType", debugType);
              lc.log(log::ERR,
                       "In GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateRetrieveJobs(): "
              ...
              otherObjects.emplace_back(
                new objectstore::GenericObject(rrup.retrieveRequest->getAddressIfSet(), objectStore));
            }
...
void GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(
  objectstore::Agent& agent,
  objectstore::AgentReference& agentReference,
  objectstore::Backend& objectStore,
  cta::catalogue::Catalogue& catalogue,
  ...
  for (auto& go : otherObjects) {
    ...
    if (go->exists()) {
      objectstore::ScopedExclusiveLock goLock(*go);
      go->fetch();
      ...
      lc.log(log::INFO,
               "In GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(): garbage collected owned "
               "object.");
      ++garbageCollectedOtherObjects;
    } else {
      lc.log(log::INFO,
               "In GarbageCollectRoutine::OwnedObjectSorter::lockFetchAndUpdateOtherObjects(): "
